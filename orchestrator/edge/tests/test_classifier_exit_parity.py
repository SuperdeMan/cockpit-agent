...
_GOLDEN = [
    ...
    ("音乐别停", None, False),
    # ⚠ 2026-08-27（QA N2）改过这三行，**留痕**：原来「停止/关闭」与「暂停」一律
    # 落 `media.pause`——`media_map` 把 stop/close 折进了 pause。折掉的那个终态
    # (`stopped`) 恰好是 VAL 的初始态，于是它成了「只能离开、回不来」的态，
    # 探针每一轮车态恢复都留一条不可能达成的差异。**行为锁按设计工作**：
    # 修出口必须显式改这几行，而不是让路由悄悄变。
