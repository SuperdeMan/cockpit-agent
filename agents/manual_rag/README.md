...
生产状态（2026-09-04）：release `7b594f379c1fbfb5156c55c4e0dc573957b49d28` 使用 v2
shared-model 只读图文包，5/5 endpoint healthy；最近成功统一verify仍只属于旧release
`434a046`。旧release的完整36题生产链路36/36，14个高风险问法全部repeat 3，合计64/64；
这些数字不转借给当前release。

整本范围复核把分母扩为 PDF outline 的187个原子叶子。当前生产自然化首轮181/187，
6个失败后两轮均通过，故全部为2/3、无0/3稳定章节失败；受控视觉首轮30/35，其中
`位置灯/左转向/右转向/后雾灯/近光灯`五个三字caption均0/3。候选0.3.2已在本地把显式
手册来源落域222/222、视觉35/35，但尚未发布，不能把本地结果转借给生产。

当前release补跑用户点名问法：`雨刮器怎么打开`与“小人背宝剑”各3/3，均返回预期PDF页和
