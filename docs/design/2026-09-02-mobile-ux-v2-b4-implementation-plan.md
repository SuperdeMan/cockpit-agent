...
**未验的一半（本轮没做完，adb 卡死打断）**：我点追问 chip 造的第二轮到底有没有产生 `process` 帧。
`edge.falseAt=0` 有两种解释——(a) 那一轮没有 `process` 帧；(b) 有，但标的仍是 `driving:true`。
(b) 与云栈回读矛盾，所以 (a) 可能性大，但**没有直接证据，不定死**。收起层看那条气泡有没有过程区即可分辨。
