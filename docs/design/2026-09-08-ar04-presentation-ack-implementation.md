...
日期：2026-09-08。状态：**接手核查和改前基线完成；跨页产品范围待确认，尚未修改产品代码。**
...
推荐采用以下范围；用户选择后在本节回填决定，根布局迁移再开始。
...
备选是只允许对话页语音：每次离开对话页显式暂停采集和播报，其他页只显示待呈现提醒及返回入口。
它减少宿主迁移，但改变“切去看地图时继续问答”的体验；若选择它，本页的控制器迁移范围需要收缩。
