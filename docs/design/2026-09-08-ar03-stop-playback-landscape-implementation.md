...
> 状态：客户端与共享 FSM 修复、本地回归与变异验证完成；OPPO 上取得 R06 主证据（同题 A/B），Xiaomi 对照机上取得 R09 横屏证据（结构 + 功能）；**层内停止键与 S2S/多段/主动消息仍未验，AR03 未整批签收**（末节列格）。
...
- **主动消息**播报中停播：不能按需触发。
- **横屏行车态确认可达：已在对照机上取证关闭**（见第八节）。仍未验的是同形态下的**层内停止键**与 **200% 字号 / 长摘要**。
...
逐项 XML/PNG（`t*` = OPPO，`x*` = Xiaomi；决定性的是 `x24-landscape-dock-under-sheet.png` 与同帧 XML）、
...
~~~text
批次：AR03
对应 R 编号与关闭/剩余项：R06 客户端修复完成 + OPPO 同题 A/B 取证；R09 横屏部分客户端修复完成 + Xiaomi 结构与功能取证；仍未验：层内停止键、多段、S2S 自答、主动消息、200% 字号、盲听；R09 跨页部分属 AR04 未启动
...
本轮执行的检查与原始证据：mobile 70 suites / 695 tests + tsc exit 0；hmi 308/308 + Vite build；15 条变异注入全部判红；OPPO 上同题 A/B（停播窗口 ≥15.7s → 单个采样）与 8 次播报零麦克风；Xiaomi 上 driving-landscape（w840dp h360dp land）下层与 Dock bounds 零重叠且 Dock 可点
失败/阻塞/污染样本：reanimated 官方 mock 在本环境不可用（已手写替代）；测试渲染器不跑布局（横屏用例手动放 onLayout 并先证层在场）；ColorOS 拒绝 adb 改动画/尺寸/密度/旋转 ⇒ driving-landscape 在 OPPO 不可达（改用对照机）；升层与播报结束同时发生，n 太小未定性；HyperOS 上 120ms motionevent 触发不了普通 Pressable；旋转进入横屏后语音层升不起来（冷启动可以），留给 AR04
