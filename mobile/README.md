...
- **端到端时延复盘（2026-09-16）**：[2026-09-16-android-e2e-latency-location-wait.md](../docs/reviews/2026-09-16-android-e2e-latency-location-wait.md)——同一句「今天天气怎么样」App 比 HMI 慢 20s 的成因（发送前等新定位、室内每轮等满 20s）与修法（`core/location/fixPolicy.ts`：缓存即用 / 最多等 2.5s / 陈旧回落 / 后台预热）；**§10（2026-09-17）坐标来源换成系统 LocationManager（`modules/platformlocation`）——GMS fused 那份缓存在测试机上三天没动，在家答出公司地址就是它**；端云共有的规划 LLM 3–4.5s 三个可选项待裁决
- **流式文字匀速上屏（2026-09-18）**：[2026-09-18-android-stream-text-pacing.md](../docs/design/2026-09-18-android-stream-text-pacing.md)——「一大段跳变」是服务端 delta 成簇到达、客户端原样即时上屏（JS 线程不忙）；`core/session/streamReveal.ts` 把显示与记录分开、显示按节拍追（`useRevealedText`，MessageBubble / VoiceSheet 消费）；「长文本展示不全」是 chitchat `max_tokens` 当长度控制，服务端已改为兜底上限。真机 A/B 与清洁包待构建机内存释放
...
- **页面展示层评审**（2026-09-10；P01–P30 布局 / 文案 / 视觉、D1–D7 测试味、V1–V10 显示不全，含两台真机的用法）：[2026-09-10-android-ui-polish-review.md](../docs/reviews/2026-09-10-android-ui-polish-review.md)；分批计划 [2026-09-10-android-ui-polish-batches.md](../docs/design/2026-09-10-android-ui-polish-batches.md)（批 A–G，已批准一次性推进，文末有 Goal 文本与授权边界；落盘时零代码改动）
...
- **性能与链路时延评审（2026-09-12）**：[读数、归因与优先级](../docs/reviews/2026-09-12-android-performance-latency-review.md)——空闲 CPU 的大头是光球动效（1.5 核）与 VAD 线程池（已改单线程）；链路大头是云侧规划（10.5k token prompt、约三分之二轮次两次 LLM）；测试环境走 DERP 中继的路径成本单列。
