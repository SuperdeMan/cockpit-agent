...
> **进度（2026-08-01，13 张主卡清零）**：P1 ①② M-B（§10）／③ M-D（§12）／④⑤ M-C（§11）
> ／⑥ M-A（§9）。P2 ①② M-B ／③④ M-A ／⑤ M-C ／⑥ 历史已修 ／⑦ M-D。
> 各里程碑的「明确未做」逐条附判据在 §9/§10.2/§11.2/§12.2——**未做不等于没账**。
...
## 12. M-D 外部生态收口（2026-08-01）——13 张主卡清零

M-D 关的是 §1「MCP 写操作生命周期五项两项纸面合规」、§7 P1-03 与 P2-07。
三张卡里**只有一张是用户真的缺能力**：下了单之后没有任何办法查它或取消它。

### 12.1 已修（本批）

| # | 缺口 | 修复 |
|---|---|---|
| 1 | 补偿「仅准入期存在性校验、运行期零调用、用户零入口」（§1） | `order.cancel` 从一开始就在商户侧存在、也被 `order.create` 声明为 `compensate_tool`，但**从没进过准入清单**。放进清单它才成为能力——**声明存在 ≠ 能用**。仍走确认闸，**不做未经用户确认的自动补偿** |
| 2 | MCP 订单查询入口不存在（§7 P1-03） | 新增 `order.get`，按订单号**或幂等键**查。幂等键那条是关键的一半：**下单超时那一单根本没有订单号**（响应没回来），但幂等键是我们自己生成的、商户按它索引——「到底下没下成」由此第一次可以核对，而不是让用户「先去商家处核实」 |
| 3 | 话术承诺过不存在的入口 | 验收当时先改成不承诺（治标正确），M-D 接入能力后才把承诺加回来。**先有能力再有话术**——顺序反过来就是把不确定包装成「有办法查清楚」 |
| 4 | Ledger「先查再插」允许多实例同时取得执行权 | `(user_id, idempotency_key)` 活跃态 partial unique + `ON CONFLICT DO NOTHING`；竞争输了回读对方账目按 `Duplicate`，**不当失败**（那一单正在被别人执行）。**partial 是必须的**：终态行要能共存。配源码断言防「改回先查再插」——那个写法在单实例下永远测不出问题 |
| 5 | toolcall provider 能力位（§7 P2-07） | `supports_toolcall` 声明式能力位，缺省 True（零行为变化）、不认识的档不定罪。声明 False 的档当场退回纯文本——此前每轮白打 2 次上游（primary 与 fast 各一次 400），planner 还要再走一遍 JSON。**每次请求现读**，热切后不沿用旧能力 |

### 12.2 明确未做

原 M-D 计划 13 个 task。未做项与判据：

| 未做项 | 判据 |
|---|---|
| `mcp_operation` 独立业务状态表 + owner-scoped 兼容层 | **商户是订单状态的真相源**，本地镜像就是第二真相源。要知道「有哪些单」用 Ledger 已有的记录；要知道「状态如何」问商户。这是本批最重要的一个减法 |
| Ledger `owner_v2` forward-only cutover（控制行 / `FOR SHARE` barrier / 冻结备份） | 那套仪式是为「有损重分配」准备的，而本批不重分配任何数据——只加了一个 partial unique index |
| HMI operation card、双 bridge acceptance profile、真栈迁移矩阵 | 覆盖面证据，价值在回归而非发现 |
| GDPR `retained_audit` / `external_reference` 目标 | 与 M-B/M-C 后置的跨域 saga 同批，单独接一个目标没有消费方 |
| `patch_missing` 声明式补槽与枚举校验 | 验收 §6「已知残余」记的是 provider 输出方差（size 幻觉/畸形谱），本批未观察到它阻断查单取消链路；真要做应在通用引擎侧，不在 MCP 桥 |

### 12.3 §7 十三张主卡终态

P1：①② M-B ／ ③ M-D ／ ④⑤ M-C ／ ⑥ M-A。
P2：①② M-B ／ ③④ M-A ／ ⑤ M-C ／ ⑥ 历史已修 ／ ⑦ M-D。
**13 张全部有归属，无遗留含糊状态。** 各里程碑的「明确未做」逐条附判据，
分别在 §9（M-A）、§10.2（M-B）、§11.2（M-C）、§12.2（M-D）。

### 12.3b 真栈跑出三个单测测不到的缺陷

单测全绿后跑真栈全链路（下单→查单→取消）：

| # | 缺陷 | 性质 |
|---|---|---|
| 1 | **mcp-bridge 镜像从来没装过 asyncpg** | **既有缺陷，不是本批引入**。`_sdk/requirements.txt` 没有它，deep-research/reminder/scene 各自补了，只有 mcp-bridge 没有——启动即打「TaskLedger: PG 不可用，任务账本禁用」，**幂等受理从 M3 上线起就一直关着**。降级是诚实的（WARNING + 代码容忍 `task is None`），但**没人读那行 WARNING**，于是它安静地少了一个安全特性 |
| 2 | 取消复用写路径后追问词还是下单的「要点什么？」 | 改为按**槽位名**索引问法（槽位名来自 capability 声明，不是 intent 字面量） |
| 3 | 确认词说错动作：「取消我的咖啡订单」被问成「准备下单：DC03…，确认吗？」 | **用户正要点头同意的就是这句**。动词是领域语义，移进工具声明（`confirm_prompt`），桥核心不认识「下单」「取消」；配 AST 源码断言（只看可执行代码里的字符串常量——注释里引用这个词是在解释缺陷不是在犯它） |

**这三个单测一个都测不出来**：①是镜像依赖，②③在只有一个写工具时永远看不出问题
——**加第二个写工具才让「顺手写死的那一句」显形**。

### 12.4 一条判据

**「声明存在」不等于「能用」。** `compensate_tool` 被准入校验查了存在性、被文档写进
「写操作生命周期五项」、被测试断言过——唯独没有一条路径能真的调到它，因为那个工具
自己不在准入清单里。校验的是**声明**，没人校验**可达性**。同族的还有 M-B 的
`nlu.theta_high/theta_low`（运行时零消费方）与 skills 的 `few_shots`（文档有、代码不读）。
