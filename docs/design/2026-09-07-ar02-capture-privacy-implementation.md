...
> 状态：客户端修复、本地回归与 OPPO prod release 定向验证完成，代码已合入并推送 main；**AR02 尚未整批签收**，R02/R07 完整设备矩阵见末节。旧缓存已于 2026-09-08 经用户授权清除。范围仅 R01 / R02 / R07；启动基线 `9297848`。2026-09-07 开发批没有生产部署、环境/密钥/CI/CD/数据库修改或真实车控、商户写、付款；2026-09-08 清理时发生的误操作单列如下。
...
- 旧版本遗留 2 个 Camera 缓存文件、4,408,670 bytes 已于 2026-09-08 经用户授权清除；清理结果见下节，该项不再挂待授权。
...
用户明确授权仅清理 OPPO 小舟 App 缓存。包仍为 `70365389e`，安装时间仍为 2026-09-07 20:51:42。命令行 cache-only 请求未返回，adb 短暂断开后以 exit 1 结束，没有计为成功；随后在系统应用详情确认“小舟随行”后，进入存储占用，点击“清除缓存”。系统缓存由 8.6MB 降至 0B，应用数据保持 19.9MB；没有点击“清除数据”、卸载或重装。

冷启动后回读 `cacheFileCount=0 / cacheBytes=0`，构建身份不变；清理前后设置页的服务器地址及掩码凭证完全一致，麦克风/相机权限仍授予。证据在 `%LOCALAPPDATA%\car-agent\artifacts\AR02-cache-20260908`，核心记录为 `cleanup-result.json`、前后 XML 与系统存储截图。未重跑代码测试、未构建新包；不改变上一节设备矩阵的未验状态。

清理过程出现一次前台切换：系统设置截图后切回 App，坐标点击后出现座椅加热识别页。只读核对 collector trace `16500021e5d4f77b`，2026-09-08 00:27:10 的 `seat.heating.on` 产生 1 个 action，VAL 记录 `seat_heating: false → true`，随后读取仍为 true。第一次点击未即时复核前台，存在误点风险，已向用户说明并请求恢复 false 的单独授权。之后发现同一工作树有并行 AR03 源码和计划改动，结合页面切换，操作归属尚需确认；不能仅凭截图把该 action 归给任一测试。本会话未发恢复命令，也没有推定业务已回滚。后续清缓存点击校验实际前台组件，成功后冷启动读回完成；已结束设备操作，避免继续干扰并行工作。
