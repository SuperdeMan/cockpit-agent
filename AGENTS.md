...
| 候选 | 为什么现在能做 / 不能做 | 入口 |
|---|---|---|
| **Android 陪伴端 App（新客户端 `mobile/`，2026-08-23 立项）** | **可以开工**：三条需求（与 HMI 共存的独立用户端 / 生产标准 / 细节授权代决）与五项决策（RN+Expo、CNG、token 策略、Android 10+、M2 语音随首轮）已定，方案已批准（方向）。纯客户端新增，与下方编排/商户面候选**零文件交集**（唯一交集=只读引用 `hmi/src` 白名单模块）。⚠ 开工前有一次性环境准备 E1–E6（Android SDK/JDK 全局安装=红线，须泓舟执行） | 接手从 [`docs/design/2026-08-24-mobile-app-implementation-plan.md`](docs/design/2026-08-24-mobile-app-implementation-plan.md) **§0 接手须知**开始（逐任务执行真相源）；需求/选型/架构判断在 [`docs/design/2026-08-23-hmi-android-app-plan.md`](docs/design/2026-08-23-hmi-android-app-plan.md)（§9 决策记录） |
