...
- **端到端时延复盘（2026-09-16）**：[2026-09-16-android-e2e-latency-location-wait.md](../docs/reviews/2026-09-16-android-e2e-latency-location-wait.md)——同一句「今天天气怎么样」App 比 HMI 慢 20s 的成因（发送前等新定位、室内每轮等满 20s）与修法（`core/location/fixPolicy.ts`：缓存即用 / 最多等 3s / 陈旧回落 / 后台预热）；端云共有的规划 LLM 3–4.5s 三个可选项待裁决
