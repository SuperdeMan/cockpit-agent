...
| **P2** | `nq.hvac-keep.dont`「空调先别关」：五字短句在两条通道上都检不回任何范例 | 该句 `exemplars=[]`（词法 <0.34、语义 <0.65 全不达），读数 10/12 且失败形态是真下 `hvac.off`。**不是知识缺**——常驻否定 policy 每轮都在；是**短句检索够不着**。修法要么给短句族补对照范例、要么重看阈值对短文本的适用性，别硬塞进上一条的范例里 |

> 2026-08-10（下午批 · 生产侧）已收口：**`nq.hvac.reported` 的真缺陷已修**——
> 根因不是「模型不懂否定」（常驻 policy 每轮都在），是**否定这一侧对「空调」没有对照物**，
> 检回来的是 `volume#1`「空调震动，把声音调小一点」这条**含空调的肯定式**范例。
...
> 判据＝**给一个对象写范例时两侧都要有对照物，只有一侧就是吸引子**（`sunroof.yaml`
> 那条判据的镜像——同一个机制两个方向都咬过人）。findings §21。
>
