...
**14** 个领域 Agent · **30** 个服务一键起栈 · **80** 条端侧 capability / **67** 个车控对象 · 后端 **4864** 个通过用例 + **36** 条旅程级语料 · 流式 TTS 首帧 **469ms** / 端到端语音首音频 **609ms** · 数据源全真实（高德 / 和风 / Exa / Tushare / api-football）· M0a→M4 智能化升级**通过跨阶段组合总体验收**且 13 张验收主卡全部收口 · M5 数据飞轮落地——**修落域 badcase 的标准产物是数据不是正则**
...
| `mcp-bridge` | 受控 MCP 生态桥：一个 Agent 承载 N 个外部 MCP server，**人工准入 + 版本锁定 + schema 指纹**三重锁定；写操作有幂等键/账本落账/超时不确定口径/补偿声明/审计。已接入**麦当劳/瑞幸官方 MCP**（stdio+streamable_http 双传输；v1 只读：麦当劳营养餐品、两家订单查询，商户长响应经 LLM 重述成一句人话；下单归二期）+ 演示商户全流程（卡片与话术恒标注） |
...
| 层 | 是什么 | 现状 |
|---|---|---|
| 单测 / 契约 | 全服务 pytest（编排 / Agent / 安全 / 记忆 / 网关 / 共享运行时）单命令一次跑通 | **4864 passed / 14 skipped / 0 failed**（2026-08-11） |
...
| L3 旅程级 | 36 条语料：跨 Agent 自主执行（把事办完）× 全场景连续对话 | 回归级常绿（红灯单条重跑定性制） |
...
- **文档先行**：79 篇按日期编号的设计与落地记录（`docs/design/`，截至 2026-08-09），每个主题先对齐设计再动手；「架构唯一真相源」制度化。
...
| 各主题设计与落地记录（79 篇，按日期，截至 2026-08-09） | [`docs/design/`](docs/design/) |
