...
## 6. 第三轮真机反馈：提醒句被端侧体感规则劫持成开空调（c9bcf8c2）

**病灶**：「再帮我加一个明天晚上10点提醒我**冷萃**咖啡过滤。」被端侧快路径按
`hvac.on` 置信度 0.9 秒回「空调已开启」并**真执行了车控动作**（path=local，10.7ms，
零 LLM）。根因：`classify_structured` 空调分支入口的体感共现规则
`(热|冷) × (度|一点|再)` 宽到「**冷**萃」+「**再**帮我」即命中，进分支后内部
子规则全不匹配、落到兜底 `open aircon`。「提醒我」在端侧完全没有对应的让路。

**修复**（`orchestrator/edge/fast_intent.py`）：

1. **提醒话术全局让路**：`_is_reminder_utterance`（提醒我/提醒一下/别忘了/
   设个闹钟/待办/记得提醒/到时候叫我）在 `classify_structured` 入口最前返回
   None → 整句上云归 reminder Agent。刻意不用裸「提醒」——「打开限速提醒/
   疲劳提醒」是 ADAS 功能开关（端侧设置域），不受影响。R7 拨号分支的同类
   局部让路由此统一承接。多意图拆句路径逐子句过 classify_structured，守卫天然
   per-clause 生效（「打开空调，明早提醒我开会」车控子句照常端侧）。
2. **体感入口收窄**：共现规则收成显式词形白名单 `_FEELING_FORMS`
   （热一点/冷一点/再热/再冷）+「(热|冷) × 可解析出度数」——刻意只取旧规则的
   **合理子集**，不扩端侧接管面（「有点冷/好热」裸体感陈述历史上就走云端隐式
   车控，维持不变）。

**验证**：edge 单测 363（+6：badcase 原句/提醒动词族上云、限速提醒仍端侧、
体感相对词形与度数句仍本地、共现误伤面不再进空调）；`eval_fast_intent` 基线
57/57 无回归；全量语料覆盖率 75.79%（-0.38pt，全部来自提醒/闹钟句让路上云，
方向正确、阈值 1pt 内）；真栈原句复验 → `reminder_card`「好的，明天 22:00
提醒你：冷萃咖啡过滤」零车控动作（探针建的提醒已按具体说法定点清理，
用户既有提醒未动）。
