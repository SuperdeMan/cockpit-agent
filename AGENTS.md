...
| safety focus 持续阻断后续 charging plan（T47） | 安全状态解除时机的产品裁决 | **已裁 A、已发布 `0d414816`、真栈 T47 格 3/3 `charging.plan`**（`alert_resolved` 一份判据四个消费方）；`dest_choice` 挂起吞安全陈述已修并发布 `b342e3bb`；五条提前出口跳过输入侧登记的漏点 `_register_input_facts` 待发布；收口页 §3.3–3.5 |
