...
- **提醒存量处置**（P1-07 的遗留）：云库有 ~80 条过期提醒 + 若干垃圾标题（见 C10），
  删除属红线动作，**先出逐条清单、经泓舟点名再删**（同 2026-08-16 数据清洗批的口径）。
...
- **B（必做，独立于 A）**：挂起不冻结兄弟步——NEED_SLOT 只挂起**该步及其下游依赖**，无依赖的兄弟步照常执行完再挂（T50 的 navigate 与 nearby 无依赖）。这需要动 engine 的挂起语义，**单独评估**：多步部分完成后的恢复语义（恢复时跳过已完成步——`completed_results` 机制已存在，缺的是「挂起前先跑完无依赖步」的调度顺序）。
...
- **D（必做，数据治理）**：过期条目加终态流转：fired 超过 N 天自动转 `expired`（新状态或复用 cancelled+reason），不再参与 ACTIVE 查询与序数参照；hygiene 脚本加「过期沉积」召回族（dry-run 先行，**删除仍走人裁点名**——本轮那 80 条先出清单给泓舟）。探针跑批结束补 reminder 清理段（按 run 号标题匹配自己建的、逐条取消并验证计数回落）。
...
- 裁决需要泓舟确认的一句话：**「真栈出现如实标注的 mock，算不算 QA 失败」**——①②都只是把这个判断显式化。
...
**三处要泓舟拍板的**：① C15 的「如实标注的 mock 算不算 QA 失败」；② C10-D 的过期提醒状态流转（schema 变更红线）与 80 条存量的删除清单；③ C5-B 改挂起语义（执行调度是编排核心行为变更）。
