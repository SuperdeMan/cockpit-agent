...
**增量 —— 信源质量加权（✅ 已落地 2026-06-27）**
原信源选择**只按相关性**（Exa 神经排序），学术/官方与内容农场平等进池（实测「混少量内容农场」）。
新建中立内核 `agents/_sdk/source_quality.py`：`domain_tier(url)→3 学术/官方/百科·2 权威媒体·1 默认·0 内容农场`
（覆盖国际+中文学术期刊/院刊/出版平台与行业媒体，含 `.edu/.gov/.edu.cn` TLD 与 `docs./developer.` 启发）
+ `rerank_by_authority`（**稳定**排序：档位降序、同档保留检索相关性序，**只重排不丢弃**——低质源沉底、
仅 top-N 不足时才用到，诚实优先）。接入两处（改一处全覆盖）：① 深调研 `synthesize` 合成前按权威重排每子
问题证据（决定哪几条进 top-N 材料）+ `_assign_global_sources` **按权威全局编号**（`[1]`=最权威，来源区/引用
都以学术打头）；② 共享 `grounded_synthesis`（info.search 同源）合成前对来源按权威重排进 top-5。
**学术兜底**（仅深度异步、不收窄整体）：某子问题证据「薄」(<2) 时用 Exa `category="research paper"` 补权威
文献回填，不新增小节。*验收*：真栈探针「深入调研固态电池」前 5 来源平均档位 **1.00→2.80**（idx1-4=材料导报/
