...
**5836 passed / 14 skipped 零红**（2026-08-16 QA 批阶段 3·Q13 后实测，26m50s）。
较阶段 2 的 5739 净 **+97**，逐条点号：`test_classifier_exit_parity.py` 93 +
...
| **3 T2 语义与真实性** | 🔄 **进行中** | **Q13 ✅ 完成**（两个分类出口收敛成唯一实现，分歧 15/38→0/38；三条守卫见卡 §4；真栈「暂停音乐」154ms 端侧秒回。**它让 Q7 变得可测**：极性缺陷从「云侧判错」变成「端侧当场按反」，验证路径确定性、零 LLM）→ **下一步 Q2** 候选集升格 → Q7 极性/顺序/省略 → Q5 记忆写入闸 → Q12 槽值保真 → Q6 执行账本 → Q10 查单绑会话；**清洗 `--apply` 排在 Q5/Q11 之后** |
