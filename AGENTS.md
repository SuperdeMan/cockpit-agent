...
| 生产 release | `d532c6d816e662fa60740565d6230774c02ff200` |
| 上一生产基线 | `74852a737a53e2bd75f72087ee94834cccc63874`（再上一跳 `d425b9c`）；本轮未执行回滚 |
| status | 2026-09-11 `d532c6d` 发布后独立复核：5/5 endpoint healthy，零 warning，`running_release_sha` 与 `release_sha` 一致 |
| verify | `verified`；artifact `20260911T111910Z-d532c6d.json` |
| 代码验证 | 本机全量固定口径 **8233 passed / 32 skipped / 0 failed**（绑 `d532c6d`，`TZ=UTC0`、`-n 8 --dist worksteal`；上一基线 8231/32 绑 `74852a7`）；同轮 mobile 951 + tsc/lint 0（绑 `e95d077`）、四道门禁 + `smoke_edge` 13 全过；hmi / gateway 本轮未动、未重跑。历史：`74852a7` 时 mobile 876、hmi 333、`go build/vet/test ./gateway/...` 全绿；AR05 批 **8205 / 32**（绑 `00d1925`）；其后 `d425b9c` 跑 orchestrator+security+registry+scripts 3710、mobile 792 + tsc 0、hmi 333 + build、`go build/vet/test ./gateway/...` 全绿。`573ad46` 的 CI 八任务 success 是历史读数，不转借 |
...
| 证据边界 | 当前部署/status/verify 绑定 **`d532c6d`**（2026-09-11：push→dry-run→apply 经用户一次授权，CI/CD digest `a50e46e3…` 未变、无 blocking_changes；该次 deploy 只带上批 G 的服务端改动，其余 9 个提交是 mobile/docs）。批 G 真栈复验绑 `d532c6d` + OPPO 追加包 `e95d07725`（APK SHA-256 `069a45a4…052c`）：「open the trunk」的 Dock 标题读到「打开后备箱」。Android 打磨批 A–F 的真机证据绑固定包 `7525784b6` / 最终包 `7fc8d9894` / 追加包 `e95d07725`，逐态在 [打磨批文档](docs/design/2026-09-10-android-ui-polish-batches.md) 文末回填。上一跳 **`74852a7`**（2026-09-10：push→dry-run→apply 均经用户单独授权，含 CI/CD digest `a50e46e3…`；该次 deploy 同时把上一轮已 push 未部署的 AR06～AR09 整批带上了生产）。受限身份请求面、E-06 车控负例、`running_release_sha` 对账三项真栈证据均绑 `74852a7`；本轮真机证据绑固定包 `1f241c8c5`（APK SHA-256 `4c2c16bd…6985`）。AR05 早前的真栈契约证据仍绑 `d425b9c`；OPPO 固定包绑 `d425b9c2d`（其后的设置页文案修复未进该包）；锁屏提醒绑 `573ad46`、手册整本与旧全量各自保持 SHA；后续 docs/test 不冒充 release。2026-09-10 的 AR05 V01–V12 补测**只读不改**：真栈读数绑运行中的 `d425b9c`（开工前独立 `status` 5/5 healthy、零 warning），真机截图绑固定包 `d425b9c2d`，本轮未 deploy、未 verify、未改产品代码 |
...
| Android 语音输入采纳 | [实施与验收](docs/design/2026-09-11-android-voice-input-acceptance.md)：`f8fd151` 完成来源透传、手动录音拒识入口、回声待核对及终态恢复；本地全量 8234/32/13 warnings、mobile 962，OPPO 已装同 SHA prod 包。云端仅 dry-run、未 push/apply，声学误采纳与误拒绝率未验。 |
