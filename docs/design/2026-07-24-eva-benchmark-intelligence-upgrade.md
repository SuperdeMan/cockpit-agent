...
> **总体验收（2026-07-26）：M0a→M4 六期全部完成并通过跨阶段组合验收**——两个确认链 P0
> 与一批组合缺陷当日修复，结构性遗留已立卡；验收报告与修复/立卡清单见
> `docs/reviews/2026-07-26-acceptance-review-m0a-m4.md`（M3 DoD P2-③「CDP 用例」在该报告
> §1 改口径为 e2e_mcp 承载）。
...
- **落地记录（2026-07-25 续，记忆图谱 P0/P1/P2 全落地；细节见子 RFC §10）**：三个决策点泓舟拍板「都按你建议的来」后单日完成。**P0 偏好加权**：`memory_item` 加 weight/evidence_count/half_life_days/consent 四列 + `memory/weighting.py` 纯函数（显式偏好不衰减、推断类 90 天半衰期、重复证据每次 +0.1 封顶 +0.4）+ 巩固期「等价→加权」（就地更新不刷新 `valid_from`——那是衰减基准）+ 召回打分与注入两段式渲染（强度人话词、top-N 3→5）；**存量兼容是硬要求**：weight=0 逐字回confidence 口径。**P1 关系边**：`memory_relation` 新表 + 封闭 rel 词表 + 一跳解析 + **GDPR 同事务级联删（红线）** + navigation 人称目的地消费——「导航去接孩子放学」→ 阳光小学真栈走通（§1.2-E2 的 Eva 例子）。**P2**：routine 时间加权（旧习惯自然沉底）+ emotion **prompt-only 不进 tool schema**（B4-1 教训复用）→ HMI 下一轮 TTS 语气。**对母提案 §4.D 的两处修正已兑现**：不建 `preference` 新表（真缺口只有三个字段，建表会推翻2026-06-25 单表合并决策）、emotion 不进记忆层（短 TTL+不入画像=会话态；TTS 要的是当轮情绪）。验证：全量 pytest 零回归、`e2e_memory_graph.py` 五场景全绿（含 GDPR 级联红线）、journeys regression 13/14+1 数据真空 skip（红灯逐条复验为方差：A2-3 充电编织概率性、B3-4 和风 provider 瞬时不可达，均单跑即绿）。真栈三坑=亲属称谓自然语言变体（「用户的女儿」存进去就查不到→入库归一）、口语「接我妈」（词表漏裸「妈」+ filler 漏「我」致静默不触发）、播报要用自然称谓。**M2 至此全部完成。**
...
- **落地记录（2026-07-25，P0/P1/P2 单日完成；全量细节见子 RFC `2026-07-25-m3-proactive-engine-mcp-bridge-rfc.md` §10）**：
  - **口径修正**：§4.E 说的「四路主动」实际是**六路**（deep-research 三处 + info 早报是 M1 之后新增，提案盘点时口径没跟上）；迁移清单按六路 + 位置提醒 = 七路。「复用 scene 的三态求值器」复用的是**设计不是代码**（跨包 import 会让基础设施服务反向依赖 Agent 包）。
  - **P0 统一主动引擎**：新服务 `proactive/`（契约登记 conventions §9.8）。六道闸=情境断言投递期复核（unsat/unknown 一律丢）/ **跨生产方**去重 / 免打扰（默认不启用，§9-3 拍板）/ 驾驶负荷延后（**unknown 放行**，唯一故意背离「unknown 不打扰」处：拿缺数据定罪会把冷启动那 30 秒静默掐死）/ 全局频控 / 同窗合并成一条。**落点判据是「它必须可以随时死掉」**——生产方走 request/ack，没人 ack 就直发老主题，停容器即一键回退（真栈手验 `NoRespondersError` 毫秒级回落）。**单条通过 = 剥治理键后原样转发**，下游 Go 网关与 HMI 零改动。**零 kind 字面量**由源码断言测试钉死。
  - **DoD 场景补齐**：此前**没有任何生产方会因为低电量说话**（road-safety 只看天气预警），故新增 `charging-planner` 低电量顺路建议；它与 scene 低电量触发同窗到达 → 合并成一条（e2e 实测「电量只剩 18% 了…另外，要开启省电出行模式吗？」一条消息）。拿不到桩就整条不发（铁律③：主动播报不该拿「服务不可用」打扰用户）。
  - **P1 位置提醒**：`kind=location` + 地点数据存既有 `extra` JSONB（字段级对照后**不加新列**——它只在按 kind 选出后被读，从不参与过滤排序）；ETA 族词形主动让路（防两条链路抢同一句话）；**地点解析不出就诚实追问，绝不存一条永远不会触发的提醒**；首次观测只播种不触发（人已经在目的地时创建提醒，立刻响一声不是用户要的）。
  - **P2 受控 MCP 桥**：`agents/mcp_bridge/`（契约登记 conventions §9.9）。三重准入=版本逐字锁定 + tool 白名单（server 多提供的直接忽略）+ schema 指纹；**写操作五项缺一不接**（幂等键=**请求指纹**不是 task_id、订单状态机复用 `task_ledger` 不建表、timeout 报「不确定」不假装成败、`compensate_tool` 缺失即拒载、审计进结构化日志）。首批演示商户**三重诚实标注**（`_prov.mode=mock` + 卡片角标 + 话术前缀）。**第三方互操作已验**：官方 `@modelcontextprotocol/server-everything` v2.0.0 握手/列表/调用全通，并确认 MCP 区分「协议错误」与「工具执行错误（result.isError）」两条路径。
  - **顺手修的存量缺陷**：road-safety 把每 30s 的**全量车况快照**当成「进入新区域」，无条件查天气预警——一个查不到的地名每 30 秒打一次和风 400，**把共享的 qweather 熔断器打开**，天气域跟着一起垮（journeys 因此红 3 条）。教训可复用：**周期性全量快照会把「变更回调」变成「定时器」**，语义是「变了才做」的消费方必须自己比对上一次。
...
- **落地记录（2026-07-25/26，P4 声纹 + 视觉收官；全量细节见子 RFC `2026-07-25-m4-p4-voiceprint-vision-rfc.md` §11）**：
  - **M4 两条 DoD 至此全部兑现**：「语音双链路可切换」由 S2S 线兑现，**「多用户记忆隔离旅程」由本批 P4a 兑现**（真栈 e2e 7 场景全绿）。
  - **侦查推翻了这一期的重心**：记忆层的 occupant 维度早就全套就位、`recall` 本来就是 occupant 精确过滤——**缺的从来不是记忆能力，是身份来源和一条透传管道**（`PlanContext`/`_sdk.Context` 里压根没有 `occupant_id` 这个字段）。于是 memory 侧零改动，只把既有字段沿既有 meta 管道多传一段（七个改动点）。
  - **P4a 声纹**：网关 `speaker_embed`（音频→192 维向量）× memory `voiceprint`（向量→是谁 + 模板表 + GDPR 级联删）分家——**模板绝不下发到无状态服务**，生物特征扩散了就删不干净。识别走**独立端点不旁路 ASR/S2S 流**（那两条刚验完，可选增强件不该焊进关键件）；**边说边识别**（1.5s 即发，send 前软等 ≤150ms）；**一次唤醒锁一次**（轮内改判会同时污染两个人）；**认不出一律回 primary**（不是 guest——那等于凭空造一个空记忆空间）。
  - **探针把三个阈值从经验值钉成实测值，并推翻一个想当然**：单句余弦分布**是重叠的**（同人 p5≈0.49 / 异人 p95≈0.65，同性别合成音色互相很像），但端到端识别在 thr∈[0.45,0.70] **结果完全相同**——**真正起作用的控制量是 margin 不是 threshold**；两次跑认对率 83%/100%、**认错率恒 0%**（混淆对被 ambiguous 档拦成 primary，而不是认错人）。`min_consistency` 按实测从 0.55 下调 0.50（原值高于合格注册下界，会误拒合法注册）。
  - **红线：声纹不作鉴权因子**，源码级断言 9 条钉死（`occupant_id` 不得进 granted_scopes/权限/VAL/确认闸/支付）。理由不止「可被录音重放」——**身份识别与授权是两件事**，识别错了只该损失个性化。
  - **P4b 视觉**：新 Agent `agents/vision`（50077）+ 网关内存帧库。**图像永不进对话链**（proto 里只有 16 字节 frame_id，图像本体 TTL 120s 内存 LRU，不落 Redis 不落盘）；**采集门控在端侧**（命中触发词才抓一帧，默认一帧都不采）。
  - **两条被实测逼出来的设计修正**：① 看图必须走**独立 `qwen-vl` 档**——实测 `qwen3.7-max` 对多模态 content 直接 400，而档位解析对不认识的模型是**静默回落 primary**，不独立成档就会把看图请求打到看不了图的模型上**且毫无报错**；② 帧过期必须**显式失败**——静默只发文本时 VL 模型答「看不清，画面有点模糊」，**它在假装看到了一张模糊的图**，比说不出更糟。
  - **新 scope `camera.frame` ≠ `camera.read`**：后者是连续流、conventions §3 维持 ❌ 禁。沿 `location.read`/`location.precise` 的精度分级先例，**不为了让 Agent 可路由就把连续流权限偷偷放开**。
  - 验证：全量 `pytest` **2270 passed / 7 skipped**（M4 S2S 基线 2189，净 **+81**）；HMI node **192**（+21）；tsc 21=基线、build 通过；真栈 `e2e_voiceprint` 8/8、`e2e_vision` 6/6，两个新 e2e 已挂 `run_e2e`（模型/key 缺失自动 SKIP）。
  - **真机三修（2026-07-26，子 RFC §11.6）**：①**我引入的回归**——为等声纹结果把控制器 `onSend` 改成异步（150ms 软等待），破坏了「用户气泡由 send **同步**接管」的不变量，对话窗回答与问题错位；**修法是整个删掉而不是缩短**（识别在说到 1.5s 时就发出、端点还要再等 800ms 静音尾，这 150ms 几乎不生效）。②**名字只写在 `voiceprint` 表里答不出「你知道我是谁」**——那张表除了比对没有任何消费方会读；补 `identity.name` 记忆 + `occupant_name` meta 注入。③注册交互缺「进行中反馈」与「完成后自证」→ 每段独立可重录 + 倒计时 + **「试一试」当场验**。
  - **余项**：跨乘员共享记忆 v1 不做（§10-2 拍板「纯隔离」）；`min_consistency` 的 0.05 窗对真人可能要重调；真麦声学验收（识别率/误认率）同 R4.3 惯例留泓舟；声纹语音注册入口、模板漂移巩固、视觉多轮追问均 v2。
