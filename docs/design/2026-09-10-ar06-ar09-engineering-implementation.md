...
## 附录 C：AR09 性能矩阵首批读数（P0/P3/P5）——两个真缺陷，其中一个推翻了我自己的第一读法

> ### ⚠ 2026-09-10 晚：C.1 的 F1 已被单变量实验推翻，C.2 的 F2 随之改写
>
> **F1 量到的是装置自己的走法，不是 App 的缺陷。** 逐条更正与真正的缺陷见
> [AR01～AR11 余项收口](2026-09-10-ar-residuals-closeout.md) §2。一句话：
> p3 当时用深链一路前进、**从不返回**，于是 30 轮之后有 120 个屏活在导航栈里；
> 换成用户真实路径（进页面后按返回键）之后，同一台设备、同一个包、同一装置，
> Views 静置后**逐字回到冷基线 151**、PSS 回到 256MB。屏是会被正确释放的。
>
> 下面 C.1/C.2 的表**原样保留**（读数本身没错，错的是它被读成了什么），
> 但**不得**再被引用为「内存泄漏」的证据。

> 装置：[`scripts/probe_ui_perf.py`](../../scripts/probe_ui_perf.py)，只用 adb 读设备自己持有的事实
> （`gfxinfo` / `meminfo` / `am start -W`），**不改任何设置、不发业务、不采集音视频**。
> 12 份原始 JSON 在仓库外 `%LOCALAPPDATA%\car-agent\artifacts\ar09-ui-perf\`。
>
> 冻结条件：APK `f3825e28…`（`7481cb5ee-dirty`，prod、非 DEBUGGABLE）；OPPO PEUM00 / Android 14；
> **活跃屏是外屏 988×1972 @440dpi，刷新率 60Hz ⇒ 帧预算 16.667ms**（内屏那块 120Hz 的 `state OFF`，
> 拿它当分母会把结论说反）；电量 100%、29.8℃、省电关；免唤醒**关**（0 个活动录音轨，最近一次录音是两天前）。

### C.1 F1（严重）：路由循环让进程内存单调涨，静置两分钟不回落

30 次 `chat → settings → vehicle → map` 循环，每次都从**冷进程**起算：

| 臂 | 冷基线 PSS | 循环后静置 2min | 增长 | Native heap | Graphics |
|---|---|---|---|---|---|
| A：不含 map | 213 MB | **761 MB** | **+548 MB（+257%）** | 49 → 414 MB | 33 → 42 MB |
| B：含 map | 212 MB | **1313 MB** | **+1102 MB（+520%）** | 49 → 508 MB | 33 → **308 MB** |
| B′：含 map（复跑） | 212 MB | 1309 MB | +1097 MB | 49 → 539 MB | 33 → 307 MB |

AR09 §4.1 的建议门槛是「静置 2min 后 PSS 相对预热基线增长 ≤ `max(20MiB, 10%)`」。
实测 **超出约 27 倍（不含 map）/ 55 倍（含 map）**，且**不是**用强制 GC 掩盖过的读数——本装置从不触发 GC。

单变量归因（两臂只差一条路由）：

- **地图解释 Graphics 那 275 MB**（33 → 308 MB 只在 B 臂出现）；
- **Native heap 那 ~365 MB 在不开地图时照样涨**——那不是地图，是每次路由挂载都留下的东西。

⇒ 至少**两个**独立的增长源。修的时候不能只盯地图。

### C.2 F2：99% 卡顿是 F1 的**后果**，不是对话屏的属性

对话页空闲 180s，同一台设备、同一个包、同一装置，只有**进程状态**不同：

| 进程状态 | PSS | 帧数/180s | janky | p50 | missed vsync |
|---|---|---|---|---|---|
| 冷进程 | 216 MB | **10795（≈60 fps）** | **0.01%（1 帧）** | 27 ms | **0** |
| 撑大后 #1 | 1303 MB | 4906（≈27 fps） | **99.43%** | 69 ms | 4855 |
| 撑大后 #2 | 1308 MB | 4969（≈28 fps） | **98.83%** | 69 ms | 4866 |

**我第一小时的读法是错的。** 首批读数里对话页 99.43% janky 是在跑完 30 次路由循环之后测的，
我差点把它写成「对话屏常驻动画扛不住 60Hz」。冷进程复跑当场推翻：同一块屏 **60fps、0.01% jank、
零 missed vsync**。两次独立的「撑大后」都回到 ~99%，因果方向因此是钉住的：
**内存涨上去 → 渲染掉到 27fps**，不是反过来。

### C.3 F3（背景事实，本身不是缺陷）：只有对话页在空闲时持续渲染
...
同一装置在两条路由上报 0 帧、在另一条报 10795 帧 —— 这也是**装置有分辨力**的内部对照：
「对话页那个数」不是仪器恒亮造成的。

对话页的常驻动画是一笔真实的持续负载（60fps 满帧渲染），但在健康进程上它**在预算之内**。
要把它归到「极光动画」这一层还需要 `reduceMotionForce` 的开/关对照——见 C.5，那一臂被工具卡住了。

### C.4 F4：冷启动

`am start -W` ×10，TotalTime **p50 590ms / p95 620ms**（min 570 / max 620，nearest-rank）。
⚠ 这是系统报的「显示出来」时刻，**不是「可交互」**；AR09 §4 的 P5 要的是后者，本轮没有测到。
...
| P2 语音并发 | 需要真人说话 |
| P4 省电/低电量 | 需要改系统设置并回读，且要真实低电量 |
| 触摸响应 p95 ≤100ms | 需要可重复的输入注入 + 帧时间对齐，本轮装置不测输入 |
| 诊断开/关的开销对照 | 轮次时间线是**常驻有界摘要**（AR09 §3.1 允许），没有开关可比；高频采样档尚未做 |
| P3 三轮 | 本轮只跑了 1 轮 ×3 个配置（A/B/B′），不是同一配置的 3 轮 |

### C.6 归属

F1/F2 归 **AR09**，是这一批要修的正题；修完必须在**同一协议**上复测，
且**不能**用「换个包重测」来绕过——`probe_ui_perf.py` 的每份 JSON 都带包身份与设备指纹，
换包换设备的读数不许并进同一张表。

> **2026-09-10 晚更正**：这条「归属」本身是对的，但对象错了。F1 不是 App 的内存泄漏，
> 修的对象是**装置**（p3 的走法）与**一条真实存在、但小得多的缺陷**（外部深链只推不弹）。
> 见 [AR01～AR11 余项收口](2026-09-10-ar-residuals-closeout.md) §2。
> 上面那句「修完必须在同一协议上复测」在新结论下仍然成立，而且更要紧：
> **协议本身变了，旧读数与新读数不可并表。**
