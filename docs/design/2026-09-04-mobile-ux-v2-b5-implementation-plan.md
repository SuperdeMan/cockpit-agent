...
**开工基线（2026-09-05，本会话自己跑出来的数）**

| 项 | 读数 |
|---|---|
| `check_android_env.ps1` | 退出码 **0**（18 pass / 0 warn / 0 fail） |
| `dev_stack target show` | `{"source":"file","status":"target","target":"cloud"}` |
| adb server（`:5037`） | LISTENING **PID 23900** = `D:/Android/Sdk/platform-tools/adb.exe`（= `ANDROID_HOME` ✅，**与第 2 批同一 PID**，本轮没自发重启）。孤儿 PID 14604 仍在，未碰 |
| `mobile: npm test` | **51 suites / 514 全绿**（54.5s）——与 §6.2 收口的 514 对得上 |
| `mobile: npm run typecheck` | **0 error** |
| 设备 `lastUpdateTime` | **`2026-09-04 23:41:40`** ✅ = §6.2 T10 记的新主线包锚（**不是 §0 第 1 条写的旧锚 `09-02 16:45:36`**） |
| `git log --oneline origin/main..HEAD` / `HEAD..origin/main` | **空 / 空**；`git status` 干净；起点 SHA `5e4b79b`（= `origin/main` = HEAD） |
| Metro | 8081 `packager-status:running`，PID **75696**（本仓，`CreationDate 2026/9/3 22:26:56`，非本会话所起）⇒ 按纪律不停不重启 |
| `adb reverse` | 开工时 **空**（第三轮确认它会自己掉），本轮重建 `UsbFfs tcp:8081 tcp:8081` |
| `haptic_feedback_enabled` | **1** ⚠ §6.2 收口记的是 **0** ⇒ 这一位在本机确实会自己动（第二次观测到）。本轮**没有动它** |
| worktree | 未分树（泓舟未授权），主工作树 |

**⛔ 开工即撞到的两条设备侧阻塞（决定了本批哪些格子取不到）**

1. **设备被人在用**：开工核 `mCurrentFocus` = **`com.tencent.mm`**（微信小程序），外屏 `mState=ON`、
   `mWakefulness=Awake` ⇒ 按 §0 第 1 条纪律**不抢焦点**，先做完全部宿主侧任务（T12–T15 的判据、
   组件、jest），设备空出来后才取证。
2. ⛔ **Maestro 主包又不在了**：`pm list packages | grep maestro` 只剩 **`dev.mobile.maestro.test`**，
   主包 `dev.mobile.maestro` 不在 ⇒ `hierarchy` / `test` 一律 `INSTALL_FAILED_USER_RESTRICTED`。
   `hierarchy --no-reinstall-driver` **也拦不住**（它照样先装）。
   ⚠ **这与 §6.2 T10 步骤 3 记的「重装后仍在 ✅」矛盾**——两个包不是一起消失的，隔一天主包自己没了。
   ⇒ **T16 步骤 4（Maestro 01/02/03/06/08/09 回归）本轮取不到**，需泓舟在开发者选项里放行 USB 安装。
   下一轮开工把 `pm list packages | grep maestro` **数两条**列进固定五步。

**逐任务**

**T12（语音层去底栏 + 顶缘把手带）** — 提交 `aef13d3`（4 files，+188/−191）

- 步骤 1 先改判据与测试 ⇒ **实红 4 条**（计划没数条数，这是实测）：`缺陷 A 症状二`、`外屏竖三档`、
  `split 0.78`、`最小高的构成`。`Expected: 240` 与计划手算的「0.4 档行车最小高 = 56+32+120+12+20 = 240」逐 dp 对上。
- 步骤 2 实现后**仍剩 2 条红，而且是事实变了不是测试写错**：
  **撤底栏省下的 chrome 恰好 29dp（把手 12 + 底栏 17），而 B4 时 0.78 档的缺口正好也是 29dp**
  （内屏纯比例 447 < 当时下限 476）⇒ 下限降到 447，与纯比例**逐 dp 相等**；外屏竖 0.78 下限 447 <
  比例 451 ⇒ **这一档从「受下限」变成「走比例」**。两条用例按新事实改写，并各补一个下限仍绑得住的
  容器（内屏那条补 `call(500,0.78)`）**保住判别力**——否则 `call >= need` 在 447=447 处退化成恒真。
  ⚠ **下一轮真机量 0.4 档层高应是 240 不是 B4 的 269，别拿旧读数对**（已写进测试注释）。
- 反向验证（副本回写，未用 `git checkout --`；每条先 `grep` 证明变异落盘）：

  | # | 变异 | 计划预期 | **实红** |
  |---|---|---|---|
  | M1 | chrome 里把手带写死 56 不过 `scale` | 恰红「大字档」 | **恰 1 条**（`最小高的构成`）✅ |
  | M2 | 删掉 `SCROLL_PAD_DP` | 全部构成用例红 | **5 条**（含两条新改写的）✅ |

- 步骤 3 组件：底栏整段删除（`voice-sheet-collapse` + `voice-sheet-interrupt` 两个 Pressable）；
  外层 `GestureDetector` 去掉、Pan 只包把手带；`onInterrupt` prop 删除；`busy` 局部变量随之删掉。
- 步骤 4：`tsc` 0、全量 **514 不变**（组件层）。

**T13（发送与打断合一 ⬆·■）** — 提交 `61cb59c`（2 files，+23/−23）

- 步骤 1 `Select-String -Recurse` 在本机 PowerShell **不存在这个参数**，改用 `grep -rn 'name="send"' src/`
  ⇒ **只命中 `Composer.tsx:268` 一处** ✅ ⇒ 按计划把 `send` 换成 `arrowUp` / `stop` 两条。
  `returnKeyType="send"` 是 RN prop 不是图标，不受影响。
- 步骤 2/3：原「■ 打断」pill 整段删除；`composer-send` 忙闲两态合一；`tsc` 0、全量 **514 不变**。

**T14（Scanner 三条）** — 提交 `4de8f69`（4 files，+80/−13，新建 `test/theme.test.ts`）

- `theme.ts` 的两份 Palette **名字已经是 `DARK` / `LIGHT`**，只是没导出 ⇒ 只加 `export`，不另起名。
- 步骤 2 跑红 ⇒ **实红 3 条**（与计划手算逐条对上）：`dark fg3` **2.995**、`light fg2` **4.124**、
  `light fg3` **2.095**；自检两条绿。
- 步骤 3 一次过线，**不需要 0.02 步进**。实际对比度（跑出来的数，不是手算）：

  | 项 | 旧值 | 旧对比度 | 新值 | **新对比度** |
  |---|---|---|---|---|
  | dark `fg2` | .58 | 6.877 | 不动 | **6.877** ✅ 本来就达标 |
  | dark `fg3` | .34 | **2.995** | **.48** | **4.971** |
  | light `fg2` | .55 | **4.124** | **.62** | **5.210** |
  | light `fg3` | .32 | **2.095** | **.60** | **4.867** |

- 步骤 4 全量 **514 → 519（+5）**、`tsc` 0。反向验证：

  | # | 变异 | 计划预期 | **实测** |
  |---|---|---|---|
  | M1 | dark `fg3` 改回 .34 | 恰红 dark fg3 | **恰 1 条** ✅ |
  | M2 | `composite` 忽略 alpha | 四条全绿 | **5 条全绿** ✅ |
  | **M2′** | M2 **＋ 把色板一并退回未修的 .34/.55/.32** | （计划没要求） | **四条对比度仍全绿** ⇒ **这才是「alpha 先合成」那半的真反证**：不合成会把 B4 那份不达标的色板整体判成达标。只做 M2 只能证「变异不红」，证不到「它会漏掉什么」 |

  ⚠ 顺带一条：`公式自检：黑白 21:1、同色 1:1` 那两条在 M2 下**仍然绿**（黑白与同色都是不透明色）
  ⇒ **自检对「合成」那半零敏感**，它守的是别的东西，不要拿它当合成那半的护栏。

**T15（缺陷 A 横屏半）** — 提交 `a7b01e7`（5 files，+143/−31）

- ⛔ **步骤 1「先量」本轮没做**：要物理横屏 + 角色 C + 行车档，横屏是人手动作。
- ⚠ **因此期望值没有按计划步骤 2 写的 `columnH = 98.67 + 157` 去拼**：那个 157 是 **B4 读的**
  chips+输入区高，而 **T13 刚撤掉「■ 打断」pill、T14 刚把顶栏钮 40 → 48/56** ⇒ B4 的两个数在本批之后
  已经不成立，拿来填期望值就是 `sheetHeight.test.ts` 头注禁的「估值冒充实测」（那条头注正是被 728dp
  那次坑出来的）。**改用不依赖新实测的边界表达**：阈值 = 0.4 档最小高本身，两个字号档各验「恰好装得下
  ⇒ 120 / 差 1dp ⇒ 88」。真机落在阈值哪一侧归 T16 待泓舟。
- 新增 5 条（`sheetOrbDp`）：容器够 ⇒ 120 / 外屏横实测 **98.67** ⇒ 88 / 泊车永远 88 / 阈值边界两档 /
  球降了最小高跟着降（差额恰是两个球径之差）/ `sheetHeightDp` 把降级球传下去。全量 **519 → 524（+5）**。
- 反向验证：

  | # | 变异 | 计划预期 | **实红** |
  |---|---|---|---|
  | M1 | `sheetOrbDp` 恒 120 | 恰红「98.67 ⇒ 88」 | **3 条**（另两条是新加的阈值边界与传参链，都该红） |
  | M2 | `sheetHeightDp` 不把 `orb` 传下去 | 红「整列」那条 | **恰 1 条**（`把降级后的球传下去`）✅ 判据有分辨力，不是 fixture 恰好装得下 |

- 顺手纠一处头注：`OUTER_LANDSCAPE = 192` 是 B4 的**推导值**，B4 §6.4 后来**直接量到 98.67dp**（差一倍）。
  老用例的读数绑在 192 上不动它，**B5-15 的新用例一律用 98.67**，头注写清两者关系。
- 布局（lever ②）：`driving-landscape` 下 `hideChips`、语音层挪到整列容器并读 `columnHeight`、
  层内大球接 `onOrbTap`。**非 split 路径的渲染树逐字节不变**（层仍住在记录区容器里）。
  球径两个分支都读 `sheetOrbDp` 的同一份结果——不这么做就会「判据按 88 算空间、渲染画 120 的球」。

**T16 第 3 批真机轮 — 单人能取的都取了，需泓舟的四格 ⬜**

**先证仪器再用仪器**（§6.2 同一条）：设备重启 App 后落到 `DevLauncherActivity`（**看起来像坑 74 的
Metro OOM，但 `/status` 仍 `packager-status:running`、进程 PID 75696 364MB 活着**）——真因是
`force-stop` 把 dev-client 的已连服务器清了。用 `xiaozhou://expo-development-client/?url=...` 接回去
之后，主屏截图上**发送键已是 ⬆ 箭头、顶栏两枚钮明显变大** ⇒ 设备跑的是本轮 JS。

**步骤 1 目标 ≥56dp 新演员（`uiautomator dump` + `target_probe --density 480`）**

⚠ 打开路径是 **App 内**设置 → 实验室 →「减少动效（强制）」（**不是系统设置**，§6.2 坑 ⑬ 记错了）。
开了之后同一屏从「拿不到 idle / 被 kill」变成 **56–60KB 完整树**。用完已关回并回读。

| 演员 | 泊车（目标 48） | 行车 + 角色 C（目标 56） | B4 读数 |
|---|---|---|---|
| `voice-sheet-collapse`（**把手带，新演员**） | **358.0×48.0dp PASS** | **358.0×56.0dp PASS** | 底栏那枚 96.0×56.0 |
| `health-dot` | **48.0×48.0dp PASS** | **56.0×56.0dp PASS** | **40**（两态都不达 48） |
| 顶栏「车辆」（按 `content-desc` 读，无 testID） | **48.0×48.0dp** | **56.0×56.0dp** | **40×40** |
| 顶栏「设置」 | **48.0×48.0dp** | **56.0×56.0dp** | **40×40** |
| `composer-orb` | 56.0×56.0 PASS | 56.0×56.0 PASS | 同 |
| `composer-send` | 44.0×44.0 PASS（泊车阈值 44） | **56.0×56.0 PASS** | 同 |

**阴性三条**（每条读数都配了阴性）：① 泊车树按 `--min 56` 判 ⇒ 三条全 **FAIL**、`rc=1`（探针真在判阈值，
不是恒 PASS）；② `voice-sheet-interrupt` ⇒ **NOT FOUND、`rc=2`**（底栏那枚打断键真的不在了）；
③ 角色 C 行车档下 `composer-input` **不在**树里（§6.0 C 身份输入框隐藏）、`voice-sheet` **在**（行车档层常驻）。

**步骤 6 把手带手势正反（T12 步骤 5，在新包上取的）** — 升层走 `xiaozhou://voice`，
**全程 `dumpsys audio` 的 `active? true` = 0**（免唤醒是关着的，这条判据有分辨力，坑 ⑰）：

| 手势 | 期望 | 实测 |
|---|---|---|
| 把手带向下 `input swipe` 400px/600ms（单进程慢速） | 收起 | ✅ `b5-12-pos.png` |
| **层内球区**同样距离下拖 | **不收起** | ✅ 层顶纹丝不动 `b5-12-neg.png` ——**这条才是 Pan 挪位的判据** |
| 轻点把手带 | 收起 | ✅ `b5-12-tap.png`（另存 `b5-12-t0.png` 证明 tap 前层升着） |
| 点暗区 | 收起 | ✅ `b5-12-scrim.png` |
| 底栏是否还在 | 不在 | ✅ 截图与 dump 双证（`voice-sheet-interrupt` NOT FOUND） |

**T13 忙闲两态（单人可做，发一句英文文本轮——不碰车控 chips，`input text` 送不了中文是坑 ⑫）**

| 态 | 实测 |
|---|---|
| 闲 | ⬆ 极光渐变圆键，`content-desc="发送"`（`b5-16-home.png`） |
| **忙** | **■ 琥珀**（琥珀底 + 琥珀边框 + stop 图标，极光渐变消失），**旧「■ 打断」pill 不在**（`b5-13-busy.png`） |
| 点忙时的 ■ | 气泡定格**「已打断」**（灰字，**不改红** ✅ B2 规则 4）、键**回 ⬆ 极光**（`b5-13-interrupted.png`） |

⚠ 顺带观测到助手气泡里「已打断」**出现两次**（大字正文 + 下方灰字标记）。**不是本批引入**——
Composer 的 `onInterrupt` 回调逐字节未变（原 pill 与新合一键同一个 `cancelCurrentTurn`）。
**只记录，未定性**，交下一批看要不要并进 Scanner 那类账。

**T14 chips a11y**：dump 里三条 `content-desc="快捷指令：打开空调26度 / 打开主驾座椅加热 / 播放音乐"` ✅
出账③ 成立。副文本对比度的**视觉侧证**：设置页那一屏的 `fg3` 描述文字明显可读（量化判据在 jest，不靠看图）。

**⚠ 本批真机抓到一个缺陷，已修（提交 `dd7c6ef`）**

`uiautomator` dump 里 `content-desc="收起语音层"` 有 **2 条**——一条是**暗区** Pressable（360.0×562.7dp），
一条是新的**把手带**（358.0×56.0dp）。**读屏会把同一个动作念两遍**，正是 T14 刚修掉的
「多个项目具有相同的说明」（B4 Scanner 出账③）的同一形态，而**这一处是本批自己造的**
（把手带的 label 是 B5-12 新写的，暗区那条是原有的）。
⇒ 暗区加 `importantForAccessibility="no-hide-descendants"` + `accessibilityElementsHidden` 从无障碍树拿掉；
