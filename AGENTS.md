...
云端 release **`5177198`**（2026-08-19 由 QA Q2 残余批 `deploy --apply` 推上去，
...
> ⑦ **跑全量单测必须带 `--import-mode=importlib`**（即 `make test` 的口径），
> 否则 `agents/*/tests/test_agent.py` 同名模块冲突 ⇒ **一条 collection error 截断整趟**，
> 读起来像「只红了一条」，实际是后面几千条根本没跑。
> ⚠ 同族一条：**跑单测期间别在旁边跑 PowerShell 真栈命令**。
> `scripts/tests/test_e2e_wrappers_ci.py` 那条要启 PowerShell 子进程（单跑 83s），
> 与 `deploy`/`status` 轮询撞在一起就会红；独占复跑 **1091/52 零红**。
...
对抗语料 **596 唯一输入**（上界 596，余量 0）、catalog **145 条**、架构 **v1.30**
...
**探索式 QA 轮的 Q8 与两处卡内残余**（~~Q2 的确定性消费方~~ ✅ 2026-08-19 交付、
Q10 的双入口收敛（+ Q2 的候选集下发面）、
...
流水 history **§41–§53 + §58**，本轮归档索引 **§47.5**。）
...
| **3 T2 语义与真实性** | 🔄 **进行中** | Q13 ✅ / **Q2 ✅（载体 + 确定性消费方，2026-08-19、§58）** / **Q7 ✅ 三维度全收口** / Q5 ✅ / **Q10 ✅ 三块（第四块移第 7 步）** / **Q6 ✅** / **数据清洗 `--apply` ✅** / **Q12 ✅ 主体**。逐卡状态见下表；⚠ 同批掛开一条**记忆语义召回失效**新卡 |
...
| 卡 | 状态 | 残余（**接手要做的就是这一列**） |
|---|---|---|
| **Q2 候选集升格** | ✅ **确定性消费方已交付**（第 6 步完成，2026-08-19） | **CD1 red→3/3 `[det]`、CD4（I-023 首次有探针）red→3/3 `[det]`**，history **§58**。⚠ **卡上那两个缺口上游还有两个洞**，只做卡上写的会「机制通了、读数照红」：白名单 7 个字段名是**猜的**（`open_hours` vs 产生方的 `open_today`）⇒ 营业时间**从来没进过候选集**、且被测试自己的 fixture 盖住；商户菜单 items 只在 `ui_card` 而 `extract_focus` 只读 `data` ⇒ **菜单从来没进过候选集**。**残余=① 下发通道（移到第 7 步，B4 判据：本批没有真实消费方）② I-030 跨组比较**（要先有「哪一组」的指代解析，独立能力） |
| **🆕 记忆语义召回失效（云端）** | 🔴 **新发现、未开工**（← **接手先验这一条**）| 2026-08-19 云端复跑：**XS3 稳定 0/3**（history §51 记的是 0/3→3/3）。症状不是「出处没说」而是**记忆压根没召回**（三次取样分别答「我没法查到哦」/「我并不掌握您女儿的具体信息」/「这个我没办法知道呢」）。psql 只读取证：库里**逐字有**「用户的女儿在深圳市南山实验小学上学」（`predicate=person.child`）、`embedding` **非空**、371 条 `embedding_model` **全是 `llm-gateway`（统一）** ⇒ **不是数据丢失、不是模型不匹配，是召回/注入这一环**。时间线上它夹在 **2026-08-18 切云 + 三存储迁移** 之后；Q2 那一批与记忆域零交集（短路判据已逐条核过，不劫持这句话）。⚠ **影响面可能大于 XS3 一条**：XS1 同批也红（同样依赖语义召回），而 XS2/XS4/XS8 绿（不依赖）。**先验它，别先信 §51 的读数**。history **§58.5** |
...
第 5 步于 2026-08-17、**第 6 步于 2026-08-19** 完成 ⇒ 新会话从**第 7 步**（Q10 双入口收敛 + Q2 下发面）接手**）：
...
| ~~**6**~~ | ~~**Q2 残余**：I-018/I-023 的确定性消费方~~ ✅ **完成**（2026-08-19，history **§58**）<br>⚠ **下发面按 B4 判据移到第 7 步**（云侧消费方成立后它在那一批才有真实消费方）| **它是三条残余的共同前置**（Q10 双入口收敛、Q12 规格维、以及后续任何要引用候选集的卡），而原来排在 Q8 后面——**前置排在叶子后面是排序 bug**。属性已跨轮存住，缺的只是算它的人 + 一条到 Agent 的下发通道 |
| **7** | **Q10 残余**：文本/按钮双入口收敛 **+ Q2 的候选集下发面**（两者同批） ← **接手从这里开始** | ⚠ **2026-08-19 调整**：下发面从第 6 步挪到这里。第 6 步的消费方落在**云侧**（不依赖下发），于是那条通道在那一批没有真实消费方——B4 判据：**无消费方的声明只会漂移**。它真正的消费方就是本步。<br>**必须排在第 6 步之后**：修法是「文本路径先在当前候选集里做确定性匹配」，而 `Focus.candidate_sets` 只活在云侧 `context.py`、**没有下发到 Agent 的通道**（取证见 history **§49.6**）。同理 **Q12 的规格维**（I-025②「少冰」验不到）也卡在这一处 |
...
⚠ **对抗语料唯一输入 596 / 上界 596**（`suites.yaml` 的 `max_cases`，权威值）——当前余量仍为 0。下次加 L0 语料必须先说明新增
能力或边界为何值得占额度，再有原则地调整 `suites.yaml` 的 `max_cases`；不得删旧尺子压数字，
也不得先加语料撞闸后再补理由。560→596 的十次递进与逐项占用写在 `suites.yaml` 头部。
