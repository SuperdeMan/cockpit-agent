...
`python -m pytest -q -n 8 --dist worksteal` = **7710 passed / 32 skipped**。
⚠ **末趟有 1 条红，已定性、不是回归**：`test_e2e_stack_lease.py::
test_restore_failure_is_identity_cleanup_and_overrides_pass`——**隔离复跑 61 passed /
2 skipped 全绿**，命中 §4.2 里已立账的那条（它抢**真实 OS 锁**，被并行方污染；
本机此刻有另外三条会话在同一棵树上工作）。按「隔离」那条纪律处置，**不说 flaky**。
本跳 **7691 → 7710 = +19**，与本批新增用例数**逐字相符**：
...
`orchestrator/cloud/tests/test_engine_confirm.py` +2（重列换题一正一反）。
四道离线门禁与基线**逐字相同**。
...
| B4 | 系统 | **I-024**（编号序列最后一条残余）：用户看得见的选择卡，它的候选也要进候选集 | §9.39 C |
