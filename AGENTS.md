...
| status | 2026-09-11 发布后独立复核：5/5 endpoint healthy，零 warning，running SHA 与 release SHA 均为 `f8fd151` |
| verify | `verified`；artifact `20260911T134859Z-f8fd151.json` |
| 代码验证 | 固定 `f8fd151` 本机全量 **8234 passed / 32 skipped / 13 warnings**，0 failed（`TZ=UTC0 -n 8 --dist worksteal`）；mobile **962** + tsc/lint 0，HMI **333**，smoke_edge **13** + 四道门禁通过。gateway 本批未改未重跑。详情见 [语音采纳实施记录](docs/design/2026-09-11-android-voice-input-acceptance.md) §5 |
...
| 证据边界 | 当前部署/status/verify 绑定 **`f8fd151`**，OPPO 包 `f8fd15152`（APK SHA-256 `6fc093a9…66103`）。来源与拒识出口已贯通，但真实 MiniMax-M3 文字注入续测：正常请求6/6、背景静默拒识6/12；另有首轮播报句误命中端侧 `media.play`，已停止探针造成的模拟播放。**拒识仍未验收，声学未验**；[逐条结果](docs/reviews/2026-09-11-voice-input-acceptance-live-findings.md)。手册整本与旧 Android 验收继续保留各自历史 SHA |
