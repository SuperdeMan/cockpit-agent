...
**第 1、2 批均已于 2026-08-28 落地**：第 1 批 = C1 安全告警链 A-D + C2 端侧三 bug 与
探针诊断出口 + C16 的 1·6·8（另加实施时扫出的 N8/N9 两个新缺陷）；
**第 2 批 = C4**（会话账本扩「数据源」维 + 编排层三条确定性读出口 + 候选集重列算子；
D 项评估后不做，T44 换了个更小的修法当批做掉）。
**下一批 = 第 3 批（C3 wait_slot + C10 提醒域）**。
⚠ **本 release 的云端读数没有因此改变**——两批都只跑了本地全量与离线门禁，
**真栈迷你集与「修正后计分」都还没跑**（探针本身在这两批里都改过）。
...
**最新后端全量基线（2026-08-28 MiniMax QA 修复批**第 2 批**，`target=cloud` +
本地 Docker 已退）**：`python -m pytest -q -n auto --dist worksteal` =
**7397 passed / 32 skipped 零红**（4:06）。本跳 `7314 → 7397` = **+83，全部是第 2 批（C4）新增断言**
...
对账链：**7397**（08-28 QA 修复批第 2 批）← 7314（08-28 第 1 批）← 7225（08-26 发布治理/测试族）← 7106（08-25 MiniMax QA 闭环）← 6969（08-24 MiniMax QA 批）← 6933（08-22 I-030 批）← 6902（08-22 复验批）← 6897（08-21 规格值域批）
...
| 候选 | 为什么现在能做 / 不能做 | 入口 |
|---|---|---|
| **2026-08-26 MiniMax QA 修复批（17 症状卡 → 16 根因卡 C1–C16；六批，**第 1、2 批 2026-08-28 已落地**）** | **可以开工（当前首选）：从第 3 批开始 = C3（wait_slot 反转 + slot_shape 契约）+ C10（提醒域 A/B/C/E/F）**——会话状态机族；C10-D 的过期治理已在 2026-08-27 单独执行完（90 条全清）。✅ **第 2 批 C4 已完成**：账本扩「数据源」维（proto `TurnSource`，写读两侧全接通）+ 编排层三条确定性读出口（挂起状态 / 数据源 / 执行史，判据下沉 `runtime/session_facts.py`、挂点在 plan 构建之前）+ 候选集第四种算子「重列」；**C4-D 评估后不做**（够不到自己那张卡 + 会挤掉真实候选组），T44 换成「回顾指代夺回股票焦点」当批做掉；**T43 归 C11**（第 5 批的防编造面），不是漏做。两处显式裁决见 fix plan「第 2 批终态」。✅ **第 1 批已完成**：C1 安全告警链 A-D（云侧问句写闸落 `build()` 唯一出口 + 告警登记改成扫本轮原话 + 同槽严重级比较 + 「机油机油灯」拼接）、C2 端侧三 bug（后挡除雾错字 / `media.stop` 出口 / 胎压规格问句让路）与探针诊断出口拆分、C16 的 1·6·8；另在实施中扫出并当批修掉 **N8**（规则产的对象名知识库不认 ⇒「胎压是多少」一直答「暂不支持哦」）与 **N9**（「停止播放」被执行成开始播放）。⚠ **第 1 批只跑了本地全量 + 三道离线门禁，真栈迷你集与「修正后计分」都还没跑**——别把它读成「QA 那几条已验证转绿」。✅ **三处拍板已于 2026-08-27 裁定并落地（按方案推荐项，方案 §4 有细节）**：C15 如实标注的 mock=WARN 不判 fail + `deterministic` 收编（契约 §9.3 已同批改）；C10-D 过期提醒用 `cancelled+extra.reason` 清扫——**90 条存量已按泓舟「全清」点名当日执行**（单事务 UPDATE 90、u1 ACTIVE 归零、审计标记 `extra.batch=20260827-hongzhou-approved-90`，证据 `.artifacts/sweep-apply-20260827.result.txt`）；C5-B 放行。**无外部阻塞项**。⚠ §4.2 两条既有账的启动条件已被本轮满足（I-033 数据源账本→C4、多意图复合句→C5），落地时一并销账 | [`docs/design/2026-08-27-minimax-qa-root-cause-fix-plan.md`](docs/design/2026-08-27-minimax-qa-root-cause-fix-plan.md) **§0 接手须知 + §4 实施顺序（含「第 1 批终态」读数与未做项）+ §6 第 1 批实施时新发现的缺陷**；问题原始记录 [`docs/reviews/2026-08-26-minimax-cloud-qa-findings.md`](docs/reviews/2026-08-26-minimax-cloud-qa-findings.md) |
| **Android 陪伴端 App（新客户端 `mobile/`，2026-08-23 立项）** | **M0–M3 四阶段全部完成并收口**（08-25 ~ 08-28，M3-6 验收结论见计划文档）。读数：`tsc` 0 / mobile jest **209** / hmi node:test **288** / APK 325MB / **Maestro e2e `4/4 Flows Passed in 7m 43s`**（真机，第二遍）。§8 三张验收清单已逐条打钩（**打钩纪律**：✅ 只给有真机读数的，样本写样本，没跑写未验）。**⚠ 没有「默认下一步」——三个候选各自带条件，挑一个再开工**：**① 清 M3 遗留 R1–R4**（无 key 引擎回退**预期本身存疑**、需先定「谁换引擎」／来电抢占需真实事件／蜂窝 PTT 需真人语音／keep-awake 在 dev build 上**验不出**）——**四条都要人工配合，不是写代码能推进的**；**② M4 进阶语音**（触发=M2 验收通过 ∧ 泓舟确认手机形态要免唤醒/S2S。⚠ **第一个条件严格说尚未满足**：M2 清单还差 R1–R3 那三条 + 一条被后端挡住（`asrProvider=off`，云栈 `/api/asr` 401）。**要不要因此卡住 M4 由泓舟裁**）；**③ M5 生产化**（触发=泓舟决定发布；范围含后端三条线，**开工前先立独立实施计划**）。⚠ **开工硬前提**：每次先跑 `check_android_env.ps1`（退出码 0 才动手）；跑 e2e **必须带 `--no-reinstall-driver`**（MIUI 每 session 弹安装确认，只给 5 秒且默认「拒绝」）。⚠ **挂账给后端/QA（App 侧修不了）**：`route_plan`/`charging_route` 契约无 `lat`/`lng`（折线画不了）；澄清按钮 `send_text` 丢原句位置依赖性（共享闸 `location.mjs`，**HMI 同病**）；RN 的 `URL` 把路径/查询串里的 `@` 读成 userinfo（fail-closed，已钉成可见测试）；MiMo key 失效卡住 ASR 批处理兜底（全体客户端含 HMI）；**上行帧无 request_id 幂等** ⇒ 探活判死前那条丢失的消息不能自动重发（R5）。⚠ **两条要泓舟动手**：`AUTH_TOKENS` 只有一个条目（u1/v1 带 `vehicle.control`），README 写的「App 专属条目、手机档不含 `vehicle.control`」**没有落地**（R7）；E3「两台真机」始终只有一台，平板形态一直是折叠屏展开态强制验的、**不等于真平板**（R8）。⚠ **M5 合规两条**：amap3d 硬编码同意隐私政策；模板 debug keystore 是全世界 RN 项目共用那把，换正式签名要同步在高德控制台加 SHA1。 | 接手从 [`docs/design/2026-08-24-mobile-app-implementation-plan.md`](docs/design/2026-08-24-mobile-app-implementation-plan.md) **§0 接手须知** → **§M3-6 的「M3 遗留出账」表**（R1–R8 逐条写了「为什么现在不能做 / 解锁条件」）；实施记录在 §3/§4/§5/§6 末（M0–M3 + **M3-V** Aurora + **M3-W** 收尾），**坑账 §9 已积到 42 条、开工前读一遍**；日常命令看 [`mobile/README.md`](mobile/README.md)、e2e 看 [`mobile/e2e/README.md`](mobile/e2e/README.md)；需求/选型/架构判断在 [`docs/design/2026-08-23-hmi-android-app-plan.md`](docs/design/2026-08-23-hmi-android-app-plan.md)（§9 决策记录） |
...
| **`test_e2e_stack_lease` 抢真实 OS 锁**（2026-08-28 新记，**不是本批引入**）| **根因已定性，有复现配方**：该文件多处 `repo_root=Path.cwd()`，而 `identity_lock_path()` 把它解析成 **`.git/car-agent-e2e-identity-stack.lock` 这个真实 OS 锁文件**——它是**仓库级共享**的，谁持有别人就拿不到。取证：单跑 3/3 绿、`scripts/tests -n auto` 3/3 绿（1335×3）、全量单跑 2/3 绿；而**同时跑两趟全量必红**，且两趟各红了该文件里**不同的一条**（`test_restore_failure_…` / `test_single_owner_…`）——症状随「谁先抢到」漂移，正是共享锁的签名。⇒ 修法是让这几条用例各自用 `tmp_path` 当 `repo_root`（或给锁名加 worker 后缀），**不是加重试**。⚠ 在修掉之前：**不许两个会话同时跑全量**，全量报绿时也要单独确认它这一条。⚠ **2026-08-28 第 2 批又撞到一次，而当时只跑了一趟全量**（该趟 6:00，相邻两趟 4:06/4:09）——**宿主负载本身就是第二个竞争源**，「同时跑两趟」不是必要条件。隔离复跑该文件 61 passed / 2 skipped，随后整趟全量 7397 零红。 | 文件本身；本行由 QA 修复批第 1 批的三趟全量读数触发，第 2 批补一次单趟复现 |
