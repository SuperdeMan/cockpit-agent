...
（2026-08-15 立卡 / 2026-08-16 推进八批。报告
...
| Q5 身份/作用域/记忆写入 | 🔄 | ① **出处披露实测 0/3 稳定红**（清洗后复跑，**不是**此前记的「方差」——旧的方差结论只是采样不足）。做成确定性需要一个「本轮答案是否由记忆驱动」的机制。② **实体归一**（`孩子`/`女儿`、`妈妈`/`用户的妈妈` 苏州 vs 杭州）与 **N3**（`导航去过济南市南山实验小学` 两条）都是 `memory_item` 里的事实级脏数据，**清洗脚本刻意够不着**（history §48.5） |
...
**接手顺序**（下面这个次序是有理由的，别按卡号做。**第 0 步已于 2026-08-16 完成**）：
...
| **1** | **Q10** 查单绑会话 ← **接手从这里开始** | 最小且依赖已满足（Q2 候选集已就位）。一张卡同时收掉 I-021+I-026 与「文本/按钮双入口不等价」那半。**分母已由清洗后复跑钉住：XS4 = 1/3** |
