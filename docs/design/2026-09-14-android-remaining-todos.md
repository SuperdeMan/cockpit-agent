...
| D-09 | D | 系统音频五维（声音 / 播放器 / FSM / 采集 / 上行）× 四态（主 TTS / S2S / LISTENING / FOLLOWUP）：焦点丢失、耳机断开、系统中断 | **上机前先核事件源，核出两条真缺陷（E-24 / E-25，均已修并真机复验）**。已取：主 TTS 出声中 × 中断五维齐（`5286c3ba5` 2ms / `b08579f6c` 6ms 停声、1s 后放焦点、不进 FOLLOWUP、App 不切后台）、THINKING（TTS live）× 中断 ×3、**LISTENING × 中断（G-07 后）：持焦点、`-2` → 放弃半句回 ARMED**、出声才持 / 收尾放、别人的音乐压低不停。**未取**：语音轮 SPEAKING / FOLLOWUP / S2S × 中断（屋里的视频对白让 VAD 一直判有人说话、端点不来——协议已改成整轮持焦点事后分类，电视关掉即可采）、耳机断开 OS 层（OPPO 零 bonded 蓝牙、无有线口） | remediation §6.15；在场轨迹 `system_stop:<reason>` + logcat `MediaFocusControl` / `[handsfree] fsm` + `ps -T` 线程 |
