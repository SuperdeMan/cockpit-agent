...
> 状态：客户端与共享 FSM 修复、本地回归与变异验证完成；OPPO 上取得 R06 主证据（同题 A/B）与层内停止键证据，Xiaomi 对照机上取得 R09 横屏证据（结构 + 功能）；**多段段链 / S2S 自答 / 主动消息 / 系统 200% 字号 / 盲听仍未验，AR03 未整批签收**（末节列格）。
...
**R09 横屏那格在 OPPO 上不可达。** `driving-landscape` 要 `宽 expanded(≥840dp) ∧ 高 compact(<480dp)`：
外屏横 = 1972×988px ÷ 2.75 = **717×359dp**（宽只到 medium）；内屏 = 1920×1792px ÷ 2.75 = **698×652dp**（高不是 compact）。
两块屏都落不进那个分支，而密度/尺寸/旋转覆盖又被系统拒了。**这条本身要带进 AR10 的支持范围裁决**：
方案承诺的横屏车载形态，测试机根本进不去。该格改在对照机上取，见下节。

**八、R09 横屏：Xiaomi 对照机（用户当场授权接机验证）**

| 项 | 读数 |
|---|---|
| 设备 | Xiaomi 24072PX77C / HyperOS 3 / Android 16，外屏 1080×2520 @480dpi ⇒ **360×840dp** |
| 包 | 同一份 APK，`pm path` 取到的安装文件 SHA-256 `0583e297…` 与本地一致；非 DEBUGGABLE；`install -r` 后 lastUpdateTime 2026-09-07 10:28:41 → 2026-09-08 10:13:00；设置页构建行 `v0.1.0 · prod · b5c471832 · 2026-09-08 08:10` |
| 形态 | 强制横屏后 `dumpsys window` 直接读出 `sw360dp **w840dp h360dp** 480dpi … **land**` ⇒ expanded × compact ⇒ `driving-landscape` |
| Dock 项 | **位置征询**（关掉「使用定位」后问「附近的充电站」触发），不是车控——本轮没有触发任何 `require_confirm` 动作 |

**结构（层升起时同一帧的三个 bounds）：**

| 节点 | bounds |
|---|---|
| `voice-sheet-scope`（层的覆盖域） | `[0,361][2520,690]` |
| `voice-sheet`（层本体，含 60% 暗区） | `[0,360][2520,690]` |
| `focus-dock` | `[0,690][2520,1032]` |

**零重叠**：Dock 完整落在覆盖域之外。截图 `x24-landscape-dock-under-sheet.png` 同帧可见——层压住了记录区、胶囊与 Composer（都在暗区里），
而底部 Dock 的「使用当前位置 / 拒绝 / 允许」亮度未减、琥珀边框完整。修前层挂在整列上，这块正好在暗区之下。

**功能：层还盖着时直接点 Dock 的「拒绝」**——征询关闭、`focus-dock` 出账（层随之收起是因为 `hasAttention` 落了，不是被点收的）。
**不是**被暗区吞掉。⚠ 第一次用 120ms motionevent 点没触发（那一次不作任何证据，见下条限制），改用 `input tap` 生效。

**这台机器上的三条取证限制：**

- **120ms 同点 motionevent 序列**（AR02 为 RNGH 光球定的那条）在 HyperOS 上**触发不了普通 `Pressable`**，`input tap` 可以。
  「点了没反应」先换注入方式，不能直接当成产品缺陷——本轮第一次点 Dock 就是这么差点误判的。
- `screencap` 在多屏机上会**先打一行 `[Warning] Multiple displays…` 再输出 PNG**，必须按 `PNG` 偏移截断，否则存出来的文件是坏的。
- **旋转进入 `driving-landscape` 之后语音层升不起来**（多次尝试，`voice-sheet` 节点始终不出现）；横屏冷启动后正常。
  怀疑是 `columnHeight` 的 `onLayout` 在 `splitLandscape` 由 false→true 那一帧没有重新触发（层的挂载条件是 `containerHeight > 0`）。
  本轮未进一步定性，**留给 AR04**——它接手层宿主时正好要碰这块。

**对照机的设备状态处置。** 它自己的「免唤醒对话」原本是开的，App 一启动麦克风就工作，一段真实环境语音被转写并上传 ASR
（只到草稿，没有派发成一轮，无助手回复）。发现即 `force-stop`。随后**经用户当场授权**关掉免唤醒再继续，全程麦克风计数为零。
本轮在对照机上改过五项：免唤醒（开→关→**已还原为开**）、行车档（关→开→**已还原为关**）、使用定位（开→关→**已还原为开**）、
减少动效强制（关→开→**已还原为关**）、旋转（0→1→**已还原为 0**，auto-rotate 未动）。
另有一处**误操作**：按「开关与标签文字带重叠」配对时把「记忆」当成了「使用定位」关掉，**当轮发现并还原为开**——
多行说明的 SwitchRow 会把开关对整行居中、掉到文字带之外，正确规则是**标签之后的第一枚开关**（已写进取证脚本 `switch_for`）。
结束时 App 已 force-stop、退到 Launcher、crash FATAL 计数 0。

**九、层内停止键与放大字号（OPPO 第二轮，2026-09-08 11:0x–11:22）**

上一轮把这一格记成「升层与播报结束同时发生、分不清因果」。本轮用**两条互不相同的升层路径**各跑一次，把因果证伪了：

| 升层方式 | 时间线（合一键态 / 层在不在 / 「停止播报」节点数，采样 ~2.5s） |
|---|---|
| 深链 `?voice=1` | 10.5s 播报中·无层·1 → 发深链 → **17.0s 与 19.5s：层在·2** → 25.2s 自然播完 |
| 点状态胶囊 | 39.4s 播报中·无层·1 → 39.5s 点胶囊 → **46.0s：层在·2** → 46.8s 按下层内那枚 → 49.3s 回「发送」 |

**升层之后播报分别继续了 ~9s 与 ~6.5s**，所以上一轮那两次是 TTS 时长方差，不是升层停了播报——该条**从「待查」改为已证伪**，不再留给 AR04。
同时这也把层内停止键坐实：层升起时**两枚停止入口同时在场**（层内一枚 + Composer 一枚），按下层内那枚播报即停。

**放大字号复跑（App「大字」档）**：层 `(0,751)-(988,1480)`；层内键 `[733,759][964,905]` = **84×53dp**，Composer 键 `[829,1769][961,1901]` = **48×48dp**（泊车目标 48），两枚都达标且都不在对方的覆盖域里；按层内键后播报停止。截图 `r5-largefont-two-stops.png` 同屏可见琥珀色「停止播报」在层的把手带右侧、■ 停止键在右下。
⚠ 第一次抓的 `cellB2-two-stops.png` **拍在层展开动画中途**，那枚键还没画出来——「XML 里有、截图里没有」先怀疑抓帧时刻，不是控件缺席。

本轮麦克风计数全程为 0，播放计数 `starts:4 / stops:4` 配平。**系统 200% 字号取不到**：ColorOS 同样拒绝 `settings put system font_scale`（读数仍 1.0）；本轮用 App 自己的「大字」档覆盖了同一条 `scale()` 路径，但它不等于系统 200%，那一格保持未验。

另记一条取证边界：深链 `?voice=1` 是**一次性消费**（`voiceParamConsumed` ref，「同一次进入只升一次」），第二次发同一个深链不会再升层——要么冷启动，要么改点胶囊。
...
- **多段回答**（divergent / mixed 段链）中途停播：**构造不出来**。这两种形态取决于云端 `final` 与已流内容的关系，客户端点不出来；prod 也没有 `segments` 的读数出口（`[speech-turn]` 只在 dev 打）。要验得先给 prod 加一个只读的段数出口，那是另一个批次的事。
- **S2S 自答**真机停播：需免唤醒 + 端到端挡位 + **说话**，没有声学输入通道（同 AR02 的限制）。
- **主动消息**播报中停播：不能按需触发。走「建一条提醒等它到点」这条路需要**中文文本输入**，而 `adb shell input text` 只能打 ASCII、默认快捷指令里也没有提醒类——本轮没有可用的输入通道。
- **系统 200% 字号**：ColorOS 拒绝 `settings put system font_scale`（读数仍 1.0）。App 的「大字」档已覆盖同一条 `scale()` 路径（第九节），但那不等于系统 200%。
- **横屏形态下的层内停止键**：横屏那一轮没有同时构造出播报。横屏确认可达已在对照机闭合（第八节），层内键本身已在 OPPO 竖屏闭合（第九节），两者的交叉未验。
- 与停播/接段直接相关的一次**混合意图盲听**回归——需泓舟参与。

云端未部署，未做真实车控/商户写/付款。全程只用了快捷指令「讲个笑话 / 我今天有点不开心 / 今天天气怎么样 / 附近的充电站」四条，
没有触发任何 `require_confirm` 动作。OPPO 上先后改过六项（播报、减少动效强制、设备角色、行车档、字号、以及第二轮重开的前两项），
Xiaomi 上改过五项，**两台都已逐项还原并回读**；系统设置一项都没改（ColorOS 全拒、HyperOS 只用了 `user_rotation` 并已还原）。
两台结束时 App 都已退出前台、crash buffer 的 FATAL 计数为 0。
...
逐项 XML/PNG（`t*` / `cell*` / `r*` = OPPO，`x*` = Xiaomi；决定性的三张是
`x24-landscape-dock-under-sheet.png`（横屏 Dock 在暗区外）、`r5-largefont-two-stops.png`（两枚停止入口同屏）与同帧 XML）、
三条 A/B 时间线（`cellA3-natural.txt` / `cellA4-stopped.txt` / `cellB-control.txt`）、取证脚本 `probe.py`；
不含原始音频、图像、token 或密钥。
...
~~~text
批次：AR03
对应 R 编号与关闭/剩余项：R06 客户端修复完成 + OPPO 同题 A/B 与层内停止键取证；R09 横屏部分客户端修复完成 + Xiaomi 结构与功能取证；仍未验：多段段链、S2S 自答、主动消息、系统 200% 字号、横屏×层内停止键、盲听；R09 跨页部分属 AR04 未启动
代码 SHA：b5c471832061e4744dc42128e346462be6f41bc8（8bb13ab 的停播顺序、fe798cc 的缓冲段两处缺陷均在同批修掉）
APK 构建行与设备：`v0.1.0 · prod · b5c471832 · 2026-09-08 08:10`，SHA-256 0583e297…；OPPO PEUM00 08:26:38、Xiaomi 24072PX77C 10:13:00 两台装机，设备文件均同哈希
云端 release SHA、provider/model：本批未涉及真栈
本轮执行的检查与原始证据：mobile 70 suites / 695 tests + tsc exit 0；hmi 308/308 + Vite build；15 条变异注入全部判红；OPPO 上同题 A/B（停播窗口 ≥15.7s → 单个采样）与 8 次播报零麦克风；Xiaomi 上 driving-landscape（w840dp h360dp land）下层与 Dock bounds 零重叠且 Dock 可点；OPPO 第二轮两条升层路径各证「升层后播报继续」（~9s / ~6.5s）、两枚停止入口同屏、按层内键即停
失败/阻塞/污染样本：reanimated 官方 mock 在本环境不可用（已手写替代）；测试渲染器不跑布局（横屏用例手动放 onLayout 并先证层在场）；ColorOS 拒绝 adb 改动画/尺寸/密度/旋转 ⇒ driving-landscape 在 OPPO 不可达（改用对照机）；上一轮「升层与播报结束同时发生」已被两条升层路径证伪；HyperOS 上 120ms motionevent 触发不了普通 Pressable；旋转进入横屏后语音层升不起来（冷启动可以），留给 AR04
