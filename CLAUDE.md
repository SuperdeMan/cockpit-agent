...
M0a→M3 全部完成，M4 的 S2S 线也已落地**，主线是「把每一种智能供给都声明式化」：
...
**下一站：M4 剩下的 P4——声纹多用户（让 `occupant_id` 真实化、兑现「多用户记忆隔离旅程」DoD；
声纹不作鉴权因子=红线）+ 视觉入口（「那是什么」单帧图片问答，走 escalate 同构）。**
边界速写见 `docs/design/2026-07-25-m4-s2s-fullduplex-rfc.md` §8；按 v1.2 既定制度，动手前先出设计。
