...
test('收音初始：草稿未出现 ⇒ 层内容为空（sheetBody=empty、0.4）；草稿一出现 ⇒ turn（收集器把 draft 事实喂给 derivePresence）', async () => {
  ...
  const hfSpy = jest.spyOn(handsFreeHooks, 'useHandsFree').mockImplementation((opts) => {
    const h = realHf(opts)
    return { ...h, fsm: 'LISTENING', availability: { ...h.availability, usable: true } }
  })
  settingsStore.getState().update({ handsFree: true })
  const view = await mount('/')
  try {
    ...
    expect(runtime!.snapshot.sheetBody).toBe('turn')
    expect(has(view, 'voice-sheet-transcript')).toBe(true)
  } finally {
    await unmount(view)
    hfSpy.mockRestore()
