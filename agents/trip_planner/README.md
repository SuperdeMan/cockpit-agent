...
| intent | 说明 |
|---|---|
| `trip.plan` | 目的地/天数/偏好生成行程；天气联动（雨天优先室内）；**G4 主题行程**（`theme` 槽：「跟着《X》游Y」→ LLM 提议候选地名、高德接地 `name_matches` 校验后入池——池外名字仍无条件丢弃）；**G9 多城市**（destination 按口述序连写「杭州、苏州」→ 逐城建池、按序分天标 `Day.city`、跨天衔接 leg 进充电编织） |
