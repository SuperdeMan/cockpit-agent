...
- **第二梯队问题**：端云"长连接"实为逐请求建流（持久版是 250 行死代码）；权限引擎双轨（PermissionEngine 从未被调用）；全链路零鉴权（PoC 已知）；两个 1200-1600 行巨型文件（info agent、fast_intent）。
...
#### A4 权限模型双轨制，PermissionEngine 是死代码
...
#### A5 跨 Agent 隐式耦合通道（memory KV 私有契约）
...
| D6 | info agent 巨类 | agents/info/src/agent.py（1269 行） | 天气/搜索/新闻/赛事/股票五域 + 早报 + 画像排序 ~60 方法一个类。域间无隔离，改新闻碰坏赛事的风险真实存在 |
