...
async def _run_turn_with_empty_transport_retry(
    key: str,
    model: str,
    session,
    ws,
    ...
    *,
    ...
    """Retry one acoustic-empty transport failure on a fresh provider session."""

    result = await probe.run_turn(pcm)
    if not _is_empty_acoustic_transport_failure(result):
        return result, session, ws, probe

    # A real-time provider can close a healthy session before ASR emits any
    # observation. Retry only that evidence-free transport failure. Completed
    # wrong routing, transcripts, and tool calls remain first-attempt verdicts.
    print(
        f"  {label}轮无转写/工具事件，使用新 session 有界重试一次："
        f"status={result.get('status')} error={result.get('error', '')[:120]}"
    )
    await ws.close()
    await session.close()
    session, ws = await _session(key, model)
    probe = Probe(ws, model)
    await probe.open()
    result = await probe.run_turn(pcm)
    return result, session, ws, probe
...
async def case_escalate(key: str, model: str, rep: Report) -> None:
    ...
    try:
        p = Probe(ws, model)
        await p.open()
        r1, s, ws, p = await _run_turn_with_empty_transport_retry(
            key,
            model,
            s,
            ws,
        ...
        r2, s, ws, p = await _run_turn_with_empty_transport_retry(
            key,
            model,
            s,
            ws,
            p,
            synth_pcm16k(UTT_CONTROL),
            label="车控",
        )
        tc = r2["tool_call"]
