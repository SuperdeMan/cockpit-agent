...
| 生产 release | `ca4bf3705a8e73d19e56fb713c36c3773af7a5c9`（2026-09-19 13:4x apply）——QA 交接页 §5 收口批当日第四个 release：T24 安全问句错域（road-safety manifest 描述 + 122 续驾 hint）、T47 裁决 A（`alert_resolved` 解除陈述清焦点、四个消费方一份判据）、`dest_choice` 挂起不再吞安全陈述、五条提前出口前登记输入侧事实，外加 barge-in 探针判据 / trip fixture / `_canonical_bytes`。当日链 `1eb25a70` → `0d414816` → `b342e3bb` → `ca4bf370`，逐个 release 的内容与真栈证据见 [2026-09-19 收口页](docs/design/2026-09-19-qa-residual-closeout.md) §2.5 / §3.3–3.5 与 history。上一个非本批 release `3c389465`（2026-09-18，搜索合成 `max_tokens` 兜底等，见 [2026-09-18 设计](docs/design/2026-09-18-android-stream-text-pacing.md)） |
| 上一生产基线 | `b342e3bbf6e9102989823d5d4769cc8f8e353f5a`；本轮未回滚 |
| status | 2026-09-19 13:4x 独立复核：`ok`、5/5 endpoint healthy、零 warning，`release_sha` 与 `running_release_sha` 均为 `ca4bf370` |
| verify | `verified`；artifact `20260919T054445Z-ca4bf37.json`（`minimax:MiniMax-M3`，lock `e2e`，`e2e_remote_safe`）；当日前三个 release `20260919T051116Z-b342e3b.json` / `20260919T041208Z-0d41481.json` / `20260919T030356Z-1eb25a7.json` |
| 代码验证 | **全量固定口径（`82c8c7fa` = `ca4bf370` + docs，`TZ=UTC0` `-n 8`）：8397 passed / 1 failed / 31 skipped / 12 warnings，272s。** 那 1 红 `test_e2e_stack_lease.py::test_parallel_owner_cleanup_failure_overrides_both_passes`：同文件串行 61 passed / 2 skipped、单独 `-n 8` 三趟各红 2–3 条不同用例——parallel-owner 那组用例拿**真仓库根**当 `repo_root` 去抢同一把仓库级身份 OS 锁（`identity_lock_path`），xdist 多 worker 同跑必撞（§6.1 点名的「OS lock 污染读数」形态；测试隔离债，未修）。31 skipped = 32 基线 − 1（Docker Desktop 此刻在跑）。12 warnings = Starlette ×8 + WordPiece ×2 + audioop ×1 + 一条 stack-lease 子进程读线程 GBK 解码告警。分层：`test_route_hints` 37、`eval_route_hints` 119/119、范例 328 条契约 OK、四门禁 + smoke_edge 13、runtime 13、`test_safety_focus` 25、`test_engine_input_facts` 4、cloud 1345、agents+registry+security 2103、mobile `voiceTts` 22 + tsc/eslint 0；十处变异各自判红。真栈（`minimax:MiniMax-M3`，collector 逐 trace）：T24「红色机油灯亮了还能继续开吗」干净会话 3/3 `safety.driving_advice` + deterministic 卡、对照「机油灯亮了怎么办」仍 `manual.query`、safety 组 `--repeat 3` 15/15（`1eb25a70`）；T47 persona 同形序列 ×3 解除轮零动作、「规划去广州路上的补能」3/3 `charging.plan`（`0d414816`）；带 `dest_choice` 挂起的六轮序列 ×4 解除句 0/4 被吞、落技术失败出口那趟也清掉了告警（`ca4bf370`）。逐个 release 的读数见收口页 |
