...
**两件账**：
- ⚠ **长跑批被外部中止就全丢**：探针在**全部 persona 结束后**才落盘，那次腰斩让四个
  已完成 persona 的 trace / 卡片明细全部丢失（只剩控制台 ✓/✗，已存
  `.artifacts/…-console-killed.log`）。**改法是逐 persona 落盘**——
  一次跑批四十分钟，把它的产物押在「跑完不出事」上本身就是个设计缺陷。
- ⚠ **两条待定性**（单次取样）：`adversarial T35` 疲劳驾驶答成反问、
  `family T39`「哪家最晚关门」。下一步 `--repeat 3`。
