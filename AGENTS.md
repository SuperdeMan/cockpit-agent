...
| 生产 release | `ca4bf3705a8e73d19e56fb713c36c3773af7a5c9`（2026-09-19 13:4x apply；`_register_input_facts`——技术失败 / 授权缺失 / 澄清 / 取消未命中 / 没听清五条出口前仍登记输入侧事实，带挂起序列 ×4 解除句 0/4 被吞、落技术失败出口那趟也清掉了告警，收口页 §3.5）。上一 release `b342e3bb`：2026-09-19 13:0x apply；`_is_topic_change` 安全信号一律判换题——`dest_choice` 挂起不再把解除/告警陈述当目的地吞掉，带挂起序列 ×3 零吞句，收口页 §3.5）。上一 release `0d414816`：2026-09-19 12:0x apply；T47 裁决 A：`alert_resolved` 解除陈述清焦点、四个消费方同一份判据；真栈 persona 同形序列 ×3 T47 格 3/3 `charging.plan`，收口页 §3.4）。上一 release `1eb25a70`：2026-09-19 10:5x apply；QA 交接页 §5 收口批：T24 安全问句错域修在 road-safety manifest（描述 + 122 续驾 hint）、barge-in 探针判据、trip fixture、`_canonical_bytes` 读长；全程见 [2026-09-19 收口页](docs/design/2026-09-19-qa-residual-closeout.md)。上一 release `3c389465` 的内容：2026-09-18 19:1x apply；搜索合成 `max_tokens` 600→1200 只做兜底、prompt 管长度、撞上限切句并说明。当日前几批：`a4b47748` 改派流式直通、`3abd325e` chitchat 兜底上限 + D0 截止 60s；mobile 侧 `d32f81c2` reveal 跟到达速率走。全程见 [2026-09-18 设计](docs/design/2026-09-18-android-stream-text-pacing.md)） |
