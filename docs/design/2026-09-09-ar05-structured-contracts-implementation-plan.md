...
> 2026-09-10 晚（release `74852a7`）：计数未变，但两格的缺口实质缩小——
> **V05 的负例已闭合**（受限 token 的车控端到端，两条闸各验一遍、车态零变化），
> 只剩 PoC 默认与伪造 meta 两档仍是离线覆盖；
> **V07 的业务 `scope_missing` 已闭合**（请求面 6/6），只剩设备权限拒绝未验。
> 逐条见[余项收口 §3.4/§3.5](2026-09-10-ar-residuals-closeout.md)。
