...
- **M2-5 验收（2026-08-26 首轮，机器可判的部分已跑完；纯人工项待下一轮）**：
  ...
  | § 8.2 条目 | 状态 | 读数 / 说明 |
  |---|---|---|
  | PTT partial 上屏 ≥1 / 松手自动发送 | **协议面 ✅ / 交互面待人工** | `asr 直灌`（绕开麦克风只验协议）：fun-asr partial×4 首个 @476ms、qwen3 partial×10 首个 @764ms，final 均逐字一致。**待验的是「人按住说话」那一段**：按钮手感、320ms 误触门槛、首次权限弹窗 |
  | 蜂窝网络下重复 | ⬜ | 需人工切网 |
  ...
  | TTS 首音 <1.5s | **⚠ 取决于引擎** | cosyvoice 551/617ms（三倍余量）；**minimax 1353/1390ms，冷启 1881ms**——贴线甚至越线 |
  ...
  | 无 key 引擎回退出声 | ⬜ | 需人工选一个无 key 引擎听 |
  | 来电/焦点抢占四场景 | ⬜ | M2-4 代码已落，四场景需真实事件 |
