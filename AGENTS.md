...
**最新后端全量基线（2026-08-19 Q10 残余批一趟根跑实测，`target=cloud` + 本地 Docker 已退）**：
`python -m pytest --import-mode=importlib` = **6722 passed / 32 skipped 零红（24m16s）**
（Q10 残余 + 第 7.5 步两批合计 `6623 + 38 + 41 + 10 + 10`，逐条闭合）。
其中 `6623 → 6702` 那一跳：**+38** 是新增用例
（桥侧 `test_candidate_ref.py` 31 + 云侧 `test_candidate_sets.py` 7），
**+41 由 skip 转 passed**，成因是本轮把 `CAR_AGENT_DEPLOY_HOST`/`_USER`/`SSH_IDENTITY`
设进了操作者 shell（**RC17 那条的另一面**：§57 记的是「只因 export 就从绿变红」，
这次是由 skip 转 passed 且全过）。⇒ **skip 数不只随部署形态变，也随操作者 shell 变**，
对账先问「这一跳的 shell 和上一跳一样吗」。此后 `6702 → 6722`：桥侧三通道 H 组 +10、
云侧序数取值 +10。上一跳基线 6623/73（洁癖批，`--collect-only` 6696）。
...
**探索式 QA 轮的 Q8 与三处卡内残余**（~~Q2 的确定性消费方~~ ✅、
~~Q10 的双入口收敛 + Q2 的候选集下发面~~ ✅ 2026-08-19 交付，真栈 MC1 **0/3 → 3/3 `[det]`**
（含同日第 7.5 步「序数取值」把 2/3 补成 3/3）；
**Q12 的规格维**、**Q10 的门店侧（I-024）**、**I-030 跨组比较**仍未做——
三条各有独立成因，见 §4.1 ① 逐卡表；阶段 0/1/2 与
Q13/Q2/**Q7 全维度**/Q5/Q11/Q10/Q6/Q12 主体、**存量数据清洗**已完成，
逐卡状态见 §4.1 ①、流水 history **§41–§53 + §58/§59/§61**、归档索引 **§47.5**）。
...
> 另两行（Q12 规格维 / Q5 实体归一）**刻意排在它后面且不编号**：前者的修法是 B6 远期
> 字段、后者跟着 person-pickup 卡走，都不是「下一个动手」的对象。
...
**第 0–7.5 步已全部完成**（2026-08-16 → 08-19，逐步的落点/判据/读数在 history
**§48–§59 + §61/§62**，本表不再复述）⇒ **新会话从第 8 步开始**。
...
| # | 做什么 | **为什么排在这里** |
|---|---|---|
| ~~7.5~~ | ~~候选在手时 chitchat 编造~~ ✅ **同日收口**（history §62）| 编号带小数是刻意的：它是第 7 步的**直接产物**（复跑读数掀开的），不是新一轮的事。修法=`candidate_query` 第三种算子「序数取值」，**MC1 2/3 → 3/3 `[det]`**。判据沉淀两条：**凡是「系统持有的事实」，判据面就得是闭合的**；**把一件事从「路由对了才对」变成「不经过路由」，方差就不是靠运气压下去的** |
| **8** | **Q8** 能力缺席 + **Q11** offer 准入 + P2 长尾 ← **接手从这里开始** | 收尾——它是**叶子**，不解锁任何别的卡，所以放最后。⚠ **Q8 开工前先决策**：对抗语料唯一输入 **596/596 余量为 0**，而新增 active intent 必须补对抗覆盖（正 2/硬负 2/对照 1）。**先说明新能力为何值得占额度、再有原则地调 `suites.yaml` 的 `max_cases`**——不得先加语料撞闸后补理由。⚠ 本卡另含 Q12 转来的 `navigate_to` 缺 `origin` 槽一条 |
