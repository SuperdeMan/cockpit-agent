...
> 版本：v1.15（当前架构基线；版本规则见文末「附录 C：版本记录」）
> 日期：2026-07-30（v1.15 定稿归档——端侧语义 NLU 影子层合入：§3.2 双阈值伪码首次拿到真概率，挡位停 shadow）
...
> **实现对应（2026-07-30 校准，M5 P3a）**：上述双阈值伪码写下半年后**第一次拿到真概率**——
> 端侧语义 NLU（`orchestrator/edge/nlu.py`，4 层中文 encoder ONNX，35MB/4.8ms）已建成
> **识别侧**，产出真 softmax 置信度；但**执行侧刻意未接**，挡位停在 `EDGE_NLU_MODE=shadow`
> （只算不用，判定与规则的分歧落 obs `nlu_vs_rule` 三态）。**`on` 挡位刻意不存在**：θ 按
> transfer 车道标定到 85% 覆盖须付出约 1% 请求识别成错对象的代价（三闸挡危险动作、挡不住
> 「合法但不是用户要的动作」），放量前提与开工判据见 `AGENTS.md` §4.0 P3b 卡。当前生产路由
> 仍由 1727 行规则 `fast_intent` 承担，置信度仍是硬编码字面量——本节伪码的完整兑现在 P3b。
...
HMI 侧责任**：S2S 交互进行中主动消息只出气泡不出声（两个播放器互不知情、同放即混音，且
主动 TTS 的生命周期回调会误推语音 FSM）；治理器侧按 `s2s_speaking` 情境位延后是完整方案，
已立卡未做。
