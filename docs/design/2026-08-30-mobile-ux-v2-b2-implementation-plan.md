...
- **闸的结论（第一行）**：**过 → B3/B4 可开工。** 泓舟 2026-09-01 逐项验收后裁定放行。
  五项闸**全部有读数且全部达标**：G1 ✅（首反馈时延 **3ms**，不带唤醒词追问成功，追问窗 **8.02s**）／
  G2 ✅（长播报下回声提示 0 次、无环，**非预期值但是真读数**）／G3 ✅（两半）／G4 ✅（两方向层与草稿都不丢）／
  G5 ✅（60.0 fps 零掉帧、同屏循环动画 1 个）。
  ⚠ **唯一被「裁定」而不是被「测量」的一格是 5 人外部小样本**：实际做的是**泓舟本人自评 6/6**（1 人、非外部），
  泓舟裁定「算过」。**接手的人要知道这一格没有分布读数**——§11.4「状态可读性」在 B2 仍然没有外部基线。
  Accessibility Scanner 仍 ⬜（装 APK 需单独授权，本批没有）。
  验收当轮还查出**三条缺陷**（S4 / S6 / `plateGesture`，见遗留出账），泓舟裁定**都记给下一批**，不在本批修。
  ...
  **D8 两条声明用 `git grep` 在 `HEAD~1` 上取了证**：`activityLog.list()`（单例）与 `activityLog.push('location'` 各 **0 命中**，
  ...
  - **真机 ✅**：设置 → 调试分区末行「在场轨迹（B2：光球为什么变了 / 麦为什么开了）」可见并可进；
    ...
    **采集激活 7/20 有真实数据**（`mic · 唤醒词命中` ×6 + `mic · 按住说话` ×1）⇒ `activityLog.list()` 的消费方带真数据工作正常。
    ✅ **`location` 产出方也验到了**（设备重插后补取）：关掉「使用定位」→ 问「附近有什么好吃的」→ 位置授权征询弹出
    （Dock attention 态 +「拒绝/允许」+ 胶囊「等你确认」，`d8_c2.png`）→ 点「允许」→ 轨迹页
    **`采集激活 1/20` 里出现 `19:19:00.194 location · 位置授权 · 同意`**（`d8_trail.png`）。
    轨迹把整条链路都记下来了：`19:15:06.314 attention「等你确认」输入 pendingLocation,voice` →
    `19:19:00.194 location · 位置授权 · 同意` → `19:19:00.202 armed 输入 pendingLocation` → `19:19:20.390 thinking`。
    **⚠ 第一次尝试失败过一次**：点「允许」那一下 adb 恰好报 `no devices`（daemon 自重启），tap 落空，
    屏上表现成「activityLog 里就是没有 location」——**与「功能没做」完全同形**（见坑⑨）。
...
| # | 项 | 目标 | 读数 | 结论 |
|---|---|---|---|---|
| G1 | 真人语音轮：唤醒 → 说 → 答 → 8s 内不带唤醒词追问；首反馈时延 | ≤100ms；追问成功 | **全部达标**（泓舟 2026-09-01 真人轮，语料「小舟小舟」→「深圳天气怎么样」→ 答完后不带唤醒词说「那明天呢」）。**首反馈时延 = 3ms**：`23:53:14.790 ◇ fsm:LISTENING` → `23:53:14.793 listening ·「在听…」`。整条 FSM 链都在轨迹里：`THINKING`(2ms) → `SPEAKING`(**133ms**) → `FOLLOWUP`(3ms，胶囊「可以接着说」) → `LISTENING`(3ms) → `FOLLOWUP` → `ARMED`(3ms)。**追问成功**（记录里有第二轮问答：「那明天呢？」→ 明天天气卡）。**追问窗 = 8.02s**（`23:53:15.946 FOLLOWUP` → `23:53:23.970 ARMED`），是 §11.2「8s 追问窗」的机器读数。副产品：`SPEAKING` 那一跳 **133ms** 是全轮最大（其余都是 2–3ms）——它不是首反馈，是「音频起播 → 屏上变 speaking」。⚠ 昨天那个 2ms 走的是 `fsm:ARMED` 那一路，**今天这个 3ms 才是 G1 定义的 `fsm:LISTENING` 那一对** | **✅** |
| G2 | 回声降级：胶囊「像是我自己的声音，没算数」出现次数 / 是否成正反馈环 | 无 AEC 包：提示出现、无环 | **回声提示 0 次 / 无正反馈环 / 无 barge-in**。前置条件三条都成立：APK `lastUpdateTime=2026-08-29 17:22:24` **早于 AEC 提交 `96a6830`（17:27:50）⇒ 这个包没有 AEC**；A2DP `STATE_DISCONNECTED`（vivo TWS 4 已断开，音频走扬声器）；**媒体音量原本是 0/150，已抬到 140/150**（见坑③）。两轮取样：①「讲一个三百字的故事」播报 `19:33:16.921 speaking` → `19:33:25.515 armed` = **8.6s**；②「详细介绍深圳…」播报 `19:47:55.352 speaking` → `19:48:28.057 armed` = **32.7s**（过程区走了理解需求→规划步骤→深度调研→整理结果）。两轮的轨迹里**都没有 `notice` 轴条目、没有回声胶囊、也没有 speaking→listening 的 barge-in**（`g2_trail.png` / `g2_trail2.png`）。⚠ **这不能读成「提示功能是好的」**——提示从未被观测到触发过；能读的只有「这台设备这两轮没有出现回声环」。可能是麦克风没把扬声器声压收够，也可能是 FSM 的 `_overlapsTts` 在更上游就短路了（那条路径**本来就没有 metric**，§0 第 5 条） | **✅ 有读数（非预期值）** |
| G3 | 键盘：Maestro 08 退出码；层开着时键盘弹起 → 层重排、发送键仍在树里 | 0；在 | **两半都过**。① Maestro 08 **rc=0，墙钟 133s**（B1 是 126.0s）；② 层开着时弹键盘（专门写的探针流，rc=0，七条断言全过）：轻点光球 → `voice-sheet` 可见 → 点 `composer-input` 弹出键盘 → **`composer-send` 仍在树里** 且 **`voice-sheet` 仍在树里** → 再 `inputText "a"` 后 `composer-send` 依然在 | **✅** |
| G4 | 折叠切换：层开 + 草稿存在时外屏↔内屏各一次 | 两个方向都不丢 | **两个方向都不丢**（泓舟 2026-09-01 手折：长按光球说「帮我查一查今天的新闻」，趁层开着且层内有转写字，外屏→内屏、内屏→外屏各一次，**层都在、草稿逐字未变**）。机器侧佐证：该时段 `sheetDetent` 轴重算 **3 次**——`00:06:34.283`（输入 `turn,voice`）、`00:06:47.365`（输入 `partial,voice`）都是内容驱动；**`00:07:04.921` 那次 `输入 —`**，即 14 条输入投影全部没变而 `sheetDetent` 仍重算 ⇒ 触发源不在 `PresenceInput` 里，只能是容器高度变了，**与折叠的签名相符**。⚠ 同一条里 `commitment` 轴也变了，所以**不唯一归因给折叠**，只记「有一次与折叠相符的、非输入驱动的 detent 重算」。口径：`cmd device_state print-states` = 0 CLOSED / 1 TENT / 2 HALF_OPENED / 3 OPENED …（外屏 `4630947090644569220` 1080×2520；内屏 `4630946481727302019` 2224×2488） | **✅** |
| G5 | 帧率 `framestats` 逐帧口径；同屏循环动画实例数 | ≥55fps（dev build）；1 个 | **呈现节奏 = 60.0 fps**（`IntendedVsync` 逐帧差中位 **16.66ms**、**p90 也是 16.66ms**，120 帧**零掉帧**；`FrameInterval` 16.657ms ⇒ 采样期间屏在 60Hz）。GPU 段中位 7.16ms / p95 7.77ms。**循环动画 = 1 个**：层开时 Composer 主球 f1 vs f2 **0.00%**（22500 px 逐字节相同）／同两帧层内大球 **28.87%**（通道自检）／层关时同一区域 **100.00%**（阳性对照） | **✅（有代价，见下）** |

> ⚠ **G5 的「循环动画 1 个」这一格是拿状态可读性换来的**：那个 `0.00%`（层开时 Composer 主球逐字节相同）来自 `ChatScreen.tsx:530` 的 `orbAnimated={snapshot.input !== 'voice-sheet'}`——层开着就把 Composer 球的动画停掉。而 `speaking` 态的识别特征恰恰是**三青环连续动画**（`AuroraOrb.tsx` 注释原话：「与 speaking 的三青环区分：一次 vs 连续」）⇒ **在语音轮里，输入框旁那个球是「静止版的 speaking」，看不出在播报**（泓舟 2026-09-01 验收当场指出，编号 S4）。**同一个读数，一面是性能达标，一面是状态不可读**——记进遗留，由 B3/B4 重新权衡。
...
**⬜ 没有外部分布读数；泓舟裁定「算过」。** 实际做的是**泓舟本人自评（1 人，非外部）**，2026-09-01：

**① 状态识别 6 张（`D:/Android/b2-sample/S1–S6.png`，用纯 stdlib 的 `pngcrop.py` 把标题行与 `transport=…` 判据行裁掉——
不裁就等于把答案印在图上）**：

| 图 | 态 | 泓舟判读 |
|---|---|---|
| S1 | armed | ✅ 光球对；「真机上没见过『说「小舟小舟」』这个胶囊」→ 追问后确认是 **D2 的 3s 后隐藏**，非缺陷 |
| S2 | listening-ptt | ✅ |
| S3 | thinking | ✅ |
| S4 | speaking | ✅ 识别对，但指出**输入框旁那个光球状态不对**（→ 查实为 `orbAnimated` 停动画，见 G5 那条的代价说明） |
| S5 | attention-confirm | ✅ |
| S6 | offline-with-confirm | ✅ 识别对，但指出**光球边缘被截断**（→ 查实为 `filter` 裁子元素，见遗留） |

**⇒ 识别 6/6**（目标 ≥5/6）。**但这是 1 个人、而且是本项目最熟悉这套状态的人**，
它证明不了「外部用户读得懂」——**§11.4「状态可读性」在 B2 仍然没有外部基线**，B3/B4 接手时别把 6/6 当成分布读数。

**② 录音手势**：泓舟自评「**找到比较久**」。目标是「≤15s 内发出」，**没有秒表读数**。
这一条当场牵出一个真缺陷：他要求「按住对话框也能进录音」，而**计划 §5 第 15 条声称这条能力已经有了**——
实测发现它**在真机上不工作**（见遗留）。所以「找得久」至少有一部分不是发现性问题，是**功能没生效**。
另提一条纯设计项：**发送按钮改成图标**（对标小艺 / 小爱同学），记 B3/B4。
...
| ④ | T6 两格 | **a) 15s 硬上限 ⬜**：本次轻点走的是 `hf.wake()`（免唤醒开着），一轮在 **6.7s < t ≤ 9.7s** 之间收尾（连拍文件大小从 825–835KB 掉回 ~712KB 为判据）——**不是硬上限**，而且轨迹页实录显示环境里有持续人声被收进来（`listening · voice-sheet ·「啊？知识库，没有…这飞书的这知识过」`）⇒ **「安静环境」这个前提本身不成立**，要验硬上限得先关免唤醒**并且**换安静环境。**b) 输入框有字长按：上半 ✅ 下半弱证据**——输入框里有 `a`、键盘弹着时长按光球（三段式注入按住 4s），**层照常升起并在收音**（顶缘极光在、层内实时转写大字、胶囊在听、Composer 光球带青环，`a4b_orb.png`）⇒ **输入框有字时光球仍可 PTT** ✅；而长按输入框区域那次**没有触发 PTT**（光球无青环，`a4b_input.png`），但**没拍到原生选择弹层**（松手 2s 后再截图，弹层可能已消失）⇒ 「走原生选择」这半只有「没走 PTT」的弱证据。**c) 2026-09-01 补的正例实测：空输入框长按 4s ⇒ 层不升、PTT 不启动**（见遗留 `plateGesture` 那条）——**反例过了，正例是坏的** |
...
- **本批踩的坑**：
  ...
  7. **⚠ 取证会话末尾 adb / USB 链路失效，剩余取证被迫中断**。演进顺序：`dumpsys bluetooth_manager | grep` 卡住 →
     ...
     很可能就是 `check_android_env` 报的那个 shadow 副本），怀疑它占着 USB 句柄。**修法需要物理重插数据线或提权杀进程，两件都得泓舟做。** 中间探过一次：`adb devices` 本身 rc=0 但**设备列表是空的** ⇒ 设备已完全不在枚举中，不是 adb 慢。
     **结局：泓舟重插数据线后恢复**——但**重插本身不够**，还要 `adb kill-server` + `start-server` 让 server 重新枚举 USB（重插后第一次 `adb devices` 仍是空列表），并且**再重建一次 `adb reverse`**。
     那个杀不掉的 `adb.exe` PID 14604 至今仍在，恢复后没有再捣乱。
  ...
  11. **⚠ Metro 会中途死掉，症状是 dev-client 报「There was a problem loading the project」**，
     而 `adb reverse --list` 明明是好的。真因判据只有一条：**宿主侧 `curl http://localhost:8081/status`**——
     本批读到 `http=000`（连不上），`netstat` 显示 8081 **没有监听**、只有一个 `SYN_SENT` 的客户端，
     进程列表里也不再有 expo/metro 的 node 进程。⇒ **「应用加载失败」有两个长得一样的真因（reverse 没了 / Metro 死了），
     必须分别用 `adb reverse --list` 和 `curl /status` 两条独立判据区分。** 重启 Metro 后一切正常。
  12. **⚠ 只验反例、不验正例，缺陷就正好躲在没验的那半**（`plateGesture`）。第 3 批 T6 的验收项与我昨天的附加④b
     **验的都是「输入框有字时长按不该录」**（这条一直是过的），而「没字时该录」**一次都没验过**——它是坏的。
     这正是本计划 §0 第 5 条自己写着的那句「**『没取证』的那一格可能正藏着缺陷**」，**在同一份计划里第三次应验**
     （前两次：PTT 在免唤醒下坏、`settingsMeta` 对剥旧键零敏感）。**判据：一条能力如果只有否定式验收项，就等于没验过。**
  13. **一次「用户体验反馈」里可能混着缺陷**：泓舟报的「找入口找得久 + 建议按住对话框也能录音」，
     听起来是**设计建议**，查下去是**计划声称已有、实际不工作的能力**。⇒ 收到体验类反馈时，
     **先去代码里确认「这件事我们是不是本来就说有」**，再决定它是需求还是缺陷——两者的优先级和修法完全不同。
...
- **遗留出账（前三批逐条核过再写现状）**：
  ...
  | 第 3 批 | T11 回声未触发 | **有读数了（非预期值）**：三条前置条件全成立（无 AEC 包 / 走扬声器 / 音量 140）下，8.6s 与 32.7s 两轮长播报**都没有触发提示、也没有回声环**。⇒ 缺口从「没条件测」变成「**提示路径从未被观测到触发过**」，B3/B4 要么造一个能确定触发的装置（直灌回声音频），要么承认这条提示在真实使用中可能永远不出现 |
  ...
  | **本批新增（验收轮，泓舟裁定记下一批）** | **S4：语音轮里 Composer 光球看不出「在播报」** | `ChatScreen.tsx:530` 的 `orbAnimated={snapshot.input !== 'voice-sheet'}` 为满足 §11.4「同屏循环动画常态 1 个」把层开时的 Composer 球动画停掉，而 `speaking` 的识别特征正是**三青环连续动画** ⇒ 该态在语音轮里不可读。**G5 的 0.00% 读数与这条缺陷是同一件事的两面**，要一起重新权衡 |
  | **本批新增（验收轮，泓舟裁定记下一批）** | **S6：muted（离线）光球被裁成方框** | `AuroraOrb.tsx:162` 给外层 View 加 `filter: [{ saturate: 0.4 }]`，RN 在 Android 上会**把子元素裁到容器盒内**；而 `:291` 的离线灰环与环境辉光**刻意画在盒外 10%**（`top/left/right/bottom: -size*0.1`）⇒ 被裁成方框。**只有 muted 同时用了这两样，所以只有它被截断**。三种修法任选：容器留出 padding／把 `filter` 下移到只包球体的内层／灰环改画在盒内 |
  | **本批新增（验收轮，泓舟裁定记下一批）** | **⚠ `plateGesture`：「空输入框可长按录音」在真机上从来不工作** | `Composer.tsx:91` 的 `plateGesture` 挂在 `Composer.tsx:136-138` 那个**包着 TextInput 的父 View** 上，而 Android 的 TextInput **自己消费触摸**（长按=光标/选择），RNGH 抢不到。真机实测：长按空输入框 4s，层没升起、PTT 没启动，屏上出现的是**文本光标水滴柄**。⇒ 计划 §5 第 15 条声称已有的能力实际缺席。修法在 RNGH 的手势竞争配置（`blocksExternalGesture` 一类），**必须真机复验**（单测和 tsc 都看不见这条） |
  | **本批新增（设计项）** | 发送按钮改成图标 | 泓舟 2026-09-01 提，对标小艺 / 小爱同学。纯视觉，B3/B4 |
...
- **设备当前状态（逐字段对账）**：
  ...
  - **`locationEnabled` = `true`（已还原，不需要人工处理）**：取 D8 时我把它关成 `false`，而后点位置授权卡的「允许」时
    **App 自己把它写回了 `true`**（回读 RKStorage 确认）——这也顺带说明「允许」这一下不只是本轮授权，它同时打开了设置项；
  - **`speakPolicy` = `'auto'`**（迁移后的默认档，留着即可；期间为取证短暂切到过 `'always'`）；
  - **媒体音量**：我在取证时从 0/150 抬到 140/150；验收当天读到 **60/150**（被调过，不是我），够出声，未再动；
  - **验收轮期间设备 USB 掉线两次**，泓舟各重插一次；**每次重插后都要 `adb kill-server` + `start-server`（重插本身不够，
    第一次 `adb devices` 仍是空列表）并重建 `adb reverse`**——本会话 `reverse` 前后共重建 **5 次**；
