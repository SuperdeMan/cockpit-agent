...
| 候选 | 为什么现在能做 / 不能做 | 入口 |
|---|---|---|
| **Android 陪伴端 App（新客户端 `mobile/`，2026-08-23 立项）** | **M0 地基 + M1 对话 MVP 代码面已完成（均 2026-08-25）**：M0=Expo 57 脚手架/共享接线/白名单守卫/ASCII 镜像构建定案/CI mobile job（首构 21m48s，坑账 §9.11-17）；M1=M1-1..M1-7 全过——会话状态机（8 型帧分发+每轮看门狗+挂起台账，测试 8 正常+8 必测边界逐条对号）、sendRouter 候选拦截+位置闸（18 例）、chat UI+**17+1 卡族**（兜底卡铁则+单卡 ErrorBoundary）、设置页（buildMeta 键集与 HMI 逐键钉住）、双形态外壳、前台主动回执+车况镜像页；jest **80/80**、tsc 0；新增原生依赖（expo-location/clipboard/flash-list/zustand）后镜像构建复验 **BUILD SUCCESSFUL 15m15s**、243MB dev-client APK。**真机首轮已完成（2026-08-26 晨，MIX Fold 4 实机联调）**：M0 五条补验全过 + M1-8 §8.1 主体全过——双确认条并存逐条确认/取消、断网补发（后台冻结与飞行模式两种断法）、失败态后台账存活（后备箱挂起跨离线周期确认执行）、提醒到点 33s 送达+完成闭环、折叠展开实时切双栏+车况镜像、深浅主题、trip_itinerary 走兜底卡=铁则实证；首轮修两处（SafeArea top / 定位 last-known 兜底，`5c8fad9`）；**坑账 §9.18-21 新增四条**（Metro CI 缓存红屏 / watch 丢事件 / adb 驱动三雷 / GMS 网络面死=定位结构账挂 M2）；**勘误：车窗非危险动作**，require_confirm 五对象=后备箱/门锁/油箱盖/充电口盖/前备箱（计划 M1-3 验收语料写错，已在实施记录勘误）。**遗留**：多端并发、零重投复验（首轮两次观察窗均无效——假绿差点入账）、weather 卡单证与整轮复跑（第二轮）、App 专属 AUTH_TOKENS 条目 ⚠ 泓舟（手机档不含 vehicle.control）、mobile-apk `workflow_dispatch` 待泓舟点一次；三条后端观察移交 QA（同一提醒重复 offer×3 / 「我有哪些提醒」upcoming 读侧存疑+50 条过期存量 / 行程天数自述 4 天）。**M2 语音代码面已完成（2026-08-26）**：M2-1 采集定案（A/B 都在真机跑过才选，定 react-native-audio-api——理由不是「A 坏了」而是它要拖第二个音频库进来）、M2-2 PTT 流式 ASR、M2-3 流式 TTS（注入契约三处差异全由 App 侧适配层吸收，**共享 pcmPlayer 与 hmi/ 一行未改**）、M2-4 音频焦点。**真栈机器读数**：ASR 直灌 fun-asr partial×4（首个 476ms）→ final 逐字一致；TTS 流式 cosyvoice 首音 **617ms** / minimax 首音 **1.35~1.39s**（冷启 1.88s，**贴着 <1.5s 判据**）＋麦克风能量 0.12~0.31 vs 底噪 0.06 证明确实出声；jest **133/133**、tsc 0。**引擎偏好已落**（泓舟 08-26：ASR fun-asr 主 / qwen3 次、TTS minimax）——「其次」做成了真的模型回退链（主模型失败→换模型重连+全量重放），因为**批处理兜底在云栈上是 401**（MiMo key，**移交后端**，它是全体客户端含 HMI 的兜底）。**遗留**：真人说话交互面、播报中按 PTT 即停、断网兜底、M2-4 四场景、语音设置持久化、平板（E3 仍未办）、minimax 首音是否接受 ⚠ 泓舟。**M2-5 首轮已跑完机器可判的部分**：barge-in 物理面（播报中 peak 0.145→停后稳态 0.004，**是声音真的没了不是「代码调了 stop」**）、语音设置持久化、断网兜底——断网那条**抓到并修了一个真挂死**（换模型重连清了兜底 timer 而新 ws 在飞行模式下停在 CONNECTING，两处批处理又都是裸 fetch；修后 29 秒出错并回 idle，回归测试已反向验证）。**08-27 泓舟真机补验**：真人按住说话（含手感/误触门槛/权限弹窗）✅、播报听感 ✅。**minimax 首音 1.4s 已查清根因**：慢的不是引擎（它一次性喂全文首音 406ms、四家最快），是网关的 `MiniMaxStreamingTTSProvider` 走 **HTTP-per-sentence**、必须等一整句，而 `_SENTENCE_END` 不含逗号——同句把句号从第 18 字挪到第 6 字，首音 1468→688ms，cosyvoice 则纹丝不动（它是 WS 长连接）。**修法在 llm-gateway 首段切分，超 App 边界已挂账**。**仍未验**：蜂窝网、无 key 引擎回退听感（mimo TTS 现成全 error 可用来验）、M2-4 四场景、平板（E3）；**外部阻塞**：`asrProvider=off` 与批处理兜底要等后端修 MiMo key（ASR 401 / TTS error）。**下一步：M2-5 人工轮**（逐条状态见计划 §5 M2 实施记录末的表）。每次开工先跑 `check_android_env.ps1` | 接手从 [`docs/design/2026-08-24-mobile-app-implementation-plan.md`](docs/design/2026-08-24-mobile-app-implementation-plan.md) **§0 接手须知**开始（逐任务执行真相源，**M0/M1 实施记录已写在其 §3/§4 末**）；需求/选型/架构判断在 [`docs/design/2026-08-23-hmi-android-app-plan.md`](docs/design/2026-08-23-hmi-android-app-plan.md)（§9 决策记录） |
