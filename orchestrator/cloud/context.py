...
# always-include：无论语义预筛结果如何都要在 catalog 里的 agent。
# chitchat 是全局降级兜底；trip-planner 是确定性行程兜底的目标（planning._ensure_trip_*）。
_ALWAYS_INCLUDE = ("chitchat", "trip-planner")
...
class ContextManager:
    ...
    async def _catalog(self, text: str) -> list:
        ...
        for aid in _ALWAYS_INCLUDE:
            if aid in by_id:
                picked.setdefault(aid, by_id[aid])
