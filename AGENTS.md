...
| 候选 | 为什么现在能做 / 不能做 | 入口 |
|---|---|---|
| **Android 陪伴端 App（新客户端 `mobile/`，2026-08-23 立项）** | **可以开工**：三条需求（与 HMI 共存的独立用户端 / 生产标准 / 细节授权代决）与五项决策（RN+Expo、CNG、token 策略、Android 10+、M2 语音随首轮）已定，方案已批准（方向）。纯客户端新增，与下方编排/商户面候选**零文件交集**（唯一交集=只读引用 `hmi/src` 白名单模块）。**开工前置 E1–E6：2026-08-25 已完成 E1/E2/E5/E6**（JDK 17 + Android SDK 命令行工具链 + 用户级环境变量 + 非 ASCII 路径 subst 预案；已用最小 AGP 工程真实构建出 APK 验证整条链）。**只剩 E3/E4 须泓舟**：真机开 USB 调试（`adb devices` 当前为空）+ 设备端 Tailscale 上线（tailnet 里已有 `superdeman-xiaomi-mix-fold-4`，离线中）。**每次开工先跑 `powershell -ExecutionPolicy Bypass -File scripts\check_android_env.ps1`**（逐条验 E1–E6，退出码 0 才动手） | 接手从 [`docs/design/2026-08-24-mobile-app-implementation-plan.md`](docs/design/2026-08-24-mobile-app-implementation-plan.md) **§0 接手须知**开始（逐任务执行真相源）；需求/选型/架构判断在 [`docs/design/2026-08-23-hmi-android-app-plan.md`](docs/design/2026-08-23-hmi-android-app-plan.md)（§9 决策记录） |
