...
**2026-09-20/21 第三批**（remediation §6.15）：G-05 首跑红（工作流自 M4 起没构建过）、D-08 替身 + dev 包 `8df1d1173` 真机闭合、D-09 核出 E-24 / E-25 两条真缺陷并修、真机取到主 TTS × 中断五维与 THINKING × 中断 ×3、E-26 顺带修、G-07 立卡；提交 `59088760` → `8df1d117` → `6faa3c75` → `e7a83638` → `2ed312fb` → `5286c3ba`（CI 一行，用户批准）；OPPO 常驻包 **`5286c3ba5`**。
...
| D-09 | D | 系统音频五维（声音 / 播放器 / FSM / 采集 / 上行）× 四态（主 TTS / S2S / LISTENING / FOLLOWUP）：焦点丢失、耳机断开、系统中断 | **上机前先核事件源，核出两条真缺陷（E-24 / E-25，均已修并真机复验）**。已取：主 TTS 出声中 × 中断五维齐（`5286c3ba5`：-2 后 2ms 停声、1s 后放焦点、不进 FOLLOWUP、App 不切后台）、THINKING（TTS live）× 中断 ×3、LISTENING × 中断 / × 永久 LOSS 按现状记录、出声才持 / 收尾放 9 次、别人的音乐压低不停。**未取**：语音轮 SPEAKING / FOLLOWUP / S2S × 中断（声学路径抖动 + 屋里有电视，铃三次落在 THINKING）、耳机断开 OS 层（无硬件）——装置就绪，安静环境补 | remediation §6.15；在场轨迹 `system_stop:<reason>` + logcat `MediaFocusControl` + `ps -T` 线程 |
...
| G-07 | H | LISTENING / FOLLOWUP 期要不要持焦点：持了才能在采集时把别人的音乐压低（ASR 听得清）、也才收得到来电 / 闹钟的回调（= 评审 F02 的「采集策略」、D-08 那条「系统占麦事实」）；代价是每次收音都压一次音乐 | 产品裁决。判据面已备（`captureFacts.asrUploading / s2sUploading` 可并进 `syncFocusToPlayback`）；现状：收音期不持焦点、来电响铃时麦照开，用户接听 ⇒ InCall 前台 ⇒ AR04 前后台闸停采集 | `core/voice/audioFocus.ts`、`core/voice/captureFacts.ts` |
