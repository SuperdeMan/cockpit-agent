...
| 格 | 为什么做不了 | 谁能解 |
|---|---|---|
| 步骤 1 后半（Edge 标 `driving`）、步骤 5（过程区单行）、步骤 6（VAL 拒绝「打开天窗」）、步骤 11②（「换一批」chip） | **本机发不出中文**（坑①） | 泓舟手输一句；或另想输入通道 |
| 步骤 12（Maestro 06/07/08/09 回归）、步骤 3 的 Scanner 替代读法之外的部分 | **Maestro driver 装不上**（§6.2 坑⑩） | 泓舟在设备侧放行安装 |
| 步骤 8 的外屏横 / book / tabletop 竖 / 分屏、T7 的 PTT 松手与「层不收起只重排」、建议胶囊正例 | **要手折 / 转屏**（改 `user_rotation` 命中系统配置红线） | 泓舟 |
| 步骤 2（提示音人耳）、步骤 10（`shutter` 盲测）、步骤 13（TalkBack / Scanner） | **要人耳 / 人手 / 授权** | 泓舟 |
| 步骤 7 的系统开关那一路 | 改系统设置需授权 | 泓舟 |
...
| `STREAM_MUSIC` speaker 音量 | 0 ⇒ **60** | **未还原，交泓舟裁** | 抬它是为了让音频读数有意义；**泓舟的提示音 / shutter 盲测也需要它非 0**，还原回 0 反而挡住下一轮。要还原就 `keyevent 25` 按到 0 并回读 |
| 设备 `/sdcard` 临时文件 | 本轮 12 个 | **已清** | `ls /sdcard` 只剩第 1、2 批留下的 `b3-*`/`b4-0*`（**别人的，不动**） |
| `cmd device_state` / `wm size` / `wm density` | **一次都没改** | — | 全程只 `print-state` / 只读；`wm size`、`wm density` 无 Override 行 |
...
| # | 要求 | 状态 |
|---|---|---|
| ① | 形态矩阵五截图 | **部分**：外屏竖（本轮多张）/ 内屏双栏 / 抽屉 / tabletop 在 §6.2；**外屏横、book、分屏 ⬜**（需泓舟手折转屏） |
| ② | `driving=true` 帧 → 目标 ≥56dp 读实、文本输入按身份、过程区单行 | **两格拿到**：目标 ≥56dp 有机器读数 + 阴性（56.0dp PASS vs 44.0dp FAIL，探针 rc 0→1）；文本输入 A/B/C 三张 + `composer-input` 节点在/不在的机器读数。**过程区单行 ⬜**——它要一句中文复杂任务，本机发不出中文（坑①）。云栈注入这一半已通（`speed_kmh=30`/`gear=D` 逐字下发到客户端），**Edge 在 process 帧上标 `driving` 那一半 ⬜** |
| ③ | VAL 拒绝 → 无全屏层 + Dock `safety_blocked` 原话 | 可验的半 ⬜（要说「打开天窗」——同样卡在中文输入）；**不可验的半按 §5 第 2 条记 ⬜ 待 Q16**——协议无结构化标记、**无产出方**，不是「没做」而是「做不了」，且不许客户端正则猜 |
| ④ | reduce-motion → 光球静帧 | **本轮在新形态下重新取到**（行车档 + 语音层常驻的 120dp 大球）：整屏 **0.00%**（0 字节）/ 层内大球区 **0.00%**，活体对照 **8.76% / 99.49%**。§6.1 那次（Composer 球，0.00% vs 31.90%）仍有效，两次量的不是同一颗球。**系统开关那一路 ⬜**（需授权） |
...
1. **T13 大部分已做完**（09-03 10:18–10:47 真机轮，见上）。**剩下的按四类原因分类**，第 4 批开工前照这张分类推进，别再当成一堆平铺的 ⬜：
   ① **卡在中文输入**（坑⑥）——步骤 1 后半 / 步骤 5 / 步骤 6 / 步骤 11②。这四格共用同一个前提，**解一个就解四个**；
   ② **卡在 Maestro driver 装不上**（§6.2 坑⑪）——步骤 12 全部。`target_probe` 已改走 `uiautomator`，不再依赖它，但流级回归还要它；
   ③ **卡在手折 / 转屏**——步骤 8 的外屏横 / book / 分屏、§6.2 T7 的 PTT 松手与「层不收起只重排」、T10 的建议胶囊**正例**；
   ④ **卡在人耳 / 授权**——步骤 2 提示音盲测、步骤 10 `shutter` 盲测、步骤 13 TalkBack 与 Scanner、步骤 7 的系统开关那一路。
   ⚠ **音量已经抬到 speaker 60 了**（开工时是 0），③④ 里的音频类格子不再被「音量 0」挡着——这是本轮解掉的一个长期前提（§6.1 遗留①）。
2. **`tintOverBlur = 0.40` 仍是待证参数**（§6.2 遗留③ 原样转出）：真模糊这条生产路径在真机上一次都没跑过，本批也没碰它。
3. **`foldstate`「订阅即查询」不成立**（§6.2 遗留①）原样转出，仍等泓舟裁：并进 B5 那趟重建，还是单开一趟。**两个待裁项（§0 第 3 条①②）本批同样一字未碰。**
4. **C 档那枚点不动的发送键**（见上「一处设计代价」）——第 4 批收口时如果泓舟裁了「不渲」，T13 步骤 3 的清单要同步划掉 `composer-send` 那一格。
5. **`Msg.driving` 至今没有真实产出方的读数**：`ProcessFold` 现在读 `msg.driving || driving`，前者靠 Edge 在气泡上标（协议里有、`types.ts:28` 写着），后者是此刻的行车档。本批只证明了「读得对」（jest 没覆盖组件层，连这个都是靠类型），**「Edge 真的会标」要等 T13 步骤 1 的云栈注入**。
