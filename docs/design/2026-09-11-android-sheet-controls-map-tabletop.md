...
- **桌面姿态真机（Xiaomi MIX Fold 4，同一份 `71d33dc3f`，lastUpdateTime 23:48:47、哈希端本一致）**：用户把内屏折到 90°、下半平放，App 停在对话页。uiautomator（480dpi）：`stage-tabletop` 807×252dp、`stage-mode`「舞台 · 桌面」、左列 `stage-orb-column` 136dp 宽（= 120 球 + 16，上半 252 ≥ 200 ⇒ 120 球，判据 `tabletopStage`）、右列 `stage-pane` 627×199dp `scrollable=true`；**三格车况完整**——`72 %` / `396 km` / `P` 三个数值与「电量 / 续航 / 挡位」标签都在右列可视区内（`xm-02-inner-second.png`），下半是记录 + Composer，分界与铰链对齐。右列下方还有「提醒」占位与「路线」卡头（可滚区），`adb input swipe` 在 HyperOS 上没有推动这个 ScrollView（注入触摸在这台机器上本就时灵时不灵，非产品判据），手指滚动由用户自验。
