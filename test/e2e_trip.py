...
async def ask(recorder: CaseRecorder, payload: dict, desc: str) -> dict:
...
async def _run(recorder: CaseRecorder) -> int:
    ...
    mod_sid = recorder.session_id(2)

    # 轮1：多日行程规划
    m1 = await ask(recorder, {"text": "周末去杭州两天带老人不要太累", "session_id": sid},
    ...
    # 轮2：确认收尾
    m2 = await ask(recorder, {"text": "确认", "session_id": sid, "is_confirmation": True},
    ...
    m_nav = await ask(recorder, {"text": "下一站", "session_id": sid},
    ...
    m_st = await ask(recorder, {"text": "行程到哪了", "session_id": sid},
    ...
    m_rs = await ask(recorder, {"text": "时间不够了精简一下行程", "session_id": sid},
    ...
    m3a = await ask(recorder, {"text": "周末去成都三天轻松点", "session_id": mod_sid},
                    "轮3a 先规划成都三天")
    ...
    m3b = await ask(recorder, {"text": "第二天换一个景点", "session_id": mod_sid},
...
async def main() -> int:
    ...
    with recorder:
        ...
        if rc == 0:
            recorder.pass_case("trip_multiturn_contract")
        else:
            recorder.fail_case(
                "trip_multiturn_contract",
                "assertion_failed",
                "one or more trip assertions failed",
            )
