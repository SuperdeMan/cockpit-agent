...
- **遗留 / 给第 4 批的话**：
  ...
  - **需泓舟真人的那两条已于 2026-08-31 验收通过**（见真机段 T6⑫⑬），**只剩 TalkBack 一条**：MIUI 的触摸浏览要在系统设置里人工确认，adb 写 `touch_exploration_enabled=1` 不生效（第 1/2/3 批连续三次卡在这一处）。判据两句：非录音读 `ORB_A11Y[当前态] + '，开始说话'`（待机=「小舟，开始说话」）、录音中读「小舟，结束并发送」（`Composer.tsx:93`）。
