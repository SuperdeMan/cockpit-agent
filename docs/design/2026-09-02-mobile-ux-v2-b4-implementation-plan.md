...
> 状态：**四批收口（2026-09-03）**——**§11.2 B4 四条 ✅✅✅✅**（形态矩阵五张齐 / Edge 标 `driving` 自动进入行车档 + 目标 ≥56dp 机器读数 56.0 PASS vs 44.0 FAIL / VAL 拒绝原话「高速行驶中请勿打开车窗/天窗」是安全话术非权限拒绝、无全屏层 / reduce-motion 静帧 0.00% 配活体 99.49%），**各自只剩一个具名 ⬜**（分屏、系统开关那一路、Dock `safety_blocked` 待 Q16）。两个缺陷已修（`sheetDetent` 纯比例 → `ui/layout/sheetHeight.ts` 加内容最小高下限；`/native-spike` 的取证装置自己在说谎）。**仍开：T13 八格 + T14 材料等一次真机轮**（其中 Maestro 四流卡在 driver 装不上，需设备侧放行 USB 安装）。§0 第 3 条①已裁「另开 KWS 阈值 A/B 批」、②待澄清；`foldstate` 定性并进 B5 重建。读数与逐条核销见 §6.4
...
| T8 材质（真模糊 + 减少透明度 + 胶囊浅色实底） | `cca08e0` | **467**（+1） | 09-03 补取：真模糊过、`tintOverBlur=0.40` **裁定保持**、20 次开合零崩；**浅色对比不达标且暗盘救不了 ⇒ 出账** |
| T9 发送键图标 + 无障碍补项 | `d7635e6` | 467（零增量） | 09-03 补取：发送键深浅两张 + a11y 树 `Button/「发送」/44×44dp`；Maestro 回归与 TalkBack 仍 ⬜ |
...
#### T8 / T9：代码当轮全绿；真机读数**当轮全 ⬜，次日补取轮已拿到大部分**

做到 T8 步骤 5 的第一条时 **USB 被拔、`adb devices` 空**，当轮到此为止。
T8 的 `settingsMeta` 那条与 `tokens` 那条都做了「先跑红再实现」：两条各自红在自己那一句上
（`● tokens 数值照 A-1 设计系统 › 材质三档…` 与 `● …B4-8：旧库没有 reduceTransparency…`），不是「红了两条」而已。

**读数见下一节「2026-09-03 08:00–08:25 补取轮」。**

#### T8 / T9 真机读数——**2026-09-03 08:00–08:25 补取轮**（USB 回来后）

> ⚠ **口径先说清**：补取时 `HEAD` 已经是 **`7f7445a`**（第 3 批 T10–T13 由另一个会话在此期间落地）。
> 也就是说这些读数取自**含第 3 批代码**的树，不是纯第 2 批。开量前逐点核过 T8/T9 的判据面
> 自 `cca08e0` / `d7635e6` 起没被动过：`GLASS.frosted.tintOverBlur = 0.4` 原值、`VoiceSheet` 壳底的
> `blurTarget ? BlurView + tintOverBlur : tint` 那段 `git diff cca08e0..HEAD` 零命中、
> 胶囊 `p.dark ? p.glassBg : '#FFFFFF'` 原样、`Composer` 的 `Icon name="send"` 原样。
> 设备：`5d432b6d`，包锚 `lastUpdateTime=2026-09-02 16:45:36`（未变），`device_state=HALF_OPENED(2)`
> 机身竖持 ⇒ 内屏 **741×829dp 双栏**，主题深色（浅色那几格另标）。

**① 真模糊真的在（T8 步骤 5-1）——达标**

取法：`adb logcat -c` → 单条 `adb shell 'input motionevent DOWN 112 2336; sleep 4; screencap …; sleep 1; input motionevent UP …'`
（**按住不放**把层稳在收音态，同一个 shell 进程内注入，见坑③）。图：`b4-08-sheet-blur-dark.png` / 反例 `b4-08-sheet-tint-dark.png`。

- `W ReactNativeJS.*blurTarget` **0 条** ⇒ ref 时序没回落（`blurReady` 那个模板是对的）。
- **最大水平梯度**（`hgrad.py`，复用 `png_probe.decode`，沿 x 求相邻像素 luma 差）——同一段答案气泡文字：

  | 情形 | 带 A `x[60,360) y[1800,1855)` | 带 B `x[60,1040) y[1930,1980)` |
  |---|---|---|
  | 无层原样 | **208.9** | **210.9** |
  | 真模糊层内 | **12.0** | **12.0** |

  判据是「≤ B3 ③ 块量级（19.2）」⇒ **过**。两条带都刻意避开层自己的文字（见坑②）。
- **反例**：实验室开「减少透明度」⇒ 同类带最大梯度 **34.0**，且**肉眼上背景记录逐字可读**
  （`b4-08-sheet-tint-dark.png` 里「tell me a short joke」「哈哈，早高峰堵车时…」全都认得出来）。

**② `tintOverBlur` 的裁决（T8 步骤 5-2）——保持 0.40，不抬**

最大梯度对「可读性」不够灵敏（它吃单个亚像素边），改按计划的原口径量**幅度**（`amp.py`：一条横带内 luma 的 p95−p5）：

| 情形 | 幅度 p95−p5 | 层内/层外比 | 判据 ≤0.5 |
|---|---|---|---|
| ① 无层原样 | **211.0 / 222.9** | 1.00 | — |
| ② **真模糊（`tintOverBlur=0.40`）** | **8.0 / 8.9** | **0.04** | **过**（B2 无模糊 .58 时是 0.45） |
| ③ 纯 tint（`tint=0.58`，减少透明度开） | 35.0 / 79.2 | 0.17–0.36 | 也过，但差 4–9 倍 |

⇒ **`tintOverBlur = 0.40` 达标，不抬到 .50、更不回 .58**；「待证参数」这四个字可以摘掉了。
顺带一个说明力更强的读数：真模糊那档壳底**中位亮度 27.8 比纯 tint 的 16.3 还亮**——
**壳更薄却把背景藏得更干净**，这正是「真模糊在场时 tint 可以更薄」那句设计话的实测形态。

**③ 浅色对比（T8 步骤 5-3 / B2 出账⑫）——⛔ 不达标，且计划给的补丁方向是反的**

图 `b4-08-sheet-light.png`。用 `radial.py` 按圆心分环取平均 luma（比两个方框稳，不用猜边缘在哪）：

```
r=[  0, 12) luma=172.9  RGB=(181.5,164.7,228.7)   ← 球心
r=[ 96,120) luma=164.6  RGB=(155.9,161.8,218.1)   ← 球体边缘
r=[192,216) luma=176.9  RGB=(175.8,175.7,192.1)   ← 紧邻壳底
r=[240,264) luma=181.2  RGB=(180.4,180.8,187.5)
```

- 球体边缘 **164.6** vs 紧邻壳底 **177.0** ⇒ 差 **12.4/255**；整段剖面的最大差也只有 **13.8**。
  判据 **≥40/255 ⇒ 差得很远**。**浅色下的光球几乎没有亮度对比，它只靠色相区分**
  （B 通道 228→187，R 通道反而是球更低）。
- 按计划的处置加了浅色专用暗盘 `rgba(10,14,26,0.08)`（`orb×1.5`）**并实测**（`b4-08-sheet-light-disc.png`）：
  球体边缘 164.3 vs 紧邻（暗盘上）158.3 ⇒ **6.0/255，比不加还差**。
  **机制**：光球是半透明的，垫在它后面的暗盘会**连球一起压暗**——球心从 172.9 掉到 146.8，
  surround 从 177 掉到 158，两边一起走，边缘对比原地不动。**这条补丁不是「不够」，是方向反了。**
- ⇒ 按计划的升级路径**出账**：真要过 ≥40 得动光球的浅色高光参数（A-1 §10），
  而「B4 不动光球」是本计划自己写死的。补丁已 `cp` 还原、`git status` 干净（工作树零残留）。

**④ 胶囊浅色实底（T8 步骤 5-4）——达标**

`b4-08-sheet-light.png` 里胶囊底 `png_probe px`：`(600,2020) = (255,255,255)`、`(490,2036) = (250,252,254)`
（边缘抗锯齿）⇒ 浅色下是**不透明白底**，不再是 B1 那种透出深空 blob 的 76% 白。

**⑤ 稳定（T8 步骤 5-6）——达标**

层开合 **20 次**（同一条 adb shell 里 `while` 循环做 DOWN/UP）：

- 进程 pid **21036 全程未变**、`Running "main"` **0 次** ⇒ 没重启、没重载；
- 该 pid 的 `E/` `F/` **0 条**——**这条先做了通道自检**：同一过滤器下该 pid 有 **1631 行**日志，
  所以「0 条」是真的零，不是过滤器没命中（见坑①）；
- 全 buffer `FATAL EXCEPTION` / `ANR in com.xiaozhou` / `has died` **0 条**。

**⑥ 发送键图标（T9 步骤 4-1）——图已备好，等泓舟看**

`b4-09-send-icon-dark.png` / `b4-09-send-icon-light.png`：圆形极光底 + 线性纸飞机，深浅各一张。
**这一格是「泓舟看图定方案」，不是我判**。

**⑦ 无障碍树（T9 步骤 4-3 的单人替代读法）——`composer-send` 拿到了，卡片按钮 ⬜**

用 `uiautomator dump`（不是 `maestro hierarchy`，见坑④）读到的 `class=Button` 全部节点：

| testID | class | content-desc | bounds | 换算 |
|---|---|---|---|---|
| `composer-send` | **Button** | **「发送」** | `[927,2278][1059,2410]` | **44×44dp** |
| `composer-orb` | Button | 「小舟，开始说话」 | `[30,2242][198,2410]` | 56×56dp |
| `health-dot` | Button | 「连接正常；打开隐私栏」 | `[384,144][504,264]` | 40×40dp |
| `receipt-toggle` | Button | 「已执行 · 展开回执」 | `[189,1911][1008,2007]` | 273×32dp |

⇒ `accessibilityRole="button"` 在 Android 上落成 `class=Button`、`accessibilityLabel` 落成 `content-desc`，
**发送键的 role/label/44dp 三项都读实了**。⚠ 但这**只证明 label 在树里，不证明 live region 行为**
（转写/回答区的 `accessibilityLiveRegion` 要 TalkBack 才验得到，仍 ⬜ 需泓舟）。

⬜ **卡片按钮（`CardButtons` 的 role/label/48）这轮造不出来**：真栈这一轮出的两张卡都没有 `buttons`
（`charging_list` 走兜底卡渲列表行、导航走执行回执），而卡片画廊那屏 `uiautomator dump`
一直 `could not get idle state`（见坑④）。这三行改动目前只有静态样式 + tsc 兜着。

**⑧ 意外收获：T5 的两份判据第一次有了真实生产消费方**

问「附近的充电站」拿到真栈真卡（`b4-09-card-check.png`）：

- 对话流里 `charging_list` 走 **T5 的通用列表兜底卡**——渲出 5 行「站名 + 距离」+ `soc: 72`，
  正是 `cardFields.cardListRows` 的设计形态（B2 出账⑨ 的 mobile 那一半**在真卡上成立**）；
- 右舞台**同一张卡**同步显示为场景卡（`stageScene` → `focus`），
  即 **T6 的 `StagePane` 与 T5 的 `stageScene` 在真数据上串起来了**。

§6.1 遗留③ 说「五份判据里有四份此刻零生产消费方」——`cardFields` 与 `stageScene` 这两份**已经兑现**。

**⑨ 仍 ⬜ 的格子与各自的前提**

| 格 | 前提 |
|---|---|
| Maestro **01 / 06 / 08** 回归（T9 步骤 4-2） | ⛔ **Maestro driver 装不上**：`dev.mobile.maestro` 不在设备上（只剩 `.test`），装它被 MIUI 拒（见坑⑤）。**需泓舟在设备侧放行**。⚠ 注意 07 与抽屉那两跑是 09-02 夜里做的，读数有效，只是本轮没复跑。 |
| TalkBack 三项（转写/回答 live region、卡片按钮读法） | 需泓舟开无障碍服务（改系统设置） |
| 卡片按钮 a11y 节点 | 需要一张带 `buttons` 的真卡，或让画廊那屏 idle 下来 |
| 外屏横 / 分屏 / book / PTT 松手 / 层不收起 / ①语音层 BACK | 需泓舟的手（见上文各处） |
...
3. **MIUI 拒绝 USB 安装**：Maestro 第一跑死在 `INSTALL_FAILED_USER_RESTRICTED: Install canceled by user`，
   而 `--no-reinstall-driver` 并不能跳过（driver 当时确实不在设备上）。解锁后一次就过。
   ⚠ **当轮据此把它归因成「锁屏」是错的**——09-03 补取轮里设备解锁、亮屏、应用前台，**照样拒**（见坑⑩）。
   解锁那次能过大概是巧合或另有条件。**唯一站得住的结论是：报错文案里的 "canceled by user"
   不是「有人点了取消」**；到底是什么，本批没定性。
...
7. ⛔ **`$pid` 是 PowerShell 的只读内置变量**：`$pid = (adb shell pidof com.xiaozhou.companion)` 会报
   `Cannot overwrite variable PID` 但**不中断脚本**，后面的 `adb logcat --pid=$pid` 于是拿 **PowerShell 自己的进程号**
   去过滤设备日志 ⇒ 打出「`E/` `F/` **0 条**」。**这是一条彻头彻尾的假读数**：那个 pid 在设备上根本不存在。
   ⇒ **凡是判据形如「0 条」的读数，先做通道自检**（同一过滤器下的总行数 >0 才算数）。改名 `$appPid` 后
   自检到 1631 行，「0 条 E/F」才成立。（坑账「观察对象缺席时零事件=零证据」的第 N 次，这次缺席的是过滤器。）
8. **「按住不放」是本轮唯一能把语音层稳住的仪器**，而且必须在**同一条 `adb shell` 里**
   `input motionevent DOWN …; sleep N; screencap …; input motionevent UP …`——拆成两次 adb 调用会各自带
   自己的 `downTime`（B3 坑）。胶囊那条路（坑⑤）三跑三败，这条一次就成。
9. **`uiautomator dump` 要窗口真的 idle**：默认在对话页只吐到 `ComposeView` 一层（**3.7KB，零 RN 节点**），
   看着像「RN 树不暴露给 uiautomator」；**开了「减少动效（强制）」之后同一屏变成 80KB 的完整树**。
   ⇒ 本批新加的那个开关意外成了无障碍取证的仪器。⚠ 但卡片画廊那屏即使开着它也一直
   `ERROR: could not get idle state`（那屏还有别的东西在动），所以它不是万能钥匙。
10. ⛔ **Maestro driver 装不上，且与锁屏无关**（推翻坑③ 的定性）：设备上只剩 `dev.mobile.maestro.test`，
   主包 `dev.mobile.maestro` 不在，装它一律 `INSTALL_FAILED_USER_RESTRICTED`。解锁、亮屏、应用前台都试过。
   ⇒ **需泓舟在设备侧放行**；在此之前 Maestro 类读数全部取不到（07 与抽屉那两跑是 09-02 夜里做的，仍有效）。
11. **量「层内高频」时把层自己的文字圈进带里**，读到 129.1，看着像「模糊没生效」——实际那是
   `收起` 两个字的边缘。计划里「层内一条**没有文字**的横带」不是修辞，是判据的一部分；
   换成真正无文字的带之后同一张图读到 12.0。
12. ⛔⛔ **做完对照实验只 `print-state` 不 `reset`，把设备留在了强制态——泓舟折上手机后外屏不亮**
    （2026-09-03 08:33 泓舟报障）。T7 那次「切 `device_state` 看 fold 事件」的实验最后一条是
    `cmd device_state state 2`，之后我只反复 `print-state` 读、**没有还原**；机身折上时框架仍以为是
    HALF_OPENED ⇒ 驱动内屏、外屏不点亮。`cmd device_state state reset` 后 `print-state`=0、
    `wm size` 回 1080×2520、内屏 `mState=OFF`、外屏点得亮，已恢复。
    ⇒ **坑② 说的是「reset 之后要回读」，这一条说的是「压根忘了 reset」**——两件事。
    凡是 `cmd device_state` / `wm size` / `wm density` 这类**改设备状态**的动作，
    **必须在同一条命令里带上还原**，或者当场记进待还原清单，不能靠「等会儿一起还」。
    本轮其余状态回读全干净：`wm size`/`wm density` 无 Override 行；系统设置类（旋转锁定 / 动画缩放 /
    无障碍服务）**全程只读没写**，一次 `settings put` 都没发过。
...
3. ✅ **`tintOverBlur = 0.40` 已实测裁定「保持」**（09-03 补取轮②：层内/层外幅度比 **0.04**，判据 ≤0.5）。
   真模糊这条生产路径（`BlurTargetView` 包 `FlashList` + `blurTarget` 指过去）也在真机上跑通了
   （`blurTarget` 告警 0 条、背景幅度 211→8）。**本条原来的「只有 tsc 兜着」已解除。**
   ⛔ 但**浅色下光球与壳底的对比不达标**（12.4/255，判据 40），且计划给的暗盘补丁**方向是反的**
   （光球半透明 ⇒ 垫暗盘连球一起压暗）——补丁已还原，转出给「动光球浅色高光参数」那条（A-1 §10，B4 不动光球）。
...
7. ⛔ **未推送清单在补取轮期间变了，别照本批收口时那份念**。本批 4 个代码提交
   `0ff7c10`(T6) / `91d1113`(T7) / `cca08e0`(T8) / `d7635e6`(T9) + 本节记录提交，开工时
   `origin/main..HEAD` 是 0 条（`HEAD` = `origin/main` = `d3dde0a`），**这五条里没有别人的东西**。
   但 09-02 夜里到 09-03 早上，**第 3 批（T10–T13）由另一个会话落在了它们上面**
   （`f6d0159` / `832d9a3` / `525e405` / `7f7445a`）⇒ 现在 `origin/main..HEAD` 是 **9 条以上，其中 4 条不是本批的**。
   ⛔ **push 的粒度是分支不是提交**（M4-R1 教训）：推之前必须**当场重列** `origin/main..HEAD`
   并单独取得泓舟授权，不能拿本节这份清单当准。
8. **两个待裁项（§0 第 3 条①②）本批一字未碰**，仍等泓舟裁；本批也没有捎带任何语音类读数。
9. **给收口批的一条**：09-03 补取轮里 `charging_list` 的真卡同时跑通了 T5 兜底卡与 T6 舞台场景卡
   （见补取轮⑧）——B2 出账⑨ 的 **mobile 那一半可以关掉了**；hmi `types.ts` / `_prov` 那一半原样仍开。

### 6.3 第 3 批「行车档」（T10–T13）

**本批开工授权**：泓舟 **2026-09-02** 当轮批准第 3 批开工，并指定**顺序改成「先码后验」**——T10 → T11 → T12 只做代码 + jest + tsc，三个任务各自的真机步骤（T10 步骤 6 / T11 步骤 7 / T12 步骤 6）不在任务内做，**全部并进 T13 统一验，读数仍记在各自任务名下**。计划头部「草案待批」状态行本批不改（四批走完由收口批统一改）。

**等第 2 批收口**：开工时第 2 批仍在飞（T9 未提交、工作树 6 个文件在改、§6.2 是空壳），按泓舟交代等到它收口（`d7635e6` T9 + `91a986e` §6.2 记录）才动手。等待期间**一个字节没落盘**——往 `mobile/test/` 加文件会改掉它 `npm test` 的条数、进而污染它 §6.2 的收口读数。

#### 开工基线（2026-09-03，本会话自跑；有效期只到下一次改动）

| 项 | 读数 |
|---|---|
| `powershell -File scripts\check_android_env.ps1` | 退出码 **0**（17 pass / 1 warn / 0 fail；唯一 WARN = E3 `no device attached`） |
| `python scripts/dev_stack.py target show` | `{"source":"file","status":"target","target":"cloud"}` |
| `cd mobile && npm test` | **47 suites / 467 tests 全绿**（53.1s） |
| `npm run typecheck` | **0 error** |
| `git log --oneline origin/main..HEAD` | **5 条**，全部是第 2 批的（`0ff7c10` T6 / `91d1113` T7 / `cca08e0` T8 / `d7635e6` T9 / `91a986e` §6.2）——与 §6.2 遗留⑦ 的清单逐条对得上，里面没有别人的东西；`git status` 干净 |
| 设备（`adb devices` / `adb reverse --list` / `lastUpdateTime`） | **⬜ 三项全缺**：`adb devices` 空列表、`adb reverse --list` 报 `no devices/emulators found`、`lastUpdateTime` 取不到。代码段不需要设备；**T13 整轮因此没有开工**（见下） |

#### 逐任务

| 任务 | 提交 | jest 条数 | tsc |
|---|---|---|---|
| T10 身份与行车档设置 | `f6d0159`（5 文件 +102/−8） | 467 → **471**（+4） | 0 |
| T11 行车档布局全套 | `832d9a3`（16 文件 +522/−121，2 新文件） | 471 → **481**（+10） | 0 |
| T12 播报收紧 | `525e405`（6 文件 +177，2 新文件） | 481 → **486**（+5） | 0 |

**条数比计划预期多**（计划 +2/+5/+4 ⇒ 476；实际 +4/+10/+5 ⇒ **486**）：按泓舟「正例反例分开 test」的要求，计划里把多条断言打包在一个 `test` 里的都拆开了。拆的理由不是洁癖——**同一个 `test` 里 jest 在第一个失败断言就停**，两侧同时红时只看得见前一条（B4 第 1 批坑）。确实要「一次摆多格」的（画廊三样本 / 三档映射 / 三优先级）改成**单条数组断言**，一次全看得见。

#### 一件必须先说的事：**「让位 / 反例」类用例在实现之前就是绿的**

T10 的 3 条让位、T11 的 5 条（A 不常驻 / 3s 内仍 0.62 / 播报中不回落 / 有卡 0.78 / 非行车不受影响）在步骤 2 跑红时**全是绿的**——`drivingSuggest`、`answeredAt` 那时什么都不做，而期望值恰好等于现状。步骤 2 真红的只有 T10 1 条、T11 3 条、T12 5 条（模块不存在）。

这是 **B4 第 1 批坑③「期望值恰等于初值 ⇒ 对『接没接上』零敏感」的第二次应验**。它们不是废用例——它们钉的是**判据插在链条的哪一段**，红只在反向验证里看得见（T10-1 / T11-1/2/3 全都恰好红在这些「一开始就绿」的用例上）。**但这意味着：光看步骤 2 的红条数，会以为覆盖面比实际大。**

#### 反向验证（每条先 `grep` 证明变异落盘，跑完用**副本**还原并复跑全绿）

| # | 变异 | 计划预期 | 实红 | 一致？ |
|---|---|---|---|---|
| T10-① | 建议胶囊那行挪到 `errorLive` 之前 | 恰好红「错误在场时让位」 | 1 条：`× 错误在场时建议让位` | ✅ |
| T10-② | `drivingSuggested` 删掉 `landscape` 条件 | 红 `drivingMode.test.ts` 的「缺 landscape」 | 1 条：`× 缺 landscape ⇒ 不建议` | ✅ |
| T11-① | `settled` 去掉 `agent !== 'speaking'` | 恰好红「播报中不回落」 | 1 条 | ✅ |
| T11-② | `resident` 从 `sheetOpen` 里删掉 | 恰好红「B/C 常驻」，A 那条仍绿 | 1 条：`× B/C 行车：无轮也常驻`，A 那条绿 | ✅ |
| T11-③ | `DRIVING_SHEET_SETTLE_MS` 3000 → 5000 | 恰好红「3s 后回 0.4」 | 1 条：`× 答完满 3s 回落 0.4`，「3s 内仍 0.62」仍绿 | ✅ |
| T11-④（自加） | `followUpChips` 的 `max` 退回写死 `MAX_CHIPS` | —（计划无此条） | 1 条：`× max 参数：行车档 3 条封顶` | — |
| T12-① | 行车强制那行删掉 | 「**恰好红第一条**」 | **2 条**：「静音档也说」+「S2S 忙 ⇒ 抢话」 | ❌ **被推翻** |
| T12-② | `autoplay` 改成 `!== 'silent'` | 恰好红「自动档不出声」 | 1 条 | ✅ |
| T12-③（自改） | 见下「共享判据不许变异」 | —（计划原条不可执行） | 2 条：三档映射 + 三优先级 | — |

**T12-① 为什么是 2 条**：第二条用例（`{ policy: 'auto', driving: true, s2sBusy: true }` ⇒ `interrupt`）的**播报档是「自动」**。行车强制那行一去掉，它就落回共享判据，而共享判据的 `autoplay` 闸（`policy === 'always'`）当场把它拦成 `bubble` ⇒ 同样红。计划把这条算进了「不受影响」的那一半。这是 **B4 第 1 批「计划预写的『会红哪几条』是待证命题」的第五次应验**——9 条里 8 条对了，错的这条指向一个真实的事：**行车强制那行同时是两条用例的唯一出口，它俩不是独立的两格。**

**T12-③ 改法（共享判据是硬边界，不许变异它）**：计划原文的第③条是「共享 `decideSpeech` 的三条——mobile 只是透传，红了说明共享判据变了」。那不是一个可执行的变异（要动 `hmi/src/proactiveSpeech.mjs`，命中 §0 第 2 条与「共享判据一字不动」）。改成变异**透传那一行**（`return d === SPEAK || … : BUBBLE` → `return BUBBLE`），同样证明「共享判据真的在管事」，而 `git diff --stat -- hmi/` 全程为空。

#### 三处「计划片段 ≠ 源码现状」（按源码落，不照抄）

1. ⛔ **`PendingSpeech` 的 API 计划写错了**。源码（`hmi/src/proactiveSpeech.mjs:46`）是 `push({ text, deliveryId })` + `drain()`（一次给全部）；计划伪码写的 `push(text)` 与 `shift()` **两个都不存在**。计划自己标了「入参形状 / 出队方法名照源码」，所以这是待核项已核，不是变更。顺带：既然 `PendingSpeech` 按 `deliveryId` 去重，`SpeechSink.proactive` 的入参就加了 `deliveryId`（计划签名里没有）——**不传它去重就是关着的**。
2. ⛔ **补播挂点从计划的 `setSpeaking(false)` 改成「S2S 空闲 ∨ 播报自然收尾」**，这是对计划的一处**实质偏离**：`begin()` 的第一件事就是 `stop()`，而 `stop()` 会走到 `setSpeaking(false)` ⇒ 按计划挂在那里，攒下的旧话会在**新一轮开口的瞬间**倒出来，两段音频叠着放——正是共享模块头注要避免的那件事。真正的阻塞条件是 ①`s2sBusy`（`decideSpeech` 只在 s2sBusy 时给 DEFER）②自己的播报在跑，所以挂在这两条各自解除的那一刻（`setProactiveCtx` 里 busy→idle 的那一跳 + `TtsSession.onEnd`），并用 `flushing` 守住重入、`await` 串行补播（`drain()` 一次给全部，同时喂 `speakBatch` 就是几段叠着放）。
3. **`ChatScreen.tsx:511` 的过期注释**（B3 §6.1 遗留④）已清，换成说实话的一句：「点按默认打开语音层；建议胶囊 = 开行车档，做什么由 `capsule.action` 决定，判据不在这里」。

#### 一处设计代价，交泓舟看

**C（可信车载平板）行车档下，`composer-send` 是一枚永远点不动的键**：那一档没有输入框（§6.0「C 隐藏」），所以发送键恒无字可发。按计划实现成「仍然在场 + `disabled` + 透明度 0.45」——理由是 §6「目标 ≥56dp」的读数（T13 步骤 3 的 `composer-send`）**要有演员**。代价是屏上多一枚死键。另一条路是 C 档直接不渲它，那样 T13 步骤 3 的清单里就得划掉这一格。**这一条按计划落了，不自行改设计**。

#### T13 B4 真机验收——**2026-09-03 10:18–10:47 真机轮**（泓舟插上 USB 后当轮补做）

代码三任务做完时 `adb devices` 全程空列表，T13 一步没做；泓舟接上真机后本轮补做。
包锚 `lastUpdateTime = 2026-09-02 16:45:36`（与 §0 第 1 条主线包一致），`dev_stack target show` = cloud。

**步骤 0 前提**：`5d432b6d device model:24072PX77C` 在线；`adb reverse --list` 开工时**空**（B3 坑⑬ 第四次），
重建 `UsbFfs tcp:8081 tcp:8081` 后非空；`mWakefulness=Awake`、`mDreamingLockscreen=false`；
`cmd device_state print-state`=**0**（外屏），外屏 `displayId=0` / 物理 `local:4630947090644569220` / 1080×2520 / `mState=ON`，
内屏 `displayId=1` / `local:4630946481727302019` / 2224×2488 / `mState=OFF`；`wm size`=1080x2520、`wm density`=480，**均无 Override 行**。
⛔ **`STREAM_MUSIC` speaker 音量开工时仍是 0**（§6.1 遗留① 的前提）——`adb shell input keyevent 24` ×8 后
`dumpsys audio` 回读 `streamVolume:60`、`2 (speaker): 60`、`Devices: speaker(2)`，音频类读数这才有意义。
`png_probe.py selftest` = **PASS**（五种 filter + 部分解码，仪器自检）。

**App 设置基线**（截图读，`b4-13-settings-*.png`）：服务器 `car-agent-dev.tailc936bb.ts.net:8443` / token ····j2_Z；
设备角色 **手持**、行车档 **关**；主题 深色、字号 标准、保持屏幕常亮 **关**；语音挡位 三段式（默认）、看图问答 开；
识别引擎 DashScope 实时；回答长度 标准、模型偏好 自动；能力开关 全开；记忆 开、使用定位 开；
实验室四开关：状态锚 **开** / Focus Dock **开** / 减少动效（强制）**关** / 减少透明度 **关**。

**先证明「我的代码到了设备」**（B3「A 不工作先证明 A 的输入到达了 A」）：设置页出现 T10 新增的
「身份与行车」分区（在服务器之后、显示之前），说明行「手持陪伴端 · 不控车」——`b4-13-settings-identity.png`。
Metro 是第 2 批会话起的，本轮**复用不动它**（`/status` 回 `packager-status:running`）。

##### 步骤 1：云栈 `driving` 帧注入（§11.2 B4 ② 的前提）

`POST /api/debug/vehicle {"key":"speed_kmh","value":30}` → `{"ok":true}`，回读 **`speed_kmh=30`，`gear` 联动成 `D`**（后端自己改的，不是我写的）。
`/debug` 屏收到的 `vehicle_state` 帧里逐字为 `"speed_kmh":30,"gear":"D"` ⇒ **注入值真的下发到了客户端**。
⬜ **「Edge 在 process 帧上标 `driving:true`」这一半没取到**——它要一句会出过程区的中文复杂任务，而本机发不出中文（见坑①）。

##### 步骤 3：目标 ≥56dp 读实 + 阴性（**换了读法**）

Maestro 那条路本轮走不通（§6.2 坑⑩：driver 装不上）。改用 `uiautomator dump`——**RN 的 testID 原样映射成 `resource-id`**，
且 §6.2 补取轮⑨ 的发现成立：开「减少动效（强制）」后对话页能 idle 出完整树（57KB）。`target_probe.py` 因此加了 XML 分支（提交 `c852062`）。
`--density 480`（3px/dp）：

| id | 行车档 **开**（角色 C） | 行车档 **关**（同角色） |
|---|---|---|
| `composer-orb` | 168×168px = **56.0×56.0dp PASS** | **56.0×56.0dp PASS**（本来就是 `TARGET.driving`，不随行车变——与代码注释一致） |
| `composer-send` | 168×168px = **56.0×56.0dp PASS** | 132×132px = **44.0×44.0dp FAIL** |
| `voice-sheet-collapse` | 288×168px = 96×**56.0dp PASS** | **缺席** |
| `composer-input` / `composer-plate-overlay` | **缺席**（C 隐藏） | **在场** 220×42dp |
| `voice-sheet` / `voice-sheet-shell` | **在场**（常驻） | **缺席**（不常驻） |
| 探针退出码 | **0** | **1** |

⇒ 探针**量得出差别**（rc 0→1），不是恒绿。这张表同时是「文本输入按身份」与「语音层常驻」的**机器读数**，
不只是截图——比计划要求的更硬。⚠ `presence-capsule` 靠 hitSlop 达标、视觉仍 26dp，uiautomator 给的是视觉 bounds，
本轮它**根本没渲染**（idle 时 capsule 为 null，§6.2 坑⑤），所以这一格不是「FAIL」而是**没有演员**，单列说明。

##### 步骤 4 / T11 步骤 7：文本输入按身份 + 行车档布局（三张 `b4-13-input-{A,B,C}.png`）

| 角色 | 文本输入 | 语音层 | 其它 |
|---|---|---|---|
| A 手持 | **输入框常驻** | **不常驻**（欢迎态，无层） | 发送键 56 |
| B 支架/副驾 | **折叠成圆形键盘键**（`composer-keyboard`） | **常驻**、回落形态（120dp 大球 + 「⌄ 收起」，转写/回答/chips 全不渲） | 快捷指令 3 条 |
| C 可信车载平板 | **无输入框、无键盘键** | **常驻**、同上 | 发送键**明显变暗**（`disabled` + opacity 0.45） |

A 那一格是 `sheetResident('handheld', true)=false` 的真机证据。B/C 的层压在记录上、记录透过**真模糊**仍可见
（`b4-13-proactive-bubbles.png` 里两条气泡被糊但读得出）——§5.2「记录变暗 60%、仍可见」保留。
⛔ **C 档那枚永远点不动的发送键在真机上确实很显眼**（见上「一处设计代价」），交泓舟裁。

##### T10 步骤 6：身份三选 + 行车档开关 + 建议胶囊

三种角色各切一次，**说明行随之变**（三张）：
「手持陪伴端 · 不控车」/「支架 · 副驾协同 · 不控车」/「可信车载平板 · 车控由服务端 token 决定，这里只决定布局」——
`b4-13-settings-identity.png` / `b4-13-role-B.png` / `b4-13-settings-top.png`。**说明行从一行变两行会把下面的开关整体下推 53px**（坑②）。
行车档开关开/关各截图回读，App 行为随之变（上面两张表）。
⬜ **建议胶囊的正例没取到**：三条件要「身份 C + 横屏 + 常亮」同时成立，横屏需转手机或改 `user_rotation`（命中系统配置红线）⇒ 需泓舟。
阴性侧本轮一直成立（竖屏 + 常亮关 ⇒ 所有截图里都没出现该胶囊）。

##### T12 步骤 6：主动消息播报仲裁（`/debug` 屏本地回放，零后端）

云栈无 proactive 注入入口（`grep -rn proactive scripts/ observability/collector/server.py` 零命中，静态核过）⇒ 用本批加的回放按钮。
判据经历了一次修正：**`AudioPlaybackConfiguration ... state:started` 会粘住**——`sharedAudioContext` 的 AAudio 流建了就不关，
播完 12s 仍是 `started` ⇒ 它只能证明「第一次从无到有」，分不出后续每一次（坑③）。改成
**「属于 App pid 的播放配置条数」+ 每轮前重启 App 拿 0 条基线 + 阴性在前正例在后**：

| 进程 | 行车档 | priority | 播报档 | 属于 App 的播放配置（等 12–14s） |
|---|---|---|---|---|
| 26083 | 开 | **普通** | 自动 | **0 条** ← 阴性①（priority 维度） |
| 26083 | 开 | **critical** | 自动 | **有**（`piid:12095`，`type:AAudio`、`usage=USAGE_MEDIA`、`deviceIds:[3]`=speaker） |
| 1896 | **关** | critical | 自动 | **0 条** ← 阴性②（driving 维度） |
| 1896 | **开** | critical | 自动 | **有 1 条** ← **同轮正例**（同一进程，排除「这次 App 坏了」） |

⇒ 「出声」既不是 critical 单独给的，也不是行车单独给的，**是那条 `driving && critical` 规则给的**；
且它**没有误伤普通优先级**（正是 T12-① 变异要防的误伤面）。「只气泡」的那一半也验到了：
两条主动消息气泡都在记录里（`💡 前面路况还行` 与 `💡 后备箱没有关好`），信息没丢、只是不播。
⬜ 三档里的「静音 + 行车 ⇒ 仍出声」「总是 + 非行车 ⇒ 出声」两格未取（要改播报档，可补）；S2S 忙的 INTERRUPT / DEFER 两路真机不造（jest 钉住）。

##### 步骤 7：reduce-motion → 光球静帧（§11.2 B4 ④，第二个入口）

场景是本批的新形态（行车档 + C 身份 + **语音层常驻的 120dp 大球**），与 §6.1 那次（Composer 球）不是同一颗球：

| 区域 | 减少动效 **开** | 减少动效 **关**（活体对照） |
|---|---|---|
| 整屏 1080×2520 | **0.00%**（不同字节 **0**） | **8.76%**（238512 px） |
| 层内 120dp 大球区 [356,1442)-[722,1808) | **0.00%**（不同字节 **0**） | **99.49%**（133273/133956 px） |

大球区 99.49% 说明**观测通道是开的**（球确实在动）。开关开/关都当场截图回读过（`b4-13-lab-after.png` / `b4-13-lab-off.png`）。
⬜ **系统开关那一路**（`settings put global animator_duration_scale 0`）本轮**没做**——改系统设置需泓舟授权，全程一次 `settings put` 都没发过。

##### 步骤 9：`Reanimated: synchronouslyUpdateUIProps failed`（B3 §6.3 遗留⑤）

**本轮 0 命中**。通道自检先做：全库 `adb logcat -d` **55670 行**（>0），其中
`Reanimated|synchronouslyUpdateUIProps` **0 条**；限 App pid 的 `-v brief --pid=1896` **213 行**（>0），`E/` 或 `F/` **0 条**。
⚠ 第一次读 `E/F` 用的是 `^[EF]/` 配 **threadtime** 格式（行首是日期不是 `E/`）⇒ 那个「0 条」是假的，
换 `-v brief` 才成立（坑④）。场景：行车档 + C 身份 + 层常驻 + 12 次滚动 + 减少动效开关正反各一段。
**没复现 ≠ 不存在**：B3 记的「一直在发生」本轮未出现，要么场景不同、要么与某个已变的条件相关。
计划的四段分法（`state-gallery only=idle/zzz` 等）在分母为 0 时没有意义，**不再往前推，出账带场景**。本轮**未改任何相关代码**。

##### 步骤 14：性能（§11.4）

`dumpsys gfxinfo … framestats`，本机 **24 列**（表头逐列核过，与 B2 记录一致），采到 **120 帧**。
⚠ **本机 `Flags` 恒为 32**（120/120），"Flags==0 才算正常帧"那条经验在这里不成立 ⇒ 不作过滤（坑⑤）。
场景 = 语音层开（真模糊在）+ 12 次滚动记录：

| 量 | 中位 | p90 | max | 判据 ≤18.2ms（≥55fps 等价） |
|---|---|---|---|---|
| **呈现间隔**（相邻帧 `FrameCompleted` 差，剔 >100ms 空闲缝，n=118） | **8.29ms** | **9.32ms** | 13.35 | **中位 PASS / p90 PASS** |
| 单帧总延迟 `FrameCompleted-IntendedVsync` | 15.55ms | **18.71ms** | 22.03 | 中位 PASS / **p90 FAIL（超 0.51ms）** |
| 绘制耗时 `FrameCompleted-FrameStartTime` | 15.02ms | 18.16ms | 21.65 | 中位 PASS / p90 PASS（压线） |

计划判据要的是**呈现间隔**那一行 ⇒ 达标，且 8.29ms ≈ 120Hz 的理想 8.33ms（**实际按 120fps 出帧**）。
另外两个量一并记下——只报过的那个就是挑分母（B2「一个达标读数可能是另一个缺陷的度量」的反面）。

**同屏循环动画实例 = 1 个**（§11.4），而且是**同一对帧内**的对照，不跨轮：
层内 120dp 大球区 **99.49%**（在动） vs Composer 球区 [66,2310)-[160,2404) **0.00%**（静止）。

##### 泓舟协作轮（2026-09-03 11:21–12:34）——把「需要人」的格子做掉

上一节收尾时列的四类阻塞里，**中文输入、手折转屏、人耳盲测三类当轮解开**（Maestro driver 仍未放行）。
泓舟在场，按「一次一个动作、我在中间取证」的节奏做。

**① Edge 标 `driving` → 行车档自动进入（§11.2 B4 ② 的核心，此前只有手动开关造的读数）**

云栈 `speed_kmh=30`（`gear` 联动 `D`，`/debug` 屏收到的 `vehicle_state` 帧里逐字可见）→ 泓舟手输
「帮我规划从深圳到广州的充电路线」→ **行车档自动进入**。四个独立读数互相印证，**行车档开关全程是关的、角色是 A（handheld）**：

| 读数 | 值 | 为什么能定性 |
|---|---|---|
| 过程中的过程区 | `⟳ 规划充电…`（带步骤 label） | 非行车档那一支是**固定文案** `⟳ 处理中…`，拼 label 只有 `terse` 才走 |
| 答完的过程区 | `过程 3 步`（**无 ▸ 箭头**） | 非行车必带 `▸`/`▾`；无箭头 = `terse` = `Pressable disabled` |
| `composer-send` | **56.0×56.0dp** | 同角色同开关、未注入时是 44.0dp |
| `composer-input` | 在场 208×42dp | 角色 A 行车档下 `composerInputMode='always'` |

**② 过程区单行的阴性对照——同屏相邻两轮**（比跨轮对照硬）

后来注入 `speed_kmh=0`+`gear=P` 再发一句「帮我规划从广州到东莞的充电路线」，30s 宽限过后，
**同一屏上两轮并排**：上一轮「过程 3 步」**无箭头**，这一轮「**▸** 过程 3 步」**有箭头**。
同一个组件、同样 3 步、一个不可展开一个可展开 ⇒ `terse` 真的翻了。

**③ 30s 退出宽限（`DRIVING_EXIT_GRACE_MS`）**

先记一条**判据本身的真机体现**：`speed_kmh` 降到 0 之后立刻量，`composer-send` **仍是 56.0dp**——
`drivingActive` 不看 `vehicle_state`，只认 Edge 在 process 帧上的标注（§5 第 1 条「唯一裁决点在 Edge」），
所以**必须有新一轮带标才退得出**。这就是那句「代价是简单轮不带标」的样子。

发新一轮后每 8 秒 dump 一次量 `composer-send`：

| 帧 | 时刻 | `composer-send` |
|---|---|---|
| g1–g9 | 11:32:31 – 11:33:56 | **56.0dp**（连续 9 次） |
| **g10** | **11:34:07** | **44.0dp** ← 翻转 |
| g11–g12 | 11:34:17 – 11:34:28 | 44.0dp |

发送时刻由 g5/g6（键盘弹着，`composer-send` 的 y=1378）到 g7（键盘收了，y=2274）界定在 **11:33:24–11:33:35**。
⇒ 宽限 ∈ **[21, 43] 秒**，**包含判据值 30s**。采样粒度 ~11s + 发送时刻 11s 窗口 ⇒ 给不出更紧的界，如实记区间；
**但「翻转」本身是硬的**（此前连续 9 次 56.0dp）。

**④ VAL 拒绝（§11.2 B4 ③ 可验的那半）**

`speed_kmh=90` → 泓舟手输「打开天窗」⇒ 回复原话逐字：**「高速行驶中请勿打开车窗/天窗」**。

- 这是 **VAL 的安全话术，不是权限拒绝** ⇒ **手机档 token 到得了 VAL 门控**（§5 第 2 条担心的「手机档根本到不了」**没有发生**，那半可以划掉）；
- **无全屏层** ✓ 记录完整可见、Composer 在、无任何遮挡（角色 A 语音层不常驻，压根没升）；
- **UI 不据 driving 加限制** ✓ 无确认卡、快捷指令都在；
- **`sunroof=closed` 回读** ✓ 天窗真没被打开，不只是话术；
- Dock **没有** `safety_blocked` 行 ⇒ 真机确认这条回复是**普通 final 文本气泡**，协议里没有结构化标记
  ⇒ **⬜ 待 Q16 原样保留**（§5 第 2 条），客户端不猜。

**⑤ 建议胶囊正例 + 点击闭环（T10 步骤 6；此前只有阴性）**

设成 角色 C + 保持屏幕常亮开 + 行车档关，泓舟把手机横过来（外屏横，`ROTATION_270`、`land`、
`mBounds=2520×1080`、**840×360dp**）⇒ 胶囊出现 **「切到行车档? 点一下开启」**。
这是 `usePresence → drivingSuggested` 那条**零 jest 接线**的唯一读数（§6.3 坑③ 说的就是它）。
我 tap 胶囊 ⇒ **胶囊消失 + 语音层升起常驻 + 输入框消失 + 快捷指令从 7 条裁到 3 条 + 发送键变暗**
⇒ `capsule.action==='enable-driving' → update({drivingManual:true})` 这条接线通了。
同一张图也是**形态矩阵的「外屏横」**（`single` 单栏、顶栏车辆入口在、输入框常驻）。

**⑥ 形态矩阵补齐 + §7.4 切屏层不收起**

| 形态 | `device_state` | dp | layout | 图 |
|---|---|---|---|---|
| 外屏竖 | 0 | 360×840 | `single` | 本轮多张 |
| **外屏横** | 0 | **840×360** | `single`（`expanded×compact`） | `b4-13-outer-landscape.png` |
| 内屏展开 | 3 | 741×829 | `two-pane` | `b4-13-inner-after-switch.png` |
| **book（半开竖持）** | 2 | 741×829 | `two-pane` | `b4-13-book.png` |
| tabletop | 2 | — | `tabletop` | §6.2 已有 |

⛔ **§7.4「形态切换语音层不收起只重排」当轮验到了，而且推翻了 B3 的一条记录**：
外屏→内屏切换后 **pid 仍是 1896**（切屏前后两次 `pidof` 相同）⇒ **本机切屏不重启进程**，
所以「同一个层实例没消失」是**可验的**，语音层确实还在（层的「⌄ 收起」在、内容重排进左栏）。
B3 记的「切屏会重启进程」在本轮**不成立**。

**⑦ §7.3「book 强制双栏 + 铰链落 gap」——逐像素**

`FoldingFeature` 读到 `orientation: vertical`、`isSeparating: true`、
`bounds {left:1112, top:0, right:1112, bottom:2488}`（零宽竖线）、`hinge(dp) leftDp:370.67`。
屏宽 741dp ⇒ 铰链在 **370.67dp ≈ 正中**（741/2=370.5）。对话页 `uiautomator` 量：

| 元素 | x 范围（px） |
|---|---|
| 对话栏最右（`voice-sheet` / `composer-send`） | …1089 / …1059 |
| **铰链** | **1112** |
| 舞台左缘（`stage-pane`） | 1140… |

⇒ **1089 < 1112 < 1140**，铰链落在 51px 宽的 gap 里（距左 23px、距右 28px），**没有任何元素跨在它上面**。

**⑧ 提示音人耳盲测（§6.1 遗留① 卡了两批）——PASS，但第一轮无效**

先明测（我提前告知顺序）：泓舟确认**听得到、且方向判对**（第一次上行、第二次下行）。
再盲测。**第一轮作废**：随机序列碰巧连出六个 `attention`（`111111`，1/64），泓舟报「六次都是下行」**6/6 全对**——
但**全是同一种音的分母下，「真能分辨」与「闭眼全报下行」给出完全相同的结果**，这个 6/6 不含信息
（「分母脏了真阳性也不算数」的形态）。**协议漏了均衡约束**，改成强制 3↑+3↓ 打乱后重跑：
...
**6/6，判据 ≥5/6 ⇒ PASS**。全报同一种在此序列下最多 3/6 ⇒ 分母干净。
⇒ **两种音型可辨，`cueTone.ts` 的 `TONES` 表不用改**（§5 第 5 条那个口子不用动）。

##### 本轮抓到的两个缺陷（都不自行改，交泓舟裁）

⛔ **缺陷 A：`sheetDetent` 是纯比例，没有考虑「行车档内容本身更大」。** 两个症状同源：

| 形态 | 层高 | 症状 |
|---|---|---|
| 外屏横（360dp 高） | `voice-sheet` 840×**77dp** | `voice-sheet-collapse` 被压到 **53.0dp < 56**，违反 §6「目标 ≥56dp」；**120dp 大球根本放不下**（截图里确实没有球） |
| 内屏 / book | `voice-sheet` 387×**447dp** | `driving-card-title` 的 bounds 是 `[90,1915][1072,1795]`，**y2<y1 ⇒ 高度 −40dp** = uiautomator 表示「被裁到可视区外」⇒ **§6「一屏一卡」的那张压缩卡用户看不到**（滚动手势被层的下拉 Pan 吃掉） |

估算：内屏 0.78×573dp=447dp，而行车档内容需求 ≈544dp（转写 28 + 球 120 + 胶囊 ~20 + 回答 84 + chip 56 + 卡 ~100 + 按钮 56 + gap/padding）⇒ 溢出约等于一张卡的高度。
**这不是实现 bug 是判据盲区**：`presence.test.ts` 只验 detent 的**数值**，验不出「那个高度装不装得下内容」——**只有真机能露**。
两条候选修法（都要动 `presence.ts` 判据 + 测试，超出 T13 范围）：① detent 给绝对下限（`max(ratio*h, 内容最小高)`）；② 行车档另给一套 detent。

⛔ **缺陷 B（取证装置）：`/native-spike` 的 `layout` 行写死 `useLayout(false)`**（`native-spike.tsx:26`）。
第 2 批 T6 加这一行时行车档还没接上（T11 才接），当时是对的；**现在它在行车档下必然显示错的 mode**——
本轮实测行车档 + `expanded×compact` 时它显示 `single`，而真实应是 `driving-landscape`。
**以后别拿它当行车档下的布局读数。**

##### 本轮新坑

11. ⛔ **`adb shell input text` 送中文直接抛 NPE**（见坑⑤，本轮由泓舟手输解开）——四格取证共用这一个前提，**解一个解四个**。
12. ⛔⛔ **「没听到」是两个独立成因叠加，而我差点归因给第三个（音量太轻）。**
    ① `dumpsys audio` 的 `Devices:` 从 `speaker(2)` 变成了 **`bt_a2dp(80)`**——声音路由到蓝牙，根本没从手机喇叭出；
    ② 我按的坐标 x=1112 取自**内屏 book 姿态**的 bounds，而手机那时已经合上（外屏 1080 宽，按钮 x 只到 1032）⇒ **六次 tap 全打在按钮外**。
    两个都足以让人听不到。**抓到②靠的是把「按钮在不在、bounds 是多少」做成播放前的自检**，而不是重播一次看看。
    ⇒ 两条纪律：**(a) 音频类读数每轮回读 `Devices:` 行**——音量对不代表从你以为的那个喇叭出（「音频不触发」惯犯的**第三种形态**，前两次都是音量 0）；
    **(b) 靠坐标驱动的取证，坐标必须现取现用**（后来把 `uiautomator dump` 取 bounds 做进了播放脚本，坐标再没错过）。
13. **文案换行会把下面的控件整体推走**（坑⑥）**在本轮又中了一次**：还原设置时按「说明行一行」的坐标点，
    而当时角色是 C、说明行两行 ⇒ 角色与常亮两项都没点到，截图一看才发现。**同一个坑一天内两次。**
14. **盲测协议必须约束分母。** 不加均衡约束的随机序列会退化成无信息分母（本轮 1/64 撞上全同序列，6/6 却什么都证明不了）。
    ⇒ 盲测一律**强制两种各占一半再打乱**，并把「全报同一种能拿几分」写进判据旁边。
15. **uiautomator 抓不到设置页的文字**（按 `text="手持"` 定位返回空），但抓得到对话页的（`voice-sheet-transcript` 有 text）。
    ⇒ 文字定位在这个 App 上**不通用**，坐标仍要靠截图量；`resource-id` 定位则一直可靠。

##### 上一节列的「做不了」——当轮解开三类，剩下这些

| 格 | 状态 | 卡在哪 |
|---|---|---|
| 步骤 1 后半 / 步骤 5 / 步骤 6 | ✅ **当轮解开**（泓舟手输三句） | — |
| 步骤 8 外屏横、book | ✅ **当轮解开**（泓舟手折转屏） | — |
| §6.2 T7「层不收起只重排」 | ✅ **当轮解开**（pid 未变，判据可验且通过） | — |
| T10 建议胶囊正例 | ✅ **当轮解开**（横屏三条件齐 + 点击闭环） | — |
| 步骤 2 提示音人耳 | ✅ **当轮解开**（盲测 6/6，均衡分母） | — |
| 步骤 8 **分屏不崩不遮** | ⬜ | 要泓舟拖成 MIUI 分屏 |
| §6.2 T7 **PTT 松手**（按住不放 + 展开） | ⬜ | 要泓舟一只手按住光球、一只手展开；时机要准 |
| 步骤 11② 「换一批」chip | ⬜ | 要再输一句「附近的川菜馆」并等追问窗（泓舟手输可做，本轮没排上） |
| 步骤 10 `shutter` 触感盲测 | ⬜ | 要泓舟手感盲测；音量前提已解，触感与音量无关 |
| 步骤 12 **Maestro 06/07/08/09** | ⬜ | **driver 装不上**（`INSTALL_FAILED_USER_RESTRICTED`，§6.2 坑⑩）——需设备侧放行 |
| 步骤 13 TalkBack / Scanner | ⬜ | 要泓舟开无障碍服务 / 授权装 Scanner APK |
| 步骤 7 系统开关那一路 | ⬜ | 改系统设置需授权（实验室开关那一路已取，见上） |

##### 设备与云栈还原表

| 项 | 改成 | 还原为 | 回读方式与结果 |
|---|---|---|---|
| 云栈 `speed_kmh` | 30 | **0** | `GET /api/vehicle/state` ⇒ `speed_kmh=0` ✅ |
| 云栈 `gear`（随 speed 联动成 D） | D | **P** | 同上 ⇒ `gear=P` ✅ |
| App 设备角色 | mount / trusted-tablet | **handheld** | 设置页截图 `b4-13-restore.png`：说明行回「手持陪伴端 · 不控车」✅ |
| App 行车档 | 开 | **关** | 同上，开关灰 ✅ |
| App 实验室「减少动效（强制）」 | 开 | **关** | 设置页截图 `b4-13-lab-off.png` ✅ |
| `adb reverse tcp:8081` | 开工时空 ⇒ 重建 | **保留**（Metro 还要用，且第 2 批会话的 Metro 在跑） | `adb reverse --list` = `UsbFfs tcp:8081 tcp:8081` |
| `STREAM_MUSIC` speaker 音量 | 0 ⇒ 60 ⇒ **120** | **未还原，交泓舟裁** | 抬它是提示音盲测的前提（原始 0 ⇒ 什么都听不到）。收口回读 `2 (speaker): 120`、`Devices: speaker(2)`。要还原就 `keyevent 25` 按到 0 并回读 |
| ⚠ `bt_a2dp` 蓝牙音量 | 68 ⇒ **78** | **未还原（当时蓝牙已断，调不了）** | 排查「没听到」时按音量键，那几下**落在了蓝牙那一路**（当时路由是 `bt_a2dp(80)`）。**泓舟下次连蓝牙设备前先确认音量**——这是本轮改动造成的，不是原状 |
| 蓝牙音频连接 | 连着 ⇒ **泓舟断开** | 由泓舟决定要不要重连 | 断开是为了把路由拉回 `speaker(2)`；`Devices:` 行已回读确认 |
| 设备 `/sdcard` 临时文件 | 本轮 12 个 | **已清** | `ls /sdcard` 只剩第 1、2 批留下的 `b3-*`/`b4-0*`（**别人的，不动**） |
| `cmd device_state` / `wm size` / `wm density` | **一次都没改**（姿态变化全部由泓舟手折，不是 adb 强制） | — | 全程只 `print-state` / 只读；收口回读 `device_state=0`、`wm size` 1080x2520、`wm density` 480，**均无 Override 行**（§6.2 坑⑫ 那种「忘了 reset」在本轮不可能发生——我压根没 set 过） |
| App 角色 / 行车档 / 常亮 / 减少动效 | C / 开 / 开 / 开 | **handheld / 关 / 关 / 关** | 设置页截图逐项回读（`b4-13-restore-top.png` + `b4-13-restore-lab.png`）：说明行回「手持陪伴端 · 不控车」、实验室四开关回 开·开·关·关 |
| 系统设置（旋转锁定 / 动画缩放 / 无障碍服务） | **一次都没写** | — | 全程零 `settings put` |

#### §11.2 B4 四条：逐条状态

| # | 要求 | 状态 |
|---|---|---|
| ① | 形态矩阵五截图 | ✅ **五张齐**：外屏竖 / **外屏横**（840×360、`single`）/ 内屏（741×829、`two-pane`）/ **book**（半开竖持，铰链落 gap 逐像素）/ tabletop（§6.2）。**只差「分屏不崩不遮」⬜**（要泓舟拖分屏） |
| ② | `driving=true` 帧 → 目标 ≥56dp 读实、文本输入按身份、过程区单行 | ✅ **全部拿到**：云栈注入 → **Edge 标 `driving` → 行车档自动进入**（四个独立读数印证，开关全程关着）；目标 ≥56dp 有机器读数 + 阴性（56.0 PASS vs 44.0 FAIL，探针 rc 0→1）；文本输入 A/B/C 三张 + 节点在/不在；过程区单行 + **同屏相邻两轮的箭头有无**作阴性；外加 30s 退出宽限的翻转（56→44，区间 [21,43]s 含 30s） |
| ③ | VAL 拒绝 → 无全屏层 + Dock `safety_blocked` 原话 | ✅ **可验的半全部拿到**：原话「高速行驶中请勿打开车窗/天窗」（**是 VAL 安全话术不是权限拒绝 ⇒ 手机档到得了 VAL 门控**）、无全屏层、UI 不加限制、`sunroof=closed` 回读。**不可验的半 ⬜ 待 Q16**——真机确认它是普通 final 文本气泡，Dock 无 `safety_blocked` 行，协议里没有结构化标记，客户端不猜（§5 第 2 条） |
| ④ | reduce-motion → 光球静帧 | ✅ **两个形态各一组**：§6.1 的 Composer 球（0.00% vs 31.90%）+ 本批行车档语音层 120dp 大球（整屏 **0.00%** / 球区 **0.00%**，活体 **8.76% / 99.49%**）。**系统开关那一路 ⬜**（需授权） |

#### 本批踩的坑

1. ⛔ **`git checkout -- <file>` 还原变异，会把同一文件里还没提交的实现一起抹掉。**
   T10 反向验证①跑完（读数是对的）之后用 `git checkout` 还原 `presence.ts` ⇒ 退回 HEAD，**T10 的实现三处全没了**。
   抓到它靠的是计划要求的「**还原后复跑全绿**」那一步——那一步不是形式：复跑红了 1 条、`tsc` 报 4 个 `drivingSuggest 不存在`。
   ⇒ 变异器改成**变异前 cp 一份副本、还原时把副本写回**。教训的一般形式：**「还原」这个动作本身有误伤面**，而它不会当场报错——只会在下一次读数里以另一件事的名义出现。
2. **heredoc 的反斜杠失真第二次应验**（坑账 `bash-tool-heredoc-backslash-collapse`）：用 `cat <<'PYEOF'` 往 Python 脚本里写 `"…SPEAK\\n"`，落盘成了 `"…SPEAK` + 真换行 ⇒ 字符串写断、脚本成了语法错误。**注意它是静默的**：写文件那一步照样打印成功。⇒ 含转义的 Python / JSON 一律用 Write 工具落盘；变异体索性**不写转义**（要删一行就整行换成注释）。
3. **T10 的接线本身零 jest**。反向验证②（删 `landscape`）红的是 `drivingMode.test.ts`——那是**第 1 批的纯函数**。`usePresence` 把 `landscape`/`keepAwake`/`deviceRole` 喂给 `drivingSuggested` 这条**接线**没有任何单测（`testMatch` 不含 `.tsx`、hook 层无渲染测试）。计划里这一条写作「两处各红各的」，读起来像两份覆盖，实际是**判据有覆盖、接线没有**。接线的读数只能来自 T13 步骤 6 的建议胶囊三条件正反例。
4. **`derivePresence` 与 `drivingMode` 之间是「值依赖 + 类型依赖」的双向引用**（`presence.ts` 运行时 import `sheetResident`，`drivingMode.ts` 只 `import type { Identity }`）。TS 的 type-only import 不留运行时环，jest / tsc 都干净——**但这是「刚好没成环」，不是「设计上不会成环」**：哪天 `drivingMode` 需要 `presence` 的一个值就会炸。记在这里。

——以下六条来自 **09-03 真机轮**——

5. ⛔ **`adb shell input text` 送中文直接抛 NPE**：`java.lang.NullPointerException: Attempt to get length of null array
   at InputShellCommand.sendText`（KeyCharacterMap 里没有这些字符）。**这条路在本机不存在**，不是「要加转义」。
   后果不小：「说一句会出过程区的复杂任务」是 §11.2 B4 ②③ 四格取证的**共同前提**，全部因此卡住
   （步骤 1 后半 / 步骤 5 / 步骤 6 / 步骤 11②）。试过的替代都不通：ADBKeyboard 要装 APK（与 Maestro driver 同一堵墙）、
   `service call clipboard` 的 parcel 格式随版本变、`am start` 没有可带文本的路由。⇒ **需泓舟手输，或另立一条输入通道**。
6. ⛔ **说明行从一行变两行，把下面的开关整体下推 53px ⇒ 固定坐标 tap 打空，而打空是静默的。**
   切到角色 C 后说明行变成「可信车载平板 · 车控由服务端 token 决定，这里只决定布局」两行，
   行车档开关从 y=1213 移到 y=1266；我按旧坐标点，截图看起来「屏幕没变」。
   **真正的硬证据是两份 `uiautomator dump` 逐字节相同（都是 57048）**——「没变」和「没生效」是两件事，
   前者要一个能区分它们的读数。（B1 第 4 批「固定坐标探针随布局漂」的第二次；这次漂的原因是**文案换行**。）
7. ⛔ **`AudioPlaybackConfiguration … state:started` 会粘住，不能当「这一次响没响」的判据。**
   `sharedAudioContext()` 是全局唯一输出上下文，AAudio 流建了就不关——播完静置 12s 回读**仍是 `started`**。
   它只能证明**第一次从无到有**。改判据：**每轮 `am force-stop` 重启拿「属于 App pid 的播放配置 = 0 条」的干净基线，
   阴性在前、正例在后，并且在同一进程里再跑一次正例**（否则阴性的 0 条也可能是「这次 App 坏了」）。
8. **`^[EF]/` 配 `adb logcat` 默认的 threadtime 格式恒 0 条**（行首是日期不是 `E/`）——要 `-v brief` 才对。
   这是 §6.2 补取轮⑦「凡是判据形如『0 条』的读数先做通道自检」的**第二次**，上次缺席的是过滤器里的 pid，这次是格式。
   自检做法一样：同一条命令下的**总行数 >0** 才谈得上「0 条」。
9. **本机 framestats 的 `Flags` 恒为 32**（120/120 帧），「Flags==0 才算正常帧」那条通行经验在这里不成立
   （异常不会 100% 发生 ⇒ 32 是个恒定标志不是异常标记）。按它过滤会得到「0 帧可用」。⇒ 不过滤，并把这条记下来。
10. ⛔⛔ **三重失败叠在一起，而读数正好等于阴性预期。**
    一轮里同时发生：① App 还没起完 ⇒ `pidof` 返回空；② `xiaozhou://debug` 在 dev launcher 阶段发出被吞 ⇒ 停在启动器；
    ③ tap 打在启动器上；④ 空 pid 去 grep ⇒ 恒 0 条。而那一格要验的**恰好就是「0 条」**。
    ⇒ **不查 pid、不查前台窗口，我就会把这个假绿写进记录**。抓到它靠的是把 `appPid` 打出来看了一眼。
    一般形式：**当读数与预期一致时，最容易跳过「观测对象在不在」这一步**——而那正是它最该被问的时候。

#### 遗留 / 给第 4 批的话

1. **T13 基本做完**（09-03 两轮：10:18–10:47 单人轮 + 11:21–12:34 泓舟协作轮）。原来的四类阻塞里**三类当轮解开**，
   §11.2 B4 四条现在是 ✅✅✅✅（各自只剩一个具名 ⬜）。**仍开的只有六格**，见上面「剩下这些」那张表，按卡点分两类：
   ① **卡在 Maestro driver 装不上**（§6.2 坑⑪，需设备侧放行）——步骤 12 全部；
   ② **卡在还没排上的人手动作**（都不难，只是本轮时间不够）——分屏、PTT 松手、「换一批」chip、`shutter` 盲测、TalkBack / Scanner、系统开关那一路。
   ⚠ 提示音那条长期前提（§6.1 遗留①，卡了两批）**已彻底解掉**：音量抬到 speaker 120、路由确认 `speaker(2)`、盲测 6/6 均衡分母 PASS。
2. ⚠ **更正本节早先的一处引用错误**：我写 §6.3 时引的是 §6.2 的旧版本，说「`tintOverBlur=0.40` 仍是待证参数、真模糊生产路径一次没跑过」——
   **那已经过期了**。§6.2 的 09-03 补取轮里它已实测裁定**保持**（层内/层外幅度比 0.04，判据 ≤0.5），真模糊生产路径也跑通了（`blurTarget` 告警 0 条、背景幅度 211→8）。
   本批确实没碰它，但**它不再是待证项**。⇒ 教训：**并行批次的 §6 会在你写记录的过程中被别人改**，引用别节结论要现读，不能凭开工时读到的那一版。
3. **`foldstate`「订阅即查询」不成立——本轮拿到实证，定性可以收紧了**（§6.2 遗留① 原来只有冷启动那一种观察）：
   `/native-spike` 是新挂载的实例，机身**物理上已经半开**（`device_state=2`）时它读到 `posture: flat` / `events: 0`；
   请泓舟**把铰链动一下**之后，同一屏立刻变成 `posture: book` / `state: halfOpened` / `orientation: vertical` / **`events: 4`**。
   ⇒ 真相不是「冷启动时姿态停在 flat」，而是「**任何新挂载的 `useFoldState` 实例都停在 flat，直到铰链下一次物理动作**」——
   影响面比原记录宽（**每进一个新屏都会中**），但对话页因为一直挂着所以布局是对的。
   ⇒ 两条后果：**(a) 取证屏的 posture 读数在铰链没动过时不可信**；**(b) 修它仍要重建**（加原生日志或给模块补「查当前值」），
   仍等泓舟裁并进 B5 还是单开一趟。**两个待裁项（§0 第 3 条①②）本批一字未碰。**
4. **C 档那枚点不动的发送键**（见上「一处设计代价」）——第 4 批收口时如果泓舟裁了「不渲」，T13 步骤 3 的清单要同步划掉 `composer-send` 那一格。
5. **「Edge 真的会标 `driving`」已证，但 `Msg.driving` 这一支仍没有单独读数。**
   云栈注入那一轮证明了 **process 帧上真的带 `driving` 标**（否则 `recordEdgeDriving` 不会登记、行车档不会自动进入、目标不会变 56）。
   但 `terse = !!msg.driving || driving` 是**或**：两者同时为真时**分不出是哪一支在起作用**。
   要单独验 `Msg.driving`，得造一个「气泡带标但此刻已退出行车档」的状态（等 30s 宽限过后回看那条老气泡的过程区还展不展得开）——
   本轮没做，留给第 4 批：**这一格很便宜，不需要泓舟**。
6. **临时取证流仍不入库**（§6.2 遗留⑤）：`drawer-tap.yaml` / `back-order.yaml` 只活在 scratchpad。本批没有新增 e2e 流，`e2e/tools/target_probe.py` 是工具不是流。
7. **硬边界全程未破**：零重建、零新原生依赖、零后端改动、`hmi/` 一行没碰（`git diff --stat -- hmi/` 与 `git diff --stat HEAD~3 -- hmi/` 均为空）、共享判据一字未动（`proactiveSpeech.mjs` / `pendingOps.mjs` / `voiceLoop.mjs` 的 diff 为空，只消费）。反向验证撞上「要变异共享模块」这件事时**停下来换了做法**（T12-③），没有自行突破。
8. **设备与云栈还原表见上面 T13 那一节**（逐项「改成 / 还原为 / 回读方式」）。三条要点：云栈 `speed_kmh`/`gear` 已还原并回读；
   App 三处设置已还原并截图回读；**`cmd device_state` / `wm size` / `wm density` 全程只读没写、系统设置零 `settings put`**。
   唯一**没有**还原的是 **speaker 音量（0 → 60）**——还原回 0 会挡住泓舟下一轮的提示音与 `shutter` 盲测，**交泓舟裁**。
   `dev_stack target show` 开工与收口两次都是 `cloud`，没有切过。
9. ⛔ **未推送清单在本轮期间又变了一次，而且是反方向的：我的提交被别人推上去了。**
   本批 3 个代码提交 `f6d0159`(T10) / `832d9a3`(T11) / `525e405`(T12) 与 §6.3 记录提交 `7f7445a`，
   收口时用 `git merge-base --is-ancestor <sha> origin/main` 逐条核，**四条全部已在 `origin/main` 上**
   （`origin/main` 现为 `8e5a7c7`，即第 2 批补坑⑫ 那个提交）——**不是我推的，本会话一次 `push` 都没发过**。
   ⇒ **⛔「push 的粒度是分支不是提交」（M4-R1 教训）的另一面**：别人推 `main` 时，会把我落在同一分支上的提交一起带走。
   **在共享工作树上，「我不推」保证不了「我的东西没上去」**——这条纪律只挡得住我自己的手，挡不住别人的分支。
   ⛔ **收口时清单又变了第三次，而且这次里面有别人的提交**：`origin/main..HEAD` = **4 条** —— `c852062`(T13 探针) / `8c7525a`(T13 记录) / `fb24346`(协作轮记录) 是我的，而 **`9ea7353 feat(manual-rag): ship Xiaomi SU7 manual index provider` 不是本批的、是别的会话刚落的**。
   ⇒ **推之前必须逐条念一遍 `origin/main..HEAD` 并确认每一条的归属**：这条清单在一天里变了三次（开工 5 条 → 我的四条被别人推走 → 现在混进别人一条），**凭印象推 main 就会带走不该带的**（M4-R1 教训的第三个形态）。
   **别照本批开工时那份「9 条」念**（§6.2 遗留⑦ 已经为「清单会变」警告过一次，这是第二次、且方向相反）。

### 6.4 第 4 批「小样本 + 收口」（T14–T15）

**本批开工授权**：泓舟 **2026-09-03** 当轮批准第 4 批开工，顺序指定为
「第 0 步核状态 → T13 余格（他在场时做）→ 缺陷 A/B 处置（先问他）→ T14 材料与协议（做不做由他裁）→ T15 收口」，
并当轮追加一条：**所有需要真机的等他统一处理，先把其余不需要真机的全部推进**。
本节按这条切成两半——**下面标 ⬜ 的全部是「等真机轮」，不是「做不了」**。
...
| `powershell -File scripts\check_android_env.ps1` | 退出码 **0**（17 pass / 1 warn / 0 fail；唯一 WARN = E3 `no device attached`） |
| `python scripts/dev_stack.py target show` | `{"source":"file","status":"target","target":"cloud"}` |
| `cd mobile && npm test` | **48 suites / 486 tests 全绿**（与 §6.3 收口读数逐字对上） |
| `npm run typecheck` | **0 error** |
| `git status` | 干净 |
| `git log --oneline origin/main..HEAD` | **2 条**：`fb24346`（§6.3 协作轮记录）/ `71b5b47`（§6.3 未推送清单第三次校准）——**都是我的** |
| `git log --oneline HEAD..origin/main` | **1 条**：`423ed23 fix(release): keep manual config out of runtime template`——**别的会话的** |
| 分叉状态 | ⛔ **本地与远端已分叉**（各有对方没有的提交）。**本会话没有 rebase / merge / pull**，怎么合交泓舟定 |
| 设备（`adb devices` / `adb reverse --list` / `lastUpdateTime` / `dumpsys audio`） | **⬜ 四项全缺**：`adb devices` 空列表，`adb reverse --list` 报 `no devices/emulators found`。**本轮设备不在**，所以真机三项（包锚 `2026-09-02 16:45:36`、reverse 非空、speaker 音量 120）都没核到——按泓舟交代等他统一处理 |

#### 缺陷 A / B 处置（泓舟当轮裁定）

| 缺陷 | 裁定 | 落地 |
|---|---|---|
| **A** `sheetDetent` 纯比例 | **① detent 绝对下限** | `01aec93`：新增判据 `ui/layout/sheetHeight.ts` + `test/sheetHeight.test.ts`（9 条），`VoiceSheet.tsx` 改调它 |
| **B** `/native-spike` 写死 `useLayout(false)` | 直接修（取证装置） | `27b7b7a`：改读真实行车事实（订阅 `SessionCore.store` 的 `drivingEdge` + `settings.drivingManual` → 共享判据 `drivingActive`） |

**缺陷 A 的三个容器读数**（都是**记录区高度** `listHeight`，不是屏高——这一点是修法成立的前提）：

| 形态 | 记录区 | 修前 | 修后 | 说明 |
|---|---|---|---|---|
| 外屏竖 | ~~728dp~~ **实测 578.67dp** | 231 / 359 / 451 | **269 / 359 / 476**（0.4 与 0.78 受下限） | ⛔ 早先这一行写「三档一个 dp 没变」是**据估值 728 推的，错**；真机实测见下 ② |
| 外屏横 | ~~192dp（推）~~ **实测 98.67dp** | 0.4 → **77dp** | **99**（clamp 占满容器） | 77dp 连 85dp 硬 chrome 都装不下 ⇒ flex 把 `voice-sheet-collapse` 压成 53.0；修后 **56.0 PASS**，但容器本身太小，球与层内文字仍裁（见下 ③） |
| 内屏 | 573dp（推）**实测 573.33dp** | 0.78 → **447dp** | **476.0 实测** | 修后卡标题从 −40dp 变 −11dp，**上滑一次即 +24.3dp 可见**——§6.3「滚动被 Pan 吃掉」那句被实测推翻（见下 ④） |

⚠ **一处对泓舟指示的偏离，理由写在这里**：泓舟说「先在 `presence.test.ts` 写红」，实际判据落在
`ui/layout/sheetHeight.ts` + `test/sheetHeight.test.ts`。理由是这些数是 **`VoiceSheet` 自己的排版常量**
（把手 12 / padding 32 / 底栏 17 / 球 120）加 `ui/tokens` 的 `TARGET`，而**全仓 `core/` 没有一处 import `ui/`**
（`presence.ts` 头注写着「零 RN import」）；写进 core 就得在 core 里复制一份 tokens，正是「声明源只留一份」
要避免的。顺带把 `VoiceSheet` 里 120/88 两个字面量收进 `SHEET_ORB`，球直径从此一份声明源。
**判据仍只有一份，`sheetDetent` 的语义（`terse` 判定 / `presenceTrail`）一字未动。**

⛔ **缺陷 A 的一半修不掉，出账**：外屏横的记录区只有 192dp，而 0.4 档的内容需求是 **269dp**
（chrome 117 + 球 120 + gap 12 + 胶囊 20）⇒ **clamp 之后 56dp 键装得下了，120dp 球仍装不下**。
要真正装下，得让 `driving-landscape` 的语音层**脱离记录区这个容器**（覆盖到整屏），或在极矮容器下把球降级
——那是 `ChatScreen` 的布局改动，不是一个判据能解决的。**转 B5**，并等真机复取确认 clamp 后的实际观感。

**缺陷 A 的真机复取：✅ 已做**（2026-09-03 真机轮，逐条见下 ②③④）——① 外屏横 collapse **53.0 → 56.0 PASS**；
② 内屏 `driving-card-title` 初始仍 −11.0dp 但**滚一次即 +24.3dp 可见**；③ 外屏竖**不是「不回归」而是「按判据变了」**（下限在这一档也绑）。

#### 反向验证（缺陷 A；每条先 `findstr` 证明变异落盘，用**副本**还原并复跑全绿）

| # | 变异 | 计划预期 | 实红 | 一致？ |
|---|---|---|---|---|
| M1 | `sheetHeightDp` 恒返回纯比例（退回缺陷前） | 3 条症状用例 | 3 条 | ✅ |
| M2 | 去掉 `Math.min(containerH, …)` 的 clamp | 占满 + 不超容器 | 2 条 | ✅ |
| M3 | `split` 分支改成也相加 | split 构成 | **2 条**（+ 逐项对齐） | — |
| M4 | 压缩卡最小高漏掉主按钮 | 3 条 | **4 条**（+ 逐项对齐） | — |
| M5 | chrome 的按钮高写死 dp（不过 `scale`） | 只红「大字档」 | **0 条**（v2 判据） → 改判据后 **1 条** | ❌ **被推翻** |

⛔ **M5 是本轮最有价值的一条：变异落盘了（`findstr` 打得出 `MUTANT-M5`），却一条没红。**
原因不是变异无效，是**判据对「被改的那一项」零敏感**——「大字档最小高更大」这条，只要 `orbCol` 里
任何一项跟着 `scale()` 长就绿。⇒ 终版改成**逐项与内容清单等值对齐**（固定 dp 不跟字号走、文字与触控目标跟），
M5 才红在它该红的地方。**这条用例前后废了两版，两版都是反向验证抓出来的**：
- **v1** 拿 `sheetHeightDp(573, 0.78)` 比 normal/large ⇒ 同时对「下限在不在」敏感（M4 把下限压到比例线以下，
  两档一起落回同一个比例值，断言退化成「447 > 447」恒假，红得不是它要抓的那件事）；
- **v2** 改比 `drivingSheetMinDp` 的大小 ⇒ 变成上面那条零敏感。

#### T13 余格：**七格全部 ⬜ 等真机轮**（设备本轮不在，一格未取）

| 格 | 卡在哪 | 我这边的准备 |
|---|---|---|
| §6.3 遗留⑤ **`Msg.driving` 单独验** | 只差设备（**不需要泓舟在场**） | 步骤写死了：云栈注入 `speed_kmh=30` 造一条 Edge 标 `driving` 的过程区气泡 → 等 30s 宽限过、行车档退出 → **那条老气泡的过程区仍不可展开**，而退出后新一轮的过程区**可展开**（这一对就是阴性对照）。注入后 `speed_kmh=0` + `gear=P` 回读还原 |
| 步骤 8 **分屏不崩不遮** | 要泓舟拖成 MIUI 分屏 | 判据：不崩 + 落单栏 + `composer-input` 不被遮；顺带复跑 Maestro 08 |
| §6.2 T7 **PTT 松手** | 要泓舟一只手按住光球、一只手展开 | 判据：草稿转正、消息发出（§7.4） |
| 步骤 11② **「换一批」chip** | 要泓舟手输「附近的川菜馆」并等追问窗 | ⚠ 别用宽松正则（`.*换一批.*` 会匹配到卡里的提示文字） |
| 步骤 10 **`shutter` 盲测** | 要泓舟手感盲测 | **协议已按本轮教训补上均衡约束**：强制 wake/shutter 各 3 次打乱，我按 HAL 日志核每一下真振；≥5/6 则提交 `VIRTUAL_KEY` 映射并**当场复取触感读数**，否则还原并出账 B5 |
| 步骤 12 **Maestro 06/07/08/09** | **driver 装不上**，需泓舟在开发者选项放行 USB 安装 | 放行前 Maestro 类读数一格都取不到 |
| 步骤 13 **TalkBack / Scanner** | 要泓舟开无障碍服务 / 授权装 Scanner APK | 不授权就记 ⬜ + `target_probe.py` 替代读数**明标不是 Scanner** |
| 步骤 7 **系统开关那一路** | 改系统设置需泓舟授权 | 做了要还原 `animator_duration_scale=1` 并回读 |

⛔ **核销时发现「剩下这些」那张表漏了一格**：**步骤 11①（B2 验收表#6「气泡先于相机」的顺序取证）
从头到尾没被取过，也没被列进任何一张 ⬜ 表**（§6.3「剩下这些」只列了 11②）。它是 AGENTS ⑥ / B3 §6.3 遗留⑥
的**前一半**。⇒ 现在补进上表的语义里：**⬜ 等真机轮**（需相机权限 + `screenrecord` + `logcat -s CameraService`，
泓舟在场时顺手做）。一般形式：**「剩下这些」这类表是人手写的，逐行核销时要拿源头清单（§1.1 / §0 第 9 条）
去对，而不是拿上一节的表去对**——否则漏掉的那一格会一路漏到收口。

#### T14 五人外部小样本：**⬜ 材料备不了，等真机轮**

材料的第一步就是「真机截图 8 张（深色主题、外屏竖、六态 + 行车两张）」，**没有设备就一张也备不了**。
本轮做完的是**协议与计分表**（下表即成品，取到图直接填）：

| 项 | 定死的内容 |
|---|---|
| 材料 | `s1..s8.png`（**只编号、不带态名**，标签会泄题），放 `mobile/e2e/artifacts/b4-sample/`（gitignore）；六态来自**对话页真实状态**不是画廊（画廊没有胶囊的位置关系）；行车两张 = 「行车 C 常驻层」vs「泊车 同一轮」；每张裁掉状态栏与顶栏以外的无关信息，**不裁胶囊、不裁 Dock** |
| 协议 | 每人独立、不看别人答案；逐张问同一句「**它现在在干嘛？**」**自由作答不给选项**（给选项量的是识别不是理解）；行车两张追问「哪张是开车时用的？为什么？」；**记原话** |
| 计分 | 对 = 说出该态的语义（空闲 / 等唤醒·待命 / 在听 / 在想·处理 / 在说·播报 / 等我确认），同义即对；每人 x/6，五人分布 `[x1..x5]`，**均值 ≥5/6 算达标**；错的原话逐条列出（它们是 B5 视觉批的输入，比分数值钱）；行车两张：5 人里几人选对 + 理由分类（目标大 / 没有输入框 / 单行 / 球大） |
| 记录 | 一张「人 × 图」表 + 错答原话 + 达标判定。**不做就写「泓舟裁定不做，§11.4 状态可读性仍无外部基线」——不许写「裁定算过」**（B2 那句就是这么被误读成有读数的） |

**做不做仍交泓舟裁**（要找 5 个不在项目里的人）。截至本轮：**§11.4「状态可读性」仍无外部分布读数**，
唯一的那次是 B2 闸里泓舟自评 6/6（1 人、非外部），B3/B4 原样转出。

#### T13 余格补取 + 缺陷 A/B 真机复取——**2026-09-03 15:10–16:20 真机轮**（泓舟插上 USB 后当轮做）

**设备基线**（每项都回读过）：`5d432b6d`；`lastUpdateTime=2026-09-02 16:45:36` ✅ 与开工要求一致；
`adb reverse` 本轮**重建了两次**（adb server 生命周期变化各一次）；Metro `packager-status:running`；
`device_state=0`、`wm size 1080x2520`、`wm density 480`（**均无 Override 行，本轮一次 `wm`/`device_state` 都没写过**，
姿态变化全部由泓舟手动完成）；`dumpsys audio`：**`STREAM_MUSIC` speaker = 90**（⚠ 不是 §6.3 记的 120，
中间被人按过音量键）、`Devices: speaker(2)` 路由正确。

##### ① 缺陷 B 复取——**完整闭环，四格**

`/native-spike` 的 `layout` 行修前写死 `useLayout(false)`，无论行车事实怎么变都恒 `single`。修后：

| 形态 | 行车档 | `driving` 行 | `layout` 行 |
|---|---|---|---|
| 外屏竖 `compact×medium` | 关 | `false（manual=false edge.trueAt=0 edge.falseAt=0）` | `single` |
| 外屏竖 `compact×medium` | 开 | `true（manual=true …）` | `single` |
| **外屏横 `expanded×compact`** | 关 | `false（manual=false …）` | **`single`** |
| **外屏横 `expanded×compact`** | 开 | `true（manual=true …）` | **`driving-landscape`** |

⇒ **同一形态下随行车事实翻转**（第 3、4 行），这是修前不可能出现的读数。
外屏竖两档都是 `single` 是**正确答案**不是缺陷——`layoutMode` 只在 `height compact` 才给 `driving-landscape`
（§6.3 那次读到 `single` 之所以是假值，是因为它在**外屏横**读到的）。
顺带：新增的 `driving` 行把 `manual` 与 `edge` 两个来源分开打出来，下面缺陷 C 就是靠它定的性。

##### ② 缺陷 A 复取——外屏竖：**下限精确生效，逐 dp 对上**

关键前提是**记录区容器**（`listHeight`）而不是屏高。本轮直接量到它：uiautomator 里
`[0,313][1080,2049]` 那个节点——顶接头栏底 313、底与 `voice-sheet` 底重合。

| 场景（角色 C，外屏竖，detent 0.4） | 容器 dp | 纯比例 0.4 | 实测层高 | `voice-sheet-collapse` |
|---|---|---|---|---|
| 行车档 **ON**·常驻 idle | 578.67 | 231 | **269.0** | **56.0 PASS** |
| 行车档 **ON**·PTT 按住 | 544.67 | 218 | **269.0** | **56.0 PASS** |
| 行车档 **OFF**·PTT 按住 | 568.33 | 227 | **227.0** | 48.0 FAIL |

- **两个 ON 组容器差 34dp 而层高纹丝不动** ⇒ 纯比例做不到这件事，**绑的是下限**。
  单看一个 269 分不出「下限给的」还是「比例恰好也是 269」——这一对才是判别式（§6.3 坑10 同族）；
- **269.0 逐 dp 等于实现算的下限**（chrome 117 + 球 120 + gap 12 + 胶囊 20）；
- OFF 组 **227.0 = round(568.33×0.4)** 逐 dp 等于纯比例 ⇒ 泊车路径一字未动，探针活着；
- `target_probe.py` **rc 0→1**（开：`composer-send` 56.0 / `voice-sheet-collapse` 56.0 PASS；关：44.0 / 48.0 FAIL）。

⛔ **这一组顺带推翻了我自己写进本节早先版本的一个数**：外屏竖的记录区是 **578.67dp**，
不是我按「840 屏高减估算 chrome」凑的 728。据 728 写的「外屏竖三档一个 dp 没变」是错的——
实际 0.4 与 0.78 两档都受下限影响（0.62 仍走比例）。测试里那个 `OUTER_PORTRAIT = 728`
是**估值冒充实测**，已改成实测值并补了上面这一对的判据（`976dfe9`）。

##### ③ 缺陷 A 复取——外屏横：**症状一修好，另一半坐实为出账**

| 量 | §6.3（修前） | 本轮（修后） |
|---|---|---|
| 记录区容器 | —（当时按 77/0.4 **推**成 192dp） | **实测 98.67dp** |
| `voice-sheet` | 840×**77dp** | 840×**99.0dp**（= 容器高，clamp 生效） |
| `voice-sheet-collapse` | **53.0dp FAIL** | **56.0dp PASS** ✅ |
| `voice-sheet-transcript` | — | **y2<y1（−6.0dp）** ⛔ |
| `voice-sheet-answer` | — | **y2<y1（−6.0dp）** ⛔ |

⇒ **56dp 键这一半修好了**；**120dp 球与层内文字这一半修不掉**——容器只有 98.67dp，
而 0.4 档的内容需求是 269dp（chrome 117 + 球 120 + gap 12 + 胶囊 20）。
成因是横屏下 App 自己的 chrome 吃掉了 360dp 里的 261dp：顶栏 104dp + chips/输入区 157dp。
**转 B5**，且**推的 192dp 是错的、实测 98.67dp**——残留比出账时估的严重得多。

##### ④ 缺陷 A 复取——内屏：**下限生效，且推翻了 §6.3 的一处推断**

| 量 | §6.3（修前） | 本轮（修后） |
|---|---|---|
| 记录区容器 | 573（由 447/0.78 **推**） | **实测 573.33dp**（推得对） |
| `voice-sheet` | 387×**447dp**（= 0.78×573.33 = 447.2） | 387×**476.0dp**（= 下限，逐 dp 对上） |
| `driving-card-title` | **−40dp** | **−11.0dp**（改善 29dp ＝ 层高增量） |

⛔ **`driving-card-title` 仍是负高**，因为本轮那一屏的实际内容比「最小高」大：
回答 84dp（3 行，下限按 2 行 56 算）+ 一个下限里**根本没算**的 `receipt-toggle` 32dp。

⛔⛔ **但 §6.3 的那句「（滚动手势被层的下拉 Pan 吃掉）」是推断，本轮实测把它推翻了**：

| testID | 上滑前 | 上滑一次后 | 再滑两次 |
|---|---|---|---|
| `voice-sheet` | 476.0dp | **476.0dp**（没动） | 476.0dp |
| `voice-sheet-answer` | y=[1198,1450] | y=[1005,1257] | 不再动（到底） |
| **`driving-card-title`** | **−11.0dp** | **+24.3dp（可见）** | 不再动 |

层内 ScrollView **能滚**，向上拖既没被 Pan 吃掉（Pan 是 `.activeOffsetY(10)` 只认向下）
也没把层收起，滚一次就到底，**整张压缩卡够得到**（截图 `b4-14-inner-scrolled2.png`：
`poi_list` + 「广州 · 选择目的地」全在可视区；这张卡本身没有主按钮，所以 `driving-card-button` 不存在）。
⇒ **定性要改**：不是「用户看不到」，是「**初始不在可视区、要滚一下**」。
§6「一屏一卡」的意图（不用滚就看得见）仍没达成，但严重度与成因都和 §6.3 写的不同。
**一般形式：上一节记录里带括号的那半句「（因为 X）」往往是推断不是读数，逐条核销时要挑出来重验。**

##### ⑤ §11.2 B4 ② 的核心——Edge 标 `driving` → 行车档自动进入，**本轮独立复现**

云栈注入 `speed_kmh=30`（回读 `speed_kmh=30 / gear=D`）→ 泓舟手输「帮我规划从深圳到广州的充电路线」
⇒ **行车档自动进入，全程 `drivingManual=false`**。四个互相独立的迹象：
① `/native-spike` 的 `driving` 行 = `true（manual=false edge.trueAt=…）`——**`manual=false` 是这一格的关键**，
它把「手动开关」这一支排除掉了；② 语音层**常驻**升起（角色 C，无在飞轮也开着）；
③ **输入框消失**（`composerInputMode('trusted-tablet', true)='hidden'`）；④ 舞台面板长出「路线」段。

##### ⑥ ⛔ **新缺陷 C：车停了，行车档退不出来**（本轮抓到，交泓舟裁）

`/native-spike` 的原始事实读数（16:15:04 取）：

```
driving: true（manual=false edge.trueAt=1788422403524 edge.falseAt=0）
         edge.trueAt  → 16:00:03
         edge.falseAt → 0（从未）
```

而云栈 **16:07:12 就已经 `speed_kmh=0` + `gear=P`（回读确认）**。到 16:15 行车档仍然开着，
`drivingActive` 走的是 `falseAt <= trueAt ⇒ 恒 true` 那条分支——**30s 宽限根本没起算**。

**机制**：`recordEdgeDriving` 只在 `store.ts:509` 的 **`process` 帧**处理里被调用，
而 `process` 帧只有**复杂轮**才有（§0 第 6 条自己写着「简单轮（无 `process` 帧）不带这个标」）。
⇒ **「Edge 标 false」这个事件可能永远不到**：车停了以后，只要用户不再发起一个会出过程区的复杂请求，
行车档就一直开着（目标恒 56dp、文本输入恒隐藏、语音层恒常驻）。

**计划知道前半句、没推到后半句**：§0 第 6 条已经写明简单轮不带标，但 §6「退出：Edge 标 false 起持续 30s」
默认了那条 false 一定会来。§6.3 之所以量到了 30s 翻转（56→44），是因为那轮**特意又说了一句复杂任务**。

**未验的一半：✅ 18:32–18:34 补验完成，是 (a)。** 收起语音层拿到干净的记录截图
（`b4-14-t2.png`）：追问 chip 那一轮的助手气泡是「请告诉我目的地的具体名称或详细地址，
以便为您规划更精准的路线。」——**整条气泡上没有「过程 N 步」行**，而同屏上一轮的气泡有
（「过程 3 步」+ 卡 + 回执）。⇒ **那一轮确实没有 `process` 帧**，
所以 `recordEdgeDriving` 一次都没被喂过 `false`，`edge.falseAt` 保持 0。

**持续时长读数**：18:21 再读 `/native-spike`，`edge.trueAt` 仍是 16:00:03、`edge.falseAt` 仍是 **0**
⇒ 车 16:07 停下之后，**行车档已经连续开了 2 小时 14 分钟**（到 18:34 收尾时是 2h27m），
期间设置页里的行车档开关一直是灰的（关）——`b4-14-t3.png` 有这一张视觉证据：
**开关关着而 App 在行车档里**，用户在 UI 上找不到任何可以退出的地方。

**这不是 B4 能顺手改的**：修法要么让端侧在 `vehicle_state` 帧上也认 false（但 §5.3.1 明令
「客户端不用 `vehicle_state` 的 speed/gear 再算一份——那是第二份判据」），要么让后端在非复杂轮也带标
（后端改动，命中 §0 第 2 条硬边界③）。**两条都越界 ⇒ 停下来出账，交泓舟裁。**

##### ⛔⛔ 缺陷 C 的第二轮取证——**比第一版严重：不是「要等一个复杂轮」，而是大多数轮都不带那个标**

18:56 泓舟手输「附近的川菜馆」（角色已还原成**手持**，所以行车档里输入框仍在 ⇒ 顺带印证
§11.2 B4 ② 的「文本输入按身份」A 档）。这一轮出了正牌 `poi_list`（10 家，`周边 · 餐饮` / `amap · 10:58`）。

| 观测 | 读数 |
|---|---|
| 答完 3 分 48 秒后，`composer-send` 所在区域两帧 | **diff 0.00%（0 个不同字节）** |
| **活体对照**（同两帧） | 整屏 **2.78%**、状态栏时钟区 **15.85%** ⇒ **观测通道是开的** |
| 19:07 再读 `/native-spike` | `driving: true（manual=false **edge.trueAt=1788422403524 edge.falseAt=0**）` |

`edge.trueAt` 仍是 16:00:03、`edge.falseAt` 仍是 0 ⇒ **两个时刻一个都没动**。

**这排除了「帧里标的是 true」这条解释**：若带 `driving:true`，`recordEdgeDriving` 会把 `trueAt` 刷成 19:00。
配合两处**无条件**的代码路径——`gateway/edge/main.go:409` 的 `map` 里总是带 `"driving": p.Driving`（无 omitempty），
`store.ts:507-509` 在**每一条** `process` 帧上都调 `recordEdgeDriving(…, !!data.driving, …)`——
唯一剩下的解释是：**那一轮根本没有 `process` 帧**。

⇒ **缺陷 C 要改写得更重**：本轮三轮对话里**只有第一轮**（「帮我规划从深圳到广州的充电路线」）出了过程区
（「过程 3 步」）；追问 chip 那轮没有、`poi_list` 周边检索那轮也没有。
所以退出条件不是「等下一个复杂轮」——是「等一个**恰好会走 Planner 过程区**的轮」，而日常轮大多不是。
**实测：行车档从 16:00:03 一直开到 19:12 仍未退出，3 小时 12 分钟，期间做过 3 轮对话、
设置里那个开关全程是灰的。** 用户侧的唯一出路是杀掉 App 重开。

##### 步骤 11②「换一批」chip——**答案是「没有」，而且正是 §6.3 警告过的那个陷阱**

同一轮（`b4-14-u1.png`）：卡片底部的提示文字是「点选看详情；说「导航去第N个」直接导航；**「换一批」看更多**」，
而真正的追问 chip 只有一个——**「说『看第 1 个详情』或『导航去第 2 个』」**，里面没有「换一批」。
⇒ 用 `.*换一批.*` 宽松匹配会命中**卡里的提示文字**判成「有」（§6.3 原话警告过）。
本格如实记 **❌ 没有**：这一版 `poi_list` 的 `follow_up` 不含「换一批」。
⚠ 条件说明：本轮语音层是收起的（我先前点过「收起」⇒ `override='dismissed'`）、且是文字轮，
...
#### ✅ 步骤 10 `shutter` 触感——**盲测 6/6，保留 VIRTUAL_KEY**（2026-09-04 10:50–11:10）

B3 §6.3 遗留③「`shutter` ≡ `wake` 不可区分」卡了两批，本轮关掉。提交 `f452684`。

**先证两者真的不同**（不然盲测等于让人猜同一下）——HAL 层读数：

| | `wake` | `shutter`（改后） |
|---|---|---|
| 原生路径 | `Vibrator.vibrate(timings, amplitudes)` | `View.performHapticFeedback` → **`Vibrator perform effect 5`** |
| HAL 行 | `weakenVibrationIfNecessary time50` → `Vibrator on for timeoutMs: 50` / `set amplitude: 0.117647` | `configStrengthForEffect id:5, strength:1.000000` |
| MIUI 压平 | **中了** | **绕开**（strength 满） |

`0.117647 = 30/255`，与 `haptics.ts` 头注记的 `[0,50]/[0,30]` 逐值对上。
⚠ 那条「绕开 `weakenVibrationIfNecessary`」正是关键：头注第 1 条说「能被区分的是**时间结构**不是振幅」，
成因就是这个压平机制；换到 effect 路径之后它不再作用 ⇒ 这两下在物理上确实是两种东西。

**盲测**（协议按 §6.3 坑14 补了均衡约束）：
...
**6/6**，映射 A=wake / B=shutter，**另一种映射 0/6**（分组与真值完全同构，不是碰巧）。
真值 3+3 强制均衡 ⇒ **闭眼全报同一种上限 3/6**，判据 ≥5/6 ⇒ **PASS**。
每一下都按 HAL 日志核过「真振 + 签名与预期一致」，**6 下零异常**（通道自检每下 300–540 行日志）；
**真值只写文件不打印**，避免在对话里泄题。

##### ⚠ 协议上救回一次，值得单记

对话里先后出现两串答案（`112122` 与 `ABBABA`），而且**分组不同**。
`112122` 其实是我在说明里举的格式例子被回显进来的。
**在泓舟确认哪一串算数之前，没有去看真值**——若先把两串各算一遍再让他挑高的那串，
这轮盲测就作废了（「分母脏了真阳性也不算数」的一个新形态：**分母干净但答案有两份**，
等于允许事后选择）。⇒ **盲测里任何「答案有歧义」都必须在看真值之前解决。**

##### 顺带两条纪律的应验

- **坐标现取现用**（坑 62）：泓舟中途把手机合上，我先前在内屏取的 `(1112,1526)/(1112,1994)` 全废，
  重新 dump 才拿到外屏的 `(540,1598)/(540,2066)`。脚本里把「取坐标」做成发射前的第一步，没打空。
- **前台自检**：发射脚本第一步核 `mCurrentFocus` 属于本 App，不属就放弃（09-03 抓到私人聊天那次的补丁）。

#### ✅ 步骤 13 上半 TalkBack——**三条全过，行为读数与机器读数分开记**（2026-09-04 11:42–11:52）

泓舟开了 TalkBack（`enabled_accessibility_services` 含 `com.google.android.marvin.talkback/...TalkBackService`、
`accessibility_enabled=1`、进程 pid 25496、`dumpsys accessibility` 里 `FEEDBACK_SPOKEN|HAPTIC|AUDIBLE`）。

**行为读数（泓舟的耳朵，原话记录——这是权威读数）**：

| T9 步骤 4.3 的判据 | 泓舟原话 | 判 |
|---|---|---|
| 卡片/按钮读「按钮 + 标签」 | **「发送 按钮 点按两次即可激活」** | ✅ 角色词「按钮」+ 标签「发送」都在 |
| 识别中转写区**按 segment 播、不逐字打断** | **「一段一段念」** | ✅ |
| 开层后回答流式 ⇒ **回答区播** | **「是」** | ✅ |

**机器读数（交叉验证，不能替代上面那张表）**：TTS 引擎（`com.xiaomi.mibrain.speech`）
**不打播报文本**，但 `I/tts_speak_success` 那行给得出「谁播的 / 几个字 / 多长」：
字段实测解出来是 `[pkg, uid, pid, 字数, lang, …, …, ?, 时长ms, ?]`
——44 字→8097ms、26 字→4206ms、3 字→44ms，约 150–190ms/字；`uid 10196 / pid 25496` 正是 TalkBack。

| 量 | 读数 |
|---|---|
| 本轮 TalkBack 播报 | **23 条**（每字 ≥100ms 视为播完 ⇒ 播完 17 / 被打断 6） |
| 播完那些的字数 | 中位 **14**，分布 `[7,8,10,12,13,13,13,14,14,14,15,15,16,20,22,26,44]` |
| **「逐字重播」的签名**：连续三条字数各 +1 | **0 次** |

⇒ 若 partial 未节流、逐 token 重播，签名应是**大量 1/2/3/4… 字的短促事件 + 字数逐 1 递增**；
实际中位 14 字、零递增串 ⇒ **机器读数与「一段一段念」一致**。
被打断的 6 条里有 4 条每字只有 8–22ms（焦点切换掐断），属正常。

⚠ **两种读数刻意分开记**（计划 §T9 步骤 4.3 的要求）：`maestro hierarchy` 的 label 检查
只证明 label 在树里；`tts_speak_success` 的字数只证明**播报的形状**；
**「读出来的是不是『按钮 + 标签』」只有人耳能证**——本轮三条里第一条正是靠泓舟的原话拿到的。

⬜ **步骤 13 下半 Accessibility Scanner 仍未做**：需要装 Scanner APK（我手上没有该 APK，
且装第三方 APK 到泓舟设备属他的授权面）。⇒ 本格按计划记 ⬜，
`target_probe.py`（步骤 3 已做，rc 0→1）作替代读数并**明标不是 Scanner**。

#### ✅ 步骤 13 下半 Accessibility Scanner——**§11.4 无障碍基线的第一份 Scanner 读数**（2026-09-04 12:06–12:30）

B1 出账⑦「Scanner 装 APK 要授权」挂到现在，本轮泓舟装了
`com.google.android.apps.accessibility.auditor` 并启用（启用前基线：`enabled_accessibility_services`
**空**、`accessibility_enabled=0`；包总数 521→522 作通道自检）。同一屏对话页扫两次：

| | **泊车**（角色手持 / 行车档关） | **行车**（行车档开） |
|---|---|---|
| Scanner 结论 | **16 条建议** | **11 条建议** |
| 元素数 | 14 | 9 |

⭐ **两把独立的尺子对上了，而且 Scanner 反证了行车档的目标放大真的生效**——
泊车有、行车**消失的正好 5 条**：

- **`composer-send`**：泊车 44dp 被报「触摸目标」，行车 56dp **不再被报**
  ⇒ 与 `target_probe.py` 的 **44.0 → 56.0** 逐值印证（两个工具、两条独立路径，同一结论）；
- **4 个底部 chips**（三个可见 + 一个被裁到 0.67dp 宽的第四个）：泊车高 **32dp**（97px÷3）被报，
  行车档 chips ≤3 条且目标放大后不再被报。

**两态都在的 9 条 = 行车档没覆盖到的账**（正是 §5.9「不做全仓扫荡」留下的）：

| 元素 | 类别 | 备注 |
|---|---|---|
| `health-dot` | 触摸目标 | 40dp |
| 顶栏「车辆」钮 `[768,160][888,280]` | 触摸目标 | **40×40dp** < 48 |
| 顶栏「设置」钮 `[918,160][1038,280]` | 触摸目标 | **40×40dp** < 48 |
| 两处副文本（`[189,…]` 270×15dp） | **文本对比度** | 「已打断」「已执行 · 展开回执」那类灰字 |
| 两处气泡 | **项目说明：多个项目具有相同的说明** | **全新发现**：用户气泡「打开空调26度」与同文案的 chip 说明重复 |
| `receipt-toggle` | — | |
| `composer-input` | 触摸目标 | |

⇒ **出账三条给 B5**：① 顶栏两个 40dp 图标钮与 `health-dot` 的触控目标（**两档都不达 48**，
行车档没管顶栏）；② 灰色副文本的文本对比度；③ **重复说明**（气泡与 chip 同文案 ⇒ 读屏会念两遍）。

⚠ **口径说明（计划要求的那条）**：`target_probe.py` 的判据是 §6 的 **≥56dp（行车）**，
Scanner 的判据是 Material 的 **≥48dp**；两者在 `composer-send` 上给出一致结论不是巧合
（44 同时不满足 48 与 56），但**换一个 50dp 的元素两者就会分歧** ⇒ 引用时要带上各自的阈值。
Scanner 抓到的三类里，**「文本对比度」与「重复说明」是 `target_probe` 完全覆盖不到的维度**
——这正是这一格值得做的理由。

##### 取证过程里的两条操作事实

- Scanner 的「概况」扫描**不是同步的**：点完 `概况` 之后前台仍是被测 App，
  约 10 秒后 `ExecutionDisplayActivity` 才成为 `topResumedActivity`。
  ⇒ **截早了会拍到被测页而不是结果页**（本轮第一次就是这样，靠 `dumpsys activity activities`
  看到 `visible=true` 才发现）。
...
| 项 | 改成 | 还原为 | 回读方式与结果 |
|---|---|---|---|
| 云栈 `speed_kmh` | 30 | **0** | `GET /api/vehicle/state` ⇒ `speed_kmh=0` ✅（16:07:12） |
| 云栈 `gear`（随 speed 联动成 D） | D | **P** | 同上 ⇒ `gear=P` ✅ |
| App 设备角色 | handheld → **trusted-tablet** | **handheld** | 设置页截图 `b4-14-t4-crop.png`：说明行回「手持陪伴端 · 不控车」、「手持」高亮 ✅ |
| App 实验室「减少动效（强制）」 | 关 → **开** | **关** | 设置页截图 `b4-14-t6-crop.png`：开关灰 ✅；实验室四开关回 开·开·关·关 |
| App 行车档手动开关 | **一次都没开过**（全程 `manual=false`） | — | 设置页截图确认灰 ✅ |
| `adb reverse tcp:8081` | 本轮重建 **6 次**（daemon 每次自发重启都会掉） | **保留** | `adb reverse --list` = `UsbFfs tcp:8081 tcp:8081` |
| `STREAM_MUSIC` speaker 音量 | **没碰**（本轮开工读到 90，不是 §6.3 记的 120——中间被人按过音量键） | — | `Devices: speaker(2)` 路由正确 |
| `cmd device_state` / `wm size` / `wm density` | **一次都没写过** | — | 姿态变化全部由泓舟手动完成；收口 `device_state=0`、`wm size 1080x2520`、无 Override |
| 系统设置（旋转锁定 / 动画缩放 / 无障碍） | **一次 `settings put` 都没发过**（只 `settings get` 读过 `accelerometer_rotation=1`） | — | — |
| 设备 `/sdcard` 临时文件 | 本轮约 20 个 `b4-14-*` / `t*.png` / `probe.png` | **未清**（adb 不稳，留给下一轮） | 下一轮开工顺手 `rm /sdcard/b4-14-* /sdcard/t?.png /sdcard/probe.png` |
| ⚠ **App 仍卡在行车档里** | — | **退不出**（缺陷 C） | 唯一实际出路是**杀掉 App 重开**（`drivingEdge` 在内存 `SessionState` 里，重启即清） |
...
1. ⛔ **等待期间别的应用切到前台，取证抓到了泓舟的私人聊天。**
   等 30s 宽限那 35 秒里飞书起到了前台，`screencap` + `uiautomator dump` 抓到的是飞书消息列表
   （同事姓名与工作群内容）。两个文件已当场删除（宿主侧 + 设备侧），读数作废重取。
   ⇒ **每次取证前后都核 `mCurrentFocus` 的包名**，这条从本轮起写进取证脚本。
   「观察对象缺席时零事件=零证据」的新形态：**这次不是观察对象缺席，是别的对象顶了上来**——
   前者给你空读数，后者给你**一个看起来正常、其实属于别人的读数**，更危险。
2. ⛔ **adb daemon 中途重启，吞掉了一条 `input tap`，而它不报错。**
   命令里出现 `* daemon not running; starting now` + `adb.exe: no devices/emulators found`，
   但后续命令又正常了 ⇒ 中间那一下 tap 没送到。**判别式是两份 dump 逐字节相同**（hash 一致），
   截图不行（时钟在变）。⚠ 同时 **`adb reverse` 也跟着没了**，要重建。
3. **`input swipe` 在这个 App 的设置页无效，`input motionevent` 序列有效。**
   三次 `input swipe 540 2000 540 400` 页面纹丝不动；换成同一条 shell 里的
   `DOWN → 多个 MOVE → UP` 一次就滚。（B3 坑「拆成两次 adb 调用会各自带自己的 downTime」的邻居。）
4. **`uiautomator` 现在抓得到设置页的文字**——推翻 §6.3 坑15。当时按 `text="手持"` 定位返回空，
   本轮同一屏 `text` 属性齐全（`'行车档'`、`'可信车载平板 · …'` 都在）。条件差异未定性，
   ⇒ **「某工具在某屏不行」这类结论要带上当时的条件，别当成该工具的属性。**
5. **坑 62（文案换行推走坐标）当场复现**：角色切到 C 后说明行从一行变两行，
   行车档开关从 `[859,1175]` 移到 `[859,1227]`（+52px）。本轮因为每次都重新 dump 取坐标，没有打空。
6. **`Out-Null` 会把 `adb pull` 的失败一起吞掉**，而 PowerShell 的 cwd 是 `car-agent\mobile` 不是仓库根
   ⇒ 相对路径 `mobile\e2e\artifacts\...` 落到了 `mobile\mobile\e2e\artifacts\`。
   ⇒ **adb 的宿主侧路径一律用绝对路径**；`Out-Null` 只用在确认无害的输出上。

#### 泓舟当轮提的两条界面优化（**不在本批做**，出账）

2026-09-03 真机轮中泓舟直接提的，原话记在这里，不转述成我的理解：

> 1. 语音层 voice sheet 升起的时候，下面收起、打断按钮应该不需要，收起可以通过滑动语音层顶部来收起，打断可以和下面的打断合并；
> 2. 输入框旁边的发送按钮可以和打断合并，并且发送按钮可以简化为一个 ⬆️ 箭头图标，目前市面上很多 AI 助手都是这么设计的；

**为什么不在 B4 做**：两条都是**结构性的界面改动**，不是 §6 行车档规则的落地——
① 动的是 `VoiceSheet` 底栏的存在与否（那条底栏正是 §6「目标 ≥56dp」读数里 `voice-sheet-collapse` 的载体，
去掉它要重新安排那格判据）；② 动的是 Composer 发送键与 Dock 打断键的合并（B2 出账④ 刚把发送键改成图标，
再合并是第二次改设计）。B4 的边界是「零重建 + 只落 §6 与 §7 的规则」，这两条都在边界外。

**带过去的实测约束**（做的时候别丢）：
- **下拉手势已经在层上了**：`VoiceSheet` 的 Pan 是 `.activeOffsetY(10)`（只认向下 >10dp），
  本轮实测**向上拖走的是层内 ScrollView**（`driving-card-title` 从 −11.0dp 滚成 +24.3dp，层高纹丝不动）
  ⇒ 「滑动层顶部收起」与「层内滚动」共存是可行的，但**收起手势要限定在把手/顶缘那一条带**，
  否则会和层内滚动打架；
- **底栏不是只有「收起」**：`voice-sheet-collapse` 是 §6「目标 ≥56dp」在层内的唯一演员
  （本轮读数 96.0×56.0dp PASS，阴性 48.0 FAIL）。去掉它，T13 步骤 3 的清单要换一个层内演员；
- **横屏下底栏占 73dp**，而外屏横的记录区总共只有 98.67dp（见下「缺陷 A 的残留」）
  ⇒ **去掉底栏对横屏是净收益**，这条优化和缺陷 A 的残留是同一处约束的两面。

#### 对账一：交泓舟裁的清单（本轮已裁的写结果，未裁的原样留着）

| # | 事项 | 状态 |
|---|---|---|
| 1 | **§0 第 3 条① 唤醒率 5–6/10 不达标** | ✅ **已裁：另开「KWS 阈值 A/B 批」**（必须在同一个含 AEC 的包上做 0.2/2.0 的 A/B，不拍脑袋、不混进别的批）。排在 B5 前还是后未定 |
| 2 | **§0 第 3 条② 播报卡顿的无 AEC 对照构建** | ⚠ **待澄清**：本轮泓舟把互斥的两条同时选中了（「并进 B5 的重建趟取对照」与「单开一趟 B4′ 对照构建」）。**下一轮问定一条**；在此之前不排期 |
| 3 | **`foldstate`「任何新挂载实例都停在 flat 直到铰链再动」的定性** | ✅ **已裁：并进 B5 那一趟重建**（与 B3′ 角色启用、`expo-battery` 低电量回落同一趟）。代价：B5 之前「每进一个新屏 posture 都可能是错的」一直挂着，影响面是取证屏与任何新挂载 `useFoldState` 的页面；对话页因为一直挂着所以布局是对的 |
| 4 | **缺陷 A 的修法** | ✅ **已裁：① detent 绝对下限**，已落 `01aec93`（见上）。**新出账一条**：外屏横那一半修不掉（容器 192dp < 内容 269dp），转 B5 |
| 5 | **C 档那枚点不动的发送键渲不渲** | ✅ **已裁：保持现状**（渲、`disabled`、透明度 0.45）。⇒ **T13 步骤 3 的清单里 `composer-send` 那一格保留**，不划掉（§6.3 遗留④ 说的那件事不发生） |
| 6 | **speaker 音量还不还原（0 → 120）** | ⬜ **等真机轮当场问**。还原回 0 会挡住下一轮的提示音与 `shutter` 盲测 ⇒ 建议**盲测做完再还原** |
| 7 | **本地与远端分叉怎么合** | ⬜ **交泓舟**：`origin/main..HEAD` 与 `HEAD..origin/main` 各有内容。⚠ **远端那半在本轮 40 分钟里从 1 条涨到 3 条**（开工 `423ed23`；收口时又多了 `a406e22` / `9774932`，都是别的会话的 manual-rag）——**「清单会变」的第四个形态：这次变的是远端那一侧**（前三次变的都是本地侧）。本会话**没有** rebase / merge / pull |
| 8 | **T14 做不做** | ⬜ 材料要真机截图，等真机轮 |

#### 泓舟裁决（2026-09-04，拆计划会话记录；B5 计划另开会话拆）

| 事项 | 裁决 |
|---|---|
| **缺陷 C「车停了行车档退不出来」** | **走后端：Edge 在 `final` 帧上也标 `driving`**（裁决点仍只有 VAL 一处、与 `_stamp_progress` 同款一行；网关 `eventToMap` 的 final 分支透传；客户端 `recordEdgeDriving` 同时吃 final 帧，`process` 那一路不动）+ **UI 给一个手动退出口**（行车档开关在自动进入时显示「自动行车中 · 退出」，退出只压本次、不改判据）。**不走**客户端拿 `vehicle_state` 的 speed/gear 自算——那是第二份判据（§5.3.1）。**归 B5 第一件**；后端改动要带单测 + 云栈注入 `speed_kmh=0` 后一句简单轮即退出的真机读数 |
| 对账一 #2（播报卡顿对照构建归哪一趟）/ #6（speaker 音量 120 还不还原）/ #8（T14 四态降级版还是等补齐）、KWS 阈值 A/B 批排 B5 前后、B5 重建趟内容（`foldstate` 查当前值 / B3′ 角色启用 / `expo-battery` / 缺陷 A 横屏半 / Scanner 三条 / 泓舟提的两条界面优化） | **全部归 B5**，在 B5 计划的 §0 里逐条裁与排期；本计划不再追 |

#### 对账二：出账去向核销（B3 出账 / B2 出账 / AGENTS ⑥ 逐条）

| 出账项 | 去向 | 状态 |
|---|---|---|
| B2 出账⑨ `charging_list` 未适配 | **mobile 那一半已关**（09-03 补取轮：真卡同时跑通 T5 兜底卡与 T6 舞台场景卡）；**`hmi/src/types.ts` 的 `UiCard` 型名 + `_prov` 那一半原样转出** | hmi / agent 侧的账 |
| B2 出账⑫ 壳底与浅色光球对比度 | T8 步骤 5 实测 **12.4/255（判据 40）不达标**，且计划给的「垫暗盘」补丁**方向是反的**（光球半透明 ⇒ 连球一起压暗），补丁已还原 ⇒ **转出给「动光球浅色高光参数」**（A-1 §10；B4 不动光球） | 转出 |
| §11.2 B4 ③ Dock `safety_blocked` | 协议里没有结构化标记（真机确认它是普通 final 文本气泡，Dock 无 `safety_blocked` 行）⇒ **⬜ 待 Q16 后端挂账**，客户端不许正则猜（§5 第 2 条） | 挂账 |
| B3′ 默认助理角色（第一问过、第二问不过；spike 分支永不合并） | **转 B5「角色启用」的前置结论** | 转出 |
| 低电量材质回落（§5.11） | 无 `expo-battery` ⇒ **等有它的那一趟重建**（B5） | 转出 |
| §5.6 Dock 的 task 位 | **归主动消息批** | 转出 |
| `foldstate` 订阅即查询不成立 | **并进 B5 重建**（对账一 #3） | 已裁 |
| 缺陷 A 外屏横那一半 | **转 B5**（要动 `ChatScreen` 布局，不是判据能解决的） | 新出账 |
| 主线 APK 备份 `D:/Android/builds/_b3-mainline-apk-backup/` | **B4 不需要、不删**（删文件是红线） | 保留 |
| 方案 §11.5「v1 代码 B4 稳定后再删」 | **B4 不删**（§5 第 7 条：稳定的判据是 B4 收口 + 一轮真机使用） | 保留 |

#### 对账三：§1.1 追溯表与 §0 第 9 条逐行核销

**§1.1（27 行）**：除下面五行外，其余全部指到已落的提交或已取的读数。

| §1.1 的行 | 状态 |
|---|---|
| 方案 §11.1「无障碍与 reduce-motion」的 T13 步骤 13 半 | ⬜ Scanner / TalkBack 需授权 |
| 方案 §11.3 Maestro 07 + §7.6 键盘避让的 08 回归 | ⬜ driver 装不上（07 本身 09-02 夜跑过 rc=0，仍有效） |
| 方案 §7.4「PTT 松手」 | ⬜ 需泓舟；同行的「层不收起只重排」**已 ✅**（切屏 pid 未变，判据可验且通过） |
| 方案 §7.5「分屏不崩不遮」+ `BackHandler` 的「语音层开着 → BACK」 | ⬜ 两格都需泓舟；`BackHandler` 另两组（隐私栏 / 阴性）**已 ✅** |
| 方案 §8 触感 `shutter`≡`wake` / B3 遗留③ | ⬜ 盲测需泓舟 |
| B3 遗留⑥「B2 表#6 顺序取证」 | ⬜ **本轮核销才发现它一直没进任何 ⬜ 表**（见上「T13 余格」） |
| 方案 §11.4「可读性外部分布」/ AGENTS ⑤ | ⬜ T14，材料要真机截图 |

**§0 第 9 条（17 行）**：①② 见对账一 #1/#2；③ blur ✅（T8 真模糊生产路径跑通，`blurTarget` 告警 0 条、背景幅度 211→8）；
④ B3′ → B5；⑤ 小样本 → T14 ⬜；⑥ 拆成七条——发送按钮图标 ✅（T9）/ `charging_list` 半关半转出 /
`EdgeGlow` jest ✅（T3）/ 壳底对比转出 / `shutter` ⬜ / **Reanimated ✅ 本轮 0 命中**（通道自检：全库 55670 行、
限 pid 213 行，都 >0；出账带场景——「没复现 ≠ 不存在」）/ 表#6 与「换一批」两格 ⬜；
B3 §6.1 遗留④ 过期注释 ✅ 已清（T10）；B3 §6.3 遗留④ 不动 B3 计划；B3 §6.4 遗留⑤ APK 备份保留；方案 §11.5 不删。

#### 全量收口读数（2026-09-03）

| 项 | 值 |
|---|---|
| `cd mobile && npm test` | **49 suites / 495 tests 全绿** |
| `npm run typecheck` | **0 error** |
| **条数账**（只增不减，逐批对得上） | 开工 413 → 第 1 批 **463**（+50，五份纯函数）→ 第 2 批 **467**（+4）→ 第 3 批 **486**（T10 +4 / T11 +10 / T12 +5）→ 本批 **495**（+9，`sheetHeight.test.ts`）。**一条都没删、没 skip、没放宽** |
| `python scripts/run_e2e.py --target cloud` | **exit 0**：`e2e_protocol_smoke` PASS（1/1）、`e2e_remote_safe` PASS（8/8）。`remote_mutating:false`、`canonical:false`——**廉价保险，不当 canonical**（`warnings: runtime_unverified / stale_warning` 是没跑 canonical 的正常标记） |
| `git diff --stat -- hmi/` | **空**（`hmi/` 一行没碰） |
| 本批提交 | `27b7b7a`（缺陷 B）/ `01aec93`（缺陷 A）/ 本节记录提交 / `AGENTS.md` 指针提交（单独一个） |

⚠ **真机三项本轮全缺**（设备不在）：包锚 `lastUpdateTime`、`adb reverse --list`、`dumpsys audio` 的 speaker 音量。
⇒ **本批没有任何语音类读数**，也没有任何真机读数——上表全部是宿主侧的。

#### 本批踩的坑

1. ⛔ **变异落盘了却一条没红，第一反应是「变异没生效」——查下去是判据对被改的那一项零敏感。**
   见上 M5。⇒ **`grep` 证明落盘之后，「零红」是关于判据的读数，不是关于变异的读数。**
   （B1 第 1 批「变异测试要盖被改的那一对」的镜像形态：那次是变异没落盘而全绿，这次是落了盘而全绿。）
2. **同一条用例连废两版，两版都「看起来在验一件事、实际在验另一件」。**
   v1 混进了「下限在不在」、v2 退化成「任何一项跟着 scale 就行」。
   ⇒ 判据写成**逐项与独立列出的内容清单等值**之后才钉得住构成。**这不是洁癖**——
   缺陷 A 的成因本身就是「只验数值、不验装不装得下」，判据写松一格就等于把同一个盲区搬了个家。
3. **`git commit -- <新文件>` 对未跟踪文件直接报 `did not match any file(s) known to git`。**
   §0 第 7 条写的是 `git add -- <路径> && git commit -m … -- <路径>` **同一条命令链**——
   我第一次漏了 `git add` 那半，报错文案（"pathspec did not match"）读起来像路径写错了。
4. **「剩下这些」这类人手表会漏行**（见上步骤 11①）。⇒ 逐行核销要拿**源头清单**（§1.1 / §0 第 9 条）去对，
   不能拿上一节的表去对——上一节的表本身就是被核销的对象。

#### 硬边界

**全程未破**：零重建、零新原生依赖（新增的 `sheetHeight.ts` 是纯 TS）、零后端改动、
...
- **本批我的提交**：缺陷 B（`/native-spike` 读真实行车事实）、缺陷 A（`sheetHeight` 判据 + 测试 + `VoiceSheet` 接线）、
  本节记录提交、`AGENTS.md` 指针提交（**单独一个**）；
- **上一批我的提交**：§6.3 的协作轮记录与未推送清单校准两条（开工时 `origin/main..HEAD` 里就是它们）；
- **别人的**：开工时 `HEAD..origin/main` 有 `423ed23`（另一个会话的 release 修复）——**本地与远端已分叉**。

⛔ **推之前必须当场重列 `origin/main..HEAD` 并逐条念归属，再单独取得泓舟授权**
（push 的粒度是分支不是提交，M4-R1 教训；且**「我不推」保证不了「我的东西没上去」**——
§6.3 遗留⑨ 实测别人推 `main` 时会把我落在同一分支上的提交一起带走）。
**本会话一次 `push` 都没发过，也没有 rebase / merge / pull。**
