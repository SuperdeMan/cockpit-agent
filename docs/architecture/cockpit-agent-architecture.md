...
> 版本：v1.8（当前架构基线；版本规则见文末「附录 C：版本记录」）
> 日期：2026-07-26（v1.8 定稿归档）
...
契约细则（列语义、强度公式、封闭词表、级联红线）登记在 `docs/conventions.md` §9.7。
...
- **写操作生命周期五项缺一不接**：幂等键（= **请求指纹**，不得用每次都新的 task id）、
  订单状态机（复用 `task_ledger`，不建新表）、timeout/cancel（**超时报「不确定」+ 给核对路径**，
  不假装成功也不假装失败）、补偿路径（`compensate_tool` 缺失即拒载）、审计。
