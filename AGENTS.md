...
流水在 history §42–§51，**这里只留状态与残余**）：

> ⚠ **表按「还要不要动手」排序，不按卡号**——未完成的在上面。
> 2026-08-16 洁癖整理时改的：原来按追加顺序排，两张未开工的卡夹在已完成项中间。
...
| Q5 身份/作用域/记忆写入 | 🔄 出处披露已收口 | ① 出处披露 **✅ 0/3 → 3/3**（`agents/chitchat/src/mem_source.py` 确定性追加，history **§51**）。⚠ 汇总行是 `[var]` 不是 `[det]`，**这是预期的**——主张的是「出处确定性」不是「整句确定性」。② **残余=实体归一**（`孩子`/`女儿`、`妈妈`/`用户的妈妈` 苏州 vs 杭州）与 **N3**（`导航去过济南市南山实验小学` 两条）——都是 `memory_item` 里的事实级脏数据，**清洗脚本刻意够不着**（history §48.5） |
