...
> **UX v2.1 B2「语音层」的实施记录同样不在本文件**：四批（T1–T15）的读数、坑与遗留出账
> 逐批写在 [`2026-08-30-mobile-ux-v2-b2-implementation-plan.md`](2026-08-30-mobile-ux-v2-b2-implementation-plan.md)
> **§6**（§6.1 遗留修正与判据层 / §6.2 语音层骨架与记录沉淀 / §6.3 手势与层内元素 /
...
> **UX v2.1 B4「形态与行车档」的实施记录同样不在本文件**：四批（T1–T15）的读数、坑与遗留出账
> 逐批写在 [`2026-09-02-mobile-ux-v2-b4-implementation-plan.md`](2026-09-02-mobile-ux-v2-b4-implementation-plan.md)
> **§6**（§6.1 判据层 / §6.2 形态落地 / §6.3 行车档与真机验收 / §6.4 小样本与收口）。
> B4 于 2026-09-03 四批收口。四条要带进 B5 的结论：
> ① **§11.2 B4 四条全部 ✅**（形态矩阵五截图 / `driving` 帧下目标 ≥56dp 有机器读数 + 阴性 /
> VAL 拒绝无全屏层且原话是安全话术 / reduce-motion 静帧 0.00% 配活体对照），
> 各自只剩一个具名 ⬜（分屏、系统开关那一路、Dock `safety_blocked` 待 Q16）；
> ② **Edge 标 `driving` → 行车档自动进入**已实证（云栈注入 `speed_kmh=30`，四个独立读数印证），
> 30s 退出宽限的翻转也量到了（目标 56→44）；
> ③ **两个缺陷已修**：`sheetDetent` 纯比例装不下行车档内容（新增判据 `ui/layout/sheetHeight.ts`
> 给绝对下限）、`/native-spike` 的 layout 行写死 `useLayout(false)`（取证装置自己在说谎）；
> ④ **三件事要 B5 那一趟重建才做得了**：`foldstate`「订阅即查询」不成立的定性（坑账 §9.72）、
> 无 AEC 对照构建（播报卡顿不能归因给 AEC）、`expo-battery` 低电量材质回落。
> 另有**唤醒率 5–6/10 不达标**，泓舟已裁**另开 KWS 阈值 A/B 批**（必须在同一个含 AEC 的包上做）。
...
26. **Git Bash 里跑 `adb shell` 要关 MSYS 路径转换**：`adb shell uiautomator dump /sdcard/ui.xml`
    ...
    ⚠ **2026-09-02 第二次应验**（B4 T1–T5 真机冒烟）：除了 `adb pull`，`screencap -d <id> -p /sdcard/x.png`
    也中——转出来的路径含空格被拆成多个 argv，报的是 usage 不是路径错。整段改走 PowerShell 后一次就过。
    顺带：`screencap` 的参数顺序是 `-d <id> -p <file>`（`-d` 的值必须紧跟），写成 `-p -d <id>` 报 usage。
...
——以下 59–72 来自 **UX v2 B4 四批**（2026-09-02/03），只收跨批可复用的——

59. ⛔ **`git checkout -- <file>` 还原变异，会把同一文件里还没提交的实现一起抹掉**（B4 T10）：
    反向验证跑完用它还原 `presence.ts` ⇒ 文件退回 HEAD，当任务的实现三处全没了。
    抓到它靠的是「还原后复跑全绿」那一步——那一步不是形式。
    ⇒ **变异一律「变异前 cp 一份副本、还原时把副本写回」**。一般形式：
    **「还原」这个动作本身有误伤面，而它不会当场报错**，只会在下一次读数里以另一件事的名义出现。
60. ⛔ **`adb shell input text` 送中文直接抛 NPE**（B4 T13）：
    `java.lang.NullPointerException: Attempt to get length of null array at InputShellCommand.sendText`
    ——KeyCharacterMap 里没有这些字符。**这条路在本机不存在**，不是「要加转义」。
    替代都不通：ADBKeyboard 要装 APK（撞 USB 安装限制）、`service call clipboard` 的 parcel 格式随版本变、
    `am start` 没有可带文本的路由。⇒ 要中文输入就得人手输，或另立输入通道。
61. ⛔ **凡是判据形如「0 条」的读数，先做通道自检**（B4 两次，缺席的东西每次不一样）：
    ① `$pid = (adb shell pidof …)` —— **`$pid` 是 PowerShell 只读内置变量**，赋值报错但**不中断脚本**，
    后面 `adb logcat --pid=$pid` 于是拿 PowerShell 自己的进程号去过滤设备日志，稳定打出「0 条」；
    ② `^[EF]/` 配 `adb logcat` **默认的 threadtime 格式**恒 0 条（行首是日期不是 `E/`），要 `-v brief`。
    ⇒ 自检做法一样：**同一条命令下的总行数 >0，才谈得上「0 条」**。
62. ⛔ **文案换行会把下面的控件整体推走，而按旧坐标 tap 打空是静默的**（B4 T13，一天内中两次）：
    角色切到 C 后说明行从一行变两行，行车档开关从 y=1213 移到 y=1266；截图看起来「屏幕没变」。
    **能区分「没变」和「没生效」的硬证据是两份 `uiautomator dump` 逐字节相同**。
    ⇒ 靠坐标驱动的取证，**坐标现取现用**（把取 bounds 做进脚本），别隔一步复用。
63. ⛔⛔ **改设备状态的 adb 动作必须自带还原，不能靠「等会儿一起还」**（B4 T7）：
    一次 `cmd device_state state 2` 之后只反复 `print-state` 没有 `reset`，
    **人把手机折上时外屏不亮**（框架仍以为是 HALF_OPENED ⇒ 驱动内屏）。
    另两条同族：`state reset` 回的是**机身物理姿态**、不是你上次设的值（每次 reset 后 `print-state` 回读）；
    强制值与物理姿态不一致时被「激活」的那块屏可能物理是关的 ⇒ `screencap` 全黑。
    ⚠ 另：`cmd device_state 3→2` **确实能造出正确的 halfOpened 事件**（推翻旧记录），
    但当时机身本来就是物理半开的——**「能驱动事件」不等于「摊平时也能造出铰链」**，要用它当仪器得先补这个对照。
64. **`AudioPlaybackConfiguration … state:started` 会粘住，不能当「这一次响没响」的判据**（B4 T13）：
    `sharedAudioContext()` 是全局唯一输出上下文，AAudio 流建了就不关——播完静置 12s 回读仍是 `started`。
    它只证明**第一次从无到有**。⇒ 改判据：每轮 `am force-stop` 拿「属于 App pid 的播放配置 = 0 条」的干净基线，
    阴性在前、正例在后，并在同一进程里再跑一次正例（否则阴性的 0 条也可能是「这次 App 坏了」）。
65. **音频「不触发」的第三种形态是路由**（B4 T13；前两次都是音量 0）：
    `dumpsys audio` 的 `Devices:` 从 `speaker(2)` 变成 `bt_a2dp(80)`，声音根本没从手机喇叭出。
    ⇒ **音频类读数每轮回读 `Devices:` 行**：音量对不代表从你以为的那个喇叭出。
66. **盲测协议必须约束分母**（B4 T13）：不加均衡约束的随机序列会退化成无信息分母
    （1/64 撞上全同序列，6/6 却什么都不证明）。⇒ 盲测一律**强制两种各占一半再打乱**，
    并把「全报同一种能拿几分」写在判据旁边。
67. **瞬态元素「先 assertVisible 再 tapOn」是错的写法**（B4 T6）：两步之间有真实的时间缝——
    `extendedWaitUntil visible` 过了、紧接着的 `tapOn` 找不到（元素已消失）。
    对 `presence-capsule` 这类只在某一小段出现的元素，连合并成一步的 `tapOn` 也不够，
    要稳定造出那个状态得有人真做一次触发动作。
68. ⛔ **「push 的粒度是分支不是提交」的另一面：别人推 `main` 时会把我落在同一分支上的提交一起带走**
    （B4 T10–T13，实测四条提交在本会话零 push 的情况下全部出现在 `origin/main` 上）。
    **在共享工作树上，「我不推」保证不了「我的东西没上去」**——这条纪律只挡得住自己的手。
    ⇒ 推之前**当场重列** `origin/main..HEAD` 并**逐条念归属**：这份清单在一天里变了三次
    （5 条 → 我的四条被推走 → 混进别的会话一条），凭印象推就会带走不该带的。
69. **变异落盘了却一条没红 ≠ 变异无效，多半是判据对「被改的那一项」零敏感**（B4 缺陷 A 反向验证）：
    「大字档最小高更大」这条，只要**任何一项**跟着 `scale()` 长就绿 ⇒ 把 chrome 的按钮高写死 dp，
    落盘了、一条没红。⇒ 判据要么逐项等值对齐清单，要么变异就得**盖到被改的那一对**（B1 第 1 批同一条）。
70. **`uiautomator dump` 要窗口真的 idle，而「减少动效（强制）」意外是把它变成仪器的开关**（B4 T13）：
    对话页默认只吐到 `ComposeView` 一层（3.7KB、零 RN 节点），看着像「RN 树不暴露给 uiautomator」；
    开了那个开关之后同一屏是 80KB 完整树。⚠ 不是万能钥匙——卡片画廊那屏开着也一直 `could not get idle state`。
71. **本机 `framestats` 的 `Flags` 恒为 32**（120/120 帧）：「Flags==0 才算正常帧」那条通行经验在这里不成立
    （异常不会 100% 发生 ⇒ 32 是恒定标志不是异常标记）。按它过滤会得到「0 帧可用」⇒ 不过滤。
72. ⛔ **`modules/foldstate` 的「订阅即查询」不成立**（B4 T7/T13 两轮实证）：
    模块与 `useFoldState` 的注释都写着「WindowManager 注册监听时会立即回推当前值」——**不成立**。
    真相是「**任何新挂载的 `useFoldState` 实例都停在 `flat`，直到铰链下一次物理动作**」
    （机身物理已半开时新挂的取证屏读到 `posture: flat` / `events: 0`；请人把铰链动一下，同一屏立刻
    `book` / `halfOpened` / `events: 4`）。两条后果：**(a) 取证屏的 posture 读数在铰链没动过时不可信**；
