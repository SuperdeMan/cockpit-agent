...
| 生产 release | `74852a737a53e2bd75f72087ee94834cccc63874` |
| 上一生产基线 | `d425b9c2d6209adbb5ec317d90f862796bde44e7`（再上一跳 `00d1925`）；本轮未执行回滚 |
| status | 2026-09-10 `74852a7` 发布后独立复核：5/5 endpoint healthy，零 warning，且新增的 `running_release_sha` 与 `release_sha` 一致 |
| verify | `verified`；artifact `20260910T084544Z-74852a7.json` |
| 代码验证 | 本机全量固定口径 **8231 passed / 32 skipped / 0 failed**（绑 `74852a7`，`TZ=UTC0`、`-n 8 --dist worksteal`）；同 SHA 另有 mobile 876 + tsc/lint 0、hmi 333、`go build/vet/test ./gateway/...` 全绿、四道门禁 + `smoke_edge` 13 全过。历史：AR05 批 **8205 / 32**（绑 `00d1925`）；其后 `d425b9c` 跑 orchestrator+security+registry+scripts 3710、mobile 792 + tsc 0、hmi 333 + build、`go build/vet/test ./gateway/...` 全绿。`573ad46` 的 CI 八任务 success 是历史读数，不转借 |
...
| 证据边界 | 当前部署/status/verify 绑定 **`74852a7`**（2026-09-10：push→dry-run→apply 均经用户单独授权，含 CI/CD digest `a50e46e3…`；该次 deploy 同时把上一轮已 push 未部署的 AR06～AR09 整批带上了生产）。受限身份请求面、E-06 车控负例、`running_release_sha` 对账三项真栈证据均绑 `74852a7`；本轮真机证据绑固定包 `1f241c8c5`（APK SHA-256 `4c2c16bd…6985`）。AR05 早前的真栈契约证据仍绑 `d425b9c`；OPPO 固定包绑 `d425b9c2d`（其后的设置页文案修复未进该包）；锁屏提醒绑 `573ad46`、手册整本与旧全量各自保持 SHA；后续 docs/test 不冒充 release。2026-09-10 的 AR05 V01–V12 补测**只读不改**：真栈读数绑运行中的 `d425b9c`（开工前独立 `status` 5/5 healthy、零 warning），真机截图绑固定包 `d425b9c2d`，本轮未 deploy、未 verify、未改产品代码 |
