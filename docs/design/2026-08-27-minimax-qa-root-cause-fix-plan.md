...
> §4 批次表与各批终态。**第 3 批 = C3（wait_slot 方向反转 + `slot_shapes` 契约）+ C10（提醒域 A/B/C/E/F，外加 D 的两件残账）**；**第 4 批 = C6 + C5 + C7 + C8（复合句族）**，**下一批 = 第 5 批（C9 + C12 + C13 + C14 + C11）**。其余各卡文字保持
> 2026-08-27 原样，**只在卡头加落地标记**——方案是那天写的，改它会让「当时怎么判的」不可考。
...
- **先修哪张**：见 §4 实施顺序。~~C1（安全域）与 C2 的两个端侧真 bug 排最前~~
  ...
  **第 1–4 批均已完成**，从 **第 5 批（C9 天气/模板 + C12 偏好 + C13 时限 + C14 charging 范例 + C11 chitchat/聚合器）** 开始
  ——那一批是**相互独立的单点修**，可并行/穿插。
...
> ✅ **1 / 6 / 8 已落地 2026-08-28（第 1 批）**，2/3/4/5/7 仍在第 6 批。
