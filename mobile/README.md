...
- **AR04 当前交接**：[实施记录第十三节](../docs/design/2026-09-08-ar04-presentation-ack-implementation.md)。当前代码 / OPPO 包 `573ad46d9`（2026-09-08 18:26:17 安装），726 条 mobile、220 条 reminder 与 CI 通过；播放文案已在真机修复，标题修复尚未部署。默认兼容模式的物理展开/半折/合拢横屏和全屏 `drawer→tabletop` 已证草稿/本地征询保留。**设备 19:26 断开时临时全屏显示、定位关闭/减少动效尚未恢复**，恢复路径见同节；书本式/全屏返回、真正 Keyguard 提醒仍待接续，未整批签收。
