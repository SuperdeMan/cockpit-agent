...
`adb devices` 全程空列表（开工时、三个任务做完后各查一次）。按泓舟「需要真机的不要卡住，先完成代码等我统一验」，T13 **一步没做**，包括步骤 0 的前提核对。三个代码任务的真机步骤（T10 步骤 6 / T11 步骤 7 / T12 步骤 6）也一并 ⬜，并入下面这张表。

**T13 开始前必须先满足的四项**（缺一不做）：`5d432b6d` 在线 / `adb reverse --list` 非空 / `lastUpdateTime = 2026-09-02 16:45:36` / `dev_stack target show` = cloud（现已满足）。

**待做清单（本批产生的 + 从 §6.2 转来的，一起做）**：

| 来源 | 格 | 需泓舟？ |
|---|---|---|
| T10 步骤 6 | 设置页三角色截图 + 说明行随之变；行车档开关翻 `driving` 轴；建议胶囊三条件（含阴性：关常亮不出） | 横屏那一路需转手机 |
| T11 步骤 7 | 语音层常驻（120 球 + 胶囊）/ 文本输入 A·B·C 三张 / `target_probe --min 56` PASS + 阴性（行车关时 `composer-send` 44 FAIL） | 单人可做 |
| T12 步骤 6 | 三档 × 行车两态四格（`/debug` 屏新按钮回放 critical）+ 阴性（普通优先级行车档也只气泡） | 单人可做；音量需先抬起来 |
| T13 步骤 1 | 云栈 `speed_kmh=30` 注入 → Edge `driving:true` 帧 → 30s 退出宽限计时 | 单人可做 |
| T13 步骤 2 | 提示音人耳盲测（§6.1 遗留① 转来） | **需泓舟**（音量 0） |
| T13 步骤 3–5 | 目标 ≥56 读实 / 文本输入三张 / 过程区单行 + 阴性 | 单人可做 |
| T13 步骤 6 | VAL 拒绝：无全屏层 + 回复原话逐字记 | 单人可做 |
| T13 步骤 7 | reduce-motion 静帧（§6.1 已有整屏 0.00% + 活体 31.90% 可引）；**系统开关那一路** | 系统设置需授权 |
| T13 步骤 8 + §6.2 | 形态矩阵：外屏竖 ✅ / 内屏双栏 ✅ / 抽屉 ✅ / tabletop ✅（都在 §6.2）；**外屏横 ⬜ / book ⬜ / 分屏 ⬜** | **需泓舟**手折与转屏 |
| §6.2 T7 四格 | PTT 松手（按住 + 展开）⬜ / 层不收起只重排 ⬜ / 语音层开着 BACK ⬜ | **需泓舟**（前两个要手折；第三个要有人真说一句话，见 §6.2 坑⑤） |
| §6.2 T8/T9 | 真模糊在场 / `tintOverBlur` 裁决 / 浅色对比 / 胶囊实底 / 层开合 20 次 / 发送键两张 / Maestro 01·06·08 / TalkBack | TalkBack 与发送键方案**需泓舟** |
| T13 步骤 9 | Reanimated 告警定位（上限 90 分钟） | 单人可做 |
| T13 步骤 10 | `shutter` ≡ `wake` 盲测 | **需泓舟** |
| T13 步骤 11 | B2 两格（气泡先于相机 / 「换一批」chip） | 单人可做 |
| T13 步骤 12–14 | Maestro 06/07/08/09 回归、Scanner 或替代读法、`framestats` 性能 | Scanner 装 APK **需授权** |

#### §11.2 B4 四条：逐条状态

| # | 要求 | 状态 |
|---|---|---|
| ① | 形态矩阵五截图 | **部分**：外屏竖 / 内屏双栏 / 抽屉 / tabletop 四张在 §6.2；**外屏横、book、分屏 ⬜**（需泓舟手折转屏） |
| ② | `driving=true` 帧 → 目标 ≥56dp 读实、文本输入按身份、过程区单行 | ⬜ **全部未取**（设备离线）。代码与判据已就位：`target_probe.py` 已落、三种输入形态已接、`ProcessFold` 已单行不可展开 |
| ③ | VAL 拒绝 → 无全屏层 + Dock `safety_blocked` 原话 | 可验的半 ⬜（设备离线）；**不可验的半按 §5 第 2 条记 ⬜ 待 Q16**——协议无结构化标记、**无产出方**，不是「没做」而是「做不了」，且不许客户端正则猜 |
| ④ | reduce-motion → 光球静帧 | **已有读数可引**（§6.1：整屏两帧逐字节 **0.00%** + 活体对照 **31.90%**）；本批**没有**再取，**系统开关**那一路 ⬜（改系统设置需泓舟授权） |
...
1. ⬜ **T13 整轮未开工**（设备全程离线）。上面那张「待做清单」是它的全部输入——里面已经并进了 §6.2 转来的 T7 四格与 T8/T9 全部真机读数。**第 4 批开工前先确认 T13 是谁做**：它不是 T14/T15 的一部分，但 §11.2 B4 的四条读数没有它就只有一条半。
...
8. **设备与云栈还原表：本批空表**——没有连上设备，没有做任何云栈注入（`speed_kmh` / `gear` 一次都没写），没有改任何设备系统设置。`dev_stack target show` 开工与收口两次都是 `cloud`，没有切过。
9. **未推送**：本批 3 个代码提交 `f6d0159`(T10) / `832d9a3`(T11) / `525e405`(T12)，外加本节的记录提交。加上第 1、2 批的，`origin/main..HEAD` 现在是 **9 条**（5 条第 2 批 + 3 条本批代码 + 本节记录），**一条都没推**。开工时 `origin/main..HEAD` 是 5 条且逐条与 §6.2 遗留⑦ 对得上 ⇒ 里面没有别人的东西。推之前要列完整 `origin/main..HEAD` 并单独取得泓舟授权（⛔ push 的粒度是分支不是提交）。
