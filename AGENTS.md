...
| 生产 release | `b0d79dbeba1e3d45d229b0bc3b3cbc153b35aa59`（2026-09-20 17:4x apply，隔离 worktree）——W19-c 装置 `scripts/probe_history_window.py` + 设计文档 §8.2（这次发布同时把云端 `shared/.env` 新加的签名身份车道 `E2E_IDENTITY_ENABLED=true` 带进 edge-gateway，日志 `e2e_identity=true`；用户授权「切换干净用户」）。**`eb55e502`（问句闸补「几档 / 几级 / 什么条件」，W19-c 真栈逼出）已 push、CI 待看、部署被云端磁盘容量闸挡住（可用 29.7 GiB < 30 GiB），清理待批准（设计文档 §8.2.2）**。再上一个 `eb87b571`（17:0x）= 批 6 第二个 release：（W19-b：条件提醒 guide 第四分「持久订阅」+ 范例两句 `ca2725b0`、两条流式路径的 `raw_text_from` span 格 `eb87b571`）；批 6 首个 release `cc331315`（16:5x）= W16-b 步级起点原话（`Step.origin_text` / `resumed`、`step_raw_text` 一份判据、三条执行路径接线）。再上一个 `cf1d0f96`（15:4x）= 批 5 第三个 release：nearby「X 附近」中心锚定 `31e8fefc` + 本批文档，连同另一会话的四个 mobile 提交（`59088760` / `8df1d117` / `6faa3c75` / `e7a83638`，用户裁决「授权推送」）。批 5 前两个 release `485fccd1`（13:5x）→ `8e403d5c`（14:3x）：W17 读取三态 / W18 候选墓碑 + 约束读出口 / W19 视窗 pin / reminder 事件触发拒绝 + 真栈 continuity 逼出的两条修。再上一个 `9ebaa5c3`（12:3x）= P2 收尾 |
| 上一生产基线 | `eb87b571c172d788437af73fa8cbb53afbe3a882`（批 6 第二个 release）；再上一个 `cc331315`；本轮未回滚 |
| status | 2026-09-20 19:0x 独立复核：`ok`、5/5 endpoint healthy、零 warning，`release_sha` 与 `running_release_sha` 均为 `b0d79dbe`（`eb87b571` / `cc331315` 同样 5/5 零 warning） |
| verify | `verified`；artifact `20260920T110205Z-b0d79db.json`（`minimax:MiniMax-M3`，lock `e2e`，passed；首验 `20260920T094206Z-b0d79db.json`）；批 6 第二个 release `20260920T090840Z-eb87b57.json`；首个 `20260920T085748Z-cc33131.json`；批 5 第三个 release `20260920T074422Z-cf1d0f9.json`；批 5 第二个 release `20260920T063543Z-8e403d5.json`；批 5 首个 release `20260920T055706Z-485fccd.json`；P2 收尾 `20260920T043927Z-9ebaa5c.json`（`e2e_remote_safe` passed）；批 4 首个 release `20260920T032437Z-e9044da.json`；上一 release `20260919T170707Z-88a8945.json`（`e2e_remote_safe` 8/8）；批 3 前两个 release `20260919T170108Z-fb98150.json` / `20260919T165227Z-4bfebdd.json`；2026-09-19 的四个见 history。⚠ 紧跟 apply 的 verify 与**任何仍在跑的 verify 循环**都会给全空 `…-unknown.json`（远端锁），先 `TaskStop` 再在 PowerShell 前台重跑 |
