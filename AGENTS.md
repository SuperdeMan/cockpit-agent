...
| 主题 | 启动条件 / 入口 |
|---|---|
| 对话状态评审（落域 / 拒识 / 上下文 / 长会话）落地 | [评审原文](docs/reviews/2026-09-19-cockpit_conversation_review.md) + [逐条重证与分阶段落地](docs/design/2026-09-19-conversation-review-remediation.md)（2026-09-19 起唯一入口）：F01–F12 对 HEAD 逐条重证全部成立；**批 1（P0 W01–W04 + W05-lite）、批 2（P1 W08/W09）、批 3（P1 W10 / W06 / W07，2026-09-20 用户批准）已实施并发布 `88a89456`**——确认寻址 / 上下文预算硬约束 / 约束改口 / T2 判重键 / 候选批版本 / 焦点两层寿命 / 澄清进挂起表（服务端解选择、同题止损）/ `acts` 行为标签 + 活动任务帧（改口继承缺槽）；真栈逼出两条 navigation reroute 修复（改时限算改动、时刻不当地名、裸时刻就近旧时限）。**下一步 = P2（W11–W15）/ P3（W16–W19）**，入口设计文档 §5；本批留下的观察：planner 对 prompt-only 可选字段遵循率低、纯名词 / 纯偏好陈述常落技术失败出口（F09 家族，归 W13） |
