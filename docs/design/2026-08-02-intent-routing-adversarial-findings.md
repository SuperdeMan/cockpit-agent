...
`volume#1` 是「空调别动，把声音调低一点」→ `volume.dec`，note 写着
「混合否定只删除被否定动作，保留肯定的音量调整」。库里三条否定范例
（`chitchat#8` 天窗、`#9` 音响、`#10` 付款）**没有一条够得着空调**：
语义不到 0.65、词法不到 0.34（`chitchat#9` 是 0.305，见 §22.1）。

> **判据：看检索名单要问「到场的这条范例教的是不是这一条判据」，不是「有没有范例到场」。**
> `volume#1` **到场了**，可它的判据是「混合否定：删掉被否定的那个，保留并列的肯定诉求」；
> 而引述句要的是「整句一个动作步都不出」。同一个域、同一个否定词、判据不同。
> 更糟的是它给了模型一个「去找那个该保留的肯定诉求」的模板，而这句话里唯一像动作的
> 就是被引述的「关掉空调」——**「有范例」在这里不比「没范例」强**。
> 与 `sunroof.yaml` 那条（否定范例给肯定族当吸引子）合起来看：范例的风险不止「缺」，
> 还有**「在场但判据不对口」**。

> ⚠ **本节初稿把 `volume#1` 写成了「空调震动，把声音调小一点」这条「肯定式车控范例」，
> 并据此写了一条「两侧都要有对照物」的判据——原文读自 GBK 乱码的控制台输出，是错的。**
> 已按上文更正。这是 §20.6 那个坑的**第二次**发作：上一次它制造了三条假红，
> 这一次它差点把一条错的因果判据写进常青文档。**落盘再读不是可选项。**
...
## 22. 2026-08-10：两条新 P2 的收口——一条被证伪、一条改对了作用域，外加一个尺子漂移

### 22.1 `nq.hvac-keep.dont`：洞是真的，但它不是病因

「空调先别关」五个字，词法 top-1 是 `chitchat#9`「音响先别关，我还听着呢」的 **0.305**，
阈值 0.34——**差 0.035**，差在 `chitchat#9` 的尾巴「，我还听着呢」把**对称 Dice 的分母**
撑大了；语义通道也不到 0.65。于是这句 `exemplars=[]`。
> 短句不是「信息少」，是「分母小」：对称 Dice 会系统性惩罚它。

修法选得很干净：一条同形态、异对象的短范例「天窗先别关」→ `chitchat.talk`。离线实测
对目标句 **0.502 居首**，对「打开空调 / 关掉空调 / 空调开到二十二度 / 有点热空调调低两度 /
把空条打开」全 **0.000**，对「打开天窗 0.249 / 把天窗关上 0.214 / 天窗开一半 0.225」
也都在阈值下——**不给任何肯定族当吸引子**（`sunroof.yaml` 那笔账不重犯），
且天窗肯定侧已有 `sunroof.yaml`；同时让用例仍是真正的**跨对象迁移**。

live A/B（同选集、同 provider、检索零降级）：

| | before | after |
|---|---|---|
| `nq.hvac-keep.dont` | **15/18（83.3%）**，6 趟全 `exemplars=[]` | **16/18（88.9%）**，6 趟全检回 `chitchat#13@lex:0.50` |

**双尾 Fisher p = 1.000。** 范例每趟都到场了，通过率没动；七条护栏 18/18，零回归也零收益。
按「加了知识要拿对照跑证伪」的同一条纪律**退回**（与 §18 否掉 guide 同款处置），
只把结论留在 `chitchat.yaml` 的注释里。

> **判据：诊断出一个洞，不等于这个洞就是病因。**
> 「检索够不着」是可测量的事实；「补上就能修」是**另一个命题**——后者被证伪了。
> 该用例的残余从此按**模型在这句上的方差**记账，不要再当检索问题查。
> 顺带：这条否定结果也把「动 `EXEMPLAR_LEX_THRESHOLD` 治短句分母」的预期收益打了折——
> 那个改动的爆炸半径是全库 254 条 × 全部 query，**不能再拿这条用例来论证**。

### 22.2 adaptive replan 自查：把既有守卫补对称

`replan()` 早就有一次性纠偏：目标文本命中 `_CONDITIONAL_GOAL_RE` 时，模型第一版返回
`done` 或 `done=false, steps=[]` 会被重问一次。**adaptive 是同一形态、不同来源**——
`complexity=adaptive` 是**计划自己的声明**（「这轮只查前置事实，第二阶段等看结果再补」），
replan 收场就与它自相矛盾。而「今天的天气适合去哪玩」的 goal 里一个条件词都没有，
于是这一路**从来没被纠偏过**。

改法：`replan(..., adaptive: bool)`，与 `conditional_goal` 共用那一次机会，
但**两条反馈话术按来源分开写**——条件目标要模型「拿观察比前件」，adaptive 要它
「拿观察补后续步」；一段话覆盖两者会让 adaptive 那一路收到它根本没有的「条件前件」。
两条都保留「明确不满足 / 已完成时仍可 done」，**只买一次重问，不制造必须产出步骤的压力**。

### 22.3 差点写成「给 100% 的请求加成本，去修 10% 的形态」

第一版把 `adaptive` 一路传下去，对**每一次** replan 都生效。这是错的：adaptive 计划
**正常收尾**时最后一次 replan 本来就返回 `done=true`，守卫会在那里再问一遍——
等于每个 adaptive 请求白加一次 LLM 往返。收窄到 `replans == 0`：只有「第二阶段还一步
都没出就收场」才叫自相矛盾。

> **判据：守卫的作用域按「什么时候才算矛盾」定，不是按「什么时候能触发」定。**

六条单测钉住（含两次反向构造：把 `or adaptive` 摘掉 → 3 红；把 `and replans == 0`
摘掉 → 作用域那条红）。

### 22.4 尺子漂移：harness 没跟上生产的 replan 形参——**我的第一组对照因此是无效的**

带着守卫跑了 4 趟（7/12）、又 stash 掉守卫跑了 4 趟对照（10/12），正准备定性
「守卫有害」的时候才查到：**L1 harness 调 `builder.replan(...)` 时压根没传 `adaptive`**。
两臂跑的是同一条代码路径，那 24 个样本对守卫**什么都没测**。

> **判据（§13.1 判据一的第四次兑现）：执行器有这个形参，不等于尺子也在传它。**
> 更直接的一条：**A/B 之前先证明两臂真的不同。** 我差点拿一组「两臂逐字相同」的
> 读数去否掉一个自己刚写的守卫——方向还正好是「看起来变差了」，最容易被当成结论。

修法：harness 逐字对齐 `LoopController`（含 `replan_index == 0` 的作用域）。
这不只是为了测守卫——**对抗语料的 `replans` gold 一直压在一条生产里已经变了的路径上**。

### 22.5 真正的发现：首轮把 `adaptive` 判成 `simple`

harness 对齐后重跑 4 趟（守卫真正可达）：**8/12，4 次失败全部是首轮
`complexity='simple'`、`replan_count=0`，零 `empty-replan`、零 `clarify`。**

把当天下午三组 36 个样本合起来看失败形态：

| 形态 | 上午（11 样本） | 下午（36 样本） |
|---|---|---|
| 首轮 `simple`（分诊没判成 adaptive） | **0** | **10（≈28%）** |
| `empty-replan`（首轮对、第二阶段空） | 1 | 1 |
| 首轮 `clarify` | 1 | 0 |

注入逐字相同（`skills` 与 `exemplars` 的名单**和分数**在上午/下午四份报告里完全一致：
`weather-outing@lex:24` + 三条常驻，`info#7@vec:0.74` + `nearby#23@vec:0.69`），
同 SHA 族、同 provider/model 声明、检索零降级。**工作树里没有任何改动能解释它。**

结论换了：这条用例现在的主要问题是 **complexity 分诊**，不是 replan。
而 `weather-outing` guide 的 few_shot 里就明摆着 `{"complexity":"adaptive",...}` 且每轮
都被注入——**知识在场、不被照做**，与 §18 同形。
> 所以**不要再往这条上加知识**；要动就动分诊那一层，且先拿对照量清楚它今天的底噪。
...
## 25. 2026-08-10：路径 2「范例 schema 加 clarify 型」——机制建成，并测出它治不了原目标

`AGENTS.md` §4.2「裸对象澄清族」里三条路径的最后一条，也是唯一没启动过的：
路径 1（写 guide 讲判据）实测**有害**（§18，4/10→1/10，p≈0.02）；路径 3（换出预选池）
做完全量验证后由泓舟裁定回退（§19）；剩下路径 2——「给范例 schema 加 clarify 型范例，
因为范例 `plan` 必须非空，而正确产物恰恰是**先别落域**」。

### 25.1 机制：只示范两个通道共有的那一半

`clarify` 与 `plan` 互斥且必居其一，值是**澄清的理由**而不是澄清话术。这个取舍不是
风格问题——「怎么表达这是澄清」在两个输出通道里形状不同：

| 通道 | 首轮澄清的形状 |
|---|---|
| toolcall（`PLANNER_TOOLCALL=on`） | `steps=[]` + `goal` 以「需要澄清：」开头（clarify **刻意不在** submit_plan schema 里，B4-1 两轮教训：结构可见性把误澄清率从 0 抬到 50~66%） |
| 文本 | `{"addressed":true,"steps":[],"clarify":{"question":…,"options":[…]}}` |

共有的那一半是 `steps` 为空。所以渲染成
`- 用户：『…』→ []（信息不足，按上文澄清规则先反问：<理由>）`，
把具体形状交回给 prompt 里恒拼的澄清段。

> **判据（§4.3 既有条目的又一次兑现）：示范输出形状之前先确认当前输出通道。**
> 抄死一种形状会让模型输出在另一个通道被判解析失败——把「模型判断错」变成
> 「模型说不出话」。门禁反验做过：把渲染改成抄 toolcall 的 `goal` 前缀，
> `test_clarify_renders_channel_agnostic_empty_steps` 当场变红。

### 25.2 两条候选范例全部实测撤回

机制建成当天投了两条，两道对照全部不通过：

| 候选 | 实测读数 | 判定 |
|---|---|---|
| `帮我订一下` | 台账门禁拦下：与 `帮我订一家川菜馆` IDF-Dice **0.545**、与 `帮我订位` **0.539**；对「帮我订一家火锅店」排 top-2（clarify 0.514 vs nearby 0.547） | 撤——**靠 0.03 排在后面是巧合**，翻面的代价是把一个明确订餐请求变成反问 |
| `那个多少钱` | 对「**充电多少钱一度**」这个完全明确的问题抢到 **top-1**（clarify 0.365 = shop 0.365，clarify 在前） | 撤——真实伤害 |

撤回后域路由探针回到基线 `hit 61 / miss 4 / silent 95`（错配率 2.5%）。

⚠ 还有一层自我检查值得记下来：**我写这两条的动机是「让机制有消费方」，而不是它们
服务于哪张卡**——它们跟裸对象澄清族没有关系。为了给新机制找用户而投入有风险的数据，
是本末倒置；范例是最软层这句话对 clarify 型**不成立**（见 25.3）。

### 25.3 两条结构性结论——它们解释了为什么三条路径都不通

**结论一：词法通道检回的是共享实词，不是共享形态。**

裸专名之间几乎不共享 bigram，实测（IDF-Dice，阈值 0.34）：

| query | 候选范例 | 分数 |
|---|---|---|
| 华润大厦 | 环球金融中心 / 万达广场 / 杭州 / 成都东站 / 人民广场 | **全部 0.000** |
| 华润大厦 | 世纪大厦 | 0.333（仍不过阈，且靠共享「大厦」——等于抄半个原句，违反「范例不抄语料原句」） |
| 上海 | 环球金融中心 / 杭州 / … | **全部 0.000** |

对照组（共享实词的）全部轻松过阈：`帮我订一下`↔`帮我订个位子` 0.401、
`打开一下`↔`打开一下吧` 0.824、`那个多少钱`↔`这个多少钱` 0.774。

> **裸对象澄清是形态判据（「短、无动词、是个专名」），而检索是内容通道。**
> 这同时解释了路径 1 为什么会有害：判据是形态的，guide 是文本的——
> 模型每次都读到了那条 guide（`@lex:11` 未被裁），就是不照做。
> **三条路径失败在同一个地方，不是三个独立的坏运气。**

**结论二：澄清型范例天然与它的「补全版」近重复。**

澄清＝信息不足，而「不足」在检索空间里表现为「**是完整句的子串**」。于是：
能被检回的那类（缺宾语）必然与明确请求互抢，不互抢的那类（孤立专名）检不回。

> **这不是调阈值能解决的：检索表达不了「缺了什么」。**
> 向量里只有「有什么」。要抓「缺了什么」得换判定形态——句法/形态特征，
> 而不是相似度。

### 25.4 交付与状态

机制留在主干（schema + 解析 + 渲染 + 门禁 + 契约），**不带任何生产 clarify 范例**——
这是实测后的决定，写在 `skills/exemplars/README.md` 的「clarify 型」一节里，
免得下一个人把「没有 clarify.yaml」读成漏做又投一遍。

11 条契约测试用临时目录造数据覆盖机制（互斥/缺一/通道无关渲染/无 ref 不牵连/理由长度帽
/门禁四条），含一次注入缺陷的反验。

§4.2 那张卡的状态因此从「**唯一未启动项**」变成「**机制已就位，且测出它治不了原目标**」。
裸对象澄清族的三条路径至此全部走完：路径 1 有害、路径 3 回退、路径 2 机制可用但对
该族无效。**下一次要动它，得换判定形态（形态/句法特征），不是再加一层检索式知识。**

---

## 26. 2026-08-10：工具通道处置落地——泓舟拍板「维持 MiniMax + salvage 轮强制重试」

§24 把工具通道掉档定性成 **provider 属性**之后，三个选项（换主模型 / salvage 轮强制
重试 / 接受并始终分账）的代价都超出取证批次的授权，立卡进 §4.2 等拍板。
2026-08-10 泓舟裁定：**维持 MiniMax-M3 主模型 + 强制重试 + 保留分账观测**。

不换主模型的判据值得记一笔：DeepSeek 的 147/147 **不构成「它更准」的证据**——
§24.3 刚立的纪律说「通过率跨 provider 不可直比」，能跨档比的只有协议层指标。
换档还会牵动全部既有 baseline 与读数。而重试是局部改动、不动任何 baseline、可 A/B。

### 26.1 关键设计：掉档有两种，重试价值相反

`_llm_plan_tools` 的两条失败返回长得很像，但含义相反：

| 返回 | 含义 | 该怎么办 |
|---|---|---|
| `return content, None`（raw **非空**） | 模型**能说话但没用工具** | 值得再要一次工具通道 |
| `return "", None`（raw **为空**） | 协议异常 / provider 不认 tools | 重试只会再异常一次，**必须退 JSON 档** |

判据就是 **raw 是否为空**。第二档是弱 tool-calling 厂商的可用性保障
（`PLANNER_TOOLCALL` off 档存在的同一个理由），**不能被这次优化顺手吃掉**——
契约测试 `test_protocol_failure_still_falls_back_to_json_not_retry` 钉住。

> **判据：优化一条路径之前，先确认这条路径上现有的分支各自在保什么。**
> 「掉档」在代码里是一个 `else`，在语义上是两件事。

### 26.2 强制重试不许比不重试更差

抢救成功的那份计划留作回落（`salvage_kept`），重试轮解析失败时用它，
而不是掉进 `_fallback` 兜底话术。plan_mode 单列 `toolcall_salvage_kept`，
让「重试后走成」与「重试后仍掉档」在读数上分得开。

`toolcall_salvage_kept` **算掉档**并已同步进 `_OFF_TOOL_BASES`：它与
`toolcall_salvage` 的区别只在**试过几次**，不在输出分布——那一轮模型自始至终在自由
文本里作答。⚠ 与 §24.4 那个误报正好构成一对：`_no_action` 后缀说的是**判断**
（算走成），`_kept` 后缀说的是**通道**（算掉档）。**新增 plan_mode 值时先问它描述的
是判断还是通道**，分类器错一次就会让整份读数说谎。

### 26.3 代价与未验证项——如实记账

代价是硬的：掉档轮多一次 LLM 调用（MiniMax 约一半轮次），仍在原有 2 次上限内。

**有效性尚未在 live 上验证。** 本批交付的是实现 + 契约测试；
`test_salvage_retry_can_be_switched_off` 证明了**两臂真的不同**（一臂 1 次调用走 JSON、
一臂 2 次调用重试），那是 §22.4 那条纪律要求的 A/B 前置，**但它不是 A/B 本身**。
真正要回答的是「重试之后 MiniMax 的 toolcall 走成率从 45~48% 涨到多少」，
以及「多花的那次调用换来的通过率是否为正」——两个数都要一次 live 双臂跑批。
开关 `PLANNER_TOOLCALL_SALVAGE_RETRY=on|off` 就是为这次 A/B 留的。
