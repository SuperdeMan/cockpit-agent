...
# 领域路由正则（research / trip navigate·status·reschedule·modify）已迁至各 Agent
# manifest.route_hints，由通用 RouteHintEngine 消费（R2.1）。此处仅保留 trip.plan（新出行）
# 兜底所需的目的地/天数/偏好抽取（_extract_trip；P2c 后连同 trip.plan 一并迁 Agent）。
...
class PlanBuilder:
    ...
    async def build(self, text: str, working_set: WorkingSet, ctx: PlanContext,
        ...
        # 按各 Agent manifest.route_hints（priority 降序）施加——research.run / trip.navigate /
        # trip.reschedule / trip.status / trip.modify 均为 replace（互斥、取代误规划）。
        # 若无 replace 命中，再判新出行（trip.plan，append，需目的地抽取；P2c 后迁 manifest）。
        if not self._route_hints.apply(plan, text, agent_map):
