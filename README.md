...
**14** 个领域 Agent · **30** 个服务一键起栈 · **150** 条端侧意图 pattern / **62** 个车控对象 · 后端 **2287** 用例单命令跑通 + **35** 条旅程级语料 · 流式 TTS 首帧 **469ms** / 端到端语音首音频 **609ms** · 数据源全真实（高德 / 和风 / Exa / Tushare / api-football）· M0a→M4 智能化升级全线**通过跨阶段组合总体验收**（2026-07-26）
...
- **按声音区分乘员（可选挡位）**：唤醒后的第一句话边说边判断是谁在说话（说到 1.5 秒就够），每位乘员的口味、习惯、家人关系各自独立，问「你知道我是谁」叫得出名字（身份由系统直答，不交给模型编）。**常用地点（家/公司）目前仍是全车共享**——那条路走的是另一套画像存储，还没接乘员维度（已立卡，见 `AGENTS.md` 余项表）。**认不出时一律按主驾处理**——退回的是开启这个开关之前的行为，而不是某种新的坏状态；两人声音太接近时宁可不下结论。**声纹只用于区分记忆，不作为任何权限或支付的凭证**（识别错了只该损失个性化，不该损失安全）。
...
| 层 | 是什么 | 现状 |
|---|---|---|
| 单测 / 契约 | 全服务 pytest（编排 / Agent / 安全 / 记忆 / 网关）单命令一次跑通 | 3654 passed（2026-08-02） |
