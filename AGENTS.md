...
`python -m pytest -q -n auto --dist worksteal` = **7670 passed / 32 skipped 零红**（4:19）。
本跳 **7646 → 7670 = +24**，逐文件点号（`git archive HEAD` 副本 + 补齐 `gen/` 后两边
`--collect-only` 逐文件 diff）：`test_planning_safety_talk.py` **0 → 12**（新文件，安全闸两向）/
`test_safety_focus.py` **15 → 20**（驾驶员状态登记四条 + 判据两向自检）/
`test_execution_claim.py` **16 → 20**（4 条真栈原句转成常驻回归探针）/
`agents/reminder/tests/test_agent.py` **64 → 67**（标题削尾一正两反）＝ **24**，
...
#### QA 修复批的四条余项（2026-08-29 **四条逐条处置完毕**；只剩一件事＝部署后取读数）
...
> ⚠ **唯一剩下的动作：部署后取读数。** ①③ 的修法**全部只有单测证据**——安全闸要看真栈
> 落域分布、范例要看 A/B，**两者都必须部署之后才量得到**。范例的离线检回已验
> （新增两条各 1.000；**未见过的 `取消参加代号889001的评审会` 是 0.356、阈值 0.34，擦着过**），
> 但**「知识在场」和「知识有用」是两件事**——别把检回读成生效。
