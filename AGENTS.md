...
| **`pytest test/`（目录粒度）不是有效基线：裸 `server` 被 llm-gateway 劫持（新，未修）** | 本会话真栈 | 全量根跑（项目文档基线命令）通过，但**单跑 `pytest test/` 会红 12 条**——`test/support/intent_adversarial_runtime.py` 的 `from server import EdgeOrchestratorServicer` 解析到了 **`llm-gateway/server.py`**（回溯里直接印出「LLM Gateway gRPC 服务」的模块 docstring）。触发方是某条先跑的 e2e 模块把 `llm-gateway` 插进了 `sys.path`；根跑时次序不同、`orchestrator/edge` 先赢。**不是本批引入**（该文件与那行 import 本批一字未动）。判据：**裸名 import 的胜负由 sys.path 次序决定，而次序由选集决定**——所以「换个选集跑一遍」不是回归，是换了一把尺子。⚠ 另有 3 条只在 `PYTHONIOENCODING=utf-8` 下红（`test_eval_intent_adversarial_cli.py` 的 `subprocess(text=True)`，2026-08-03 记过同一坑）——**为了自己看得舒服而改的环境变量，也是环境变量**（我这次又踩了一遍）|
...
| **L3 复跑：目标级 14/20 → 18/20，`B3-3` 转绿** | findings **§12.10 / §12.11** | 修完重建 edge-orchestrator / cloud-planner 后重跑：回归级 15/15、目标级 **18/20**，`B3-3`「把空调调到我喜欢的温度」回复「26度」终态 26。`B1-2`/`B2-3`/`B5-1`/`B5-2` 本批全通过 ⇒ 方差。仍红两条：**`B3-1` 的 gold 依赖跑批当天的真实天气**（下雨那次没改行程＝真缺陷；不下雨那次「不用调整」是对的却照样红）——判据：**断言的可判性不能依赖跑批当天的外部世界**；**`B3-2` 连续两次卡在「广州塔」地标解析**（一次错认成别的公司、一次认不出），在高德侧不进本账。⚠ `$slot:` 动态期望的端到端活证据也在这里：obs 那一轮 `step.verify` 是 `hvac.set/state_match/**sat**`，而 `sat` 只在全部键都解析成功且匹配时才产生 |
