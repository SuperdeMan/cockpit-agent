...
1. **契约静态校验（硬失败）**：顶层映射 / domain=文件名 / exemplars 非空列表 /
   ...
   **全局同句冲突**（同一句话在两处被标成不同落域＝语料自相矛盾，注进 prompt 是纯噪声）
   ...
   > ⚠ **红灯不一定指向你刚加的那条范例**（2026-08-28 实测）：`lex_min` 判的是
   > **IDF 加权** Dice，而 IDF 是**语料级**的量——投一个范例文件即全表重算权重。
   > 那次给 charging 加两条范例，被顶过阈值的是一对**毫不相干**的句子
   > （'今天天气怎么样' ↔ '路上怎么样'，0.349 → 0.351）。
   > **判读方式**：先看报出来的那一对是不是真冲突，别默认「是我引进来的」；
   > 是两回事就按台账裁一次并补双向各 2 例——**这笔连带成本要算进「加一条范例」里**。
