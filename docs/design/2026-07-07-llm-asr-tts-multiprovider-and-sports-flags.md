...
DeepSeek/Qwen 结构化输出的 thinking 行为需真栈确认（thinking_style 已做成 per-provider 可调）。
...
**平台说明**：国旗 emoji 在真机 webview（Chromium/Android）正常显示；Windows 版 Chromium 缺国旗字形会
退化成 ISO 双字母（如「ES」），仍能表意——不影响真车目标平台，CDP 验证时属预期现象。
