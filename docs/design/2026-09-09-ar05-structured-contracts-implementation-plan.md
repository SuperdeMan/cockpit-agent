...
- [ ] **V01–V12 未逐格闭合**——见下方 §9.5 的逐格表：真栈闭合 3 格、部分 4 格、
      仅离线覆盖 5 格。这是本批最大的未签收面。
...
**结论：AR05 未签收。** 阻塞项按优先级：① V01–V12 的真栈格（尤其 V05 负例、V02 过期与
断线重连、V04 held 列表恢复）；② 对话页确认卡/补槽卡的真机渲染（缺中文输入通道）；
③ 旧服务端兼容与 Redis 重启恢复的真栈反例。

### 9.5 V01–V12 逐格状态（2026-09-09）

图例：✅ 真栈闭合 · ◐ 部分（离线闭合 + 真栈只覆盖一部分）· ○ 仅离线覆盖 · ✗ 未验

| ID | 状态 | 依据与缺口 |
|---|---|---|
| V01 三条挂起并存、直接处理末项 | ○ | 离线：`test_edge_scope_gate` / `ar05Contracts` 覆盖指定 operationId 不串账；**真栈只造过单条挂起**，多条并存 + 本地位置征询共存未验 |
| V02 过期/重复点击/断线排队/取消后重连/配置切换 | ◐ | 离线：`isExpired`、限龄定时器、`slotReply` 过期不上行、配置变更清摘要均有用例；真栈只验了「取消后 operation 关闭、迟到回复无效」；**过期、断线排队、配置切换未在真栈验** |
| V03 真实 Agent NEED_SLOT（有/无建议、多槽、连续追问） | ◐ | 真栈：`提醒我开会` → `slot_request{slot:time_text, state:active, 300s}` ✅；**有建议值（选择卡）、多槽、连续追问未验** |
| V04 补槽中换题、held 恢复、淘汰/取消/到期 | ◐ | 真栈：换题 → `held_operation_ids` ✅、取消 → `closed_operation_ids` ✅；**从 held 列表恢复原任务、LRU 淘汰、到期三格未验** |
| V05 无车控权限 / 有权限 / PoC 默认 / 伪造 meta | ◐ | 离线：四档全覆盖且反向验证判红；真栈**只验了「有权限」正例**（chip 往返「26度 · 已执行」）；负例缺受限 token（红线，需单独授权） |
| V06 SDK 同步/流式、Registry JSON 往返、挂起恢复、未知字段 | ◐ | Registry 逐字段往返 ✅（proto descriptor 对账，删字段判红）；**Redis 挂起恢复与流式同构未在真栈验** |
| V07 VAL/业务权限拒绝、设备权限拒绝、网络错误 | ○ | 离线：三类 code 分得开、恢复出口不指错地方（`safety.val_rejected` / `permission.scope_missing` / `device.permission_denied`）；**真栈一格没验**（要触发 VAL 拒绝需行车态或受限 token） |
| V08 ASR/S2S 回退、TTS 无声/部分失败、静音/取消 | ○ | 离线：无声按成因分档（合成失败/被打断/纯卡片轮），只有第一种产 `tts.silent`，反向验证判红；**真机语音链路一格没验** |
| V09 非法计划+无有效重试 / 合法空动作 / 拒识 / 澄清 / salvage | ○ | 离线：六条路径逐条回归 + 反向验证（停用 F09 分支 2 红）；**真栈无法稳定复现「模型两轮都给非法计划」**，未验 |
| V10 token 明确拒绝、查询失败、旧服务端、配置重建 | ◐ | 真栈：`/api/session` 401 明确拒绝 ✅（响应只说 `unauthenticated`、不泄露 token）；离线：404=legacy、超时=unknown、不判 token 失效；**旧服务端与配置重建未在真栈验** |
| V11 部分成功+问题、旧轮关闭结果、operation 更新与新轮交错 | ○ | 离线：issues 与 actions 并存、归属不互相覆盖、混合路径只盖一次；**真栈未构造部分成功轮** |
| V12 对话/设置/车辆/地图，键盘、横屏、前后台、待办展开 | ◐ | 真机：设置页完整取证、对话页首屏与 chip 往返、前后台恢复正常；**键盘态、横屏、车辆/地图页、待办展开未取证**；对话页确认卡/补槽卡渲染缺中文输入通道未验 |

真栈闭合 3 格（V03/V04/V10 的各一部分已单独标 ✅）、部分 6 格、仅离线 5 格 —— 按格计
**没有一格是「全部子项都在真栈闭合」**，故本页所有 ◐ 一律不计入签收。
