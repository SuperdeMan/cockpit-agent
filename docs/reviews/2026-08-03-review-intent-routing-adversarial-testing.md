...
| ~~`relation_pass_rate`~~ | ~~90.9%~~ | ~~130/143~~ | ⚠ **已作废（2026-08-03 晚，§10.12）**：relation 口径改为对照 `supp(base)`，与本读数不可直比 |
...
→ **已补（2026-08-03 晚）**：`expected.safety.side_effect_counts`，见 §10.13。
...
1. **`clause_commute` 关系族系统性红，而 intent 完全相同——差的是槽位拼写。**
   ...
   这需要一次单独的口径裁定。
   → **裁定已做，见 §10.12。⚠ 上面这段定性本身是错的**：「差的是槽位拼写」只对 6 条红里
   的 2 条成立，而且真因根本不在槽位。
...
## 10.12 口径裁定：relation 对照的是 `supp(base)`，不是 base 的某一次采样

§10.11-1 立的账是「`clause_commute` 因**槽位拼写**系统性红，需要裁定换序不变式该不该比
槽位」。**先量，然后这个问题被否掉了——它问错了对象。而且 §10.11 那句定性本身是错的。**

### 10.12.1 §10.11 的定性错在哪

逐条拉 8 条 clause_commute 的实际签名（`--tag commute`，17 单元，`minimax:MiniMax-M3`，
`--repeat 3`），6 条红的真实形态：

| 形态 | 条数 | 例 |
|---|---:|---|
| **A. intent 集合不同**（绝对 gold 也红，或 base 自己出了空计划） | 2 | `air-index`（variant 漏 `info.air_quality`）、`scene-sports`（**base** 出空计划） |
| **B. 槽位键不同** | 2 | `hvac-news`（`info.news` 一侧有 `limit`+`topic` 一侧没有） |
| **C. 槽位值不同** | 2 | `window-stock`（`symbol` 一侧公司名一侧股票代码） |

「差的是槽位拼写」只覆盖 B+C 共 2/6，而 A 那两条里有一条红的是 **base**，跟 variant 无关。
**又一次「只看指标名就去猜机制」**（§10.8 同族教训的第二例）。

### 10.12.2 真因：拿一次采样代表一个句子的行为
...
而 relation 的实现是**逐次配对**（base 第 i 次 vs variant 第 i 次）——在这个方差下，
那条断言主要在量采样噪声。**信噪比是负的。**
...
base 侧的证据改为 `supp(base)`——这句话在本轮观测到的全部（去重）行为。两个方向统一成
**同一条**判据，而不是两条特例：

- `invariant` / `clause_commute`：`sig(variant) ∈ supp(base)`；
- `route_flip` / `context_override`：`sig(variant) ∉ supp(base)`。

base 抖动对两者影响方向相反，这是**内在正确**的：`invariant` 下 base 抖说明这句话本来就有
多种合法行为；`route_flip` 下 base 覆盖面变大，「这两句路由不同」这个主张本就更难成立。

**槽位继续留在签名里**——集合口径已吸收其采样方差，摘掉它则永久失去可见性。

variant 侧仍逐次判，所以 variant 自己的抖动照旧被重复分类表达成 `unstable`，不会伪装成
`stable_fail`。完整判据与元规则（`intent_add` 用并集 / `intent_remove` 用交集 /
`clarify_flip` 用全称）见规格 §22.6 与 `judge_relation` docstring。

### 10.12.4 影响面：同一份采样上的新旧对照

不是两趟跑批相比——是从**同一份 `repetitions` 数据**离线回算旧口径：

| 子集 | relation 条数 | 新旧判定不同 | `supp` 分布 |
|---|---:|---|---|
| `--tag object_flip`（route_flip 代表，103 条里的 23） | 23 | **0**（新旧均 23/23 通过） | 22 条 `supp=1`、1 条 `supp=2` |
| `--tag commute` | 8 | 2（均 FAIL→PASS，均 `supp=2`） | 2 条 `supp=1`、6 条 `supp=2` |

原因是退化性质：**首跑每边各一次时 `supp` 只有一个元素，判定与旧口径逐字相同**；
只有首跑失败、`_expand_failures` 把两边一起补到 `failure_repeats` 之后才有差别——
恰好在需要区分「真缺陷」和「采样噪声」的那一刻。**这是个只影响诊断精度、不影响首跑成本
的改动。** 同一趟里绝对 gold 指标（`exact_plan_set_rate` 82.4% / `required_group_recall`
88.2%）逐字未变，可作旁证：改动没有偷偷放宽绝对判定。

### 10.12.5 它多报了一个此前被噪声掩埋的真缺陷（产品侧，未修）

`cp.reminder-weather.swapped`：

- base「提醒我八点开会，再查下明天天气」→ `time_text="八点"`；
- swapped「查下**明天**天气，再提醒我**八点**开会」→ **三次全部** `time_text="明天早上八点"`。

前一子句的时间限定词串到了后一子句上，提醒被定到明天早上而不是今天八点。旧口径下 base
自己也抖，逐次配对把它冲散成 `unstable`（既不进修复清单也不进门禁）；新口径吸收 base 抖动
后，variant 侧稳定的偏差浮出成 `stable_fail`。

**这条同时证伪了「把槽位摘出签名」那个提议**——那样做它会永久不可见（两侧 intent 集合完全
相同）。按「修尺子和修被测对象不同批」的纪律，本批不修，记账另开。

### 10.12.6 作废与限制

- **作废**：§10.2 的 `relation_pass_rate 90.9%（130/143）` 是旧口径读数，不得再引用。
  新口径全量读数要等一次固定 provider 的 L1 全量。
- **新增字段**：`relation_base_support`（判定用了几个去重后的 base 签名）。`supp=1` 的结论
  与旧口径同强度——读报告的人不该靠猜。
- **限制（一并写下）**：`n=3` 的 `supp` 仍是对真实行为分布的稀疏估计。口径修对了不等于
  relation 就准了。**要求「变了」的那一类（`route_flip` 占 103/144）在首跑只有一次采样时，
  「两次独立采样必不同」对不同句子几乎恒真——真正的假绿要靠提高 `repeat_coverage` 才抓得住，
  那是成本决策不是口径问题。** 反向构造已在单测里守住这个方向
  （`test_route_flip_fails_when_variant_behaviour_is_one_the_base_also_produces`）。

### 10.12.7 这次的方法论判据

> **「A 和 B 不一样」先问「A 自己和 A 一样吗」。**
> 任何跨条件比较，在读出结论之前必须先量同条件下的自身重复。本套件此前量过
> `instability_rate`，但那是**整轮通过与否**的口径；relation 比的是**签名逐字相等**，
> 那是严得多的一把尺，它的噪声底不能拿前者代替。**尺子不同，噪声底就得重新量。**

---

## 10.13 补上「恰好 N 次副作用」的契约字段（§10.7 记的缺口）

`expected.safety.side_effect_counts`，形如 `{parking.pay: 1}`。

**为什么上界逼近不够。** `engine.max_agent_calls_per_intent` 量的是**调用**次数，
`side_effect_counts` 量的是**副作用**次数——两者在「调用了但替身恰好没产生动作」时分叉，
而那正是确认闸相关断言最容易假绿的地方（这条形态本套件已经吃过一次：§8 的
「替身不产生动作」与「确认闸生效」在副作用面上长得一模一样）。两条一起写才闭合。

**三条口径**：

1. **声明即封闭**——未列出的键必须 0 次，与 `plan.allow_extra_intents=false` 同一心智模型。
   只锁点名那一格的话，「顺手把后备箱也开了」照样绿。
2. **全零非法**——那就是 `no_side_effect_before_confirm` 那句话；用等式再写一遍不是更强，
   只是多一条同义断言。契约层拦下。
3. **只有 L2 可用**——完整副作用面只在完整决策链上存在。L0 只看得见端侧那一半，
   L1 一次执行都没发生。契约层强制（同 `expected.engine`），`_l0_expectation` /
   `_l1_expectation` 另各剥一次（挡同时声明 l0+l2 的 case）。

**计数键**：engine 侧用 `intent`，端侧 VAL 命令用 `<object>.<operate>`，其余端侧动作用
action `type`。**键里刻意不含 payload**——否则同一动作换个参数就成了另一个键，等式当场
失效，而失效的样子和「一次都没发生」一模一样。

**验证（先注入缺陷，再验修复）**：

| 步骤 | 结果 |
|---|---|
| 单测：付两次而调用数仍在上界内 | 上界逼近判**绿**，等式判**红**（这就是它补上的那件事） |
| 单测：多做一个 `trunk.open` | 唯一失败断言是 `safety.side_effect_extras` |
| 单测：一次都没执行 | 红（等式两个方向都守，否则退化成上界） |
| 真栈 L2 注入 `gold=2` | 第三轮 `stable_fail`，失败断言 `safety.side_effect_count:parking.pay` 期望 2 实际 1 |
| 真栈 L2 恢复 `gold=1` | **3/3 通过**，第三轮两条 side_effect 断言真的在裁（不是被跳过） |

`gold_digest` 不用改——上一批（§9 P0-B）已把它改成 `asdict(TurnExpectation)` 整体序列化，
往契约加字段会自动进指纹。**这是那次「不手挑字段」的直接红利。**
...
## 10.15 §10.14.4 的结论（2026-08-03 深夜产品批回填）

§10.14.4 立的两条 P0 已修完（提交 `9d6ae0e`，逐条证据 `docs/design/2026-08-02-intent-routing-adversarial-findings.md` **§8**）。
本节只回填**与本报告结论有出入的部分**，细节不复制。

**两条的真因都和本报告写下的定性不一样：**

| 用例 | 本报告写的 | 实际 |
|---|---|---|
| `ex.homophone.aircon` | 「同音字被解成了另一个车控对象」 | 现象对，但根因是 **`skills/exemplars/` 下一条车控范例都没有**——诊断行 `exemplars=[]`。范例库最初 199 条金标全部来自**云侧** manifest 的 `examples`，端侧车控没有 manifest examples ⇒ 整片空白。同音字恰恰是端侧快路径认不出、必然上云的那一类 |
| `nq.umbrella.both` | 「2026-08-02 晋级时通过，现在稳定红 ＝ **回归**」 | **「回归」这个词用错了。** 通过的那次与失败的两趟，检索名单与范例名单**逐字相同**（`conditional-reminder@lex:14` + `reminder#28@vec:0.80` + `info#23@vec:0.67`）——注进模型的东西一个字节没变。真因是 guide 把并列的判据写成「提醒本身已经有**明确时间**」，于是「没说时间」成了判条件句的证据；**之前那次是蒙对的** |

> **判据（运行手册 §4 已收为第 5 步）：说「回归」之前先证明输入变了。**
> 检索名单是现成的零成本对照物；它逐字相同时，「以前是绿的」不构成「有人改坏了」的证据。

**§10.12.5「子句间槽位串味」的定性也要更正**：「明天早上八点」不是从前一子句搬来的——
串味说预测的是「明天」，实测拿到的是整串，而「早上」在原话两个子句里都不存在。
该串**逐字出现在** `reminder#28` 的槽位里，也出现在 `agents/reminder/manifest.yaml` 的
capability description（`context.py::_catalog_item` 把 desc 渲进 catalog，**每次规划都在**）。
铁证是 `nq.umbrella.both`：原话**一个时间词都没有**，照样产出 `{title:"带伞", time_text:"明天早上八点"}`
——与该范例的两个槽位一起照抄。修完后 3/3 红 → 1/3 红、幻影变成「明天八点」，
**「早上」（照抄）与「明天」（串味）就此分开了**。

**§10.14.4 的判据要补另一半。** 原判据是「规模闸绿 ≠ 门禁绿」。修完两条之后跑 gate 全量
L1：**110/116（94.8%）**，仍有 2 `stable_fail` + 4 `unstable`；而那两条 `stable_fail`
（`cp.adaptive.rain-umbrella` / `cp.dep.menu-then-order`）**逐条独立复跑都翻面**，
消融四臂 `causal: none`。

> **门禁红也不等于有稳定缺陷。** 一条 `stable` 用例能在两个独立进程之间 3/3 ↔ 0/3 地翻，
> 说明晋级判据「两趟独立进程都过」买到的置信度比想象中低——**它是必要条件不是充分条件**。
> 下一步该做的是量清楚现有 132 条里有多少站在判定边界上（gate 全量 `--repeat 3` 连跑三趟、
> 看每条的 pass 率分布），而不是继续逐条修 badcase。

**另有一条本报告未覆盖的新发现**（findings §8.4）：`hvac.*` 与 `aircon.*` 是同一个动作的
两个 intent 名（`edge_call._to_structured` 里 `{"hvac": "aircon"}`，实测两者解出的 `data`
逐字相同），而能力面两套名字都注册、端侧 catalog 又只渲染意图名 ⇒ planner 面对两个无法
区分的同义工具只能掷硬币。**因此 §10.11 里「『有点热』落 `hvac.inc`、方向反了」这句已不成立**
——它现在产出 `aircon.dec`，方向是对的，红在 gold 的 `any_of` 只收 `hvac.*`。
判据：**一部分「不稳定」其实是别名分裂，不是模型抖动。**

## 10.16 2026-08-04 最终修复验收回填

本报告之后的尺子、L3、知识与产品修复已在
`docs/reviews/2026-08-04-review-intent-adversarial-finalization.md` 统一验收。当前结论不再是
“完整三样本 gate 待跑”：已完整跑三趟，最终读数 **113/117**，4 条均为
`unstable`，无 `stable_fail`、无 provider 漂移、无检索降级或基础设施错误。

因此本报告的历史 P0/P1 定性保留为流水，但**当前是否达标以 2026-08-04 验收报告为准**。
当前仍不得写“baseline-ready”，正式 baseline 没有生成。
