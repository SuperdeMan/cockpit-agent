...
> **第 1–5 批已于 2026-08-28 落地**：第 1 批 C1 安全告警链 / C2 端侧车控三 bug /
> C16 的 1·6·8（覆盖 P0-01、P0-02 及其散落漏检，流水 §73）；第 2 批 C4 会话事实的
> 确定性读出口族（覆盖 P1-04 后半 / P1-10 / P1-13 / P1-16 / P1-07 追问轮，§74）；
> 第 3 批 C3 + C10（覆盖 P1-05 与 P1-07 主体、P2-17 的落域半边，§75）；
> 第 4 批 C6 + C5 + C7 + C8（覆盖 P1-03、P1-04 前半、P1-08、P1-09 与 adv T48 接地，§76）；
> 第 5 批 C9 + C11 + C12 + C13 + C14（覆盖 P1-06、P1-15、P2-14、P1-11 落域半边、
> N4/P1-10 编造半边、P2-17 话术半边，§77；**C9-B 评估后不做**——那条机制按真栈五轮
> 跑不复现，理由在方案「第 5 批终态」）。
> **第 6 批 C15 + C16 其余已于 2026-08-28 落地，六批全部完成**（§78）：C15 的裁决
> 落到探针（卡型 × 允许 mode 期望表 + 与契约 §9.3 之间的漂移闸）、C16 的 2/3/4/5/7
> 与 C9-E/C12-D/C13-C 三条配套，并新增**回放计分** `--replay`。
> **本文件那句「自动计分 282 PASS / 33 FAIL」现在有了第二个读数**：拿今天的尺子回放
> 同一份 artifact ＝ **33 红 → 38 红（转红 17 / 转绿 12 / WARN 5 / 不可回放 44）**
> ——转红 17 行就是当时那批「自动计分 PASS 里混着的假绿」，转绿 12 行全部来自
> C15 裁决（`deterministic` 收编 + 如实标注的 mock 记 WARN）。逐行见方案
...
> ✅ **长会话探针同日也在 `6a65e7a` 上跑了**：四个 persona 完整 + `information` 补跑
> 56/58。本文件里几张靠长会话才复现的症状卡**已有修复后的真栈实录**——
> **P1-06 城市漂移**「上海」→「深圳」、三张缺章卡全部盖章、**P1-10 来源追问**
> 从编造「东方财富、19:23 前后」变成念出真实 tushare、**P1-04 重列**六次都答出真列表。
> ⚠ **仍红两条**：T20「这份方案先取消」→「行程已为您取消…」**零动作**（六批没修它）、
> T23「红色机油灯亮了还能继续开吗」落 `system.clarify`（**拦住了危险执行但没答对**）。
> ⚠ **本文件的读数一律仍是 `c7c211b` 那一次的原样，不回改**；修复后的对照读数
> 逐条在 history **§79 / §79.6**。**仍不许把「58/61」或「56/58」读成「QA 全绿」。**
