...
// 语气词/口头噪声：整句只由这些字组成（1-4 个）→ 不是请求，静默丢弃（U5a）。
const FILLER_RE = /^[嗯啊哦呃额呀哈嘿哟欸诶唔嗯呐呵]{1,4}$/u
...
/** 纯语气词/口头噪声（嗯嗯/啊/哈哈/唔）→ true，静默丢弃不上云。 */
export function isFiller(text) {
  return FILLER_RE.test((text || '').trim())
}

/** 退出/dismiss 词匹配：去尾语气词后**精确**匹配词表（修「没事了」不命中「没事」）。
 *  raw 与去尾后都试，兼顾词表里带语气尾的整词（如「别说了」）。
 *  刻意用精确匹配而非前缀匹配：前缀会把「退出导航」这类真实命令误吞（「退出」在退出词表），
 *  精确匹配同样修好「退下吧/没事了」且不误杀带宾语的命令——承接设计卡 §8 anti-误杀 红线。 */
export function matchExitWord(text, words) {
  const raw = (text || '').trim()
  if (!raw) return false
  const stripped = raw.replace(TAIL_PARTICLES, '')
  return (words || []).some((w) => !!w && (raw === w || stripped === w))
}
