...
> 版本：v1.34（当前架构基线；版本规则见文末「附录 C：版本记录」）
> 日期：2026-08-20（v1.34 定稿归档——指代到人：占位与具名是同一个实体，救济路径要闭合）
...
1. 卡上写「按钮路径带结构化引用」——**并没有**。按钮送出的也只是一句中文
   （`send_text` = `在<门店>点一份<商品全名>`）。两条入口真正的差别是
   **用词是不是商家的原名**。
...
那一组候选的**最小投影**（只有序号与名字）注入声明了 `candidates` scope 的步，
Agent 侧做确定性翻译，把用户的说法换成商家原名，**之后两条入口在 Agent 里逐字同路**。

三条纪律，每条都有它挡住的具体事故：

1. **投影只留序号与名字。** 「哪些事实值得跨轮留住」与「哪些字段可以离开编排」是
   两个问题——桥是第三方信任级、连位置 scope 都没有，候选集不能成为绕过那条声明的
   第二条路；而营业时间/价格那些字段的消费方在编排自己这一侧。
   **也刻意不下发 id**：收敛目标就是按钮送出的那个规范名，再给一条 id 通道
   会让两个入口重新变得不一样，只是反了个方向。
2. **翻不准就不翻。** 命中多项一律不动，交回商户既有的追问链。
   **一个算错的确定性答案比不答更糟**——它没有 LLM 那种不确定信号，用户会照着付钱。
3. **归属只认产出这份候选的域。** 否则「先查附近的瑞幸」之后一句「点第一个」
