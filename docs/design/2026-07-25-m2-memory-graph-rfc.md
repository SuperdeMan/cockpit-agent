...
> 日期：2026-07-25
> 状态：**设计待审（本 RFC 只做设计，不含实现）**——含**对母提案 §4.D 的两处修正建议**（§2.1 不建
> `preference` 新表、§2.3 emotion 收窄），请泓舟就 §8 三个决策点拍板后再开工。
...
1. **偏好层加列 vs 建 `preference` 新表**——我建议加列（§2.1），理由是只缺三个字段、且会推翻
   半个月前刚做的单表合并决策。若你更看重「偏好是独立一等实体」的表达，我按新表做。
2. **emotion 是否进记忆层**——我建议不进，改为会话级信号直供 TTS（§2.3）。这与母提案 §4.D
   的字面表述有出入，但与它自己给的约束（短 TTL、不入画像）一致。
3. **P1 关系边是否本轮做**——它是 E2 长链路的真实前置，但也是本 RFC 唯一新增的表与隐私面。
   若想先看 P0 的收益再定，P0/P1 可拆两批走（P0 无新表、风险最低）。
