...
| [2026-07-03-r3.5-degrade-matrix-e2e.md](2026-07-03-r3.5-degrade-matrix-e2e.md) | R3.5 降级矩阵自动化：新 `test/e2e_degrade.py` 刻画架构 §3.3 四行真实现状（断网/云Planner故障/单Agent故障/LLM超时，docker stop/pause 级故障注入 + `LLM_MOCK_DELAY_MS` 测试钩子），过程中发现 3 处真实缺口（executor 丢 error 信息/planner中途崩溃静默挂起/CompleteStream 无备用模型重试）按用户决定只记录不修 | 落地中：代码已写，待本地验证 + GitHub 实跑确认全绿 |
