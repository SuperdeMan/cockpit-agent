...
- **状态**：✅ P0 + **P1a 体验闭环**已落地（2026-07-11）。**P1a（P0 审查后泓舟批「直接开干」）**：`reminder.update` 改时间（标题/序号定位+多条澄清+缺时间经 `REMINDER_PENDING(action=update)` 两轮续接）、snooze **改期原条目**（「稍后10分钟」按钮/「过10分钟再提醒我」→ 同名 fired 尸体收编不新建，根治 P0 按钮取巧造成的 fired 堆积）、重复规则 `recur`（每天/每个工作日/每周X，触发后滚动下一次、错过跳过不补发、工作日首触发周末顺延周一）、审查修复 B1 大后天列表截胡/B2 澄清序号续接补测/B3 非法日期（2月30号）诚实 FAIL/N1 文档改名/N2 LLM ISO 时区守卫、「下个月/这个月」区间查询、「过N分钟」无"后"缀词形；reminder 单测 69→93、eval 26/26（+9 用例含「空调改到26度」guard 反例）、真栈 `e2e_reminder.py` **7/7**（+snooze 无尸体断言）+ recur/update 真栈探针全通。此前 P0（同日）——Task 0-12 全绿，真栈 6/6，全量 1268 passed。集成期修 `orchestrator/cloud/context.py::_POC_DEFAULT_SCOPES` 缺 `profile.read/profile.write`（reminder 声明的合法 first_party scope 不在 PoC 默认授权集，被 `_filter_by_permission` 在规划前剔除 → 全兜底 chitchat），补为类目级默认授权、符合 §10「profile.*，无新 scope」。**真机硬化（2026-07-11 泓舟验收）**：cancel/complete 标题命中多条时旧实现取 `hits[0]` 静默少删且聚合器谎报删除数量 → 方案乙改为 `_resolve_targets` 返回全部候选、多条走 `_clarify_multi`（NEED_SLOT 反问 + 写 `REMINDERS_ACTIVE` 支持「第 N 条」续接）、单条才执行（reminder 单测 68→69，真栈端到端澄清+续接验证）。此前：已批准（2026-07-11 泓舟评审通过，含 D7 多天查看维度增补）
...
### P1 体验补全（拆两档：P1a ✅ 已落地 2026-07-11 / P1b 位置触发独立启动）
- ✅ **P1a** `reminder.update`（"把接孩子改到五点"）+ snooze 正式化（fired 后"过10分钟再叫我"直接改期原条目而非新建——同名 fired 收编；无标题时取最近 fired）。
- ✅ **P1a** 重复规则：`每天/每个工作日/每周X`（`recur` 字段生效，fire 后滚动下一次：错过跳过不补发、工作日跳周末、"完成/取消"语义=系列级取消 `cancelled`，单次确认不改状态——列表恒显示下一次）。
- ✅ **P1a** "下个月/这个月"区间查询 + 审查修复（B1 大后天/B3 非法日期/N2 tz 守卫）+ 澄清话术带动词（「取消/完成/改第几条」，配 hint 序号形，续接确定性路由）。
- ✅ **P1c** 跨域提醒（2026-07-11 真机 badcase 驱动）：「第一场提醒我观看」→ 标准化 `REMINDABLE_ACTIVE` 交接（sports 生产 / reminder 消费 / `parse_lead` 提前量），见 `2026-07-11-reminder-cross-domain.md`。
- ⬜ **P1b** 位置触发："到公司提醒我拿文件"——订 NATS `vehicle.state.changed` location 变更（road-safety 同款订阅），`context_scopes` 补 `location`。碰 NATS 订阅/围栏判定/隐私面，独立会话聚焦（泓舟 2026-07-11 批 P1a 时定档）。
- ⬜ 错过补投：HMI 重连/会话开始时对 `fired` 未 done 的当日条目补一句汇总播报（维持后置）。
- 持续：路由评测语料扩充 + 真机验收（泓舟口径）。
