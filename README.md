...
| `manual-rag` | Xiaomi SU7 真实车主手册 RAG：生产 `7b594f37` 使用 hash 绑定 `.mrag` 图文包；原36题在`434a046`闭合，当前整本187叶子首轮181/187（6项均2/3）、视觉30/35（5项0/3），本地0.3.2候选已修显式来源落域与5个三字caption但尚未发布 |
...
- **停车仍为 mock Provider**；手册已在生产 `7b594f37` 使用真实 Xiaomi SU7 v2 图文包；
  原36题与14×3高风险问法只在旧release`434a046`闭合，当前整本扩面仍有章节单轮方差和
  5个视觉稳定缺口，不能写成整本全绿。两者不得混写成同一条 mock 边界。
