...
守卫挂在计划构建的**唯一出口**（LLM 计划、降级语义路由、确定性路由兜底三条来源
在那之前已经汇合），三项判据全部从声明派生、不含任何 agent_id / intent 字面量：
这一步在端侧执行（manifest 的 deployment/kind）∧ 这一步是写操作（只看操作名）∧
这句话是在问（封闭虚词类，由源码级断言守着「不许出现领域词」）。
因此它**不违反「不改编排核心加硬编码」**：它是安全不变量放在唯一出口，
不是给某个 Agent 的路由特判。

拦下之后交给兜底 Agent 答一句，**不留空计划**——「X 灯亮了怎么办」恰恰是最需要
一个回答的那一类，而空计划会让编排宣布「没听清」。
...
会把 `search/menu/talk/status` 等云侧读取或应答能力误判成写。问句写闸因此只有一个判据：
...
以及 mixed 计划里的合法步骤逐字保持；mixed 只丢违规对象。若过滤后零步，仍走既有兜底并
保留 `question_write_blocked`，不把安全问句降级成“没听清”。这条边界也明确**不声称覆盖**
所有 `require_confirm=false` 的云侧副作用；扩面要等正式 effect 契约成立。
