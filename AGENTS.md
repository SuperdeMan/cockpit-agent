...
**5668 passed / 14 skipped 零红**（2026-08-15 双档复跑修复批后实测，16m34s）。
较 E1–E5 批的 5659 净 **+9**：时区族 3（业务时区锚定 1 + 两条源码级守卫）+ nearby 6
...
**2026-08-15 《超级EVA指令集》全量端到端验证**（栈全量重建后逐条真栈 WS 探针，
报告 [`docs/reviews/2026-08-15-eva-instruction-set-e2e-verification.md`](docs/reviews/2026-08-15-eva-instruction-set-e2e-verification.md)、
流水 history **§37**）：25 条语料 **✅12 / ⚠️10 / 🔒3 面**（车控联动/订座/主动执行=
红线搁置立场非缺失）。过程抓修 7 处（`4b975bd`/`911b7c4` + 收尾批）：**跨厂商
备份档 `LLM_BACKUP=deepseek:deepseek-v4-flash`**（泓舟指定；MiniMax 抖动时厂商内
fast=primary 同名一抖整死）、方向词目的地拦截（「北方追春天」臆断槽穿透全等改前缀）、
描述性历史指代→轨迹候选挑选、**复合取消句吞句**（wait_slot「算了X不要了先去Y」曾
4.6ms 直回已取消吞掉后半——剥词余量 ≥6 按新请求继续）、范例三投（占位符槽被逐字抄/
家人位置误落 set_place/「找杯咖啡」被派下单）。遗留六项当日晚由泓舟指示**立卡
收口**（P1–P6，`docs/design/2026-08-15-eva-e2e-residual-cards.md`、history **§38**）：
个人地点三级途经点解析/多城 `must_visit` 点名 POI 入池（大秋裤→东方之门真栈全进）/
槽校验容错+人话/散句 relation 确定性抽取+resolve 三类并集/负偏好降权两根因串修
（咖啡类目消费门禁结构性漏洞）/方差面范例四投。终态计分 **✅17/⚠️5/🔒3**
（报告 §5）；过程抓修恶性缺陷一枚（家人位置陈述被 set_place **改写本人公司地点**
——人称守卫，绝不写画像）。

**2026-08-15 EVA 余项立卡 E1–E5**（§4.2「EVA 余项」除 G10 外全部处理，卡文档
[`docs/design/2026-08-15-eva-backlog-cards-e1-e5.md`](docs/design/2026-08-15-eva-backlog-cards-e1-e5.md)、
流水 history **§39**，架构 **v1.26**）：E1 事件时刻**反推用餐窗**（`arrive_by` 的镜像半边；
时刻解析下沉 `agents/_sdk/timewindow.py` 唯一实现；路上预留是**明说的假设**、话术必须念出来；
按入座时刻筛营业中用的是真实 `opentime_today`）／E2 停车便利·无障碍·不排队属性维
（半径停车场计数**近似**，K=4 串行有界；**没有数据就说没有**；原话与记忆两路触发）／
E3 多城**确定性归城校正** + 主题接地命中率读数／E4 方差面**只读探针**给分母
（`scripts/probe_plan_variance.py`，不进 CI 不当准入闸，维持不加 hint）／E5 谓词别名老账
（**消费方误伤已命中**——`place.avoid`/`poi.dislike`/`restaurant.no_queue` 三条 scope 明明是
`profile.taste`，只因谓词不带 `taste.` 前缀、而消费方 scope∧前缀是 **AND**，就永远召不回；
归一表扩容 + 消费面**两路并集** + 存量 18 行归一，泓舟授权①不做②）。真栈：E1 4/4、E2 3/3、
E4 首跑 15/15 无方差、E5 由「地图没有实时排队数据」坐实。**三处过程抓修**：假个性化第二形态
（「和老婆吃饭」命中「父母腿脚不便」）、探针自己抽掉位置前提造出「过度澄清 100%」、
类目别名被当检索词（「为您找到 10 家吃饭」）。E3 连修四轮才够得着，末轮漏网点由日志实锤为
「潍坊池被高德限流 ⇒ 该城无质心」的已知边界。

**2026-08-15 EVA 指令集双档复跑 + 三处修复**（报告
[`docs/reviews/2026-08-15-eva-dual-provider-rerun-and-fixes.md`](docs/reviews/2026-08-15-eva-dual-provider-rerun-and-fixes.md)、
流水 history **§40**）：25 语料 × `MiniMax-M3` / `deepseek-v4-flash` 各 27 轮
（泓舟指定加跑对照，跑完已切回主模型）。**对照档的用途当场兑现**——两档相反=模型方差
（一#1 MiniMax 导航到济南同名小学 2004km 而 DeepSeek 落对、一#2/一#9/三§2#5 同理，
三§3 方向相反），**两档逐字同错=系统缺陷**。据此定位并修掉三个真缺陷：
① **容器 TZ=UTC，所有「几点」判定整体偏 8 小时**（真栈原文「预计 05:17 到达，比您要求的
17:00 早约 **703 分钟**」）——影响 timewindow/navigation ETA/焦点渲染/road_safety 夜间窗/
**proactive 免打扰时段**/memory routine/trip 起始日/股票交易日 8 处；修法=新增
`runtime/clock.py` 作全仓唯一实现 + 6 处各自定义的 UTC+8 常量收敛 + **两条源码级守卫**
（禁裸 `localtime/mktime/datetime.now()`、禁第二份时区定义，均做过注入/还原双向验证）。
② **当轮明说的忌口被记忆偏好压过**（「不要太辣」两档都推川菜）——假个性化第三形态。
③ **画像 `place.company` 存量污染**（P4 那枚恶性缺陷的残留，守卫防住新增没清存量）。
顺带修同族更大一洞：**约束词被当检索词**（「不辣」搜出一串「辣可可」、「适合带老人」
搜出家政公司、「晚饭」搜出赛百味）。修后真栈复验全部兑现（D1「早约 201 分钟」正确）。
...
| 代码回归 | 全量基线见本节顶部（**5659**，2026-08-15 E 批后）；分套件最近实测：`orchestrator/cloud` **804** / navigation **132** / trip **85** / mcp-bridge **433** / memory **229** / nearby **87** / `agents/_sdk` **+8**（新增 `test_timewindow.py`）（2026-08-15），edge **579**（2026-08-11 未重测，history §38.1）。Skill / Exemplar（**307 条 / 22 域**，2026-08-15 实数——clarify 型机制已就位但**刻意零生产范例**，见 §4.2）/ L0 门禁均通过。catalog 目录 **144 条 / 12508 字符**。端侧能力面 **80 条**（vehicle 76 + media 4）/ VAL 车控对象 **67**。⚠ 2026-08-11 起 `VEHICLE_INTENTS` **不再手写**，由 `commands.yaml` 各对象的 `edge_intents` 派生（B4），数字不变但改的地方变了 |
