...
| 安全/权限/编排/协作/支付 | ✅ PoC 链路落地。**权限已单轨化（R2.2，待合并 main）**：三处权限实现（planning 内联过滤/dispatch 内联校验/PermissionEngine 死壳）收敛为唯一决策 `security/permission.py::check_permission`，规划期 `_filter_by_permission` + dispatch 执行期同源复用；删 `engine._enforce_permissions` 空壳；fail-open 由 env `PERMISSIONS_FAIL_OPEN` 门控（默认 on 保持现状，量产翻 false fail-closed）+ 结构化审计 `fail_open_default_scopes`。纠偏见 `docs/design/2026-07-02-r2.2-permission-single-track.md`（接 effective_scopes 会因扁平交集误拒 scene-orchestrator，取零行为变化单轨、trust-cap 强上限推迟 R3.1）。真实 token、正式沙箱与真实支付仍待接入 |
