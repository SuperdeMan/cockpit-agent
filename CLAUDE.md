...
**Phase 1 工程化 PoC（截至 2026-08-13）**，运行模型 T0 端侧快路径 / T1 单次 DAG / T2 有界
...
支付基础设施真实化四批（2026-08-11：支付宝/微信双渠道扫码收单 + 商户收银登记）与
麦当劳/瑞幸官方 MCP 复合工作流（2026-08-12：选店/选品/规格、预览计价、确认后创建
未支付订单、安全支付入口、查单，瑞幸再次确认取消；**不执行最终付款**，契约 §9.17/§9.9，
流水 history §28/§29）与商户 badcase 收口七批（2026-08-13：`merchant.*` scope 发放
——此前**全仓无发放入口**、e2e 自己塞 `granted_scopes` 所以一直是绿的；两家只读当店菜单
...
**探索式真实用户 QA 轮**（2026-08-15 立卡 / 2026-08-19 持续推进，架构 **v1.30**）：
533 轮 × 5 persona × 两档跑出的 58 个问题收敛成 **13 个根因**（Q1–Q13），四阶段推进。
已交付 Q1/Q3/Q4/Q13/**Q2（载体 + 候选集聚合问答的确定性消费方）**/
**Q7（三维度全收口）**/Q5（写入闸 + 出处披露）/Q11/Q10（三块）/Q6/**存量数据清洗**/
Q12（槽值保真主体 + 中文时间词三份实现收敛）；
契约面新增 `conventions.md` **§9.19–§9.27** 九条。
⚠ **2026-08-19 云端复跑掀开的那条已当日修完**：记忆召回在云端失效，**根因是 `AUTH_TOKENS` 漏写 user_id 段**（网关把 user_id 解析成 `v1`，而记忆全在 `u1` 名下；scopes 段恰好没错位 ⇒ 权限全通、`verify` 全绿，只有记忆召不回）。
已补网关 fail-closed 守卫 + 云端 E2E 记忆探针，history **§59**。
**当前进度、逐卡残余与接手顺序一律以 `AGENTS.md` §4.1 ① 为准**（那里是唯一真相源，
本节只说到「这件事在做」）。
⚠ 该轮验的东西与 EVA 那几批**不是同一个面**：EVA 验「能力面有没有那个维度」，
QA 验**会话状态、归属、审计与真实性**——所以它抓到的三个 P0 与 44 个 P1 与
EVA 的读数不矛盾。
