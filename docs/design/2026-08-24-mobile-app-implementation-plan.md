...
- [ ] ⬜ **未验** 多端并发：座舱 HMI（浏览器）与 App 同 user 同时在线，各自会话不串、记忆共享
- [x] ✅ 主动提醒到点前台送达（33s）+ 完成按钮闭环
- [ ] ⬜ **未验** 重启零重投（M1-8 两次观察窗都无效：一次起的是 dev launcher、一次前台归属不明）
...
- [ ] ⬜ **未验** 无 key 引擎回退出声（现成可用的反例：mimo TTS 四次探测全 error）
