...
### 11.6 M-A 验收探针的有界空传输重试（2026-07-31）

M-A canonical 复跑暴露了一类**探针传输方差**：realtime provider 偶发在 ASR 产生任何转写或
工具事件前以 `ws_closed:1007` 关闭 session。旧探针只对车控轮内联重试，闲聊轮遇到同一故障会
直接把整组判红，两个轮次的证据口径不一致。

修复不是“失败就重试”，而是一个严格谓词：仅当 `transcript` 为空、`tool_call` 为空，且状态为
`None`、`error` 或 `ws_closed:*` 时，关闭旧资源并用**新 session**重试一次。已经完成但路由错误、
已有转写、已有工具调用的结果都保留为首轮裁决，绝不借重试洗掉真实错误。闲聊轮和车控轮共用同一
helper，单测钉住旧 session/WS 被关闭、新 session 只开一次以及两次音频提交；真 provider 探针
随后恢复 **12/12**。这只是验收探针对“零声学证据的传输失败”做分类，不改变生产
L-Session 的重连/降级策略。
