...
真栈迷你集新增三条（`--group candidate`）：**CD5** 组指代（点到麦当劳那组第 2 项
**∧** 不许点到瑞幸那组任何一项——**两条互补的结构判据同时成立**才算，只压前者
压不出「绑错组」，那是 CD2 那次「答了一个名字就判绿」的同一形态）、
**CD6** 跨组比较（`names_items_from` 是 `names_item_from` 的 list 形态泛化，
**两边都要点到**才叫比较）、**CD7** 误伤对照（没点名时逐字还是旧行为）。
