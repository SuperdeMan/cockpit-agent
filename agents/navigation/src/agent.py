...
class NavigationAgent(BaseAgent):
    ...
    async def handle(self, intent, ctx, meta) -> AgentResult:
        if intent.name == "navigation.search_poi":
            return await self._search_poi(intent, ctx)
    ...
    async def _search_poi(self, intent, ctx) -> AgentResult:
        ...
        results = await self.poi.search(keyword, near=near, rating_min=rating_min)
