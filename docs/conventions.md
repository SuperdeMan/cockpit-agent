...
| `trip.status` | trip-planner | cloud | day | 在途进度只读：在第几站/下一站/还剩几站/全程补电几次；**`day` 非空时按天读**（「第二天有哪些安排」→ 那一天的停靠点）——写侧的 `itinerary[].day_index` 早就够了，缺的一直是读侧（C7-D）|
...
| `_escalate` | 任意 Agent（现：chitchat 时效兜底 → `info.search`；商户桥拒绝时按店名线索 → `nearby.search`；`navigation.search_poi` 导航语境没找到 → `navigation.navigate_to`，挂起落在改派步上，评审四轮 RS39） | engine D0/executor 两路径（每轮最多 **1 跳**；已流式播报过增量的结果忽略——final 里带的话术不算；escalated 结果里的二跳声明不消费——结构性防环；T2 循环不消费，声明方的话术须自己是一句实话） | `{"intent": str, "slots": {str:str}, "reason": str}` | 「这题我不该答，改派给该 intent 的 Agent」——engine 经 `_validated_steps` 装配单步 mini-plan 走 executor（heavy/预算/权限自动带出），过程区/挂起语义与正常步一致。设计：`docs/design/2026-07-12-mode-routing-and-answer-quality.md` P1-2，契约测试 `orchestrator/cloud/tests/test_engine_escalate.py` |
...
| `_candidate_place` | 产出候选列表的 Agent（现 nearby.search：中心来自 `location` 槽或原话「X 附近」锚定时）| `context._place_hint(slots, data)` → 候选组的 `place_hint`（组指代、墓碑点名都读它）；未声明时照旧从查询槽派生 | `str`（≥2 字，>20 字截断；坐标不声明）| 「这一批是在哪一带搜的」——批 7 追加（2026-09-22，真栈 continuity T9 → T18）：planner 把「科技园附近的餐厅」填成 `{cuisine, sort}`，地名整个丢了；nearby 从原话锚定了科技园、结果也对，但编排的地点提示只从槽派生 ⇒ 这批叫不出名 ⇒ 十几轮后「科技园那批第二家」零命中退回最新那批（南山书城），答了别人家的店。**判据与 `_candidate_label` 同一条：在哪搜的只有产生方知道。**契约测试 `test_candidate_sets` +2、nearby +2 |
...
时，`AgentResult` **必须用 OK 状态承载话术**，不得用 FAILED。FAILED 仅用于
**无话术可播的真异常**（超时/崩溃），由聚合器出通用失败话术。

> ⚠ **2026-08-28（C11-B）理由更新，规则不变。** 原文写的链路事实是「聚合器对单步
> FAILED 只读 `r.error`，FAILED 上的话术会被吞成裸『抱歉，处理失败』」——**那条已经
> 修了**（§9.37-C：有话术就原样透传，连同 follow_up 与卡片）。规则留着，因为它的
> 价值换成了另一条：**FAILED 是机器面的事实**（账本、探针、重试规则读它）——
> 一次业务性拒绝报成 FAILED，用户听到的话没变，但账上多了一条没发生过的失败。
> 反过来也一样：**真的没达成目标就该报 FAILED**，别为了让话术出得去而改成 OK。
...
评分、距离。算子**四种**：最值、合计、**序数取值**（下节）、**重列**（2026-08-28 C4-C，见 §9.34 C——「重新列出刚才可以选择的项目」在前三种上一条都不认，于是整句进 Planner 重搜了一遍）。
...
- **Android 跨页呈现（AR04，2026-09-08）**：应用级宿主共用原有会话与语音控制器；
  对话、地图、车辆、设置页在前台提供采集事实和停止出口，后台/锁屏及不支持的路由撤回采集与播报，
  已发送主链请求的文字终态仍按原 requestId 落账。主动消息入 store 仅算收到，真实可见出口经窗口、
  遮挡和布局复查后才报告 presented、发送既有 `proactive_ack` 并进行一次播报仲裁；用户收起是本地 handled，
  不表示业务完成。已呈现消息重投时允许补 ACK，不重复气泡/播报；未送出的回执独立重试，不污染用户 queued。
  录音已授权时先查询 OS 权限，不重复启动 Android 权限 Activity。实现、设备范围和未闭合项见
  [`AR04 实施记录`](design/2026-09-08-ar04-presentation-ack-implementation.md)。
...
### 9.35 槽位值形状契约 `slot_shapes` + 提醒域的序数参照系与任务性准入（QA C3/C10，2026-08-28）

两张卡共一节，因为它们是同一类病的两个面：**会话状态机把「这句话属于谁」判错了**——
一边把新请求塞进旧槽，一边让序号指向用户没看见过的东西。

#### A. `wait_slot` 的默认方向反转（C3-A）

`_is_topic_change` 的旧默认是「**除非证明是换话题，否则当槽值**」。它每漏一种说法，
那句话就被整句塞进槽里：真栈 T44–T46 一条 `item_query` 挂起连吞三轮，
每轮都答「在售餐单里没查到"<用户刚说的那句话>"」。**补词表没有终点**——
规则的边界比说法的边界窄（reminder 那条 replace hint 退役时就写下的判据）。

反转成「**槽值必须长得像这个槽**」。先例就在被它取代的那段代码里：`order_id`
只接受两种显式形状、其余一律换题。本节把那个特例升成通用机制：

- **声明**：`manifest.yaml` / `servers.yaml` 的 capability 加 `slot_shapes`
  （`槽位名 -> 形状名`），经 `Capability.slot_shapes`（proto 字段 8）到编排。
  哪个槽是什么形状是**领域知识**，所以住在 Agent 一侧。
- **判据**：`orchestrator/cloud/slot_shape.py` 是**唯一实现**，**零领域词**
  （源码级断言从 `commands.yaml` 派生词表比对，ASCII 按词边界、中文按子串）。
  分工同 `verification`：领域期望由 Agent 声明，中央只跑通用求值器。
- **三值语义，刻意不是 bool**：`False`=就是槽值（定案）／`True`=长得不像（换题）／
  `None`=形状没意见，交回通用判据。第三个值是必需的——`order_id` 匹配上就是权威，
  而 `item_name` 匹配上什么都不证明（「点一杯拿铁」完全长得像餐品名，
  它却是一句完整新指令）。压成 bool 必然牺牲其中一条。
- **漏声明的代价必须是「和以前一样严」**：`DEFAULT_SHAPE_BY_SLOT` 按槽名给
  `order_id` 兜底。这条写路径身份闸在本机制诞生前就对全部商户生效，
  不能因为谁忘了写一行 YAML 就突然放松。
- **形状名的值域不在运行期校验**，故意的：判据模块住在编排侧，而桥的镜像里没有
  `orchestrator/`（依赖闭包见两个 Dockerfile）。把已知形状名抄一份到桥里就是
  第二份声明。值域由 `orchestrator/cloud/tests/test_slot_shape.py` 逐条比对
  **全部声明方**（servers.yaml + 各 manifest.yaml），拼错当场具名红灯。
- **长度上限来自真机观测不是拍脑袋**：`item_name` 取 20 字，因为 2026-08-13 真机
  菜单里最长的在售商品名是 14 字；方案草案写的 12 会当场误伤它。
- **形状表当前三条**：`order_id` / `item_name` / **`ordinal`**（2026-08-29 补）。
  `ordinal` 是**两值**的（同 `order_id`，不是三值）：`index` 与 `order_id` 同类，
  **值域封闭**——消费方 `_resolve_targets` 只认「数字 / 第N条」，别的形状
  **就算填进去也没有意义**。所以「长得不像」在这里不是「可能不像」。
  判据形态是「剥掉序号本体与虚词之后还剩不剩实质内容」，**不是穷举「第N条」的写法**
  ——穷举必然漏，而漏一种就等于把一句新指令塞进槽里。
  ⚠ 代价明写：追问语里那句「或换个更具体的说法」从此走**重新规划**而不是补槽。
  这是变好不是变差——旧路径把它填进 `index` 同样用不上，只会**再问一遍**。

  > 真栈来历（长会话 `e15ac1e`，family turn 18/73/76 连撞三轮）：系统问
  > 「要取消哪条？」，用户下一句说「**列出我现在进行中的提醒**」，整句被填进
  > `index` 槽 ⇒ 认不出序号 ⇒ 退回标题路径 ⇒ 又查出同样三条 ⇒ 又问一遍。
  > 三轮之后挂起还活着，turn 77 用户说「取消导航」被它接走
  > （见 §9.38 A）——**一个黑洞会喂大下一个洞**。

#### B. 「这是一次新检索」的词表只许有一份（C3-B）

`candidate_query.NEW_SEARCH_RE` 是**公开的唯一来源**，`_is_topic_change` 与
`slot_shape` 都从它 import。此前它只在候选集聚合那一侧生效，于是同一句
「附近的川菜馆」在那边判成新检索、在补槽这边被当成槽值——**同一判据两份实现
各自演化，给同一句话两个答案**，正是 B1 那个 bug 的成因原型。

#### C. 泛指词归一是**消费方防御**（C3-C）

`base.normalize_menu_query`：只读菜单路径把「全部/所有/都有啥/整份菜单/随便」
整句归一成空槽（= 不筛，整份菜单），两家商户共用一份。用 `^…$` 而不是包含匹配
是刻意的——包含匹配会把「全家桶」这种含泛指字的真商品名一起吃掉，
那是从「答不出来」变成「答成了另一件事」，更糟。**只在只读路径归一**：
下单路径宁可追问也不能把「全部」理解成任何一款。

#### D. 补槽黑洞的止损底线（C3-D）

`SLOT_RETRY_LIMIT`（默认 2，env `CLOUD_SLOT_RETRY_LIMIT`）：同一条挂起**连续问
同一件事**到上限还没接上就放弃它、按全新请求规划，并且**必须说一句**
（Q1-C「淘汰必须有话术」的同一条纪律）。计数只认「同一步 ∧ 同一组待补槽」——
同一步换了个槽再问是**进展**（商户流程「先问门店再问餐品」正是如此），归零。
它是**判据之外**的兜底：换题判据再准也有漏网的说法，这条保证黑洞只能吞有限轮。

#### E. 「第N条」只许指向用户最后一眼看到的列表（C10-A）

`reminder._refresh_active` 的**无参形式**此前取 `list_split()` 的整表最老 10 条。
库里沉着 80 条过期垃圾时，一次成功的取消刷新完参照系，序号就**静默指向**一份
用户从来没看见过的考古清单——真栈 T59 的「取消第一条」于是取消了
「刚才那个提醒现在几点」。改成与 `_list` 默认视图同一口径（从现在起的未来项）。
配套：`store.get` 默认只给 ACTIVE（与 `find_by_title` 拉平——两条读路径此前
一条认 status 一条不认），读终态要显式传 `statuses`。

> 判据与候选集下发面 §9.28 同源：**任何后台刷新参照系的动作都是在给序数指代埋雷。**

#### F. 按标题操作的精确度阶梯（C10-B）

`_resolve_targets`：**先逐字相等，没有再退回子串**（多条子串命中仍走澄清，
既有行为不变）。planner 转述会把用户点名的标题放宽，子串于是同时命中
「评审会」与「准备评审会材料」——单条时直接执行，**取消掉的可能不是他点名的那条**。
`reminder.cancel` 的 capability 描述同批补一句填槽指令（catalog +49 字符，
行为锁 `test_catalog_budget.py` 已显式改）：**让它一开始就别放宽，比事后补救便宜**。

⚠ **2026-08-29 补第三档：`exact` 为空那一支此前是裸奔的。**
C10-B 只覆盖「有逐字相等就只认它」，而 planner 转述**放宽到查不出逐字相等**时，
`return exact or hits` 退回子串、调用方看见只有一条就**直接执行**。
真栈实录（deployed `ed53f8f`，D 臂第 1 轮）：用户说「取消参加**代号889001**的评审会」，
系统答「好的，取消了「参加**代号926818**的评审会」」——**取消了一条他没点名的提醒**。

`_resolve_targets` 因此改成返回 `(hits, precise)`，`precise=False` 时**单条也不许直接执行**：

| 档 | 判据 | 结论 |
|---|---|---|
| 1 | 逐字相等（`exact` 非空）| 精确 |
| 2 | **库里那条标题逐字出现在用户原话里** | 精确 |
| 3 | 子串覆盖率 ≥ `_TITLE_COVERAGE_MIN`（0.6）| 精确 |
| — | 以上都不成立 | 反问（走 `_clarify_multi`，话术说清「没找到逐字对得上的」）|

第 2 档是**既有用例逼出来的**，不是想出来的：`_extract_title` 会把时间词削掉
（「完成明天带伞」的 q 是「带伞」），而库里存的是「明天带伞」——
**光看覆盖率会把我们自己的抽取算成用户含糊**（2/4=0.5），
把一条完全正常的操作变成反问。
方向由代价定：阈值不够高 ⇒ 多问一句；不够低 ⇒ 删掉一条他没点名的提醒。
⚠ **别把 `or hits` 删掉**——那会让「取消评审会」这类合理放宽**查不到**；
本档只改「查到之后要不要直接动手」。

#### G. 「这句话是不是一件待办」的准入（C10-C）

`agents/reminder/src/task_admission.py`，与 `memory/offer_admission.py`（§9.30）
同族但**独立实现面**：那边判「值不值得 offer」，这边判「这是不是一件事」。
两条形态判据，**不取关键词**：

1. **疑问收尾**——剥掉语气尾后以疑问词结束。⚠ 刻意不判「句中有疑问词」：
   `问一下张总几点开会` 是一件真事，`刚才那个提醒现在几点` 才是问句。**位置就是语义。**
2. **第三人称主语开头**（`用户/该用户/客户/乘客/车主`）——用户跟助手说话时说「我」，
   第三人称是别的系统在描述他，那是记忆的句式不是待办的句式。

两条都只覆盖观测到的那两类，**宁可漏不误杀**：漏了顶多多一条垃圾提醒，
误杀的是用户真的要做的事。拒建要**诚实说**（R9：话术型拒绝用 OK）。

#### H. 跨轮幂等与过期沉积（C10-E / C10-D 的机制化）

- `_cross_turn_duplicate`：同 owner + **逐字同标题** + 同触发时刻的 pending 已存在
  ⇒ 收编不新建。`turn` 判据与 `_reschedule_target` 同源、理由也同一条——
  **一句话被规划成两步是设计内的（同轮不收编），跨轮重复才是该挡的那个**。
- `scripts/qa_data_hygiene.py` 新增第 ⑤ 族 `--reminders-expired`：`fired` 沉积超过
  N 天（默认 7）→ `cancelled` + `extra.reason="expired_sweep"`（+ `extra.batch`），
  零 schema 变更，**不物理删除**。这是把 2026-08-27 那次 90 条点名 SQL **机制化**，
  用同一个 `extra.reason` 约定、不发明第二个标记。
- 探针跑批结束补提醒清理段（按本次 run 号匹配自己建的、逐条取消并**复核计数回落**）。
  **只观测不中止**：这一段跑在全部业务轮之后，中止已无保护价值，而一个新写的
  清理段误判成失败会把下一趟跑批截断——那正是「修正后计分」那一趟最不能出的事。

---

### 9.36 分句级路由锚定 `RouteHint.scope` + 挂起的兄弟步语义 + 焦点让路（QA C5/C6，2026-08-28）

一族症状共一节：**一句话里说了两件事，系统只回应了其中一件，而且每次少的那件还不一样。**
架构面见 `cockpit-agent-architecture.md` §5.2.16；本节只写契约。

#### A. `RouteHint.scope`（proto 字段 7）

| 值 | 语义 |
|---|---|
| `""` / `utterance` | 整句锚定（**缺省，行为逐字不变**） |
| `clause` | 按 `runtime/clause_split.py` 拆句，**逐分句**试 `pattern` |

- **`$text` 的解析范围跟着 scope 走**：`clause` 档解析成**命中的那一分句**，不是整句。
- **`guard` 不跟着走，永远对整句求值。** 放宽锚定与放宽守卫是两件事——只做前一件。
  分句档首版把守卫也收进分句，当场撞红三条既有负向锁（「接女儿放学，路上买杯咖啡，
  **然后播放音乐**」这类另一个域的诉求靠的正是整句 guard）。
  代价是一条正向句会被邻句的否定词误伤（「接孩子，别忘了充电」），**这个交换是显式的**。
- **`append` 的去重判据按 scope 分档**：整句档问「这个 intent 在不在计划里」；
  `clause` 档问「有没有哪一步已经带着**本 hint 解析出来的那些值**」。
  按 intent 去重会把「接孩子后去万象城」的接人那半再丢一次——计划里那个
  `navigate_to(destination=万象城)` 回答的是另一半诉求。
- **分句表的唯一声明处是 `runtime/clause_split.py`**（端侧 `fast_intent._SPLIT_MARKERS`
  引用它）。**表共用、语义不共用**：端侧拆完每段要各自解出一条命令（还要接「和」的
  二次拆分、并列对象展开、场景句拦截），云侧拆完只问「这一段有没有被计划覆盖」。
  落在 `runtime/` 是镜像依赖闭包决定的（云侧编排镜像没有 `orchestrator/edge`）。

#### B. 挂起的兄弟步语义（C5-B）

| 状态 | 语义 |
|---|---|
| `NEED_CONFIRM` | 对**整轮**说「先别做」→ 当场停（逐字不变） |
| `NEED_SLOT` | 只挂起**该步及其下游**；无依赖的兄弟步照常跑完再挂 |

- 下游由既有的「依赖必须 OK」天然拦住，**不加第二份判据**。
- **本层已经算出来的结果全部交出去再判挂起**：同一层是一次 `asyncio.gather` 跑完的，
  在挂起处丢掉后面的结果不是「没执行」，是「执行了但不报」。
- **挂起 final 的 `actions` 合并兄弟步的动作**，走聚合器同一份 `compose_actions`
  （navigate 去重 + 充电途经点注入）——此前只带挂起那一步的 actions，
  兄弟步跑完了动作发不出去，**话说了事没做**。
- 同轮多条挂起按**声明序**取第一条报（`_topo_layers` 的层内顺序 2026-08-28 从
  `set` 迭代序改成 `steps` 声明序：并行执行不受影响，**读数必须确定**）。

#### C. 焦点让路 `WorkingSet.suppress_sticky_places`（C6-B）

**「值得跨轮留住」与「这一轮该不该注入」是两个问题**（判据分离同 §9.28 候选集的
跨轮留存表 vs 下发表）。置位判据完全声明式：本轮命中了任一 `scope: clause` 的 hint
⇒ 这一轮有一个自带完整语义的确定性诉求，不靠指代也说得清。

| 让路 | 不让路 |
|---|---|
| 上个地点 / 上个目的地 / 上个城市 / 最新候选 | 安全告警（这轮回答的前提，Q9）、上一轮意图、车控对象/位置/属性焦点、活动路线、上个股票标的 |

置位方是 `planning.build`，且必须在两条 prompt 路径（json / toolcall）**之前**
——两档 user message 逐字一致是 A/B 单变量的前提，让路只能从这一格进去。

#### D. 覆盖度观测列 `clause_uncovered`（C5-A，**零决策**）

obs 值形如 `"1/2"`＝未覆盖的肯定分句数 / 肯定分句总数，与 `goal_value_dropped` 并列。
判据：拆句 → 丢否定分句（`runtime.polarity`）→ **肯定分句 ≥2 才有信号** →
每句问「有没有任何一步的槽值（≥2 字）落在它里面」。
**已知误报面写在源码里**（零槽步覆盖不了任何分句 / 槽值被 planner 转述过时子串够不着）
——先拿两周真实分布，再谈升级成 salvage 重试（B6 shadow 的纪律）。

### 9.37 会话内偏好约束 `session_constraints` + 执行性声明观测列（QA C11/C12，2026-08-28）

> 两件事写在一节里，因为它们是同一条判据的两半：**系统说出口的话要能对上账**。
> 一半管「用户说过的约束有没有被消费」，一半管「系统声称做过的事有没有发生」。
> 设计 `docs/design/2026-08-27-minimax-qa-root-cause-fix-plan.md` C11/C12，流水 history §77。

#### A. `Focus.session_constraints`：当轮/会话内说出来的偏好

形如 `{"no_spicy": true, "no_queue": true}`（缺省不写键）。

| 面 | 落点 | 判据 |
|---|---|---|
| 抽取 | `orchestrator/cloud/context.py::extract_focus` | **挂在本轮原话的形态上，不挂在路由上**——「我不吃辣，也不想排长队」落的是 `chitchat.talk`，只在 nearby 那条路上抽就永远抽不到（同 §9.34 安全告警登记那条） |
| 判据与词表 | `runtime/session_constraints.py`（**唯一实现**） | 两个消费方够不着彼此：抽取在云侧编排（镜像没有 `agents/`）、消费在 Agent（镜像没有 `orchestrator/`）。落点判据是镜像依赖闭包，同 `polarity` / `clause_split` / `session_facts` |
| 跨轮 | `update_focus` 合并 | **后说的覆盖先说的、没说的沿用**。「没提到」写成**不写键**而不是 `False`——合成一个 False 会让一句无关的话抹掉上一轮的约束（同 `day_offset_of` 那条：认不出与说的是今天必须分得开） |
| 下发 | `_apply_focus_meta` → `step.meta.focus_session_constraints` | 按 manifest `context_scopes: [session_constraints]` **门控，不广播**——与安全告警的取舍**正相反**：告警是所有域都得服从的约束，忌口是个人数据，给不消费它的 Agent 只是多一处扩散面 |
| 消费 | Agent（当前 `nearby`） | **当轮原话 > 会话约束 > 记忆画像**；解析失败一律当没有（宁可少一条约束，也不要按解析错的忌口改用户的检索词） |

两条配套判据，都是被真栈烧出来的：

- **限制性偏好赢过扩张性偏好**（不吃辣 > 爱吃川菜）。挡「拿爱吃的辣菜系当检索词」
  的那块板必须问**合并后的忌口事实**，不是「本轮原话说没说」——后者会让记忆里
  两条偏好并存时，同一轮确定性地拼出三句互相打脸的话（真栈 T29 原样复现）。
- **词表要按人真的怎么说来写**。旧「不排队」词表要求否定词紧贴「排队」二字，
  于是真栈原话「也不想**排长队**」连识别都没识别到——与当年「不要太辣」漏掉同形态。
- **约束要压过检索词本身，不只压过记忆偏置**（C12-C，2026-08-29 补）。
  上面那块板（C12-A）的前置条件里写着 `not (cuisine or brand or kw_slot)`
  ——**planner 自己把 `cuisine=川菜` 填进槽时，它压根走不到**。
  真栈长会话 `e15ac1e` INF-PREFERENCE T28 逐字实录：用户上一轮说「我不吃辣」，
  这一轮说「推荐附近适合晚饭的地方」，话术播成
  「为您找到 10 家**川菜**（不合口味的已排后；记得您说过不吃辣…）」
  ——**约束被拿去排序了，却没能拦住检索词**。
  判据是「**这个重辣菜系是不是他这一轮自己说出来的**」：自己说的照办
  （既有 `_taste_conflict_note` 诚实提一句，真栈 CD5「附近的川菜馆」正是这一支，
  不许动它）；不是自己说的，退回干净类目词重搜并说出来。
  > 判据面之所以要挑「是不是他自己说的」而不是「记忆里有没有」：
  > **同一个词，从用户嘴里出来是指令，从记忆里出来是建议**。

#### B. 执行性声明的观测列（C11-C，**零决策**）

`runtime/execution_claim.py::execution_claim(text)` → `done` / `ongoing` / `""`。
判据是**形态**（指向用户的完成/进行体标记，如「已为您…」「正在帮您…」），
**零领域词**（源码级断言从 `commands.yaml` 派生词表比对，同 `actionability` /
`question_shape`）。编排层在 final 的**唯一出口**按「命中 ∧ 本轮 actions 为空」
发一条 `cloud.execution_claim` span；**只观测不拦截**——直拦的误伤面
（信息类能力的合法完成语、转述历史）没量过，先拿两周真实分布（B6 shadow 的纪律）。

⚠ 判据刻意**窄**：不带「为您/帮您」的无主句声明（「路线已经算好了。」）不计入
——判据一旦不看「是谁做的」，量的就不再是「系统声称自己动了手」，
分布也就读不出东西了。探针侧（C16-2）复用这一份，**不许在 `scripts/` 抄第二张表**。

#### C. 单步 FAILED 的话术出口（C11-B，改的是既有行为）

聚合器单步 FAILED 分支此前**无条件**查 `error → 友好话术` 表，而 `AgentResult`
没有 error 字段、executor 的 `_to_result` 也不设 ⇒ 查表恒命中空键 ⇒ 每一条失败
都被换成裸「抱歉，处理失败。」（真栈 T34：桥说的是「未能确认本次订单预览清理完成，
请稍后再试」，恢复指引连同「失败在哪一步」一起被吞掉）。

改为：**Agent 有话术就原样透传**（连同 `follow_up` 与卡片），空话术才落
`_ERROR_FRIENDLY`——后者本来就只该管**没有话术可播的真异常**（超时/熔断，
`error` 由 executor 填）。§9.5「话术型拒绝用 OK」那条**仍然成立**，
但它的理由要更新：**FAILED 上的话术不再被吞**，OK 的价值现在是「机器面别撒谎」
（FAILED 是给账本/探针看的「这一步没达成目标」）。

---

### 9.38 取消判定的第三个问法 + 取消闸 + 整句型能力 `whole_utterance`（QA 轮剩余项，2026-08-29）

三件事共一节，因为它们收的是同一批真栈红灯：**一句话说出去，系统做的是另一件事，
还告诉你「已经办好了」。**

#### A. 复合取消的分界是**回指**，不是长度

`pending_cancel._is_compound_remainder`。旧判据是「剥掉取消词后余量 ≥ 6 字 = 复合句」，
于是「取消**导航**」的余量数成 2 字 ⇒ 判成**纯取消** ⇒ 挂起被清掉、回一句
「好的，已为您取消。」，而**导航从头到尾没有被取消过**（长会话 `e15ac1e` turn 77）。
同一条阈值造成的第二例：「不用了，**关掉空调**」余量 4 字，空调也没关。

换判据前先量了误伤面（全部长会话 artifact，48 次 `system.pending_cancel` 命中）：

| 形态 | 次数 | 新旧判据 |
|---|---|---|
| 裸「取消」（余量为空）| 43 | 都判纯取消，不受影响 |
| 「取消**刚才**解锁」（I-046 的原始用例）| 4 | **必须继续判纯取消** |
| 「取消导航」| 1 | 旧判纯取消（bug），新判复合 |

⇒ 分界在**余量是不是一个指着挂起的回指短语**：

1. 余量剥掉虚词与话语副词后为空 ⇒ 纯取消；
2. 余量**带回指标记**（刚才/这条/那个…）**且短**（≤6 字）⇒ 纯取消——它指的就是挂起；
3. 其余 ⇒ 复合：挂起清掉，**整句按新请求继续规划**。

第 2 条的两个条件缺一不可，都是**既有测试当场抓出来的**：
只看回指会把「算了那个不要了，先去帮我看看附近有什么景点」判成纯取消
（那里的「那个」是被取消掉那半截的**残片**）；不看回指则 I-046 当场破。
**长度不是被废掉，是作用域收窄到只剩回指这一支**——它在那里区分的不再是
「有没有实质」，而是「这个回指短语后面还挂没挂一条新指令」。

⚠ **同批还修了一处正则的坑，值得单记**：WEAK 取消词必须**在 STRONG 之后单独剥一遍**，
不能并进同一条 alternation。正则按**最左位置**匹配、不按词表顺序，
「先不用了吧」会先在位置 0 命中 WEAK 的「先不」，把 STRONG 的「不用了」劈成两半。
⇒ **「把词表按语义排好序」在同一条 alternation 里是不生效的，语义优先级要靠分遍表达。**

⚠ 代价明写：挂起本身就是 X、用户又不带回指地说「取消X」时，
新判据会先清掉挂起、再把这句话按新请求规划，用户可能多听到一句「没找到…」。
两侧代价不对称：那一侧是多一句诚实的话，这一侧是**一条指令被吞掉 + 一句假的完成声明**。

#### B. 取消闸：一句取消话不许被规划成「新建」或交给兜底编造

挂在 `PlanBuilder.build()` 的**唯一出口**，紧随两条安全闸之后（§9.35 之外的第三条闸；
三条判的都是「这份计划与用户那句话的方向对不对得上」，共用一个汇合点就不会漏掉某条计划来源）。

**为什么需要它**：2026-08-29 的 A/B **证伪了上一版修法**。给 `reminder.cancel` 补两条范例后，
词面接近的句子转绿（`取消带伞` 0/6→6/6），但**事后补的留出臂「取消交周报」仍是 1/6**
（与同趟对照臂比 p=0.0076），落域散成五处，含 `reminder.create` **反向建提醒**
（「记下了：交周报。」）与 `chitchat.talk` **零动作宣称已取消**（「好嘞，周报提醒已经取消啦」）。
⇒ **范例覆盖得了词面，覆盖不了「这个名字是不是一条提醒」这个 planner 没有的事实。**

本闸走的是三条候选路里代价最小的那条：**接受歧义，但把兜底做对**
——它只要求「**错得诚实**」，不要求「猜得准」。另两条（把提醒列表渲染进 planner 上下文 /
恢复窄 route_hint）各有更大代价，**不能混着做**。

判据三道门，逐条有误伤对照（`test_planning_cancel_gate.py`）：

| 门 | 判据 | 挡住的误伤 |
|---|---|---|
| 1 | 是「取消 X」形态：`pending_cancel.cancel_instruction_object`（**与另两个取消问法共用同一份词表**，不许写第四份）| **分句符**挡住话语承接：「算了，帮我找家咖啡店」「不用了，帮我记一下明天买牛奶」——第二条尤其要命，它就是一条合法的新建 |
| 2 | 不是问句（`runtime/question_shape`）| 「刚才那个取消了吗」该由兜底老实回答 |
| 3 | 只丢**新建**语义的步：`runtime/intent_effect.is_create_intent`（`CREATE_OPERATES` 刻意只收无歧义的那几个操作名，`set`/`open` 不进来）| 「取消静音」→ `volume.unmute` 一个字不改 |

丢完若零步、或只剩兜底 Agent ⇒ `Plan.cancel_unresolved` 记下那个宾语，
engine 出**诚实追问**：「我没找到和「X」对得上的事，你说的是哪一件？」
——**不是**「抱歉，我没听清」。两句话的区别就是本闸的全部意义：
**我们听清了，只是不知道是哪一件事，不该赖用户说不清楚。**

#### C. 整句型能力 `whole_utterance`（proto `Capability` 字段 9）

**这个 capability 消费的是整句原话**，一次把这句话里的事全办完 ⇒
编排保证**同一份计划里最多一步**（`PlanBuilder._collapse_whole_utterance_steps`，
保留第一步）。声明在 Agent、判据在中央，分工同 `heavy` / `require_confirm` / `slot_shapes`
——编排核心零 agent_id/intent 字面量（R2.1 铁律）。

真栈来历（长会话 `e15ac1e` family SL1，累计 1/11 复现）：planner 给
「明天下午四点提醒我参加代号992535的评审会，三点半再提醒我一次」排了**两个**
`reminder.create_batch` 步，而 `_create_batch` 读 `raw_text`、每步都把整组建了一遍
⇒ **4 张 `reminder_card`、4 个不同 id**。代价不止「多几条记录」：4 条一模一样的条目
让按标题取消**当场撞歧义**，这组提醒**既清不掉、又一直占着序数参照系**。
当年那条裁定算的是「多几条记录」，**没算「从此取消不掉」**。

**既有两道防抖为什么都挡不住（逐条核过，不是推测）**：

- `executor._exec_step` 的副作用指纹是 `(intent, 归一化 slots)`，而 planner 给这两步
  **发明了不同的槽名**（9 次专项取样里出现过 7 种）⇒ 指纹不同、不命中。
  而这个 Agent **根本不读槽**。
- `reminder._cross_turn_duplicate` 明写着「一句话被规划成两步是设计内的，
  **同轮不收编**」（§9.35 H）。

⇒ 本机制**不推翻那条裁定**：它管的不是「两步做了同一件事」，
而是「**这个能力按自己的声明就只该有一步**」。**没声明的能力逐字维持原行为**
——「提醒我三点半和四点各一次」真被排成两个 `reminder.create` 时，那两步各建一条是对的。

⚠ 落这个字段时在测试替身上踩了一次同族老坑：`MockAgent` 的 capability 是 `MagicMock`，
`bool(MagicMock())` **恒真** ⇒ 不显式给假值的话，**每个 mock 能力都会被当成整句型**、
同 intent 的第二步被静默收掉。`heavy` 那行注释早就记着同一笔账
（「真 bool，避免 MagicMock 恒真误标」）——**同一个坑在同一个类里踩第二次，
说明那条注释只挡住了写它的人**。

---

### 9.39 降级要降到「上一个正确答案」+ 接送目的地的接地判据 + 可见选择卡的候选（QA 轮收尾，2026-08-30）

三件事共一节：它们都是**系统已经知道正确答案、却把它丢掉了**。

#### A. 反查失败退回同坐标的上一次成功结果（`AmapGeocoder.reverse`）

真栈（长会话 `538335f` `information` T4，专项 `--repeat 3` = **1/3**）：
「**当前位置**当前有1条天气预警：深圳市气象台发布暴雨黄色预警。」
——而同一条会话的前三轮（实况/预报/空气质量）都正确说了「深圳」。

机制**从代码可证**（不是猜）：GPS meta 在场且无显式 `city` 槽时，`_spoken_place`
落到「当前位置」**当且仅当** `_display_city` 拿到空串，而那一支的唯一来源就是
反查抛 `ProviderError`。⇒ **一次瞬时失败，把系统几十秒前刚说对过的地名整个丢掉。**

修法：`reverse` 记住同坐标的上一次**成功**结果（键按 3 位小数 ≈ 110 米，TTL 600s），
**只在真查失败那一支读它**——成功路径一次都不走缓存，每次都真查。
车在动时那个键几秒就变（拿不到旧值、老实重查），停着时才命中。

> 判据：**降级要降到「上一个正确答案」，不是降到「什么都不知道」。**
> 同族先例是 `openhours` 那条「判不出返回 None 不是 0」——
> 两者都在说**别把「这次没拿到」写成「没有」**。

#### B. 接送句里没接地的目的地让位给人称（`navigation`）

真栈（同一条长会话，family）：**同一句话前段对、后段错**——
T9「接女儿放学，路上买杯咖啡。」→ 明远学校；T54 同一句 →
**深圳湾万象城桔子水晶酒店**（T3/T48「去接老婆」的地点）。
干净会话 `--repeat 3` 全对 ⇒ **跨轮污染**，不是解析能力问题。

既有两档都够不着：`_person_destination(dest)` 看到的是一个真地名，
`_person_destination(raw_text)` 对**复合句天然失效**（源码注释里早写着）。
而 `_pickup_person` 这条**局部形态**判据一直都在，此前只在常用地点别名分支里被用。

补的第三档，判据是既有那条的直接应用（**planner 改写是不可信指代通道，只信 raw**）：

| 门 | 条件 | 挡住的误伤 |
|---|---|---|
| 1 | 原话是「接<人>」形态（`_pickup_person`）| 没有接送人称的句子根本走不到 |
| 2 | `dest` **与原话一个字都不沾**（`_grounded_in_raw`，两字连续片段）| 「接孩子后去万象城」的「万象城」在原话里 ⇒ 一个字不动（卡 §4.3 的原始铁律）|
| 3 | **这个人的地点查得到，且与 `dest` 不同** | 查不到就不动——planner 为接送句填一个具体校名是**正常职责**，「不在原话里」只说明它是推断的、不说明它是错的 |
| 4 | `dest` 不是已配置的常用地点别名 | 「配过『学校』就当地址簿用」那条既有裁定不受影响 |

> ⚠ **门 3 和门 4 都是既有测试当场按住我加的**（第一版只有门 1、门 2，撞红三条反向对照）。
> **误伤面不是想出来的，是被既有用例逼出来的。**

#### D. 「再给我看一眼刚才那份可选项」不是槽位答案

`_is_topic_change` 消费 `candidate_query.RELIST_RE`（**同 C3-B 把 `NEW_SEARCH_RE`
收敛成唯一来源那一笔——判据只许有一份**）。

真栈（release `538335f`，**2/2 复现**）：充电规划出了 `dest_choice` 选择卡之后，
「请重新列出刚才可以选择的项目」被**整句填进 `destination` 槽**，答成
「暂时无法获取前往**请重新列出刚才可以选择的项目**的路线」。
与 §9.35 A 那个 `index` 黑洞**同族，只是换了个槽**。

⇒ 这一族没有终点，所以判据不是补词表，而是一条**定义**：
**「这句话是在问候选集」，那它就不是某个待补槽的值。**

⚠ **只收「重列」，一个序数都不许沾**：裸「第一个」正是选择卡的**合法答案**
（`*_choice` 白名单就是为它写的），收进来会把整条选店/选目的地流程修死。

#### C. 用户看得见的选择卡，它的候选也要进候选集（I-024，Q10 残余）

`extract_focus` 原先只扫**成功步**（`if step.id not in ok: continue`），
而商户选店卡是 `NEED_SLOT` 的产物（「要在哪家下？」）⇒ **门店候选集根本不存在**，
下一句「第一个」无处可解。

放宽面窄到只剩一种形态：**NEED_SLOT ∧ 它的卡是选择卡**，
判据**逐字复用** `engine._suspend` 那条（`purpose` 以 `_choice` 结尾或
`type == "merchant_choices"`）——那里认定「这张卡用户看得见、序号是它的合法答案」，
这里问的是同一个问题，不许写第二份。且**只取候选集这一维**：
目的地/城市/控制焦点都是「这一步做成了什么」，它没做成。

> ⚠ 为什么不能收全部 NEED_SLOT：C10-A 的铁律——**「第N条」只许指向用户最后一眼
> 看到的那份列表**。没渲染成选择卡的步，它的 items 用户一眼没见过，
> 收进来就是给序数指代埋雷。**放宽的是可见性，不是成功与否。**

⚠ **第二层：挂起轮从来不写焦点**（同批发现，真栈实测才露出来）。
三个 `_suspend` 调用点都是 `yield await self._suspend(...)` 紧跟 `return`，
而 `update_focus` 在它们**之后** ⇒ 抽取改对了，那份候选**仍然到不了存储**
（部署后重列仍答「没有您刚才那页选项的记录」）。
`_suspend` 因此在保存挂起态之前补一次焦点写入。当时判据同上（挂起步自己出的是
选择卡才写，普通确认/补槽挂起一概不写）。
**2026-09-25 改（评审四轮，`4438ea6b` 真栈 RS39）**：「普通挂起不写」被真栈证否——C5-B 让兄弟步的动作随挂起
final 发出去，那一轮的事实却一件没登记（已发出的导航没进路线会话，下一句「取消导航」答「当前没有正在进行的导航」；
已执行的控制目标、原话里的告警与约束同样丢）。现在**每个挂起轮都用同一份抽取登记本轮已执行的事实**（挂起步本身不是
OK，抽取不会把它当成做完了）；C10-A 由 `extract_focus(candidates_from=)` 守住：挂起轮的候选集**只收挂起步自己**
（它是选择卡时就是原来那条），兄弟步的列表用户没以卡片看见过，不收。选择卡谓词只剩 `context` 里那一份。

> **这是同一形态的第三例**：安全告警登记在 `clarify`/`no_plan` 的 `return` 之后
> （余项 ①）／`_refresh_active` 刷成了用户没看见的那份（C10-A）／这一条。
> 判据：**改完抽取要问一句「产生它的那条路径，走到写入之前会不会先返回」。**
> 三次都只有真栈能露出来——单测里抽取函数自己是对的。

⚠ **第三层：候选要从卡里取，不是从 `data` 里取**（同批，又是真栈才露出来）。
成功步的候选走 `data["items"]`（那是产生方给下游用的结构化结果），
而**选择卡的产生方把候选放在 `ui_card["items"]`**——
`luckin._store_choices` 的 `data` 只有 `checkout_token`，charging 那条 `data` 是空的。
于是前两层都改对了、真栈**第三次**仍答「没有您刚才那页选项的记录」。
**而这一支本来就该读卡**：它的主张是「用户看得见的那份列表」，卡片正是他看见的东西。

> ⚠ 配套一条测试纪律：首版 fixture 把候选塞进 `data`，**测试绿而真栈红**
> ——**装置不真实，读数就会指向错误的根因**（同 `_obs` 那次少喂 `actions`）。
> fixture 改成产生方的真实形状之后才守得住。
...
连接词补「一（后接 有 / 到 / 出 / 发 / 下 / 开 / 来 / 变 / 超 / 低 / 降 / 升 / 堵 / 亮 / 响 / 停 / 涨 / 跌 / 满 / 起 / 过）」
（「一有暴雨预警就告诉我」）。**连接词形态先于「…就通知我」兜底在整句上求值、剥前缀按分句**——合在一条正则里最左匹配会让兜底抢先
（真栈 RS13：「先看看路况，要是堵的话，之后只要有堵车就提醒我」念成「之后只要有堵车」）。修前 holdout 念成「盯着『往后一有暴雨预警』」。
