...
| 尚未做 | **长会话探针（`probe_qa_long_sessions.py`）在新 release 上仍未跑**——QA 那 17 张症状卡里靠长会话才复现的那些（城市漂移 / 候选重列 / 偏好反着推荐 / 时限 / 接人复合句…）**一条都没验**。**不许把「迷你集 58/61」读成「QA 全绿」。** 那一趟同时也是「修正后计分」真正的对照对象。 |
