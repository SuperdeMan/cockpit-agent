...
### 11.8 真机反馈第三批（2026-07-26，「两个人的声音还是认成同一个」）

现象：录了泓舟与阿灵两个乘员，换人说话助手仍答同一个名字。**不是模板问题**——两个模板
互相余弦只有 0.27（分得很开），自洽度 0.834/0.822 都健康。三层原因叠在一起：

1. **探针里大半不是人声（本批主因）。** 喂给识别器的 `vad.onFrame` 是**原始帧旁路、不做
   语音门控**（它同时供 pcmRing 与 PCM 直传），而识别器按「累计 1.5s 有效语音」的语义
   直接数帧。真机时序是「唤醒 → 播提示音『在呢』→ 用户才开口」，那 1.5 秒里大半是提示音
   与静音，嵌入被稀释到谁都认不出 → **一律回 primary** → 现象就是「换个人还是同一个」。
   修：识别器只收 VAD 语音段的帧（controller 转发 `onSpeechStart/End`，`arm` 带上「此刻
   是否已在说」）；并在 VAD 端点**补发一次**——短问句「你知道我是谁」约 1.2s，按 1.5s
   门槛永远攒不够，那是从「认错」退化成「永不识别」，症状一模一样。
   > **「有效语音」这类词写进契约就要在实现里逐字兑现。** 契约 §9.11 原文就写着「累计
   > 1.5s 有效语音」，实现拿到的却是不含门控的原始帧——**同一个词在两层里指的不是一件事，
   > 而类型系统看不出来**（两边都是 `Float32Array`）。
   > 另一条：**旁路点要问清它旁路的是什么**。同一份帧同时喂三个消费方，两个要「所有帧」
   > （前滚缓冲、PCM 直传），第三个要「语音帧」——共用一个出口就必然有人吃错。

2. **阈值是拿合成音色标的，方向标反了。** 修完门控后真人实测：同人 **0.5243** /
   异人 **0.1157**，而阈值 0.62 把同人一并卡掉。合成音色的异人余弦高达 0.65（TTS 音色
   共享信道特征），逼得首版阈值往上抬；真人真麦的异人分离度好得多，阈值反而该往下放。
   threshold **0.62 → 0.45**（margin 不动——一个数据点上不同时拧两个旋钮），真人实测值
   钉成回归测试 `test_real_mic_measurement_is_accepted`。
   > **用代理数据标定的常量，必须在真实分布上复标一次**——代理数据不只是「不够准」，
   > 它可能把**方向**标反：合成音色让人以为该收紧，真实数据要求放宽。

3. **判定在生产里完全不可见。** identify 端点没有日志；RFC 承诺的「四态全进 obs 供 M1b
   挖掘」实际没落地——collector 的 `apply_metric` 是**固定键白名单**，`vp_decision` /
   `vp_score` / `vp_runner_up` / `vp_occupant` 全被丢掉。于是「认不出」是一次彻底静默的
   降级，只能靠现象猜。补：网关每次 identify 打一行（occupant/decision/score/runner_up/
   probe_ms/src），memory 侧再打一行**全量排名**——降级回 primary 时 top1 的身份本会丢掉，
   而那正是调阈值最需要的一个数。obs 白名单的洞已立卡（见 §11.5 余项）。
   > **静默降级必须留痕，且要留「差一点的是谁」而不只是「结果是什么」。**

4. **认不出就不叫名字（泓舟拍板）。** 后端降级回 primary 时照样回 primary 的
   `display_name`，HMI 拿它当「你」的称呼下发 → 助手对着没认出来的人一口咬定「你是泓舟」。
   `occupant_id` 照旧回落 primary（记忆归属逐字回落=对的），但**称呼是一句断言**，
   只有 `accept` 才下发。一处收口在 `voiceprintIdentifier`，classic 与 S2S 共用。

**多人测试的正确姿势**（本批排查时才说清）：换人必须**重新唤醒**。同一唤醒窗内的续问与
打断走 `arm(false)` 不重识（「一次唤醒锁一次」是刻意的：轮内改判会让同一段对话前后半截
落进不同乘员，比认错更糟）。本批三条 trace 里泓舟那条是 `voice_bargein`——它压根没有触发
识别，助手答「不确定你的称呼」正是识别从未落地的表现，与认错人是两码事。

#### 11.8.1 收口：真正的根因是**注册与识别不同信道**（2026-07-27）

上面四条修完，真机复测仍然认不出；但泓舟发现**设置页的「试一试」能分得开**。这个差分把
问题一刀切开——两条路唯一的区别就是音频通路：

| 路径 | 采集 | 编码 | 同人余弦（真机实测） |
|---|---|---|---|
| 注册 / 「试一试」（旧） | `MicController`，`{audio:true}` | MediaRecorder **webm/opus 有损** | **0.7272 / 0.7393** |
| 主链路识别 | VAD 管道，EC/NS/AGC 全开 | 原始 16k s16le PCM | **0.4762 / 0.5301** |

**同一批人、同一批模板，系统性差 0.2**，而阈值就在这中间——于是 webm 路认得出、PCM 路
认不出，且 PCM 路的探针会塌向别人的模板（两个人都被判成 occ-2）。**声纹嵌入吃的是信道
特征**：模板用 opus 解码后的音频建，探针拿原始 PCM 去比，两者根本不在同一个空间里。

修：新增 `hmi/src/pcmRecorder.mjs`（AudioContext 16k + 与 `vadEngine` 同一个 capture worklet +
同一组 EC/NS/AGC 约束），**注册、「试一试」、主链路三条路统一到一条信道**；`enroll`/`identify`
默认格式改 `pcm16le`；契约测试源码级钉死（约束逐字一致、设置页不得再用 MediaRecorder、
两个端点默认必须是 pcm16le）。配套补两件：**行内「重录」按钮**（带原 `occupant_id`，
否则重录会分到新身份、记忆分家）与**录音头尾静音裁剪 + 「只听到 X 秒人声」反馈**。

> **可复用的判断（三条）**
> 1. **「同一段音频」在两条路上不是同一件事。** 类型系统看不出 webm 和 PCM 的区别（都是
>    「音频」），而对信道敏感的模型会因此系统性失准。**凡是要比对的两端，必须同源采集。**
> 2. **自证要走被证的那条路。** 「试一试」走了更干净的通路，于是在主链路已经认错人的
>    情况下照样显示「听出来了」——**唯一的自证手段成了假证人**。这次也正是它先报的平安，
>    把排查往错误方向带了一轮。
> 3. **差分是最省力的定位器。** 泓舟那句「但试一试能分得开」比任何日志都值钱：
>    两条路只差一个变量时，那个变量就是答案。**遇到「A 行 B 不行」，先列两者的差异清单。**

> **验收诚实交代**：本批想用 Chrome 假麦克风（`--use-file-for-fake-audio-capture`）做浏览器
> 内闭环验收，**装置本身不保真**——同一段 TTS 文件，直取三段自洽度 0.83/0.73/0.81、
> 异音色 0.29（链路与模型健康），但经假麦克风采进来后 ASR 只能转出零星几个字、与源文件
> 余弦仅 -0.03（换 48k/降 12dB/关 EC-NS-AGC 都不救）。故**它只用于验接线**（已验：
> 「试一试」确实发出 `format=pcm16le`），**音质与识别率仍须真麦复验**。
> 教训同 §11.7：**测试装置也要先被验证**，否则它会以产品缺陷的面目出现。

#### 11.8.2 同信道之后：识别对了，答案还是错的（2026-07-27 真麦复验）

真麦重录后复验，泓舟报「试一试无法区分，HMI 也都认成阿灵」。日志把两件事分开了：

**① 「试一试」压根没跑识别。** 四次调用**一行判定日志都没有**——它在打日志之前就 return 了
（`too_short` 的早返回路径）。根因是我上一步引入的：「试一试」只录 2 秒，切掉头尾静音后
不足网关要求的 **1.5 秒有效语音**，于是恒回 `too_short`。**自证功能变成了永远说「太短了」。**
修：录音窗与注册同长（4 秒）+ 文案改「连着说一句话（约 4 秒）」。
> **改了「有效时长」的口径，就要回头看所有依赖时长的地方**——我只想到识别端的门控，
> 忘了自证端录得更短。同一个常量在两处有不同的裕度时，收紧其中一处必然压垮另一处。

**② 识别是对的，答案是错的。** 同一次测试里主链路日志：
`accept -> occ-2 (0.4745 vs 0.1229)` 与 `accept -> primary (0.6665 vs 0.2030)`——两个人分开了；
但助手对着已认出的泓舟答「**阿灵啊**」。受控复现（新会话逐一注入）：

```
新会话 + 注入泓舟 → 「你是泓舟呀」                       ✓
新会话 + 注入阿灵 → 「你是阿灵呀」                       ✓
会话里先聊过阿灵 + 注入泓舟 → 「你是阿灵呀，刚才不是说了嘛～」  ✗
```

**注入是好的，被对话历史盖过去了。** 车里只有一个会话而说话人会换，`ctx.history(4)` 里
上一位乘员的称呼比 system 提示更近、更像既成事实。**先试了加强提示词**（显式写「上文的
称呼指的是别人，一律以 X 为准」）——**两个方向各两次全错，无效**。
改为确定性直答（`_identity_answer`，同 `_clock_answer` 一族），复验两次两方向全对。
> **系统自己持有的事实不要交给 LLM 回答**——这是本仓库第三次撞上同一条（墙钟、
> 声纹身份、以及更早的日期锚）。判据很简单：**这个答案系统已经知道了吗？知道就别问模型。**
> 推论：**prompt 打不过上下文**。当 system 提示与对话历史冲突时，历史通常赢——
> 靠加强措辞去压历史是在跟采样赌，不是工程。

#### 11.8.3 乘员维度全面盘点（2026-07-27，泓舟确认识别通过后）

识别通了之后按「每个角色的其他业务逻辑是否也通」逐条实测（用两个临时乘员 occ-8/occ-9，
不碰真实数据；跑完按产品自身的删除口径清理）。

**已验证通的**

| 面 | 证据 |
|---|---|
| 记忆写入归属 | occ-8 说「我最喜欢喝乌龙茶」→ 落 `occ-8/taste.tea`，不落 primary |
| 记忆读隔离 | occ-8 问「我喜欢喝什么」答乌龙茶；occ-9 问同一句答「我不确定」——**跨乘员读不到** |
| 召回层过滤 | 直调 memory：`recall(occupant='primary')` 只返 primary 行、`occ-2` 只返 occ-2 行 |
| 关系边隔离 | 「我女儿叫小满」→ `memory_relation(occ-8, 小满, family, 女儿)`，occ-9 查不到 |
| **权限红线** | occ-8 说「打开后备箱」→ 照常 `need_confirm`——**声纹既不提权也不降权**，与 primary 逐字一致 |
| routine/主动 | `derive_routines(user_id, occupant_id)` 本就按乘员分 |

**没通的**（已立卡进 AGENTS.md 余项表）

1. **显式记忆指令间歇被拒识**——最伤的一条，且**此前的归因是错的**（见余项表）。
2. **常去地点不分乘员**：`get_context` 调 `get_places(user_id)` 不传 occupant 恒取 primary，
   写侧 `save_profile` 是共享 KV。**RFC 宣传的「常去地点各自独立」没有兑现。**
3. **提醒无 occupant 维度**：`agents/reminder` 全域零 occupant。
4. **HMI 记忆面板/「忘掉全部记忆」不分乘员**：删单个乘员的记忆没有入口。
5. **端侧快路径 `AppendTurn` 不带 user_id/occupant_id**：端侧轮次归 primary 默认值且不进抽取。

> **可复用的判断：宣传语要逐条当验收项跑。** 「口味、习惯、常去地点各自独立」里，前两个
> 兑现了、第三个没有——而三者在文档里是并列写的。**并列的承诺往往由不同的代码路径兑现，
> 隔离做在了 `memory_item` 上，而 places 走的是另一条（profile KV + GetContext），
> 它压根没有 occupant 字段。** 验收要按「每条承诺各自的实现路径」拆，不能按句子拆。

#### 11.8.4 拒识误伤显式记忆指令（2026-07-27 修）

盘点抓到的最伤一条，泓舟拍板当场修。现象：「记住，我女儿叫小满」某些轮次**整轮零回话、
什么都不记**；查 trace 是 `status=rejected`——R4.4 受话判定把它判成了乘客之间的闲聊
（`addressed=false`），按设计静默短路且**不落库不进画像**。**它是间歇的**：同一句某次
3/3 被拒、换一批 6 条又只拒 1 条——是 LLM 判定的方差，不是判据写错了。

**顺带修正了一条错误归因**：AGENTS.md 原来记的是「planner 出空计划、话术降级，**而记忆
其实已经存进去了**」——不对。机制是拒识，后果比原描述严重得多：说了、没回、也没记。
> **归因写错比没写更糟**：它会让下一个人去查空计划那条路，而真正的机制在另一条链上。
> 立卡时**必须带上判据**（这次是 `status=rejected` 这一个字段），否则复查无从下手。

修法：句首祈使前缀（`记住/记一下/记下/记着/记得/别忘了/帮我记…`，去礼貌前缀后锚 `^`）
**确定性判为受话，不问模型**；置为「本次没解析出计划」走既有的重试→fallback 机制——
**刻意不直接改判成 chitchat**，重试有机会拿到正确的计划（「记住，明天八点提醒我开会」
该进提醒域而不是闲聊），只有两次都判不受话才落兜底。代价仅在误判时多一次规划调用。
真栈复验：8 条祈使记忆指令 **0 空回**。
> 同族于 §11.8.2 的「系统持有的事实不交给 LLM」：**用户用祈使句直接对助手下指令，
> 「是不是在跟你说话」这件事不需要模型判断。** 判据是句法的，不是语义的——
> 能用确定性规则判的，就别放进模型的方差里。

**③ 顺带发现的脏数据（非代码缺陷）**：识别错乱那段时间写下的记忆已经互相串了——
`primary` 名下有「用户自称阿灵」，`occ-2` 名下有「用户称呼自己为泓舟」。记忆召回的
**occupant 隔离本身是好的**（实测按 occupant 精确过滤），但脏数据会在「认不出人」时被捞
出来喂给模型。**识别缺陷会在记忆里留下持久痕迹，修好识别不会自动修好数据**——
需要人工清理，且清理属删除用户数据，必须用户确认。
