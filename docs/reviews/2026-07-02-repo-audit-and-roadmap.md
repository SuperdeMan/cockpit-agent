...
| **R4.3 语音交互前端：唤醒+VAD+全双工-lite** | WS7 | L | 🚧 **P1 大脑落地并 push**（`feat/r4.3-p1-voice-loop`）：`voiceLoop.mjs` FSM 20 例 node 全绿 + Settings 四键默认全关 + Orb armed/listening + 双源模型下载脚本（VAD 下载+pin sha256、GitHub 主源两模型 200）。`vadWorker`/`kwsWorker` 真 sherpa-onnx 绑定 + 真麦接线 + e2e + 人工验收单 **gated 于 P0+真麦**。见 `docs/design/2026-07-04-r4.3-wake-vad-fullduplex.md`；取代 R4.2 §3.4 v2 |
