...
// ─── G-07 systemInterrupt（2026-09-21）：来电 / 闹钟 / 抢焦点在任何非 IDLE 态都回 ARMED，收音不定稿 ───
test('G-07 systemInterrupt：LISTENING 中断 → 关 ASR、不发、回 ARMED；记 metric', () => {
  const h = makeHarness()
  h.vl.handsFreeOn()
  h.vl.wake()
  h.vl.vadSpeechStart()
  h.vl.asrPartial('导航到')
  assert.equal(h.vl.state, VoiceState.LISTENING)
  const closes = h.count('closeAsr')
  h.vl.systemInterrupt()
  assert.equal(h.vl.state, VoiceState.ARMED)
  assert.equal(h.count('closeAsr'), closes + 1, '收音被关掉')
  assert.equal(h.count('send'), 0, '半句不上云')
  assert.equal(h.count('endpoint'), 0, '也不请求定稿')
  assert.deepEqual(h.last('metric'), ['metric', 'system_interrupt_capture'])
  // 之后的端点 / 定稿是旧一轮的迟到回调，ARMED 下必须被忽略
  h.vl.vadSpeechEnd()
  h.vl.asrFinal('导航到机场')
  assert.equal(h.vl.state, VoiceState.ARMED)
  assert.equal(h.count('send'), 0)
...
test('G-07 systemInterrupt：FOLLOWUP 窗被中断 → 立即 ARMED，8s 定时器清掉（之后不再有二次迁移）', () => {
  const h = makeHarness()
  driveToSpeaking(h)
  h.vl.ttsEnd()
  assert.equal(h.vl.state, VoiceState.FOLLOWUP)
  const states = h.count('state')
  h.vl.systemInterrupt()
  assert.equal(h.vl.state, VoiceState.ARMED)
  assert.equal(h.count('state'), states + 1)
  h.advance(10000)
  assert.equal(h.count('state'), states + 1, 'followup 定时器已清，不会再迁一次')
  assert.equal(h.vl.state, VoiceState.ARMED)
})

test('G-07 systemInterrupt：SPEAKING / THINKING 同 stopSpeaking（停播回 ARMED，不进 FOLLOWUP）；ARMED / IDLE 空操作', () => {
  const h = makeHarness()
  driveToSpeaking(h)
  h.vl.systemInterrupt()
  assert.equal(h.vl.state, VoiceState.ARMED)
  assert.equal(h.events.filter((e) => e[0] === 'metric' && e[1] === 'system_interrupt_capture').length, 0, '播报态不算收音被抢')

  const h2 = makeHarness()
  h2.vl.handsFreeOn()
  h2.vl.wake()
  h2.vl.vadSpeechStart()
  h2.vl.vadSpeechEnd()
  h2.vl.asrFinal('放首歌')
  assert.equal(h2.vl.state, VoiceState.THINKING)
  h2.vl.systemInterrupt()
  assert.equal(h2.vl.state, VoiceState.ARMED)

  const h3 = makeHarness()
  h3.vl.systemInterrupt()
  assert.equal(h3.vl.state, VoiceState.IDLE)
  h3.vl.handsFreeOn()
  const before = h3.count('state')
  h3.vl.systemInterrupt()
  assert.equal(h3.vl.state, VoiceState.ARMED)
  assert.equal(h3.count('state'), before, 'ARMED 下不重进')
})

test('G-07 systemInterrupt：不复位会话级插话护栏（与 stopSpeaking 同一分界）', () => {
  const h = makeHarness()
  driveToSpeaking(h, { tts: '正在为您导航到首都机场' })
  h.vl.vadSpeechStart()
  h.vl.asrPartial('正在为您导航')
