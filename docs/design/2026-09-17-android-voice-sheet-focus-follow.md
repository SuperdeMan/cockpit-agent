...
### 9.6 第三轮真机：三态复验，顺带抓到升层动画本身的问题（包 `eeae0d7e0`，14:40 出包 / 15:12 装机）

用纯 Python 解 PNG 按列扫层顶缘（`sheet_top.py`，无 PIL），逐帧量层高（记录区底：无承诺面 1716 / 有 chips 行 1562 / 有承诺面 1441）：

| 段 | 读数 | 结论 |
|---|---|---|
| D2 深链重开（0.4 档）静置 dump | `voice-sheet` [0,842][988,1716] = **874px = 318dp**；头区 286px = 104dp | 0.4 档下限逐 dp ✓ |
| D2 头区下拉 500px | 层收起，记录完整 | ✓ |
| D1 / E1 / F2 按住光球连拍 | 第一可见帧（升层后 ~0.5–0.9s）层高 **365 / 413 / 410dp**，之后 1s 内落到 **317–318** 并稳住；三次都是「先高后回落」 | 不是三态判据的问题（见下） |
| F 段在场轨迹（先造路线卡 ⇒ 闲态 0.78） | 按下那一条：`轴 capture,primary,input,sheetDetent,capsule 输入 ptt`——**升层与 0.78→0.4 同帧** | 判据即刻生效 |
| jest 真宿主树诊断（临时用例，未提交） | 上一轮带卡 → 收音：VoiceSheet 传给 `sheetHeightDp` 的序列 `[{detent:0.4, terse:false, ret:318}, {同}]` | JS 侧第一帧就是 318 |
| G 段深链升层连拍（不收音，0.62 档 348） | +0.7s **229dp** → +1.7s **357dp**（过冲）→ +2.5s **348** 落定；收起再开一遍同形 | **升层动画本身要 2.5s 才落定** |

定性：`withSpring` 动的是 `height`——每帧重排整个层（模糊壳 + 光球），这台机器只跑到几帧 / 秒，reanimated 弹簧又把帧间隔封顶，
弹簧按几分之一的实时速度播，落定要 2.5s，过程中球随层漂；收音那三次「先高后回落」是同一条弹簧在不同起点上的中途帧。这不是本批引入的
（09-11 之前就是这条弹簧），但本批把球固定在头区之后它变得显眼。修法（`990466d6`）：升起 = 高度一步到位 + 整层从记录区底缘 `translateY`
滑入（transform 不触发重排，220ms ease-out）；开着时档位 / 容器变化才动高度（200ms timing、不过冲）；收起 = 滑回底缘 180ms 再卸载；
层根 `overflow: hidden` 让滑入不画到 Composer 上。下滑收起的 `dragY` 路径不变。

真机复验见 §9.7。

### 9.7 第四轮真机：动画改法复验（包 `990466d6d`，16:12 出包 / 17:31 装机，APK SHA-256 `151ecfa0…171b` 端本一致）

⚠ 出包过程：构建本身 16:12 已 `BUILD SUCCESSFUL`（14m53s），之后空闲的包装进程在 17:2x 被 Claude Code 因系统内存不足停掉（只剩 1.5GB）；
产物完整，未重启构建；停掉本会话留下的空闲 Gradle daemon 后再装机。证据同目录 `*-990466d6d.*`、`probe7.log`。

| 段 | 读数（`sheet_top.py` 逐帧量层顶；记录区底 1716） | 结论 |
|---|---|---|
| H1 深链升层连拍（当前轮有回答 ⇒ 0.62 档 348） | +0.1–0.3s 顶缘 991（滑入中途）→ +1.0s 起 **761** 并四帧不动；静置 dump `voice-sheet` [0,760][988,1716] = **956px = 347.6dp**，头区 286px | 一步到位、无过冲、无漂移（G 段旧动画是 229 → 357 → 348 用了 2.5s） |
| H2 头区下拉 500px | 层收起 | ✓ |
| H3 按住光球连拍（收音初始 → 环境 partial） | 第一可见帧起 **843 = 873px = 317.5dp**，四帧 + 松手后一帧全部 843 | 收音初始就是 0.4 档下限，且草稿出现不再改变层高（D1 / E1 / F2 的 365 / 413 / 410 中途帧消失） |

至此三条原始问题 + 真机轮抓到的三处（收音初始上一轮内容、顶缘渐隐、升层动画）都在 OPPO 上闭合。未 push；Xiaomi 对照机与折叠姿态本批未取证。
