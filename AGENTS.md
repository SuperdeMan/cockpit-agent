...
**仍未做**：G10 订座票务（搁置，诚实桩）、复合句人称接送解析（§4.1 立卡）、
**探索式真实用户 QA 轮的 58 个问题**（2026-08-15 晚，独立于上述四批的一次黑盒/灰盒探索测试
——533 turns × 5 persona × 两档；已收敛成 12 个根因立卡 Q1–Q12 并排四阶段，§4.1 ①）。
⚠ 这一轮**验的是别的东西**：EVA 四批验的是「能力面有没有那个维度」，QA 轮验的是
...
**① 探索式真实用户 QA 轮的 58 个问题**（2026-08-15，报告
[`…exploratory-real-user-qa-deepseek-minimax.md`](docs/reviews/2026-08-15-exploratory-real-user-qa-deepseek-minimax.md)；
533 turns × 5 persona × 两档，P0×3 / P1×44 / P2×10；30 个问题族两档同时复现 ⇒ 按「两档是否
同时错」判据全是系统缺口）。**已收敛成 12 个根因、立卡 Q1–Q12、排四阶段**，方案与取证在
[`…qa-exploratory-root-cause-cards.md`](docs/design/2026-08-15-qa-exploratory-root-cause-cards.md)。
接手先读那份的 §1（**四处与报告定性不同的重判**）与 §4（阶段计划），**别按报告的 T0/T1/T2
直接开工**——报告按症状分档，会把同一个根因拆到三个档里。四条重判的要点：
I-044/I-028「幻觉」实为**真实存量记忆**（`person.child`/`taste.dislike_place` 逐字在库，
真病是关系图谱写入零约束——同一个孩子三个学校、四条 `X --family--> X` 自环、`superseded_by`
从未写过）；I-036 胎压是**系统忠实转述了一份 mock 手册**（`manual_rag/src/providers/mock.py:6`
逐字写着 2.4–2.5 bar，而 manual-rag/road-safety/chitchat 三个 agent `_prov` 覆盖为 **0**）；
I-057 可观测**没丢**（`llm-gateway/server.py:270` 已按实际 serving 厂商落 obs），降 P3；
I-021「预览即下单」**代码上不成立**（`create-order` 唯一调用点在 confirm 分支
`mcdonalds.py:344`），**首偏离诊断出结论前不许碰 drafts/mcdonalds 任何一行**。
**阶段 0 进度（2026-08-15）**：✅ 0.1 完成——**I-021 定性推翻**：那笔订单属 session
`demo-7acjk0`、创建于 **08-12 14:01**，而 QA 八个 session 全在 **08-15 15:40–17:42**，
`mcp-bridge` 日志覆盖窗内**零 create-order**。三重证据 ⇒ **QA 轮没有创建过任何真实订单**，
I-021 = I-026（查单不绑会话），**整项移出 T0**。
✅ 0.2 完成——迷你集 `scripts/probe_qa_regression.py`（31 例 / 52 轮 / 6 组 + `--mapping`）
已跑出基线，并**牵出四条报告里没有的新发现**：① 安全对话第三轮「慢一点开可以吗」被端侧
执行成 `volume.dec`（恶性）；② I-011 真根因是**泛化兜底把失败搜索变成"成功"搜索**、
合法覆盖了用户点名的候选，修法与报告写的完全不同；③ **新卡 Q13**——`classify()` 与
`_to_legacy_name()` 两个分类出口 **15/38 处不一致、7 处 `is_local` 相反**，媒体域单句
一直在绕云（`fast_intent.py:1788` 那条「发现根因时要问同一形态还有几处」的判据 2026-08-04
写下后只修了一处，今天的答案是 15 处）；④「胎压补到多少」被端侧「暂不支持哦」拦截，
I-036 的方差有一半来自端云分流不是模型。
✅ 0.3 完成——四条 QA 方法论写进 `test/README.md` §5.3.1。
✅ 0.4 完成——`scripts/qa_data_hygiene.py` dry-run **10 条命中**（自环 4 / 主宾颠倒 1 /
单值谓词冲突 2 / `fire_at<=0` 仍 pending 3）。**④ 那三条闭合了 I-056 的因果链**：
「妈妈住杭州」「停车位在B2」两条**根本不是提醒、是陈述句被 `reminder.create` 吃掉**，
`fire_at=0` 让它们永远 pending 且按升序**永远置顶**——报告读成「其他会话提醒泄漏」的
那三条逐字就是它们。⚠ dry-run **当场劝退一条判据**：③ 首版把 `爸妈--family-->爸爸/妈妈`
判成冲突并准备失效掉「妈妈」（丢掉一个真人）——**「同一 subject 多个 object」不是冲突，
除非那个谓词本身是单值的**。**阶段 0 出口条件已满足。**
⚠ **两条方法论已回写卡 §3.5，直接决定后续各阶段怎么验收**：话术层断言只能用**形态判据**
（首跑 5 条假绿都是关键词排除被措辞漂移绕过）；**单次取样不能当基线**（SF4 单次 PASS、
`--repeat 3` 实测 0/3，差一点把稳定红降级成方差面）。
⚠ 阶段 0.4 的存量清洗**泓舟 2026-08-15 已授权**（关系自环/主宾颠倒/画像被写成酒店/
`fire_at=0` 提醒），但顺序不变：**先修入口（Q5/Q11）再 `--apply`**——§40 那次复位已经
证明只清不修会再脏。同批拍板 **Q1 走 A+B+C 三条**（取消判据收敛唯一实现 → 确认帧带
`operation_id` → 单槽改小容量挂起表 + HMI 多条确认条；顺序不可换，B 是 C 的寻址键）。
