...
- 路由规则（当前实现）：
  - 完全本地且无需确认的语义组 → **本地执行**，秒回聚合话术；
  - 导航、查询或需确认的主意图 → 对应语义组完整上云；
  - 无法独立分类的续接片段附着到前一个主意图，例如“走最快路线”跟随导航，
    “周杰伦的”跟随播放请求；
  - 同一句可同时本地执行车控组并把慢意图组上云，但同一语义组不可端云拆开。
...
| 阶段 | 多意图 | 上下文 |
|---|---|---|
| **P1** | ✅ 云侧 Planner 多意图拆分强化 + 黄金用例 | ✅ 短期会话记忆、规划历史注入、LLM 指代消解 |
| **P2** | ✅ 端侧切分、语义组本地/云端混合路由 | ✅ 本地轮写共享记忆；显式 slot carryover 仍可继续增强 |
...
- **端云双拆分冲突**：允许不同语义组分别本地/上云，但一个主意图及其续接限定必须
  原子路由，不能把“导航目的地/路线偏好”或“播放动作/歌手限定”拆到两端。
...
**当前边界**：端侧快意图已 best-effort 写共享记忆，记忆服务不可用时仍保持离线
快路径，不阻塞车控。显式 slot carryover 与跨会话个性化仍可继续增强。
...
- [x] **路由纪律**：按语义组原子路由；完全本地组经 VAL 执行，导航/查询/需确认组
  完整上云。续接限定必须跟随主意图，不能拆到两端。
