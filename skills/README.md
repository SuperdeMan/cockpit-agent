...
**“目标槽无真实值”的两条细则**（都来自真栈实测，别当边角料）：

1. **非空 token 不等于引用。** 真栈出现过 `slot_refs={"item":"item"}`——只有指向已声明
   producer 的 `<producer_id>.data.*` 才算有效引用，其余是模型的格式噪声。
2. **触发词被原样抄进槽位，不算用户给的真值**（2026-08-10）。`trigger_any` 列的正是
   「指向一件还不知道名字的东西」的说法（`招牌` / `最便宜` / `第一个`），它们只有等
   producer 回来才有具体值。实测：MiniMax 把 `item="招牌"` 填进 slots、`slot_refs` 留空，
   归一被“已有真值”挡住，字面量一路发到商户侧。
   > **判据：声明已经说了这个 token 意味着「值在别处」，就不能反过来拿它当值。**
   只认**全等**——`招牌牛肉面` 是用户点名的具体商品，仍不许被改写成 `items.0.name`；
   放宽成子串匹配就是替用户改单。归一时会一并清掉该占位符，免得执行期两个来源争同一个槽。
