...
冒烟通过）。当前全量单测 846 passed, 6 skipped（2026-06-25）；compose 含 info-agent。
...
记忆系统后续：把定稿并入架构 §7、自动抽取确定性兜底、全栈跨轮回放纳入 nightly。）
