...
| `SCENE_ACTIVE`（`scene_active`） | scene-orchestrator `_dispatch`（激活写）/ `_deactivate`（退出清） | scene-orchestrator `_deactivate`（恢复基准）；P2 Verify 对账 | `{scene_id,scene_name,activated_at,activation_id,snapshot{},solved_actions[],deferred[]}` | 会话内；被覆盖 |
