...
- **AR04 当前交接**：[实施记录第十五节](../docs/design/2026-09-08-ar04-presentation-ack-implementation.md)（支持页形态修正）+ 第十四节（发布与真锁屏）。生产仍 `573ad46`；Android 代码 `1c67807`——2026-09-09 用户判定设置 / 车辆 / 地图页底部常驻两栏破坏页面设计，已改为浮动在场：闲置只剩右下角光球（静帧），有事才长出状态胶囊 / 停播·打断键 / 采集点，承诺面与提醒出口只在有内容时占布局空间；地图信息条按路由上报高度、光球浮在其上。OPPO 首轮包 `de2a556` 取到闲置 / 思考 / 播报 / 一步停播证据，`1c67807` 复验读数见第十五节；临时偏好均恢复、App 已退出。第十四节的发布、真实 Keyguard ACK 与标题修复证据不变。**服务端多 operationId 实机组合及 Planner 技术失败降级策略仍未闭合，未整批签收。**
