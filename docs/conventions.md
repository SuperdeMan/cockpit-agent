...
| reminder | reminder | core | first_party | cloud | 50074 | reminder.create, reminder.list, reminder.complete, reminder.cancel |
...
| `reminder.create` | reminder | cloud | title, time_text, kind | 一句话创建提醒；确定性中文时间解析（LLM@fast 兜底），缺时刻 NEED_SLOT 追问（title 存 REMINDER_PENDING 下轮合并）；"记一下…"无时刻→待办(kind=todo)；创建回读确认 |
...
| `REMINDERS_ACTIVE`（`reminders_active`） | reminder `_refresh_active`（list/create/complete/cancel 后刷新） | reminder `_resolve_target`（「第N条」序号解析） | `{items:[{id,title}]}` | 会话内；被覆盖 |
| `REMINDER_PENDING`（`reminder_pending`） | reminder `_save_pending`（缺时刻 NEED_SLOT 追问时写） | reminder `_load_pending`（下轮 create 合并标题） | `{title}` | 一轮追问；消费即清 |
