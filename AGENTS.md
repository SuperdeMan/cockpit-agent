...
**当前部署形态（2026-08-28 更新）**：`dev-stack.local` = **`target=cloud`**。
云端 release **`6a65e7a0beb79312df157d5b5dafc9f2ef85d1d3`**
——**MiniMax QA 修复批六批全部上线**（上一版 `7ac2176`，再上一版即 QA 被测检查点
`c7c211bedb4ff504dfceaf09e652c7875bdaebb8`）。`status` = ok、5/5 端点 healthy、零 warning，
统一 `verify` = **verified**（证据 `.artifacts/dev-stack-verifications/20260828T135840Z-6a65e7a.json`）。
...
用户话术（「113.941200,22.541000当前没有生效的天气预警。」）。**当日已修 + 两条锁
（正向 + 误伤对照），但修复尚未部署——`6a65e7a` 上仍有这个坑**，逐条见 §4.2 该行。
⚠ **另一条活缺陷已量到频次**：SL1 一句话建出 4 条同样的提醒，`--repeat 3` = 1/3 复现，
**六批都没有碰到它**（踩的是「同轮不收编」那条被刻意保留的裁定），见 §4.2 该行。

⚠ **长会话探针（`probe_qa_long_sessions.py`）在新 release 上仍未跑**——上面这些读数
只来自迷你集；QA 那 17 张症状卡里靠长会话才复现的那些（城市漂移、候选重列、
偏好反着推荐…）**这一轮一条都没验**。**不许把「迷你集 58/61」读成「QA 全绿」。**
...
**7639 passed / 32 skipped 零红**（3:49）。
本跳 `7637 → 7639` = **+2**：road-safety 的「GPS 定位串不许进话术」正向锁 + 误伤对照
（`agents/road_safety/tests/test_agent.py`，14 → 16）——那是部署后第一趟迷你集抓到的
**我们自己第 5 批造的回归**，见 §4.2 该行。三道离线门禁读数逐字未变。
...
对账链：**7639**（08-28 部署后回归修复）← 7637（08-28 迷你集跑批批次）← 7631（08-28 QA 修复批第 6 批）← 7598（08-28 第 5 批）← 7547（08-28 第 4 批）← 7493（08-28 第 3 批）← 7397（08-28 第 2 批）← 7314（08-28 第 1 批）← 7225（08-26 发布治理/测试族）← 7106（08-25 MiniMax QA 闭环）← 6969（08-24 MiniMax QA 批）← 6933（08-22 I-030 批）← 6902（08-22 复验批）← 6897（08-21 规格值域批）
...
| ~~**GPS 定位串念进用户话术**（2026-08-28 部署后第一趟迷你集抓到，**第 5 批 C9-A 引入**）~~ **✅ 同日已修，但修复未部署** | 真栈用户听到过这一句：「**113.941200,22.541000**当前没有生效的天气预警。」根因在 `agents/road_safety/src/agent.py` 的兜底话术直接念 `city`，而 **C9-A 把这个值从「mock 城市名」换成了 `lng,lat` 坐标串交给 provider**。⚠ **C9-D 明明挡过同一个形态**（方案 C9 的「顺手账」逐字写着「GPS 路径下会把『114.06,22.54』坐标串念进话术」，并在 info 侧过了 `_display_city`）——**偏偏漏了 C9-A 自己改的那个文件**。判据：**新造一条数据通路时，要把它流向的每一个出口都走一遍，不能只修被点名的那几个。** 修法＝查询串与出口分离（provider 仍拿坐标，话术说「当前位置」），两条锁：正向 + **误伤对照**（用户点名城市时照原样念）。⚠ **`6a65e7a` 上仍有这个坑**，下次发版带上 | `agents/road_safety/src/agent.py::_weather_alert`、`agents/road_safety/tests/test_agent.py::test_gps_derived_locator_never_reaches_the_user_speech` |
