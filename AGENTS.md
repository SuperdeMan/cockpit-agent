...
| 主题 | 启动条件 / 入口 |
|---|---|
| 对话状态评审（落域 / 拒识 / 上下文 / 长会话）落地 | [评审原文](docs/reviews/2026-09-19-cockpit_conversation_review.md) + [逐条重证与分阶段落地](docs/design/2026-09-19-conversation-review-remediation.md)（2026-09-19 起唯一入口）：F01–F12 对 HEAD 逐条重证全部成立；**批 1（P0 W01–W04 + W05-lite）与批 2（P1 W08/W09）已实施并发布 `e8e6c949`**——确认寻址（问句不授权 / 双挂起问一次 / 裸确认不填槽 / 裸确认询问走 `system.pending_state`）、上下文预算硬约束 + 会话约束进焦点块 + `context_stats` 观测、约束按分句带时态 / 主体 / 撤销、T2 判重键 = 执行侧指纹、候选批按查询共存并版本化、焦点 5 分钟引用 / 2 小时活动状态。**待用户裁决**：W10 澄清续接（改 HMI/Android 契约）、W06 DialogueDecision（改 Planner 输出契约、要 A/B）、W07 TaskFrame；P2（W11–W15）/ P3（W16–W19）按评审原表保留为入口。真栈读数与后续观察见设计文档 §4 |
