...
**接手顺序**（下面这个次序是有理由的，别按卡号做）：

| # | 做什么 | **为什么排在这里** |
|---|---|---|
| **0** | **数据清洗 `--apply`**（需泓舟逐族点头） | 排最前**不是因为它简单，是因为它改变后面所有卡的读数**。Q10/Q6/Q5 残余都要在真栈上量，而库里现在有主宾颠倒的边、`fire_at<=0` 的伪提醒、以及 N3 那条「错误导航被记成轨迹事实」。带着脏数据量出来的读数分不清是代码还是数据。入口闸已经在了 ⇒ 现在清是「清完不会再脏」，泓舟拍板的顺序条件已满足 |
| **1** | **Q10** 查单绑会话 | 最小且依赖已满足（Q2 候选集已就位）。一张卡同时收掉 I-021+I-026 与「文本/按钮双入口不等价」那半，还能把探针 XS4 那条方差面钉死 |
| **2** | **Q6** 执行账本 | 它建立「**确定性 handler 回答系统持有的事实**」这个模式——而 **Q5 的出处披露残余复用同一个模式**。所以 Q6 必须在它前面。⚠ 先量清楚 533 轮 × N 动作的写入量与保留期再定表，别做成只涨不清的 |
...
| **7** | **Q2 残余**：I-018/I-023 的确定性消费方 | 可以并到第 2 步一起做（同属「确定性 handler」族），也可以独立。属性已跨轮存住，缺的只是算它的人 |

**第 0 步的执行顺序与判据**（脚本逐族独立授权，`--apply` **不接受一次清全部**）：

```bash
# 每一族都先 dry-run 看命中，再单独 --apply
python scripts/qa_data_hygiene.py --relation-self-loops      # ① 先重扫
python scripts/qa_data_hygiene.py --relation-inverted        # ② 主宾颠倒
python scripts/qa_data_hygiene.py --reminders-invalid        # ④ fire_at<=0
python scripts/qa_data_hygiene.py --relation-conflicts       # ③ 只看，先别 apply
```

- **① 自环**：判据已改成 `rel <> 'family'`（2026-08-16）——**family 自环不是噪声，
  是「没有名字的人」的表示法**，删了「老婆在哪上班」当场失效。重扫很可能是 **0 条**；
  若仍有命中，那才是真的非 family 自环。
- **② 主宾颠倒**（dry-run 时 1 条）：最干净。入口闸已挡同形态新数据，清完即闭环。
- **④ `fire_at<=0` 仍 pending**（dry-run 时 3 条）：展示层已经不显示它们了，清是收尾；
  写入闸已挡新的。
- **③ 单值谓词冲突**（dry-run 时 2 组）：**建议人裁后再动**。脚本只会「留最长的那条」，
  而两个不同 object 可能是**真实变化**（转学/换公司）；而且「孩子」与「女儿」
  很可能是同一个人被拆成两个 subject——**实体归一不在清洗脚本职责内**。
- 清完**重跑一遍 QA 迷你集**（至少 session + candidate 两组，`--repeat 3`）：
  清洗是改数据，改数据必须复跑同一条探针（§4.3「改完复跑同一条真栈探针」）。
