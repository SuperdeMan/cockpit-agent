...
| 主题 | 启动条件 / 入口 |
|---|---|
| 对话状态评审（落域 / 拒识 / 上下文 / 长会话）落地 | [评审原文](docs/reviews/2026-09-19-cockpit_conversation_review.md) + [逐条重证与分阶段落地](docs/design/2026-09-19-conversation-review-remediation.md)（2026-09-19 起唯一入口）：F01–F12 对 HEAD 逐条重证全部成立；**批 1（P0 W01–W04 + W05-lite）、批 2（P1 W08/W09）、批 3（P1 W10 / W06 / W07）发布 `88a89456`；批 4（P2 W11–W15，2026-09-20 用户授权提交 / 推送 / 部署 / 真栈）已发布 `e9044daf`**——确认寻址 / 上下文预算硬约束 / 约束改口 / T2 判重键 / 候选批版本 / 焦点两层寿命 / 澄清进挂起表 / `acts` + 活动任务帧 / **终态账本 `cloud.outcome` → collector `turns.outcome`（受话 / 理解 / 支持 / 执行失败分账）** / 纯偏好陈述确定性致谢与「我听到了 X，但没听清要拿它做什么」两条新出口 / 谈话步零动作的执行性声称按句剥掉 / `Capability.effect` 声明（任务帧 kind 读它）/ route_hints 从完整注册表扫、不再靠 hint 拿目录保护。**下一步 = P3（W16–W19）与 P2 余项（W12 的 planner 侧 `goal/covers` 契约先 A/B、礼貌尾词进 `question_shape` 后再评估云侧写闸）**，入口设计文档 §6；本批的裁决数据：collector 830 轮里 `clause_uncovered` 134 条 ≥95% 误报（用户可见的「还有一件事没处理」不落）、`execution_claim` 6 条里 2 条真阳性都是谈话步；留下的观察：planner 对问候句偶尔先标「需要澄清」再交不出卡（F09-b 只改措辞不改失败） |
