...
| safety focus 持续阻断后续 charging plan（T47） | 安全状态解除时机的产品裁决 | **已裁 A、已发布 `0d414816`、真栈 T47 格 3/3 `charging.plan`**（`alert_resolved` 一份判据四个消费方）；顺带修 `dest_choice` 挂起吞安全陈述（待发布）；收口页 §3.3–3.4 |
