...
- **落地记录（2026-07-25/26，P4 声纹 + 视觉收官；全量细节见子 RFC `2026-07-25-m4-p4-voiceprint-vision-rfc.md` §11）**：
  ...
  - 验证：全量 `pytest` **2266 passed / 7 skipped**（M4 S2S 基线 2189，净 **+77**）；HMI node **193**（+22）；tsc 21=基线、build 通过；真栈 `e2e_voiceprint` 7/7、`e2e_vision` 6/6，两个新 e2e 已挂 `run_e2e`（模型/key 缺失自动 SKIP）。
