...
| `manual-rag` | Xiaomi SU7 真实车主手册 RAG：生产`9a3b6f2f`使用hash绑定`.mrag`图文包；整本章节187/187、视觉35/35，雨刮/“背宝剑”各3/3带图；LLM瞬时失败有界重试后保留真实PDF卡并诚实降级 |
...
- **停车仍为 mock Provider**；手册已在生产`9a3b6f2f`使用真实Xiaomi SU7 v2图文包，整本
  章节187/187、视觉35/35已闭合。原36题完整真栈仍只在旧release`434a046`闭合；当前精确整批
  有7条旧表述被联网前安全预检拒绝，不能转借成当前36/36。停车与手册不得混写成同一条mock边界。
