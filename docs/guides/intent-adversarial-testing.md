...
### 10.1 方差面怎么读（E4，2026-08-15）

档案化的三个方差句形（组合句口头确认不执行 / 过度澄清 / slot 填整句）**不加 hint、
不动 gate 案例集**——处置早已裁定。缺的一直是**分母**：报告里写「偶发」，复现时既说不清
是不是变严重了，也说不清投下去的范例有没有用。

```bash
python scripts/probe_plan_variance.py --rounds 5            # 三句形各 5 轮真栈
python scripts/probe_plan_variance.py --rounds 3 --cases D2a --out v.json
```

只读脚本：跑真栈 WS + 从 obs collector 按 session 取 `plan_mode`，输出「有动作/有卡片/
纯口头/纯澄清」四列与 plan_mode 分布。**不进 CI、不当准入闸**（同 `test/eval_actionability.py`）。
首跑读数（2026-08-15，MiniMax-M3，每句形 5 轮）：三句形 **15/15 走成 toolcall、零方差复现**。

⚠ 两条读它的纪律：
- **探针必须带位置 meta**。首版漏了，「附近的咖啡店」5/5 全是问句，读起来像「过度澄清
  100%」——实际是 nearby 的位置缺席诚实降级。**探针抽掉一个前提，和替被测系统提供一个
  前提一样糟**，而且方向恰好是「看起来更糟」，最容易被当成真缺陷去改生产。
- 单批没复现 ≠ 方差消失。同日另一条探针就见到一次 `toolcall_degraded` 落 chitchat。
  **这张表只排优先级，逐条改动仍要单独取证。**
