...
**真栈验证（同日，WS 探针两轮）**：「跟着《琅琊榜》游南京」落 trip.plan 出卡
（主题候选未接地→诚实降级普通行程，不假装有主题内容——设计的降级形态）；
「跟着《太平年》游杭州」出卡带「已按《太平年》主题」。**首轮探针抓修一处**：
planner 把**整句**填进 theme 槽（话术念出嵌套书名号「已按《跟着《太平年》游杭州》
主题」）——slot 值补清洗（带主题标记提主体 / 干净短语直用 / 句子回落原话解析，
真路径测试 `test_theme_slot_sanitized_from_full_sentence`），复验绿。
判据同款「示范/声明双轨里，slot 优先的前提是 slot 值可信——planner 填的槽也是
模型输出，直用前先过确定性清洗」。
