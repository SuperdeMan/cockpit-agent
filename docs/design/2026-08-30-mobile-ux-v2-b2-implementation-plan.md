...
- **闸的结论（第一行）**：**未过——三格无读数：G1 的真人语音轮 / G4 折叠切换 / 5 人外部小样本（三格都需要泓舟本人在场）。**
  按本计划 §11.1 的纪律（「闸不过，B3/B4 不开工」）⇒ **B3/B4 暂不开工**。⚠ 这三格**不是失败，是没取到**：
  它们都要真人（说唤醒词 / 手折屏 / 组织 5 名外部用户），adb 做不到。
  **裁决权在泓舟**：若认为「已取到的那几格 + 代码层全绿」足以放行，可直接判过；否则补齐这三格再判。
  已取到的闸项：**G2 ✅ 有读数**（32.7s 长播报下回声提示 0 次、无环、无 barge-in——**不是「未取」，是取到了「没触发」**）、
  **G3 ✅ 两半都过**（Maestro 08 rc=0 + 层开着弹键盘时 `composer-send` 与 `voice-sheet` 都仍在树里）、
  **G5 ✅**（60.0 fps 呈现节奏、零掉帧；同屏循环动画 1 个）、
  **§11.4 首反馈时延 ✅ 2ms**（两次独立读数；但走的是 `fsm:ARMED` 那一路，唤醒词那一路仍需真人）。
  ⚠ 取证中途设备 USB 链路失效过一次（见坑⑦），**泓舟重插数据线后恢复**，D8 / G2 / G3 后半 / 附加④b 都是恢复之后补取的。
