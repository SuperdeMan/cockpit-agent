...
云端 release **`5c6300f`**（2026-08-21 Q12 规格维批一次 `deploy --apply`：
规格值域契约 `input_schema` + 真机台账门禁 + 单候选门店闸；`verify` = **verified**、
5/5 端点 healthy）；此前依次是 `032dd82`（person-pickup 批四次 apply：
...
~~**Q12 的规格维**~~ **✅ 2026-08-21 收口**（B6 §4 `input_schema` 落地 + 真机台账门禁，
history **§65**；⚠ **真栈端到端复验待营业时段**，见 §4.1 ①）、
**Q10 的门店侧（I-024）**、**I-030 跨组比较**仍未做——
两条各有独立成因，见 §4.1 ① 逐卡表；~~**Q5 的实体归一**~~ ✅ 已随 person-pickup 卡收口；
...
**存量数据清洗**已完成，
逐卡状态见 §4.1 ①、流水 history **§41–§53 + §58/§59/§61/§63/§65**、归档索引 **§47.5**）。
...
| **Q12 规格维的真栈复验**（2026-08-21 新立）| **只差一次真栈跑，但要挑时间**：代码/单测/门禁已收口并部署（release `5c6300f`，history §65）；**planner 侧那一半已验**（3/3 槽位正确，含新 `size` 槽），**没验到的是 `_apply_specs`**——它在门店解析之后，而 22:38 之后瑞幸门店陆续打烊、整条链停在「找到的瑞幸门店已打烊」⇒ **营业时段（建议 09:00–17:00）**跑 `python scripts/probe_qa_regression.py --group spec`（SP1 不加糖→官方「不另外加糖」/ SP2 超大杯 / SP3 半糖是反向对照）。⚠ 两条约束：打烊时段跑出来的红**不是红、是没跑到**；**只到 `need_confirm` 为止，绝不发确认帧**（商户写要单轮人工授权）。⚠ 别照卡上那句「少冰」去试——生椰拿铁的温度组只有 冰/热，**「少冰」在它身上本来就该被拒**，要验少冰得挑温度组含少冰的商品（如手打鲜切柠檬茶） | 下方 ① Q12 行、契约 §9.31 |
...
（2026-08-15 立卡 → 2026-08-21 收尾，逐批流水见下。**剩下的两条是 I-030 跨组比较 /
I-024 门店侧**（Q12 规格维已于 2026-08-21 收口，只欠一次营业时段的真栈复验），
...
流水 history **§41–§53 + §58/§59/§61/§62/§63/§64/§65**，本轮归档索引 **§47.5**。）
...
|---|---|---|
| ── 以下已收口，只作查证入口 ── | | |
| **Q12 槽值保真** | ✅ 完成（2026-08-21 规格维收口）| 原话回查（时间维）+ 中文时间词收敛（history §52）；**规格维 2026-08-21 收口**（history §65）——卡上「等 B6 `input_schema` 远期字段」的定性被真机推翻四处：planner **没有丢词**（3/3 填对），三个规格槽是**值域声明本身猜错了**（`ice` 查的「冰量」组在瑞幸不存在，冰档位是「温度」组的取值）⇒ 结构性死亡两个多月。修法=`input_schema` 落 `servers.yaml`（唯一声明）+ 真机台账单向门禁 + 删 `_SPEC_GROUPS` + 补 `size` 槽。**残余只剩一次真栈复验**（要营业时段，见上方挑选表）|
...
（**I-030 / I-024**；Q12 规格维已于 2026-08-21 收口，只欠一次营业时段的真栈复验，
history **§65**）、§4.1 ③ 支付余项，与 **§4.2 条件待办索引**。
