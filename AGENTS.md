...
| 通讯链路加固（comms hardening，2026-06-26）| ✅ P0 全落地：①全链路 gRPC keepalive（新 `runtime/grpcio.py` 共享工厂，Python 全 channel/server + Go 网关三处 dial + 云网关 server params；空闲也 ping，根治"依赖重启换 IP 后需重启依赖方"的断连/无响应）；②全服务优雅停机（Python `run_aio_server` SIGTERM→`server.stop(grace)`、Go `GracefulStop`/`http.Shutdown`）；③HMI 韧性（`hmi/src/ws.mjs` 指数退避重连+断线有界发送队列不再静默丢消息+请求看门狗杜绝永久"思考中"）；④熔断接线（见上）；⑤AgentClient channel 复用+keepalive 消除每调用泄漏、`fork()` 透传 parent_meta。**真栈验证**：`test/e2e_resilience.py` 2/2（cloud-planner 换 IP ~0s 自愈、chitchat-agent 换 IP ~2s 自愈，均不重启依赖方）；全量 887 passed；smoke 13/13；e2e_ws 4 链路 + e2e_context 5/6 + 中枢断言通过。设计见 `docs/design/2026-06-25-comms-link-hardening.md`。**遗留（与本次正交、main 既有）**：真实 MiMo 对"打开后备箱"偶发空计划→回退 chitchat 幻觉应答（`dangerous_trunk_confirm`/`ctx_trunk_confirm_and_edge_include` 失败，根因在规划阶段，本次未碰 planning/context/engine）|
