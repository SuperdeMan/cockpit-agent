...
python scripts/probe_manual_rag_full_coverage.py --env-root <根仓> --pdf <SU7手册PDF> --expected-release <40位SHA> --provider <id> --model <id> --output <artifact.json>  # 手工真栈：默认187叶子+35视觉；可显式`--kind natural`复用原36题契约。先过问句/FastIntent双安全闸，每轮验manual/real/page/section/image/零动作/完整车态，支持checkpoint与release绑定失败复验；安全预检失败必须零请求退出，不得放宽后拼“36/36”
...
8. **前提要显式化，否则前提失效会伪装成判据失败**（2026-08-22 I-030 批）。
   CD5/CD6/CD7 的第 1、2 轮各钉了一个 `card_type`。不钉的时候，T1 偶尔落成澄清卡
   （`intent_choice`），第 3 轮就会报「没点到第 1 轮第 2 项**『查餐品热量』**」
   ——**读起来像被测对象错了，其实是前提换了**。加上守卫后它当场变成「T1 红」。
   > 同批实证的极端形态：第二组用「看看瑞幸有什么可以点的」时，三次取样落到三个
   > 地方，其中一次是门店选择 `NEED_SLOT`，**把会话挂起、第 3 轮整句被当补槽吞掉**。
   > 那一轮的读数与被测判据毫无关系。
9. **考点用例与对照组必须共用同一份前提；分母脏了，真阳性也不算数**（同批）。
   第二组改用「附近的咖啡店」后前提稳了，考点 3/3 绿——但 nearby 对**裸类目词**
   会标 `_fallback`，于是 **N5「兜底不得顶替点名那份」本来就会把绑定送回目标组** ⇒
   **那几次是因为 N5 通过的，不是因为被测判据**。是对照组连红两次才把它顶出来。
   > 它是「分母挑得越干净假阳性越好看」的**镜像形态**：分母脏了，真阳性也不算数。
   > 终稿改用**菜系词**（`keyword=川菜 ≠ 类目标准词「美食」⇒ 恒非兜底`），前提才硬。
   > ⚠ 顺带一条已知边界：**组标签会跟着产生方降级**——「附近的星巴克」在搜不到该
   > 品牌的地方会降级成「10 家美食」，标签也变成「美食」⇒ 跨组点不到名。
   > 那是安全方向（漏而不误伤），但**写跨组探针时别用品牌词**。
   > 配套原语：`names_items_from`（`names_item_from` 的 list 形态，**两边都要点到**
   > 才叫比较）。⚠ 新判据要登记进 `_EXPECT_KEYS`——首跑就被那道 typo 守卫拦下了，
   > **守卫本身是对的**。

10. **判据必须红在它要抓的那件事上**（2026-08-28 MiniMax QA 第 6 批）。两条同批实证：
   ① 「时限解析结果与原话时刻同半天」这条判据**推不出来**——话术里没有时限本身，
   只能拿 `ETA ± 余量` 反推，而「凌晨5点」与「下午5点」反推出的小时数**模 12 相同**；
   它在真栈那一行会红，纯属 `04:59` 差一分钟没进位的**算术巧合**。
   换成「反推出的时限不许跨过午夜」才是它要抓的那个形态。
   ② 「推荐列表首位不得是忌口系（比对卡片 `tags`/`category`）」在真数据上**恒绿**
   ——首位叫「川胖虎·美蛙肥肠鱼」、`category` 是「餐饮服务;中餐厅;中餐厅」，
   忌口信号一个都不在结构化字段里。
   > **靠巧合红的判据，下次就会靠巧合绿；恒绿的断言比没有更糟。**
   > 两条都改判**分支签名**（`找到 N 家<忌口菜系>` 是 nearby 自己回显的检索词，
   > `按您的口味优先…` 是它的 taste_note 分支）。⚠ 反向对照是这类判据真正的成本：
   > **修好之后的正确话术「这次就不按平时爱吃的川菜找了」同样含「川菜」**，
   > 按词判会把修好的行为判成红。
11. **一把尺子只有把两个立场分开表达才不会自相矛盾**（同批）。
   ...
   **如实标注即合法**——前者是**部署形态期望**（真栈不该有 mock），
   ...
   > 配套：判据表按**卡型 × 允许 mode**建，与契约 §9.3 的必带清单之间有一道
   > **直接解析 `docs/conventions.md`** 的漂移闸——**一份声明两个消费方**，
   > 两份表能分头改就还是两把尺子。
12. **漏检的默认形态是「新加的那一轮没人想到写断言」**（同批，第二次沉淀）。
   补一轮的判据只值一次；**扫全组的元断言**防的是下一次。当前两条：
   safety 组每一轮必须有至少一个正向期望、且必须带 `no_capability_refusal`
   （不得以「暂不支持」这个**我们自己写死的拒绝串**收场——它是分支签名不是关键词）。

13. **下界判据量不出「做多了」**（2026-08-28 真栈跑批实测）。SL1 断言的是
   `card_items_at_least: 2`（「一句话要两条提醒时必须真的有两条」），真栈那一轮
   实际建出 **4 条**——两个 `card_group`、4 张 `reminder_card`、4 个不同 id，
   planner 产了两步、每步把整组各建了一遍；而**顶层 `items` 恰好也是 2**（那两个
   group），于是三条既有断言全绿。
   > **立卡时防的是「说了两条只有一条」，没人想过会是 4 条。**
   > 配套原语：`card_nodes: {"<卡型>": N}` —— 数的是**整棵卡片树里该卡型的节点数**、
   > 且是**精确比较**（多了少了都报）。⚠ 顶层项数与树里的卡数在 `card_group`
   > 嵌套时会分叉，而**用户看到几张卡取决于后者**。

### 5.3.2 长会话 QA 探针与回放计分（`scripts/probe_qa_long_sessions.py`，2026-08-24）

§5.3.1 那份迷你集是**每个用例一条干净会话**；这一份把同一批用例**串进 5 类 persona
的连续长上下文**（每类 50–100 轮、始终复用同一个 `session_id`），再补一条 50 轮的
信息/出行业务线。它要抓的是迷你集抓不到的那一族：**会话状态、归属、审计与真实性**
——「happy path 和干净会话证明不了会话状态是对的」。

逐轮同时保存 speech / card / action / vehicle_state / trace，并向 collector 对账
路由 / Agent / LLM provider；所有 LLM 调用必须是 `minimax:MiniMax-M3`；
商户与危险动作最多走到预览/二次确认并立即取消，**不付款、不创建真实订单、
不执行危险确认**。跑批自带多段清理（每个 case 结束按服务端 id 逐条取消挂起、商户预览零态证明、
自己建的提醒清掉、导航与车态恢复到基线），**每一段都要拿到服务端证据才算数**
...
python scripts/probe_qa_long_sessions.py --dry-run            # 只数计划轮数，不连栈

# 修正后计分：拿**今天的判据**重算一份已有 artifact（纯本地、零网络）
...
它存在的理由是：**改了尺子就要能回答「它会把当时那一趟判成什么」**——新判据的价值
全在「当时绿的哪几行会变红」上。三条边界写清楚，别让读数越界：

- **回放 ≠ 真栈复跑。** 它判的是「尺子会怎么判**当天那些话**」，不是「今天的系统
  会说什么」。**修复有没有生效只有真栈答得出。**
- **不可回放 ≠ 通过。** runner 自造的轮次（挂起清理 / 车态恢复 / 提醒清理）与
  「要上一轮按钮才有话可说」的轮次重算不出来，**原样保留存档判读**
  ——否则红数会因为尺子够不着而凭空变小。
- **反解替代标记要按它真实的作用域取。** 用例里的 `{run}` 是**一个 case 一个**，
  而带它的往往只有该 case 的第一轮；按轮反解会让后面那些轮全拿到 0，
  `speech_has` 立刻整片假红。

判据本体一律与生产共用同一份实现（`runtime/execution_claim.py` 的执行性声明形态、
`runtime/session_constraints.py` 的忌辣词表），**不许在 `scripts/` 里抄第二张表**
——抄两份就会长出「这边认得那边不认得」的分歧，源码级断言钉着这条。

单测 `scripts/tests/test_probe_qa_long_sessions.py`：每条判据两向用例
（该判红的判红、该判绿的判绿），加上面那三条元断言与漂移闸。
