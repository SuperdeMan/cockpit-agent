...
`python -m pytest --import-mode=importlib` = **6702 passed / 32 skipped 零红（25m25s）**。
三个数逐条闭合：`6623 + 38 + 41 = 6702`、`73 − 41 = 32`——**+38** 是本批新增用例
...
对账先问「这一跳的 shell 和上一跳一样吗」。上一跳基线 6623/73（洁癖批，`--collect-only` 6696）。
...
**探索式 QA 轮的 Q8 与四处卡内残余**（~~Q2 的确定性消费方~~ ✅、
~~Q10 的双入口收敛 + Q2 的候选集下发面~~ ✅ 2026-08-19 交付，真栈 MC1 **0/3 → 2/3**；
**候选在手时 chitchat 仍会编造**（复跑掀开的新卡，排第一）、
...
> 同批复跑掀开一条**新卡**（候选在手时 chitchat 仍会编造），**它排在 Q8 前面**：
...
**第 0–7 步已全部完成**（2026-08-16 → 08-19，逐步的落点/判据/读数在 history
**§48–§59 + §61**，本表不再复述）⇒ **新会话从第 7.5 步开始**
（那是第 7 步复跑读数掀开的，不是新一轮的事）。
...
| # | 做什么 | **为什么排在这里** |
|---|---|---|
| **7.5** | **候选在手时 chitchat 编造**（§61 复跑掀开）← **接手从这里开始** | 编号带小数是刻意的：它是第 7 步的**直接产物**（复跑读数掀开的），不是新一轮的事。**排在 Q8 前面**因为它是**真实性**问题——系统说出了一个不存在的价格；Q8 只是能力缺席（答不出来）。**编一个价格比答不出来严重。** 上手先读 `conventions.md` §9.27 那三段判据为什么取窄，再决定扩劫持面还是另立一条 |
