...
| 形态 | 记录区 | 修前 | 修后 | 说明 |
|---|---|---|---|---|
| 外屏竖 | 728dp | 291 / 451 / 568 | **一个 dp 没变** | 比例本来就够，下限不绑 |
| 外屏横 | **192dp** | 0.4 → **77dp** | **192**（clamp 占满） | 77dp 连「把手 12 + 底栏 17 + 56dp 键」这 85dp 硬 chrome 都装不下 ⇒ flex 收缩把 `voice-sheet-collapse` 压成 53.0 |
| 内屏 | 573dp | 0.78 → **447dp** | **476** | 内容需求 476 > 447 ⇒ 压缩卡被裁出可视区（`driving-card-title` bounds y2<y1） |
...
**缺陷 A 的真机复取（⬜ 等真机轮）**：① 外屏横 `voice-sheet-collapse` 53.0dp → **≥56**；
② 内屏 `driving-card-title` 的 bounds **y2 > y1**；③ **外屏竖三档不回归**（层高与修前逐个对得上）。
