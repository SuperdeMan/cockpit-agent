...
| 全仓审计与 Roadmap（2026-07-02）| 📋 见 `docs/reviews/2026-07-02-repo-audit-and-roadmap.md`：架构一致性 8 偏差 + 20 技术债（P0-P2）+ 8 测试/量产缺口 + 四阶段 Roadmap（R1 门禁与卫生→R2 架构还债→R3 量产硬化→R4 能力演进）。最大风险=「不改编排核心加 Agent」铁律系统性侵蚀（A1/R2.1，编排核心 4 处硬编码特定 Agent 知识）。**R1 已开工**：media action_type 判定统一（T1.5，`edge_call.action_type_for` 单一口径，+3 回归测试）、compose `restart: unless-stopped` 生存性 + redis/postgres healthcheck（T1.3）、文档同步（T1.4，conventions/架构实现现状/ws8 单轨校验）已落；CI 补全（T1.1）与死代码/空目录清理（T1.2）命中红线待确认 |
