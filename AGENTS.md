...
| **`hvac.set` 的 Verifier 核的不是那件事（新，未修）** | findings **§11.2** | `_VERIFICATION["hvac.set"]` 只声明 `{"hvac_on": "true"}`——**「设定为 N 度」的验证核的是「空调开着」**，于是 `B3-3` 里「set 了但没设成」被判 `sat`。**判据：验证的强度必须匹配主张的强度**；核了一个比主张弱的东西等于没核，且它比「挂点漏了执行路径」更难发现——**一直在报绿**。修法要 `Verification.expect` 支持**动态期望**（取该步 slots 的值），当前 `_verification_for(intent)` 是按 intent 静态构造的，是 M2 协议级扩展 |
| **planner 把值算进 goal 却没写进 slots（新，未修）** | findings **§11.2** | `B3-3` 失败那次 `llm_raw` 的 goal 写着「…最喜欢的温度（**26度**）」而 plan 是 `hvac.set` + `slots:{}`，终态温度不变、回复只剩单字「度」。记忆召回与抽取都正常，丢的是**从 goal 到 slots 那一步**。这是「**goal 是免费的对照物**」的第三例，且**机器可判到值一级**（goal 有数字、对应 slots 为空）——比前两例的「缺步」更容易写成检测器 |
