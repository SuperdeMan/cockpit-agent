...
| 主题 | 启动条件 / 入口 |
|---|---|
| Android App 剩余待办（2026-09-14 起的唯一入口） | [剩余待办总表](docs/design/2026-09-14-android-remaining-todos.md)：E 工程可独立 / D 只缺真机 / H 缺真人、外部条件或授权 / R 已裁决不改。本日推进七条（重发终态、支持页不再被常驻层盖住、播报语域受话边界端云两处、发送跟底、重发全文）落在本地提交 `c036fd14` / `696899b5` / `cbf17b96` / `883452d2`，**未 push、未 deploy**；OPPO 固定包 `696899b59` / `cbf17b96e` / `883452d29`（现常驻）逐格证据见总表 §5。下面各行是历史入口，状态以总表为准 |
