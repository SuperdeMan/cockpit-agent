...
| `manual-rag` | Xiaomi SU7 真实车主手册 RAG：生产 `434a046` 使用 hash 绑定 `.mrag` 图文包；原36题已闭合，整本187叶子首轮177/187、视觉28/35，本地0.3.2候选已修显式来源落域与5个三字caption但尚未发布 |
...
- **停车仍为 mock Provider**；手册已在生产 `434a046` 使用真实 Xiaomi SU7 v2 图文包，
  原36题与14×3高风险问法均通过；整本扩面仍有章节单轮方差和5个视觉稳定缺口，不能写成
  整本全绿。两者不得混写成同一条 mock 边界。
