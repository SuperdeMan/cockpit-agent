...
- **本地安全闸审计对象**：本地全量、门禁与 targeted 测试绑定的代码与契约 SHA 为
  ...
  **不在 cloud**，尚未 push/deploy，也没有该 SHA 的统一 `verify`、remote-safe、安全真栈或
  `information` 长会话证据。后续 `9111681` 与本次修正文档提交都只改文档，**不承接、
  不重新声称上述测试**。引用“云上跑的是什么”仍只认 `343934b`。
...
**最新本地候选全量基线（2026-08-30，QA 安全确认写闸；未部署）**：测试绑定的审计对象 SHA
...
其中只有这 **1 条 gRPC RuntimeWarning** 已稳定定性为既有 trip test-only fixture 债务：fixture
跨多个 `asyncio.run` 复用 loop-affine gRPC channel；测试绑定代码范围 `15ff116..dfad687`
相关 diff 为空。其余 12 条按原始类别保留，本轮未逐条消除。没有证据表明生产持久 loop 受影响，
**也不能据此声称生产安全已证明**。验证清单见 ignored/local-only
...
`01cc57c` → `1105829`；本地全量、门禁与 targeted 测试绑定的代码与契约 SHA 是
`dfad68730b50d094993c328d33cb774d29642e16`。后续 `9111681` 与本次修正文档提交都只改文档，
不承接或重新声称该测试证据。
...
11 条逐条归属（**零条由本批引入**）：外部/环境 5（麦当劳 `calculate-price` 抛异常 ×1、
...
| **安全问句被商户下单域劫持（长会话专有）**（2026-08-30 长会话 `343934b` info T24 新记；**本地实现+验证完成，待外部动作/真栈**）| 云端实录仍是：「红色机油灯亮了还能继续开吗」→ `luckin.order` →「想点哪一款瑞幸饮品？」并挂起；当前云端 release `343934b` **仍未包含修法**。设计裁决已从“任何云侧写”收窄到**非指令问句拦 `require_confirm=true`**，既有端侧写判据保留；测试绑定审计对象 `dfad687…` 已完成主 guard / fallback 权威字段 / focused+normal 统一终结器三层 TDD、双评审与全量 7723/32/13 warnings，`9111681` 与本次修正文档提交不承接测试声明。13 warnings 中仅 1 条 gRPC RuntimeWarning 已定性为 test-only fixture 债务，其余 12 条按原始类别保留。⚠ **这不是云端已修**。未完成清单：push 授权；deploy 摘要与 `--apply` 授权；新 SHA `status` + 统一 `verify`/remote-safe；干净会话 3/3；原 `information` persona；零商户草稿、零挂起、零探针副作用与清理核验。完成前保持未划线/active | [安全闸设计](docs/design/2026-08-30-qa-safety-confirmed-write-guard.md)；history §84/§85；云端原始 artifact `.artifacts/dev-stack-verifications/qa-long-sessions-343934b.json` |
