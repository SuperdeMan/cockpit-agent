...
> 截至 **2026-08-02**。下方长表是逐批次的历史流水（**只进不出**，查证据用）；这一小节是
> **交接摘要**——接手者先读这里，再按需下钻。

**最新状态（2026-08-02）**：两件事同批收口——① **验收报告实现结果复核：13/13 张主卡
为真**（逐卡核到 file:line + 抽跑 224 测试；无实现缺失、无测试造假；3 条低危当批修掉，
1 条入账=MCP `compensate_tool` 可达性校验未机制化，**下个 server 接入时必须顺手机制化**）；
② **「天气适合去哪玩」badcase 三连修复**（f53d2e58/c0d1a8d2/4799fb1f）——定性为
**落域准确 ≠ 智能**的标本：planner goal 全写对、步骤只拆单域一步；质疑轮 planner 改对了
「室内景点」却被 nearby 类目子串扫描降级回「景点」。修复全声明式（`skills/guides/
weather-outing.yaml` 组合判据 + 范例 nearby#23/#24 + nearby **室内扇出**商场/电影院/博物馆
串行检索 + `weather_context` 话术承接），**编排核心零改动**；真栈三句原话翻正、冷启动
adaptive 两轮链打通（查天气→replan 补推荐→聚合成一段自然回答）、guide golden live 7/7。
报告 `docs/reviews/2026-08-02-review-acceptance-impl-and-badcase-intelligence.md`
（§2.4 判据：**goal 是免费的对照物**；上下文依赖的 badcase 不标扁平 gold）。
全量 **3654 passed / 11 skipped / 0 failed**（2026-08-02 单进程实测 14m20s；=上批 3647
＋nearby 室内组/类目优先级/话术承接 7）。

**验收 13 卡收口（2026-08-01）**：2026-07-26 验收报告 §7 的 **13 张主卡全部收口**
（M-A 测试真实性 / M-B 多乘员隔离 / M-C 可靠触达 / M-D 外部生态，四批全部合入 main）。
每批的「明确未做」逐条附判据在验收报告 §9/§10.2/§11.2/§12.2——**未做不等于没账**。
上批全量 **3647 passed / 11 skipped / 0 failed**（较 M-D 时的 3597 净 **+50**＝M5 P3 收尾 +33［判别化描述 16 / 对象桥接台账 15 / 分词 golden 1 / catalog 1］＋影子四路径与 θ 档 16［`test_nlu_shadow_paths.py`］＋CI 收口 1［recover 无操作回归］，另有若干断言按新意图改写不计净增）；HMI 225/225 + build；Go 网关 vet+test 通过。
...
`if os.name == "nt": return` 把校验整层跳过了。** 复现步骤与四条判据见 §4.0 末
