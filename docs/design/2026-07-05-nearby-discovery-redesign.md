...
- ④ 「第N个详情」handoff 透传高德 **POI id**（`meta.nearby_poi_id`）精确取详情 + provider 按名取详情优先精确匹配，修『返回第一个 / 不在列表中的分店』；**CDP 实测**「看第八个的详情」→ 列表第 8 项国家大剧院西餐厅精确命中。**续修**：裸序号选择『点一下第九个』（无「详情」线索词）之前漏接 → 落后端被 LLM 当新查询、返回列表外无关 POI（理发店）；改用 `ordinalSelectIn`（要求「个/家」，避免『第一次』误判），place_list 里任何序号选择默认看详情、带导航词才导航；CDP 实测『点一下第九个』→ 第 9 项厚榭潮汕菜精确命中。
