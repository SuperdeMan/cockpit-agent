...
真机轮抓到并同日补的两条（设计 §9.4）：① 按住说话那一瞬「在听…」下挂着上一轮的卡、层按上一轮升到 0.78 ⇒ 判据「收音中而本轮草稿未出现 ⇒ 内容不画、0.4」（`VoiceFacts.draft` → `snapshot.sheetBody: turn / empty / settled`，empty 仍按 0.4 档预留转写，settled = 行车回落只剩头区）；② 跟到末尾后被裁的首行紧贴头区 ⇒ 滚动后顶缘渐隐。第二轮真机（清洁包 `8c85e6914`，设计 §9.5）：① ✅ 收音初始只有本轮转写、上一轮不再出现；② ✅ dump 到 `voice-sheet-fade-top` 24dp 紧贴头区；顺带抓到 `empty` 只剩头区时字一到层从 224 再长到 318 ⇒ 改成预留转写（`eeae0d7e`）。第三轮（`eeae0d7e0`，设计 §9.6）：三态判据在场轨迹与 jest 真宿主树里都是升层同帧到 0.4，但逐帧量层高发现**升层动画本身 2.5s 才落定、有过冲**（`withSpring` 动 `height` ⇒ 每帧重排模糊壳 + 光球、只跑到几帧 / 秒、reanimated 把弹簧帧间隔封顶）——09-11 之前就有，球固定在头区后变显眼；修成 translateY 滑入 + 高度短 timing（`990466d6`）；第四轮（`990466d6d`）复验：深链升层一步到位 347.6dp 四帧不动、按住光球第一可见帧起 317.5dp 恒定、头区下拉收起 ✅（设计 §9.7）。OPPO 常驻包现为 `990466d6d`。
...
| ID | 栏 | 事项 | 处置 / 卡点 | 判据落点 |
|---|---|---|---|---|
| E-11 | D | 回答文字一段一段蹦：服务端 delta 成簇到达（PC 探针簇内 2ms、簇间 p90 147ms），真机每 ~110ms 长出十来个字、停顿后一次二三十个字；JS 线程流式期间只有 25–52%，不是客户端卡 | **已修 + 真机 A/B 闭合（清洁包 `3abd325ea`，设计 §5）**：流式期间帧间隔从中位 ~110ms 的簇状变成稳定 15–20ms 的逐字追出，391ms 服务端停顿后不再一次蹦出二三十字；727 字回答完整收尾；JS 线程流式期间 36–79%。显示与记录分开——`Msg.text` 照旧逐片即时累积（TTS / 历史 / 语音层判据不变），显示按「到达时定速（积压 / 240ms）、到达间匀速、最低 40 字/s、33ms 一拍」追；挂载直出、final 尾巴追完、整段替换 / 换 id 直出、用户气泡不追。装置 `mobile/e2e/tools/stream_cadence_probe.py`（prep / frames） | `core/session/streamReveal.ts`（唯一判据）、`features/chat/useRevealedText.ts`；消费点 `MessageBubble` / `VoiceSheet`；`streamReveal.test` 5、`revealedText.test` 5 |
| E-12 | E | 长回答句中截断（「给我讲一个很长的故事」357 字停在「离开港口」、「history of Shenzhen in detail」426 字停在「如今，深圳是中国」）：chitchat `max_tokens=220` 被当长度控制用；final 与流式逐字一致，客户端没再截 | **已修、已 push、已 deploy `3abd325e`（status ok、verify verified）**；部署后 PC 探针 standard ×3 + detailed ×1（17.5s、约 1500 字）全部句尾完整：`_LENGTH` 上限 140/220/440 → 300/600/900 只做兜底、提示语不变；chitchat 流式 `timeout=55`；`clients.AGENT_STREAM_TIMEOUT_S`（env `CLOUD_AGENT_STREAM_TIMEOUT_S`，缺省 60）成为 D0 `call_agent_stream` 缺省截止（原 30）——不放宽的话长回答换成被「抱歉，刚才没说完」整段替掉 | `agents/chitchat/src/agent.py::_LENGTH`、`orchestrator/cloud/clients.py`；`agents/chitchat/tests/test_agent.py` |
| H-11 | H | 候选包构建：机器 commit 上限 69.1GB 里 `WindowsTerminal` 一个进程占 34.4GB，页面文件已到托管上限，三个并发 clang 就撞顶；Claude Code 以「系统内存严重不足」终止了接续循环并要求不得自行重启 | **已解决（记录保留）**：用户释放物理内存后 commit 仍只剩 1.5GB（页面文件到托管上限），清洁树 `-CompileJobs 3` 再次死于 clang abort；镜像内把生成的 ninja `pool compile depth` 3→1 接续 11m10s 出包、验包后改回 3。同机 commit 紧张时这是可复用的退让档（设计 §5.1） | 设计 §5.1 |
...
**2026-09-20/21 第三批**（remediation §6.15）：G-05 首跑红（工作流自 M4 起没构建过）、D-08 替身 + dev 包 `8df1d1173` 真机闭合、D-09 核出 E-24 / E-25 两条真缺陷并修、真机取到主 TTS × 中断五维与 THINKING × 中断 ×3、E-26 顺带修、G-07 立卡后用户裁决「都做」当晚落地（LISTENING × 中断真机闭合）；提交 `59088760` → `8df1d117` → `6faa3c75` → `e7a83638` → `2ed312fb` → `5286c3ba`（CI 一行）→ `ec7e0fc0`（G-07）→ `b08579f6`（CI 冒烟一行）；OPPO 常驻包 **`b08579f6c`**。
...
| ~~D-08~~ | D | F07 运行时压力（慢解码替身、加载 / 释放 / 重复启停）+ F02 通话期间 recorder 收到什么 | **F07 已闭合**（`59088760` 替身 + dev 包 `8df1d1173` 真机：1.5s×6 每轮 stale +1、release ≈ 剩余解码、stuck 0 / fail 0 / 零跨代并存；0.3s×10 零 stale；G-06 引擎分支 ✅）。通话期间采集：无 SIM 造不出来电；来电响铃 = 瞬时焦点丢失（见 D-09 LISTENING 格：现状零回调、麦照开）、接听 = InCall 到前台 ⇒ 前后台闸 0.4s 内释放麦（12:55 读数）⇒ 剩下的是 G-07 裁决 | remediation §6.15 |
| D-09 | D | 系统音频五维（声音 / 播放器 / FSM / 采集 / 上行）× 四态（主 TTS / S2S / LISTENING / FOLLOWUP）：焦点丢失、耳机断开、系统中断 | **上机前先核事件源，核出两条真缺陷（E-24 / E-25，均已修并真机复验）**。已取：主 TTS 出声中 × 中断五维齐（`5286c3ba5` 2ms / `b08579f6c` 6ms 停声、1s 后放焦点、不进 FOLLOWUP、App 不切后台）、THINKING（TTS live）× 中断 ×3、**LISTENING × 中断（G-07 后）：持焦点、`-2` → 放弃半句回 ARMED**、出声才持 / 收尾放、别人的音乐压低不停。09-22 凌晨安静环境补到：**语音轮 SPEAKING × 中断（2ms 停声、无 FOLLOWUP）、FOLLOWUP × 中断 ×2（续问窗 0.18s 关掉回 ARMED）**。**未取**：S2S × 中断——云端 `E2E_IDENTITY_ENABLED` 让 `/api/s2s` 对真 App 一律 close 1008 `unauthorized test identity`，端到端整档当前不可用、静默回落三段式（cloud 侧 / `.env`，归对话评审线；见 remediation §6.15）；耳机断开 OS 层（OPPO 零 bonded 蓝牙、无有线口） | remediation §6.15；在场轨迹 `system_stop:<reason>` + logcat `MediaFocusControl` / `[handsfree] fsm` + `ps -T` 线程 |
...
| G-05 | H | CI 冒烟 x86_64 模拟器 vs 原生插件只打 ARM ABI | **已应用 `96b39b26`**；09-20 首次 dispatch（run #35494521133）**在 assembleDebug 就红**：onnxruntime 的 Gradle 9 `VersionNumber` 垫片只在本机 init script 里，CI 从没带 ⇒ 工作流自 M4 起没构建通过。垫片拆成 `scripts/gradle_rn_compat.init.gradle`（`e7a83638`）+ CI 一行 `-I`（用户 09-21 批准，`5286c3ba`）⇒ 第二次 dispatch **构建 24 分钟成功**（M4 以来第一次）、ABI 读数 = CI debug 包四个 ABI 全有（`ABI_MATCH=native`，评审「只打 ARM」的前提对 CI 包不成立）、x86_64 镜像装包 Success；冒烟没跑到——emulator-runner 逐行执行多行 `script:`，`if…fi` 被拆开语法错。逻辑搬进 `scripts/ci_mobile_smoke.sh` + workflow 一行调用（`b08579f6`）；第三次 dispatch：Maestro driver 15s 超时、App 没被拉起 ⇒ 脚本改成自己 `am start` 取 .so 读数 + driver 180s（`781e7530`）；**第四次 dispatch run #35620321623：三个读数齐——四 ABI native、装包 Success、App 在 x86_64 上原生跑起来（无 UnsatisfiedLinkError / dlopen failed、ndk_translation 0）**。Maestro 离线冒烟本身：flow 09 红在 `composer-input is visible`（fresh install 落引导页，flow 前提）+ job 45 分钟被取消——CI 预算 / flow 前提的事，另立（remediation §6.15 末） | `.github/workflows/mobile-apk.yml`、`scripts/ci_mobile_smoke.sh`、remediation §6.15 末 |
