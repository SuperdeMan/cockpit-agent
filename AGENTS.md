...
**第 1、2、3 批均已于 2026-08-28 落地**：第 1 批 = C1 安全告警链 A-D + C2 端侧三 bug 与
探针诊断出口 + C16 的 1·6·8（另加实施时扫出的 N8/N9 两个新缺陷）；
**第 2 批 = C4**（会话账本扩「数据源」维 + 编排层三条确定性读出口 + 候选集重列算子；
D 项评估后不做，T44 换了个更小的修法当批做掉）；
**第 3 批 = C3 + C10**（`wait_slot` 方向反转成「槽值必须长得像这个槽」+ `slot_shapes`
声明式契约 + 补槽黑洞止损线；提醒域序数参照系统一 / 标题精确度阶梯 / 任务性准入 /
跨轮幂等 / 范例一条，外加 C10-D 的两件接手残账）。
**下一批 = 第 4 批（C6 接人分解 + C5 覆盖度 + C7 trip 三修 + C8 reroute origin）**。
⚠ **本 release 的云端读数没有因此改变**——三批都只跑了本地全量与离线门禁，
**真栈迷你集与「修正后计分」都还没跑**（探针本身在这三批里都改过）。
...
**最新后端全量基线（2026-08-28 MiniMax QA 修复批**第 3 批**，`target=cloud` +
本地 Docker 已退）**：`python -m pytest -q -n auto --dist worksteal` =
**7493 passed / 32 skipped 零红**（3:57）。本跳 `7397 → 7493` = **+96，全部是第 3 批（C3/C10）新增断言**
