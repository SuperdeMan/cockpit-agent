...
日期：2026-07-12　发起：泓舟　状态：**已落地**（9 切片 + §8 收尾续修全部完成并真栈验证，见 §7/§8）
...
### 8.7 badcase 736e4bba / 1de7e50c：赛事预测答非所问 + 赛果阶段误标

同会话两轮（@deepseek）：轮1「判断世界杯半决赛结果+决赛对阵+谁胜出」——回答把
**四分之一决赛**赛果（英格兰2-1挪威、阿根廷3-1瑞士）说成「今天刚结束的半决赛」，
错推「决赛=英格兰vs阿根廷」（实为半决赛对阵）；轮2 用户纠正后要求预测，仍被
info.sports 答成单场已结束的 1/4 决赛详情。两个缺口：

1. **预测/前瞻类问题不适配结构化赛果源**：api-football 只有已定事实、免费档只开
   今天±1 天（未来对阵/分析根本给不了）。修：`_PREDICTIVE_HINT`（预测/判断/前瞻/
   谁会赢/胜出/赔率/看好谁…）——`_maybe_sports` 命中即让路（planner 路由 info.search
   的预测句不再被赛事短路劫持）；`_sports` intent 直达命中则以**原话整句**为 query 转
   `self._search`（保住「半决赛/决赛/预测」上下文，联网检索对阵+接地综合，不回环）。
   manifest desc 补反界定「仅限已定事实；预测/分析走联网搜索」。
2. **赛果 speech 不带比赛阶段（round）**：`Fixture.round` 早就在卡片里但 speech 从不念
   ——聚合 LLM 拿到裸赛果便脑补阶段。修：`_round_zh` 映射（Quarter-finals→四分之一
   决赛…，注意 quarter/semi 须在 final 前匹配），列表首句（同日同轮点明）与单场详情
   head 都带阶段。
