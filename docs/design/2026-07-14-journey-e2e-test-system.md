...
所以已为你设置了明天早上8点的提醒」——条件链+天气结论都在）。②「明早」无时刻→追问
维持原判（半合理，未动 timeparse 的 NEED_TIME 语义）。
...
**收口回归**：全量 pytest **1616 passed**（1603+13）；挂起面密集旅程批 A2-3/A5-3/B2-1/
B2-2/B3-1 全绿（trip 确认、B2 中断-恢复三态、多意图确认均无双重播报）。**观察项（非本
次引入，登记不擅改语料）**：B2-3 当日两采样均被 planner 路由到
`navigation.navigate_to{destination:惠州,stop_category:充电}` 顺路途经点流（waypoint
候选「想顺道去哪家」），不出 charging 的 dest_choice 候选卡——与 B5-2 校准记录同族
（R1 后城市级目的地常被直接成路线，双合法路由采样翻面），obs trace `c03a87cf35934837`；
该轮无挂起、与本次改动无关。处置（修 charging hint 优先级 / 改用例前提，同 B5-2 先例）
留泓舟三选一。
