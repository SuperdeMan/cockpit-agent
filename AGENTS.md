...
云端 release **`7ac2176712c5a68dd65f39118502c60505774fa4`**（上一版 `c7c211be`；本次带的是 MiniMax TTS 换 WebSocket 长连接，首音 1453→516~563ms）；`status` = healthy、5/5 healthy、零 warning，
...
| 候选 | 为什么现在能做 / 不能做 | 入口 |
|---|---|---|
| **Android 陪伴端 App（新客户端 `mobile/`，2026-08-23 立项）** | **M0 地基 / M1 对话 MVP / M2 语音 三阶段代码面全部完成**（2026-08-25 ~ 08-27）。M1 真机首轮 08-26 通过；**M2-5 验收轮进行中**——机器可判的已跑完（采集 A/B 定案、ASR 协议面直灌逐字一致、TTS 首音、barge-in 物理面、语音设置持久化、断网兜底），**08-27 泓舟真机补验真人按住说话与播报听感两条通过**。jest 133/133、tsc 0、APK 281MB。⚠ **仍待人工验**：蜂窝网 / 无 key 引擎回退听感 / M2-4 中断四场景（来电·闹钟·抢焦点·蓝牙）/ 平板形态（E3 设备仍未办）。⚠ **外部阻塞**：`asrProvider=off` 与全体客户端（含 HMI）的 ASR 批处理兜底都卡在**云栈 MiMo key 失效**（ASR 401 / TTS error），**移交后端**。⚠ **待泓舟**：App 专属 `AUTH_TOKENS` 条目（手机档不含 vehicle.control）、`mobile-apk` 的 `workflow_dispatch` 点一次。**M3 A 批代码面已完成（2026-08-27）**：卡片全量补齐 **14 个渲染器 / 16 个注册键**（M1 18 + M3 16 = types.ts 的全部 **34** 个 type 字符串；**§2.6 原文那个「29 型」是错的、已改**，守卫改成从 `types.ts::UiCard` **派生**，加卡型不再改测试）、车况三格指标 + 平板三段面板（复用 `vehicleStage`/`reminderStage`）、keep-awake + 弱网条、新增 `/card-gallery` 画廊屏（全卡族样本 + 主题过检，**每条标「真栈已验/样本」**）。`tsc` 0、jest **174/174**、hmi **285/285**。**真机两轮跑完**：第一轮 8 张卡真栈已验；第二轮（泓舟解锁后）画廊全卡族 + **深浅主题实时切换** + `payment_qr` 三分支 + **平板三段面板**（`device_state state 3` 强制展开，车况三格是真实推导 72%×550=396km）全过，返回键二级页=关层 ✅。**真机 8 张卡真栈已验**（sports_scores 空态 / scene_card danger 角标 / scene_list / research_report gaps / trip_itinerary 分天 / mcp_result 只读 / parking_fee / place_list「看菜单」）。本轮真机逼出三处修复，最要紧一条：**`CardBoundary` 兜不住原生组件缺席**——Fabric 挂载期在原生线程抛，`getDerivedStateFromError` 不触发、整屏红屏，⇒ 铁则「异常卡不许崩整列表」只覆盖 JS 侧，引入原生支撑的卡片必须先探 `TurboModuleRegistry`（坑账 §9.27，**M5 的 OTA 同形态**）。⚠ **待泓舟拍板一条**：计划写的「根屏返回=后台」被实测推翻——HyperOS 上根 Activity 返回是 finish、JS 上下文重建、会话清空（**换 `monkey -c LAUNCHER` 从桌面起重测才坐实，deeplink 起的那次结论不作数**）；与 PoC「前台交互档」承诺不矛盾，但要选：维持现状／M3 内加 intent hack／并入 M5 前台服务。**B 批已完成（08-27 下午）**：一次构建带上 svg + amap3d，`BUILD SUCCESSFUL 24m6s`、APK 325MB；**svg 端到端通**（付款码渲出真二维码，上一轮那条降级守卫自动让位）；**amap3d 能编译能挂载**（Paper ViewManager 在 Fabric interop 下正常；我对它的定性被实测改了两次、两次都先判悲观——`react-native:+` 那条 RN 0.86 仍在做依赖替换，差点据此否掉一个能用的库）。**M3-3 地图 ✅ 打通并真机验过**（08-27）：首次跑出 `Key验证失败 [INVALID_USER_SCODE]`＝包名+签名 SHA1 与 key 注册的不符，指纹从 **APK 本体 apksigner 读实**（非从 keystore 推断——本机有两把 debug.keystore，推错就给错指纹）；泓舟当日在高德控制台加上 `com.xiaozhou.companion` + `5E:8F:16:06:…:F6:25` 后即通。真实瓦片、marker 落点正确、回中实测、浅色跟随。⚠ M5 账：模板 debug keystore 是全世界 RN 项目共用那把，换正式签名要同步加 SHA1。⚠ **M3 仍未做**：Maestro、§8.3 全清单、地图 key 通过后的余项（marker 落点/回中/浅色）；`route_plan`/`charging_route` 的折线还等**后端在卡里带坐标**（这两类契约里根本没有 lat/lng）。⚠ **未达真栈的 6 个卡型**（charging_route 异步未回 / sports_scorers 落域到检索 / merchant·payment 族被门店营业时间挡住 / vision_answer 需 M4 开关）**只在样本上验过，不算读数**。**下一步：B 批原生重建（一次性带上 `react-native-svg` 与地图库）→ M3-3 地图 spike → M3-5 Maestro → §8.3 全清单**（M2-5 剩余人工项随 M3 验收轮一并跑）。每次开工先跑 `check_android_env.ps1` | 接手从 [`docs/design/2026-08-24-mobile-app-implementation-plan.md`](docs/design/2026-08-24-mobile-app-implementation-plan.md) **§0 接手须知**开始（逐任务执行真相源，**M0–M3 实施记录分别写在其 §3/§4/§5/§6 末**）；需求/选型/架构判断在 [`docs/design/2026-08-23-hmi-android-app-plan.md`](docs/design/2026-08-23-hmi-android-app-plan.md)（§9 决策记录） |
