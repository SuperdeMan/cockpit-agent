...
- 位置提醒的 `location` 今天只能由 debug 通道注入（PoC 没有真实 GPS 流）；接上真车 GPS 后
  围栏模块零改动——它只消费广播。
- **地点解析走 `nearby` 的邻近搜索，不是通用地理编码**：车在 50 公里外时目标 POI 落在
  搜索半径外 → Agent **诚实追问**而不是乱猜（行为正确，但「到很远的地方提醒我」今天建不了单）。
  要覆盖这一族需要一个真正的 geocode 出口（navigation 有，但 `data` 不跨 Agent 传，
  卡片里也没有坐标）——留作后续小卡。
- **两个 e2e 的前置是显式的**：`e2e_geofence` 先把车况位置压到市区（否则依赖上一个脚本
  留下的车况）、`e2e_mcp` 先重启桥拿演示商户净初态（否则第二遍跑「确认后真的下单了」
  拿到的是幂等复用响应，断言名不副实）。**顺序依赖靠显式前置消掉，不靠运气**。
