...
⚠ **02（气泡内确认，v1 路径）与 `subflows/set-dock.yaml` 已于打磨批 E（2026-09-10，裁决 J1）删除**：
`uxV2Presence` / `uxV2Dock` 两个回滚开关连同 v1 的状态条、通知条、PTT 提示行、连接 pill、弱网横幅、
气泡内确认按钮一起删掉，代码里只剩 v2 一条呈现路径，06（承诺面）是唯一的确认流。
此前「02 与 06 前提互斥、要分两趟」的纪律随之作废；回归清单为 **04 / 09 / 10 / 11 + 01 / 03 / 06 / 08**。
下面历史段落里提到 02 的读数与互斥说明按当时的包理解，不再适用。
...
- **flow ③ 断网后必须等够 ~45 秒再发**：RN 的 WebSocket 在飞行模式下 `onclose` 不来，
  连接靠 App 探活判死（4s 超时档实测 **25.7s**；2026-09-12 起单次超时 8s，判死更晚）。早发的那条会落进
  已知残留窗口、根本不入队——2026-09-14 起它会在判死那一刻被结算成「发送状态未知」+ 重发
  （`settleLinkLost`），`msg-pending` 也会消失，所以 flow 末尾多了 `bubble-resend` 不可见的断言：
  等得不够长 flow 诚实地红，红的原因是那个已知窗口、不是回归。计时器不再用 `waitForAnimationToEnd`
  （光球 2026-09-13 起空闲 30s 静置，画面会静），改成等一个永不出现的元素 + `optional: true`。
