...
- **P2 ✅（2026-07-17 当日）**：
  - **严格闸**：`REQUIRE_REAL_PROVIDERS=on`（默认 off）双侧落地——agents 侧在
    `log_resolution` 决议收口处拒 mock；llm-gateway 侧 LLM mock active / embed mock
    （§7 EMBED_DIM=384 边缘就此关死）/ ASR/TTS auto→mock 同闸；豁免
    `REQUIRE_REAL_EXEMPT`（默认 `parking,knowledge`）；compose `python-env` 锚全服务下发。
  - **`_prov` 全量推广**：info forecast / stock_quote（东财降级路径如实标
    `mode=degraded, note=Tushare 失败降级东方财富`）/ news_brief / sports_scores /
    sports_scorers + navigation poi_list×6 / route_plan×2 / poi_detail + charging
    charging_route×2 / poi_list(dest_choice)。**刻意不标**（卡内已有更强证据链）：
    trip_itinerary（每停靠点 grounded 布尔粒度更细）、research_report（sources + 全局
    权威编号）、内部数据卡（reminder/scene/vehicle——无外源真值可标）。
  - **泄漏探针**：新 `test/e2e_strict_stack.py`（active=mock 自动 SKIP；带 `_prov` 卡
    <2 张判探针失效防假绿），接入 `scripts/run_e2e.{sh,ps1}` 清单。
- **P2 验证**：+9 严格闸单测（默认 off 零行为变化 / on 拒 mock / 豁免放行 / llm/embed/
  asr/tts 各闸）；真栈：一次性容器 `REQUIRE_REAL_PROVIDERS=on` + 抹掉股票凭证 →
  `ProviderConfigError: provider[stock] 决议为 mock——严格栈禁止` 实拒；
  `e2e_strict_stack.py` 三问全 real（weather=qweather / place_list=amap / route_plan=amap，
  推广族真栈顺带验证）。全量 **1695 passed / 7 skipped**。
