...
- **状态**：已落地 + 红灯修复收官 + 遗留卡清零（P0-P3 全落地，9 项红灯按泓舟拍板全部
  修复；2026-07-15 收口两张遗留卡与尾项——A1-4（挂起前缀+「明早」段位默认）/B3-3
...
### 遗留卡收口（2026-07-15，两张全清；A1-4/B3-3 真栈转绿）

**卡① A1-4：`_suspend` 携带前序已完成步结论。** `engine._suspend` 增 `prior` 参数
（本轮**新完成且尚未播报**的步骤结果）：多步/adaptive 计划中途挂起时，聚合器不会跑、
复杂任务又不逐步流出话术、挂起 final 还会整体替换 HMI 气泡——现在挂起 final 前缀这些
步骤的脱敏简报（复用过程区 `result_summary` 首句/安全计数口径，`strip_markdown_speech`
保零泄漏；「好的」类短回执过滤）。**防双重播报三道闸**：挂起步自身不进前缀（trip/scene
确认话术本就是完整叙述）、确认/补槽续接的种子结果切掉（`results[len(seed_results):]`，
上轮已播报）、T2 流式路径已流出话术的结果按 `id()` 排除（loop `spoken` 集合；步 id 跨
replan 可撞名，身份比较不用 step_id）。接线四处：engine executor 挂起、executor 路径
escalate 挂起（`_run_escalated` 透传 prior）、loop 流式/unary 两处挂起；D0 单步无前序
不传。契约测试 `test_suspend_prior.py` 6 条（前缀/单步不变/续接不复读/短回执过滤/loop
跨迭代携带/流式+种子排除）。真栈证据：多步挂起探针「查明天天气，另外提醒我带充电线」→
「深圳当前阴，气温29℃，体感33℃，南风2级。好的，带充电线。什么时候提醒你？」（修复前
只有后半句）；**A1-4 旅程转绿**（该轮 mimo replan 自带时刻未挂起，「明天深圳会下雨，
所以已为你设置了明天早上8点的提醒」——条件链+天气结论都在）。
**②「明早」追问也已收口（2026-07-15 泓舟拍板按建议修）**：timeparse 新增段位默认时刻
`_SEG_DEFAULT_HOUR`（dawn 6/am 8/noon 12/pm 15/eve 20）——**仅「日+段位」同时在场**
（明早/明晚/明天下午）才默认成单，display 回读完整时刻、可一句改期（P1a update）；
裸日期（「明天提醒我开会」）与裸段位（「晚上提醒我」——过点会被滚到明晚，语义存疑）
仍 NEED_TIME 追问，「中午」12:00 既有默认不变。timeparse 用例 +6（黄金表迁移
「明天早上提醒我」到 OK）。真栈 A1-4 达到理想形态：「明天有雨，气温25℃左右，出门记得
带伞。已经帮你设好明早8点的带伞提醒了」——单句条件链零追问；A1-3 回归绿（显式八点
不受影响）。

**卡② B3-3 记忆 M1/M2。** 真栈 PG 实证病灶：u1 有 8 条 `demo-*` 会话（07-14 场景编排
开发日）抽出的钓鱼模式条目——**semantic + user_stated + confidence 1.0 + scope
profile.preference**，文本含「空调设为22度」直接污染温度类召回（M1 实锤）；而「记住我
最喜欢26度」**从未入库**——一问一答 2 轮的会话永远凑不满 4 轮抽取节流窗（M2 真根因，
`server._CONSOLIDATE_EVERY=4`）。三针修：
- **M1 抽取黑名单补场景参数**（`memory/extract.py`）：prompt 明令场景/模式配置参数非
  偏好 + **确定性后置过滤**（不信 prompt）——偏好类候选的参数锚点（数字/颜色
  `_ANCHOR_RE`）若只能溯源到「模式/场景」语境的用户话轮（该话轮又无「记住/我喜欢」
  偏好口吻 `_PREF_STATE_RE`）即场景配置→丢弃；episodic（「用户创建了钓鱼模式」）照常保留。
- **M2a 显式记忆陈述立即抽取**（`memory/server.py`）：用户轮命中「记住/我喜欢/我习惯」
  `_REMEMBER_NOW_RE` → 绕过 4 轮节流立即触发 consolidate（触发后计数清零防紧邻双跑）。
- **M2b 谓词归一 + 等价类 supersede**（extract+store）：LLM 自由造词（hvac.temperature
  / climate.temp / ac.temperature…）导致 `current_by_predicate` 精确匹配失手、新旧偏好
  并存——常用车控偏好归一到 canonical（`climate.temperature`/`media.volume`/
  `light.ambient_color`/`seat.heating`，prompt 同步给词表），consolidate 冲突查找按
  **谓词等价类**逐一回退，命中任一别名即 supersede。
- 存量清理：2 条现行钓鱼模式污染行按记忆模型语义**软标记 superseded**
  （`superseded_by='cleanup-m1-scene-config-20260715'`，可审计可回退，不硬删）。
  单测 +6（场景参数丢弃×3/别名类 supersede/立即触发/归一断言更新）。真栈证据：
  **B3-3 旅程转绿**（「记住26度」纯记忆应答→新会话「调到我喜欢的温度」→26 落地终态
  校验过）；PG 回读 26 条目 `climate.temperature`/semantic/现行。

**收口回归**：全量 pytest **1616 passed**（1603+13，②收口后 **1622**）；挂起面密集旅程批
A2-3/A5-3/B2-1/B2-2/B3-1 全绿（trip 确认、B2 中断-恢复三态、多意图确认均无双重播报）。

**B2-3 前提翻面观察与处置（2026-07-15 泓舟拍板按建议修=改用例前提，B5-2 先例）**：
当日两采样均被 planner 路由到 `navigation.navigate_to{destination:惠州,stop_category:充电}`
顺路途经点流（直接出路线+waypoint 候选「想顺道去哪家」，**无挂起**——对「路上/找个」
形态这是更优体验非缺陷；obs `c03a87cf35934837`），不出 charging dest_choice——根源
=charging manifest **无 route_hints**、路由纯 LLM 掷硬币（B5-2 校准同族）。不为测试
掰产品路由：语料改用 charging 例句形态「去惠州怎么充电」（真栈探针 **6/6 稳出
dest_choice**，三个候选句式全稳）。B2-3 两采样转绿：dest_choice 卡→插话问时间→
「第一个」回填惠州站→路线+电量足够直达的诚实判定。附带观察的墙钟 badcase 同日修复（见下）。

### canonical 全量重跑（2026-07-15 15:51，33 条连续 881.7s，@MiniMax-M3）

收口后首次全量：**回归级 15/15 全绿**（保护底座跨 provider 站住）、目标级 13/18；
时延 P50 4.9s / P95 25.5s / max 39.5s（M3 显著快于 mimo 基线的 P95 40s）；记分卡
honesty 4/4 · proactive 4/4 · safety 1/1 · autonomy 16/19 · continuity 18/21 ·
interaction 4/6。**注意：run 期间全局 active provider 被切到 MiniMax-M3**（当日早些
采样均 @mimo，切换非本会话所为；收口修复 B2-1/B2-3/B3-1/B3-2/B3-3/A2-4 在 M3 上
全部保持绿——机制跨 provider 成立）。5 红三分类处置：
- **真产品缺陷已修**：B2-2——M3 planner 对「提醒我吃降压药」误填 `kind=todo` 槽，
  agent 盲信槽位走待办路径静默跳过时间追问 → reminder「原话优先」护栏（显式
  「提醒/叫我」话术永远走定时提醒，槽位只在与原话不冲突时生效，同 scene
  custom_params 原则；单测 +2，@mimo 复跑绿）。
- **语料校准**：A2-1——导航直达真店名已成功，禁词「第几个」被顺路补能 offer 的
  通用话术误伤 → 移除该禁词（反问断链语义仍由 哪一家/想去哪/请选择 钉住），复跑绿；
  B5-2——裸序号协议层 LLM 方差（M3 采样反问「想看哪一家」）→ 按 B1-1 先例加
  `retry: 1`，复跑绿。
- **记录不改**：A1-4——M3 对条件句只答当日天气未建条件链（planner 条件规划质量即
  目标级标尺本身，mimo 采样绿）；B5-1——「取消」轮解析到**跨旅程残留**提醒
  （A2-2b 等 cleanup 遇多条同名候选澄清后断链遗留）→ 残留 8 条（带伞家族×7+已 fired
  宝安机场，全部当日测试产物）已定向软取消清理；跨旅程 u1 store 污染是已知运行卫生
  成本，cleanup 多候选断链留观察。

**墙钟 badcase 修复（2026-07-15，B2-3 插话轮暴露）**：obs 复核发现「现在几点了」两采样
**全错**（实际 14:25:44 答 14:43、14:27:13 答 10:06——14:43 只是编得像）。两层病灶：
①chitchat 锚只有日期没有时刻，被问钟点时 LLM 必然编造；②planner 会把该句误路由
`info.search`（真栈复现：exa 搜墙钟→「联网检索暂时不可用」降级话术）。三件套修复
（系统自己持有墙钟，这类问题不该让 LLM 答）：chitchat `_clock_answer` 确定性直答
（钟点/日期/星期问句**占据整句**才拦：礼貌前缀+语气尾词剥净后 ^$ 锚定，防劫持
「几点提醒我」「明天几点有比赛」；接 handle/handle_stream 双路，零 LLM）+ 锚点补
星期与时刻（长尾时间相对话题不再瞎编）+ **manifest route_hints 钉路由**（priority 61
> info 59，占据整句 pattern，policy replace——R2.1 声明式机制，不改编排核心）。
语料 `route_hints_cases.yaml` +8（含误路由纠正原型与三条反向护栏）84/84、基线刷新
（原基线停在 07-12 的 47 条）；chitchat 单测 +5。真栈：「现在几点了」三采样
「现在是下午2点52分」@墙钟 14:52 全对且 2.2s 秒回、「今天几号」带星期正确、
「讲个笑话」对照仍走 LLM。
